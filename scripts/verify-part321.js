#!/usr/bin/env node
/**
 * Verify Part 321: NL Kindergarten Enrichment — Themes 1-19
 *
 * Checks all 19 theme files have the 10 enrichment fields in their kindergarten section.
 */
...
for (const theme of themes) {
  ...
  const block = content.substring(kindergartenIdx, firstGradeIdx);

  // 1. seoTitle contains "Groep 1-2" and "LessonCraftStudio"
  const seoTitleMatch = block.match(/seoTitle:\s*'([^']+)'/);
  if (!seoTitleMatch) {
    errors.push('seoTitle missing');
  } else {
    if (!seoTitleMatch[1].includes('Groep 1-2')) errors.push('seoTitle missing "Groep 1-2"');
