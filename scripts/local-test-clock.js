...
const SHOT = has('shot');
const ACTIVITY = 'clock.set-clock.1-md-b-3';
...
(async () => {
  ...
  for (const loc of LOCALES) {
    ...
    const url = `${BASE}/clock-activity.html?lang=${loc}&activity=${ACTIVITY}&embed=1`;

    /* set the time deterministically via the keyboard fallback (Enter steps
       hour +1 / toggles minute) — robust against the 12:00 hand overlap. */
    async function keyboardSet(H, M) {
      const cur = await page.evaluate(() => ({ h: window.ClockActivity.setHour, m: window.ClockActivity.setMinute }));
      if (cur.m !== M) { await page.$eval('.clk-hand-minute', e => e.focus()); await page.keyboard.press('Enter'); }
      let hp = ((H - cur.h) % 12 + 12) % 12;
    ...
    try {
      ...
      /* prompt shows the locale's WORDED time (half-idiom), not a bare key */
      const prompt = await page.$eval('.lcs-activity-prompt-text', e => e.textContent.trim()).catch(() => '');
      note(prompt.length > 0 && !/\d+:\d\d/.test(prompt), `${loc}: prompt "${prompt}" looks like a bare H:MM key (worded time missing)`);
      ...
      // minute hand at start points up (12); grab near its tip (radius ~34 units), drag to bottom (180°)
      ...
      note(m1 === 30 && m1 !== m0, `${loc}: minute-hand radial drag did not set minute to 30 (got ${m1})`);
      ...
      await keyboardSet((tgt.H % 12) + 1, tgt.M);   // wrong hour
      ...
      note(!wrong.c && !wrong.ro, `${loc}: a wrong time still celebrated/locked`);
      ...
      note(ok.correct, `${loc}: keyboard-set to target did not satisfy isCorrect()`);
      note(ok.c && ok.ro, `${loc}: correct time (${tgt.H}:${tgt.M}) did not celebrate + lock`);
  ...
  console.log(`CLOCK LOCAL TEST PASSED — ${LOCALES.length} locale(s): render (face + hands + readout) + localized title/worded-prompt + radial-drag (minute) + keyboard set (correct celebrates, wrong doesn't) + ≥7-round reshuffle + no mobile overflow.`);
  process.exit(0);
