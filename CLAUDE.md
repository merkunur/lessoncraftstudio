...
**Position (2026-08-01).** `#36 number-sieve` · `#37 arrow-strip` · `#38 draw-bag` · **`#39 lids` LIVE — WAVE 1 IS CLOSED.** → **NEXT = wave 2: B3 Unit Handle**, then B4 Unrolling Tape · A5 Comparison Tape · B7 Cold Line. Full wave order in the companion.

**Constants for the next build:** header ordinal **#40** · `PREV = 'lids'` · `TOOL_WRAPPER_VERSION` **7.52 → 7.53** · it becomes `TOOL_KEYS` position **43**. ⚠ **The header ordinal is a LEDGER, not the array index — it runs 3 behind** (`class-timer`/`letter-studio` both claim #25, `wodb`/`syllable-splitter` both claim #22, three early tools carry none). Take the next ordinal from the last shipped tool's header. **Do not renumber anything.**
