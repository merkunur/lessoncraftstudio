#!/usr/bin/env node
/**
 * gen-b4-waves.js — nt10-D wave files, two per locale:
 *   waves/wave-b4-<loc>.json     the 20 base specs
 *   waves/wave-b4var-<loc>.json  the 100 variation faces
 *
 *   node tools/gen-b4-waves.js [--locales=en,de,…] [--dry-run]
 *
 ...
 * every apply-b4-locale (Phase 4) — until then those waves carry only what builds.
 */
'use strict';
const fs = require('fs');
...
// nt10-D theme rulings (design finals §1): tangram / human-body / five-senses / recycling / cloze / pronouns /
...
function writeWave(id, locale, m) {
  ...
  const plan = {
    ...
    _refused: m.refused,
    _note: 'nt10-D (generated by tools/gen-b4-waves.js — every themed type pinned by a MEASURED build; _refused = specs that build on no candidate in this locale, dropped from the wave and recorded, never padded)',
  };
