#!/usr/bin/env node
/* =====================================================================
   _dot-probe.js — multi-state control-bottom probe for "Dot's Story Spine"
   (RL.K.3, clarity-first redesign of #62). Drives two role rounds (initial)
   + a wrong-scaffold + resolved, and MEASURES the lowest control INCLUDING
   the shell `.lcs-activity-check` across 320·360·412·768·1024·1366, plus tap
   ≥44px on the panel-cards, no overlap, no horizontal overflow.
   ===================================================================== */
'use strict';
...
(async () => {
  ...
  const solve = (ok) => page.evaluate((correct) => {
    const t = window.StorySpineActivity, r = t._round, story = t._stories[r.storyId], C = window.StorySpineCore;
    const oi = C.oracle(r, story), idx = correct ? oi : ((oi + 1) % story.panels.length);
    const b = Array.from(document.querySelectorAll('.ds-cand')).find(x => +x.getAttribute('data-pi') === idx); if (b) b.click();
  }, ok).then(() => sleep(50));

  const states = {
    'prob': async () => { await force('ds-mit-prob'); },
    'sol': async () => { await force('ds-pic-sol'); },
    'wrong': async () => { await force('ds-car-set'); await solve(false); },
    'resolved': async () => { await force('ds-car-sol'); await solve(true); }
  };

  async function controlBottom() {
    return await page.evaluate(() => {
      const sel = '.ds-root,.ds-say,.ds-strip,.ds-cand,.ds-scene,.ds-hear,.ds-line-msg,.lcs-activity-check';
  ...
  async function checks() {
    return await page.evaluate(() => {
      const taps = Array.from(document.querySelectorAll('.ds-cand'));
  ...
  if (!(await page.evaluate(() => window.StorySpineActivity._resolved))) fails.push('a correct tap did NOT resolve');
  await states.wrong();
  if (await page.evaluate(() => window.StorySpineActivity._resolved)) fails.push('a wrong-role panel resolved — must not advance');

  const order = ['prob', 'sol', 'wrong', 'resolved'];
  for (const [w, h] of VIEWPORTS) {
    ...
    for (const label of order) {
      ...
      if (tt.minTap < CTRL_MIN && tt.minTap !== 999 && !subFloor) fails.push(`${label} @${w}×${h}: a panel is ${tt.minTap}px (<${CTRL_MIN}px)`);
      if (tt.overlap > 0) fails.push(`${label} @${w}×${h}: ${tt.overlap} panel pair(s) overlap`);
  ...
  console.log('');
  if (fails.length) { console.error(`DOT-PROBE FAILED — ${fails.length}:`); fails.forEach(f => console.error('  • ' + f)); process.exit(1); }
  console.log(`DOT-PROBE PASSED — role rounds/wrong/resolved: a correct tap resolves + a wrong-role panel does not advance; lowest control (incl. shell Check) clears the fold by ≥${MARGIN}px across 320→1366; panel-cards ≥${CTRL_MIN}px (off the 320 sub-floor), no overlap, no horizontal overflow.`);
  process.exit(0);
