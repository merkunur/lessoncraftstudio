...
(function (global) {
  ...
  function fixitSVG() {
    return '<svg class="fic-mole-svg" viewBox="0 0 100 100" role="img" aria-label="' + (LANG === 'de' ? 'Fixit, der Reparatur-Maulwurf' : LANG === 'fr' ? 'Fixit, la taupe réparatrice' : LANG === 'es' ? 'Fixit, el topo reparador' : 'Fixit the repair-mole') + '">' +
  ...
  global.FixItCornerActivity = {
    ...
    strings: {
      title: { en: "Fixit's Fix-It Corner", de: 'Fixits Reparatur-Ecke', fr: 'Le coin réparation de Fixit', es: 'El rincón de arreglos de Fixit' },
      instruction: { en: 'Fixit can\'t read the tiny number — you\'re his sharp-eyed helper. Work out the part each gadget needs, pop it in, then tap Check.', de: 'Fixit kann die winzige Zahl nicht lesen – du bist sein scharfäugiger Helfer. Finde heraus, welchen Teil jedes Gerät braucht, setz ihn ein und tippe dann auf Prüfen.', fr: 'Fixit n’arrive pas à lire le tout petit nombre — c’est toi son aide aux yeux perçants. Trouve la pièce qu’il manque à chaque appareil, place-la, puis touche Vérifier.', es: 'Fixit no alcanza a ver el numerito — tú eres su ayudante de buen ojo. Descubre qué parte le falta a cada aparato, colócala y toca Comprobar.' },
      promptMake10: { en: 'Fill it up to exactly 10!', de: 'Füll es genau bis 10 auf!', fr: 'Remplis-le jusqu’à exactement 10 !', es: '¡Llénalo justo hasta 10!' },
      promptMissingSum: { en: '{a} and {b} — how many in all?', de: '{a} und {b} – wie viele sind das zusammen?', fr: '{a} et {b} — combien ça fait en tout ?', es: '{a} y {b}, ¿cuántos son juntos?' },
      promptMissingAddend: { en: '{a} and how many more make {c}?', de: '{a} und wie viele mehr ergeben {c}?', fr: '{a} et combien de plus pour faire {c} ?', es: '{a} y ¿cuántos más hacen {c}?' },
      promptMissingSubtrahend: { en: '{a} take away how many leaves {b}?', de: 'Von {a} wie viele weg, dann bleibt {b}?', fr: '{a}, on en enlève combien pour qu’il reste {b} ?', es: 'De {a}, ¿cuántos quitas para que queden {b}?' },
      promptMissingFirstAddend: { en: 'What plus {b} makes {c}?', de: 'Was plus {b} ergibt {c}?', fr: 'Combien plus {b} font {c} ?', es: '¿Qué número más {b} es {c}?' },
      promptMissingMinuend: { en: 'Take away {b}, leaves {c} — start with what?', de: 'Nimm {b} weg, dann bleibt {c} – womit hast du angefangen?', fr: 'Enlève {b}, il reste {c} — on part de combien ?', es: 'Quitas {b}, quedan {c}: ¿con cuántos empezaste?' },
      promptBalance: { en: 'Make both sides match.', de: 'Mach beide Seiten gleich.', fr: 'Rends les deux côtés égaux.', es: 'Equilibra los dos lados.' },
      promptProduce: { en: 'Tap a number to hide — make the next puzzle!', de: 'Tippe eine Zahl an, um sie zu verstecken – mach das nächste Rätsel!', fr: 'Touche un nombre pour le cacher — fabrique la prochaine énigme !', es: 'Toca un número para esconderlo y arma el siguiente reto.' },
      hop: { en: 'Hop', de: 'Hüpf', fr: 'Saute', es: 'Brinca' },
      slide: { en: 'Slide together', de: 'Zusammenschieben', fr: 'Rassemble', es: 'Junta' },
      whole: { en: 'the whole', de: 'das Ganze', fr: 'le tout', es: 'el todo' },
      winMake10: { en: 'Ten! It whirs to life — it can butter ten slices of toast at once!', de: 'Zehn! Es surrt los – es kann zehn Scheiben Toast auf einmal buttern!', fr: 'Dix ! L’appareil s’anime en vrombissant — il peut beurrer dix tartines à la fois !', es: '¡Diez! Arranca zumbando: ¡puede untar diez panes tostados a la vez!' },
      winSum: { en: 'That\'s the whole! POP — it clicks awake!', de: 'Das ist das Ganze! KLACK – es springt an!', fr: 'C’est le tout ! CLAC — il se réveille !', es: '¡Ese es el todo! CLIC, ¡y arranca!' },
      winAddend: { en: '{x} more! It catches, clicks, and whirs to life!', de: '{x} mehr! Es greift, klickt und surrt los!', fr: '{x} de plus ! Il accroche, cliquette et s’anime !', es: '¡{x} más! ¡Agarra, hace clic y cobra vida zumbando!' },
      winSubtrahend: { en: '{x}! It catches, clicks, and whirs to life!', de: '{x}! Es greift, klickt und surrt los!', fr: '{x} ! Il accroche, cliquette et s’anime !', es: '¡{x}! ¡Agarra, hace clic y cobra vida zumbando!' },
      winFirstAddend: { en: '{x}! The fact-family fits — it springs alive!', de: '{x}! Die Aufgabenfamilie passt – es springt an!', fr: '{x} ! La famille de calculs colle — il se met en marche !', es: '¡{x}! La familia de operaciones encaja, ¡y arranca!' },
      winMinuend: { en: 'You started with {x}! Slid back together — it hums awake!', de: 'Du hast mit {x} angefangen! Wieder zusammengeschoben – es brummt los!', fr: 'Tu es parti de {x} ! Rassemblé — il se réveille en ronronnant !', es: '¡Empezaste con {x}! Se volvieron a juntar, ¡y arranca traqueteando!' },
      winBalance: { en: 'Level! Both sides match — it balances and dings!', de: 'Im Gleichgewicht! Beide Seiten sind gleich – es pendelt sich ein und klingelt!', fr: 'À l’équilibre ! Les deux côtés sont égaux — il se stabilise et sonne !', es: '¡En equilibrio! Los dos lados iguales: se acomoda y suena una campanita.' },
      winProduce: { en: 'A new puzzle! Fixit fixes it right up.', de: 'Ein neues Rätsel! Fixit repariert es im Nu.', fr: 'Une nouvelle énigme ! Fixit la répare en un clin d’œil.', es: '¡Un reto nuevo! Fixit lo arregla en un dos por tres.' },
      missOver: { en: 'Whoa — it over-revs! That\'s too many. {a} is a part of {c}, not all of it.', de: 'Hoppla – es dreht über! Das sind zu viele. {a} ist ein Teil von {c}, nicht das Ganze.', fr: 'Oh — il s’emballe ! C’est trop. {a}, c’est une partie de {c}, pas le tout.', es: '¡Uy, se pasa! Son demasiados. {a} es una parte de {c}, no el todo.' },
      missCopy: { en: 'Clunk — it stalls. That\'s the whole, not the missing part.', de: 'Klonk – es stockt. Das ist das Ganze, nicht der fehlende Teil.', fr: 'Clonk — il cale. C’est le tout, pas la partie qui manque.', es: 'Clonc, se atora. Ese es el todo, no la parte que falta.' },
      missOne: { en: 'Half a turn… so close! Count once more.', de: 'Eine halbe Umdrehung … fast! Zähl noch einmal.', fr: 'Un demi-tour… tout près ! Recompte une fois.', es: 'Media vuelta… ¡casi! Cuenta una vez más.' },
      missGeneric: { en: 'Not yet — let\'s check it together.', de: 'Noch nicht – lass es uns zusammen prüfen.', fr: 'Pas encore — vérifions ensemble.', es: 'Todavía no… vamos a revisarlo juntos.' },
      model: { en: '{a} plus {x} is {c}… or {c} take away {a} is {x}.', de: '{a} plus {x} ist {c} … oder {c} minus {a} ist {x}.', fr: '{a} plus {x} font {c}… ou {c} moins {a} font {x}.', es: '{a} más {x} es igual a {c}… o {c} menos {a} es igual a {x}.' },
      escalate: { en: 'Fixit slides over the counting-rail — let\'s build it together!', de: 'Fixit schiebt die Zählschiene herüber – lass es uns zusammen bauen!', fr: 'Fixit fait glisser la règle à compter — construisons-le ensemble !', es: 'Fixit se desliza por el riel para contar: ¡vamos a armarlo juntos!' },
      hintCheck: { en: 'Pop a part into the gadget, then tap Check.', de: 'Setz einen Teil in das Gerät ein und tippe dann auf Prüfen.', fr: 'Place une pièce dans l’appareil, puis touche Vérifier.', es: 'Pon una parte en el aparato y toca Comprobar.' }
    },
