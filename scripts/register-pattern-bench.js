#!/usr/bin/env node
/* =====================================================================
   register-pattern-bench.js — the five wiring points for TOOL #32.
   ...
   Usage: node scripts/register-pattern-bench.js
   ===================================================================== */
'use strict';
const fs = require('fs');
...
const E = require('./_pattern-bench-content.js');
...
for (const rel of ['frontend/config/live-tool-slugs.ts', 'frontend/lib/seo/tool-content.ts']) {
  ...
  const anchor = `, 'number-balance'] as const;`;
  if (!s.includes(anchor)) die(`${rel}: TOOL_KEYS anchor not found`);
  s = s.replace(anchor, `, 'number-balance', '${KEY}'] as const;`);
...
(function () {
  ...
  const adds = [
    [`  'number-balance': '/mini-tools/number-balance.html',`,
     `  '${KEY}': '/mini-tools/${KEY}.html',`, 'TOOL_MINI_URL'],
    [`  'number-balance': 'number-balance',`,
     `  '${KEY}': '${KEY}',`, 'TOOL_ACTIVITY_PREFIX'],
    [`  'number-balance'?: ToolEntry;`,
     `  '${KEY}'?: ToolEntry;`, 'ToolContent interface']
  ];
...
(function () {
  ...
  for (const loc of LOCALES) {
    ...
    /* insert before "labels" so the file keeps its shape */
    const out = {};
