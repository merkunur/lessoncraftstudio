#!/usr/bin/env node
/**
 * alloc-b3var-ids.js — the nt20-C variation-face id allocation (Phase 2).
 *
 ...
 * K-325+ · G1-311+ · G2-320+ · G3-378+ in file order. Deterministic, so twenty
 * builders working in parallel cannot collide. Writes
 * docs/worksheet-gen/b3-designs/_records/b3var-id-allocation.json and refuses if the
 * count is not exactly 100 or an id block would overlap a shipped spec on disk.
 */
'use strict';
const fs = require('fs');
const path = require('path');
const ROOT = path.join(__dirname, '..');
const DESIGNS = path.join(ROOT, '..', '..', 'docs', 'worksheet-gen', 'b3-designs');
const FAMILIES = ['K-317', 'K-318', 'K-319', 'K-320', 'K-321', 'K-322', 'K-323', 'K-324',
  'G1-305', 'G1-306', 'G1-307', 'G1-308', 'G1-309', 'G1-310',
  'G2-315', 'G2-316', 'G2-317', 'G2-318', 'G2-319', 'G3-377'];
const NEXT = { K: 325, G1: 311, G2: 320, G3: 378 };
...
if (out.length !== 100) throw new Error('expected 100 faces, got ' + out.length);
...
fs.writeFileSync(path.join(recDir, 'b3var-id-allocation.json'), JSON.stringify({ generated: new Date().toISOString().slice(0, 10), next: NEXT, faces: out }, null, 1) + '\n');
const per = {}; for (const r of out) per[r.band] = (per[r.band] || 0) + 1;
console.log('allocated 100 face ids:', per, '→ next free', NEXT);
