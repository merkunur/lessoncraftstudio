#!/usr/bin/env node
/* Wave 6 final enum (READ-ONLY): shadow-match/find-shadow × EN K (readiness; silhouette-match).
 * Group by theme; canonical = earliest publishedAt; siblings = rest (~2.9x collapse expected — heaviest in the wave).
 * MUST run on Hetzner. Usage: node scripts/seo-landing/enum-wave6-shadowmatch.js
 */
'use strict';
...
(async function () {
  ...
  var rows = await c.deck.findMany({
    where: { language: 'en', status: 'published', exerciseType: 'shadow-match' },
  ...
  var coords = Object.keys(groups).sort().map(function (theme) {
    ...
    return { mode: 'find-shadow', theme: theme, slugTheme: theme.replace(/_/g, '-'), n: members.length,
