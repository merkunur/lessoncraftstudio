#!/usr/bin/env node
/* =====================================================================
   audit-class-graph-locale-layout.js — the sweep, in the WORDIEST
   locales.
   ...
   Usage: node scripts/audit-class-graph-locale-layout.js [--shot]
   ===================================================================== */
'use strict';
...
(async () => {
  ...
  for (const loc of LOCALES) {
    ...
    await p.goto('http://127.0.0.1:5463/class-graph.html?lang=' + loc + '&embed=1', { waitUntil: 'domcontentloaded' });
    ...
    for (let i = 0; i < 5; i++) {
      await p.evaluate((c) => { document.querySelectorAll('.cgr-vote')[c].click(); }, i % 3);
      await wait(30);
    }
    await p.evaluate(() => {
      const b = document.querySelectorAll('.cgr-chip')[2];   /* show the numbers */
      if (b) b.click();
    });
    await wait(350);
    ...
    for (const [w, h] of V) {
      ...
      const m = await p.evaluate(() => {
        const c = Array.from(document.querySelectorAll('.cgr-chip,.cgr-vote,button'))
