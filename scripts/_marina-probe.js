#!/usr/bin/env node
/* =====================================================================
   _marina-probe.js — multi-state control-bottom probe for "The Halfway
   Harbors" (3.NBT.A.1, clarity rebuild). Drives nearest / big-trail /
   halfway (initial) + a wrong-scaffold + resolved, and MEASURES the lowest
   control INCLUDING the shell `.lcs-activity-check` across 320·360·412·768·
   1024·1366, plus tap ≥44px, no overlap, no horizontal overflow.
   ===================================================================== */
'use strict';
...
(async () => {
  ...
  const solve = (ok) => page.evaluate((correct) => {
    const t = window.HalfwayHarborsActivity, r = t._round, Core = window.HalfwayHarborsCore;
    const oi = Core.oracle(r), arr = r.cog === 'halfway' ? r.buoyMarkers : r.harbors;
    const val = correct ? arr[oi] : arr.find((v, i) => i !== oi);
    const b = Array.from(document.querySelectorAll('.hh-cand')).find(x => Number(x.textContent) === val); if (b) b.click();
  }, ok).then(() => sleep(50));

  const states = {
    'nearest': async () => { await force('hh-near-63'); },
    'big': async () => { await force('hh-big-247'); },
    'halfway': async () => { await force('hh-half-40-50'); },
    'wrong': async () => { await force('hh-near-63'); await solve(false); },
    'resolved': async () => { await force('hh-big-360'); await solve(true); }
  };

  async function controlBottom() {
    return await page.evaluate(() => {
      const sel = '.hh-root,.hh-say,.hh-prompt,.hh-board,.hh-nl,.hh-row,.hh-cand,.hh-line-msg,.lcs-activity-check';
  ...
  async function checks() {
    return await page.evaluate(() => {
      const taps = Array.from(document.querySelectorAll('.hh-cand'));
  ...
  if (!(await page.evaluate(() => window.HalfwayHarborsActivity._resolved))) fails.push('a correct tap did NOT resolve');
  await states.wrong();
  if (await page.evaluate(() => window.HalfwayHarborsActivity._resolved)) fails.push('a wrong harbor resolved — must not advance');

  const order = ['nearest', 'big', 'halfway', 'wrong', 'resolved'];
  ...
  console.log('');
  if (fails.length) { console.error(`MARINA-PROBE FAILED — ${fails.length}:`); fails.forEach(f => console.error('  • ' + f)); process.exit(1); }
  console.log(`MARINA-PROBE PASSED — nearest/big/halfway/wrong/resolved: a correct tap resolves + a wrong one does not advance; lowest control (incl. shell Check) clears the fold by ≥${MARGIN}px across 320→1366; controls ≥${CTRL_MIN}px (off the 320 sub-floor), no overlap, no horizontal overflow.`);
  process.exit(0);
