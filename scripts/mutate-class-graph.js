...
const M = [
  ...
  /* ⚠ FIVE, not four: the free line moved to FREE_CATS 4 (2.MD.D.10 "up
     to four categories"), so a four-answer starter is now legal and this
     mutation had to move up with it or it would survive silently. */
  ['a starter exceeds the free tier', 'cats: ["Walked", "Car", "Bus"]', 'cats: ["Walked", "Car", "Bus", "Bike", "Train"]'],
  ...
  /* ---- C14: the dead-button class. Each of these is a way the editor
     could go back to leaning on `st` as its working copy, which is what
     made "Add an answer" do nothing at all. ---- */
  ['THE DEAD BUTTON: add commits through setSetup again',
  ...
  ['the editor reads st.cats instead of the draft',
   "    var draft = this._draft.labels;",
   "    var draft = this.st.cats.map(function (c) { return c.label; });"],
  ['the draft field is gone', "  _draft: null,", "  _draftGone: null,"],
  ['remove commits, throwing away the votes',
   "          self._syncDraft(qin, inputs);\n          self._draft.labels.splice(i, 1);",
   "          var vals = inputs.map(function (x) { return x.value; });\n          vals.splice(i, 1);\n          self.st = self.setSetup(self.st, qin.value, vals);"],
  ['a one-answer survey is allowed', "if (kept.length < self.MIN_CATS) return;", "if (kept.length < 1) return;"]
];
