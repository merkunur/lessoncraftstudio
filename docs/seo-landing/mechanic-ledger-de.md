...
| more-less / image-number, check-cross | ⚠️ **DEFERRED.** image-number compares a pile to a NUMERAL (numeric → interrupt). check-cross unverified (no source-read) → deferred pending a ledger-lock. Both keep `/decks/`. |
...
_Remaining readiness type: picture-sort (de-orphan) — appends its row at its sub-slice's source-read. Future numeric slices (subtraction; code-addition/crossword/Anlaute) per their later steps._
