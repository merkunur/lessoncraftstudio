const fs = require('fs');
const content = fs.readFileSync('C:/Users/rkgen/lessoncraftstudio/docs/TPT-LISTINGS-PT.md', 'utf8');
...
allSections.forEach((section, idx) => {
  ...
  const hasRespostas = section.includes('Gabarito') || section.includes('Respostas') || section.includes('Chave de corre\u00e7\u00e3o') || section.includes('respostas') || section.includes('gabarito');
  const hasCTA = section.includes('LessonCraftStudio.com');
  const hasNiveis = section.includes('N\u00edveis');

  console.log(`${num}${variant}: [${titleLen} chars] [${titleStatus}] ${title}`);
  console.log(`    Tags: ${tags.length} [${tagStatus}] | Respostas: ${hasRespostas ? 'YES' : 'NO'} | CTA: ${hasCTA ? 'YES' : 'NO'} | N\u00edveis: ${hasNiveis ? 'YES' : 'NO'}`);
});
