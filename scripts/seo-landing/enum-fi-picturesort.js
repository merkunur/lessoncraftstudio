#!/usr/bin/env node
/* FI picture-sort "-vs-" PAIR enumeration (READ-ONLY). subjectTags[0]="X-vs-Y" combined key.
 * Group by pairKey; canonical = earliest publishedAt; siblings = rest. Parse left/right themes.
 * Clone of enum-sv-picturesort.js (language='fi'). MUST run on Hetzner (DB).
 * Usage: node scripts/seo-landing/enum-fi-picturesort.js > /tmp/fi-picture-sort.json
 */
'use strict';
...
(async function () {
  ...
  var rows = await c.deck.findMany({
    where: { language: 'fi', status: 'published', exerciseType: 'picture-sort' },
  ...
  console.log(JSON.stringify({
    locale: 'fi', exerciseType: 'picture-sort', rawRowCount: rows.length, pairCount: coords.length,
