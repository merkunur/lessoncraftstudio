...
(function (global) {
  ...
  function speak(text) {
    try { if (global.LCSAudio && global.LCSAudio.speak) { global.LCSAudio.speak({ type: 'word', text: text, lang: (LANG === 'es' ? 'es-MX' : LANG === 'pt' ? 'pt-BR' : LANG === 'it' ? 'it-IT' : LANG), rate: 0.95 }); return; }
      if (global.speechSynthesis && global.SpeechSynthesisUtterance) { var u = new global.SpeechSynthesisUtterance(text); u.rate = 0.95; u.lang = (LANG === 'es' ? 'es-MX' : LANG === 'de' ? 'de-DE' : LANG === 'fr' ? 'fr-FR' : LANG === 'pt' ? 'pt-BR' : LANG === 'it' ? 'it-IT' : 'en-US'); global.speechSynthesis.cancel(); global.speechSynthesis.speak(u); } } catch (e) {}
  }
  function shuffle(arr) { var a = arr.slice(), i, j, t; for (i = a.length - 1; i > 0; i--) { j = Math.floor(Math.random() * (i + 1)); t = a[i]; a[i] = a[j]; a[j] = t; } return a; }

  function goatSVG() {
    return '<svg class="ggg-goat-svg" viewBox="0 0 100 100" role="img" aria-label="' + (LANG === 'es' ? 'Gauge la cabra' : LANG === 'pt' ? 'Palmira, a cabra' : LANG === 'it' ? 'Spanna la capra' : 'Gauge the goat') + '">' +
  ...
  global.GaugeGoodGuessActivity = {
    ...
    strings: {
      title: { en: "Gauge's Good Guess", de: 'Gauges gute Schätzung', fr: 'La bonne estimation de Gauge', es: 'El buen ojo de Gauge', pt: 'O bom olho da Palmira', it: 'La stima azzeccata di Spanna' },
      instruction: { en: 'Picture the real size, then tap the estimate that makes sense.', de: 'Stell dir die echte Größe vor und tippe dann auf die Schätzung, die passt.', fr: 'Imagine la taille réelle, puis touche l’estimation qui a du sens.', es: 'Imagina el tamaño real y luego toca la estimación que le queda bien.', pt: 'Imagine o tamanho real e toque na estimativa que faz sentido.', it: 'Immagina la grandezza vera, poi tocca la stima che ha senso.' },
      prompt: { en: 'Tap the estimate that makes sense.', de: 'Tippe auf die Schätzung, die passt.', fr: 'Touche l’estimation qui a du sens.', es: 'Toca la estimación que le queda bien.', pt: 'Toque na estimativa que faz sentido.', it: 'Tocca la stima che ha senso.' },
      gaugeIntro: { en: 'I guess sizes by eye — picture how big it really is!', de: 'Ich schätze Größen mit dem Auge – stell dir vor, wie groß etwas wirklich ist!', fr: 'Je devine les tailles à l’œil — imagine la taille réelle !', es: '¡Yo calculo tamaños con la vista! Imagina qué tan grande es algo de verdad.', pt: 'Eu calculo tamanhos de olho — imagine o tamanho real!', it: 'Indovino le misure a occhio: immagina quanto è grande!' },
      hintPick: { en: 'Think about the real object. Is it small, or really big?', de: 'Denk an den echten Gegenstand. Ist er klein oder richtig groß?', fr: 'Pense à l’objet réel. Est-il petit, ou vraiment grand ?', es: 'Piensa en el objeto real. ¿Es chiquito o de verdad enorme?', pt: 'Pense no objeto real. Ele é pequeno ou bem grande mesmo?', it: 'Pensa alla cosa vera: è piccola o proprio grande?' },
      ...
      win: { en: 'Yes! That is a sensible estimate. 📏', de: 'Ja! Das ist eine sinnvolle Schätzung. 📏', fr: 'Oui ! C’est une estimation qui a du sens. 📏', es: '¡Sí! Esa es una buena estimación. 📏', pt: 'Isso! Essa é uma boa estimativa. 📏', it: 'Sì! Questa è una stima sensata. 📏' }
    },
    defaults: {},
