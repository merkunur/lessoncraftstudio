...
import { importImage, StudioMediaError } from '@/lib/studio/media';
...
// Story Studio tenancy — general picture upload (props drawer "upload my
// own"). Raw image body → sanitized webp under uploads/.
...
export async function POST(request: NextRequest, { params }: Params) {
  ...
  try {
    const saved = await importImage(row.id, buf);
