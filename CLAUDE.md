...
**Position (2026-07-31).** `#36 number-sieve` **LIVE** · `#37 arrow-strip` **LIVE** · `#38 draw-bag` **LIVE** → **NEXT = A2 Splat Mat**, which CLOSES wave 1; then wave 2 (B3 Unit Handle · B4 Unrolling Tape · A5 Comparison Tape · B7 Cold Line). Full wave order in the companion.

**Constants for the next build:** header ordinal **#39** · `PREV = 'draw-bag'` · `TOOL_WRAPPER_VERSION` **7.51 → 7.52** · it becomes `TOOL_KEYS` position **42**. ⚠ **The header ordinal is a LEDGER, not the array index — it runs 3 behind** (`class-timer`/`letter-studio` both claim #25, `wodb`/`syllable-splitter` both claim #22, three early tools carry none). Take the next ordinal from the last shipped tool's header. **Do not renumber anything.**
