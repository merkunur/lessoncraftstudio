...
**NEXT (operator's call — do NOT auto-open):** (1) the **remaining 8 languages** (nl/fr/it/pt/sv/da/no/fi) — EACH needs its own ~20-min autocomplete harvest (right regional Google) BEFORE its build, so it's born keyword-correct (no retrofit — the es/en/de lesson); (2) **tracing/handwriting deck-PRODUCTION** (grafomotricidad / Schwungübungen / tracing — 3-locale-validated demand, the strongest content signal; a deck-creation product decision, NOT a landing task — no decks exist, the writing/drawing-lines apps are PDF-only). **Carry-forward (browse-hub, NOT a landing-program task):** `/en/topic/sudoku` 404 (§22.2; confirmed per-axis-key, not systemic).
