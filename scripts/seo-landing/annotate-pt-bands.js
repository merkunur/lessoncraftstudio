#!/usr/bin/env node
/* Per-coordinate band annotation for the band-split pt arithmetic types (math-worksheet, math-puzzle,
 ...
 * Clone of annotate-pt-bands.js (1o-ano/2o-ano instead of classe-prima/classe-seconda). MUST run on Hetzner
 * (reads /var/www/lcs-media/decks/pt/).
 *
 * Band map (pt ledger; concrete-vs-symbolic → 1o-ano FLOOR, NO infanzia for symbolic calc):
 ...
 * Usage: node annotate-pt-bands.js <coords.json> --type=math-worksheet [--locale=pt] [--source=deck-html]
 */
'use strict';
const fs = require('fs');
...
const locale = (argv.find(a => a.indexOf('--locale=') === 0) || '--locale=pt').split('=')[1];
...
function bandFor(type, max) {
  if (type === 'math-worksheet' || type === 'math-puzzle') {
    if (max <= 10) return { level: '1o-ano', standard: 'K.OA.A.2' };
    if (max <= 20) return { level: '1o-ano', standard: '1.OA.C.6' };
    if (max <= 100) return { level: '2o-ano', standard: '2.NBT.B.5' };
    return null;
  }
