...
Read first: `_STUDIO-BRIEF.md` → `_SUBSTRATE.md` → your type's entry in `_PANEL-FINDINGS.md` → the final design files of the nt20-C neighbours named in your type's Boundary (`../b3-designs/<ID>-<key>.md`) → the existing specs your Boundary names (`scripts/worksheet-gen/types/<band>/<ID>-*.js`, read `build()` AND `verify()`). Measure every pool you cite with read-only node over `scripts/worksheet-gen/cache/manifest.json`, `lib/b2-common.js entriesFor(theme, loc)`, `lib/b3-common.js bank('instructions', loc).objForms`, `data/science/*.json`, `data/b2/sentences.js`. Write NOTHING outside your output file; scratch under the session scratchpad with a `<ID>-` prefix.
...
**D. Data + gates.** The bank shape (fenced), which existing bank it reuses, the validator rules (numbered) and ≥10 poison cases (each must FAIL; the correct draft is the control), plus what `qa/verify-b4-<key>.js` asserts on the render (floors, uniqueness, no answer printed, the 722 stack).
