/**
 * b6-healthy-habits-harness.js — the K-380 `healthy-habits` render harness shared by
 * its gates (verify-b6-habit-pictogram.js, verify-b6-healthy-habits.js).
 * Type-scoped clone of qa/b5-road-safety-harness.js; nothing shared is edited.
 *
 *   withBrowser(fn)                 one headless Chromium for a whole gate
 *   openDoc(page, name, bodyHtml, { filter })
 *                                   writes out/dev/K-380-<name>.html with the
 ...
 *                                   out/dev/K-380-<name>-{colour,grey}.png
...
async function openDoc(page, name, bodyHtml, opts = {}) {
  ...
  const f = path.join(OUT, `K-380-${name}.html`);
...
async function sheet(page, name, bodyHtml) {
  ...
  for (const [tag, filter] of [['colour', ''], ['grey', 'filter:grayscale(1);']]) {
    ...
    const p = path.join(OUT, `K-380-${name}-${tag}.png`);
