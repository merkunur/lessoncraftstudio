...
(function (global) {
  ...
  function speak(text) {
    try { if (global.LCSAudio && global.LCSAudio.speak) { global.LCSAudio.speak({ type: 'word', text: text, lang: (LANG === 'es' ? 'es-MX' : LANG === 'pt' ? 'pt-BR' : LANG), rate: 0.95 }); return; }
      if (global.speechSynthesis && global.SpeechSynthesisUtterance) { var u = new global.SpeechSynthesisUtterance(text); u.lang = (LANG === 'fr' ? 'fr-FR' : LANG === 'de' ? 'de-DE' : LANG === 'es' ? 'es-MX' : LANG === 'pt' ? 'pt-BR' : LANG === 'it' ? 'it-IT' : LANG === 'nl' ? 'nl-NL' : 'en-US'); u.rate = 0.95; global.speechSynthesis.cancel(); global.speechSynthesis.speak(u); } } catch (e) {}
  }
  ...
  function wormSVG() {
    ...
    return '<svg class="btb-worm-svg" viewBox="0 0 100 100" role="img" aria-label="' + (LANG === 'fr' ? 'Bea le ver de livre' : LANG === 'de' ? 'Bea, die Bücherraupe' : LANG === 'es' ? 'Bea, la oruga lectora' : LANG === 'pt' ? 'Bea, a lagarta dos livros' : LANG === 'it' ? 'Bea, il bruco dei libri' : LANG === 'nl' ? 'Bea de boekenrups' : 'Bea the bookworm') + '">' +
  ...
  global.BeaTwoBookshelvesActivity = {
    ...
    strings: {
      ...
      title: { en: "Bea's Two Bookshelves", de: 'Beas zwei Bücherregale', fr: 'Les deux étagères de Bea', es: 'Los dos libreros de Bea', pt: 'As duas estantes da Bea', it: 'I due scaffali di Bea', nl: 'De twee boekenkasten van Bea' },
      instruction: { en: 'Tap the story book, or the fact book, that Bea asks for.', de: 'Tippe das Geschichtenbuch oder das Sachbuch an, das Bea sucht.', fr: 'Tape le livre d’histoire, ou le livre documentaire, que Bea te demande.', es: 'Toca el libro que Bea te pide: un cuento o cosas de verdad.', pt: 'Toque no livro que a Bea pediu: uma história ou um livro de verdade.', it: 'Tocca il libro di storie, o quello dei fatti, che Bea ti chiede.', nl: 'Lees de titels en tik op het juiste boek.' },
      promptStory: { en: 'Which book tells a make-believe STORY?', de: 'Welches Buch erzählt eine ausgedachte Geschichte?', fr: 'Quel livre raconte une HISTOIRE inventée ?', es: '¿Qué libro es un cuento?', pt: 'Qual livro conta uma história inventada?', it: 'Quale libro racconta una STORIA inventata?', nl: 'Welk boek vertelt een verzonnen verhaal?' },
      promptFact: { en: 'Which book gives you real FACTS?', de: 'Welches Buch erklärt dir echte Dinge?', fr: 'Quel livre donne de VRAIES infos ?', es: '¿Qué libro te explica cosas de verdad?', pt: 'Qual livro ensina coisas de verdade?', it: 'Quale libro insegna cose VERE?', nl: 'In welk boek staan echte feiten?' },
      beaIntro: { en: 'Story books tell a tale. Fact books teach real things!', de: 'Geschichten sind ausgedacht. Sachbücher erklären echte Dinge!', fr: 'Les livres d’histoire racontent un conte. Les livres documentaires apprennent de vraies choses !', es: 'Los cuentos son inventados. ¡Los otros te enseñan cosas de verdad!', pt: 'Histórias são inventadas. Livros de verdade ensinam!', it: 'Le storie raccontano, i fatti insegnano cose vere!', nl: 'Een verhaal is verzonnen. Feiten zijn echt!' },
      hintStory: { en: 'A story has made-up characters and a tale. Read the covers!', de: 'Eine Geschichte ist ausgedacht und hat erfundene Figuren. Lies die Buchdeckel genau!', fr: 'Une histoire a des personnages inventés et une aventure. Lis les couvertures !', es: 'Un cuento es inventado y tiene personajes. ¡Lee las portadas!', pt: 'Uma história é inventada e tem personagens. Leia as capas!', it: 'Una storia ha personaggi inventati e una trama. Leggi le copertine!', nl: 'Zoek naar iets verzonnen, zoals draken, kabouters of toveren.' },
      hintFact: { en: 'A fact book teaches you about real things. Read the covers!', de: 'Ein Sachbuch erklärt dir echte Dinge. Lies die Buchdeckel genau!', fr: 'Un livre documentaire t’apprend de vraies choses. Lis les couvertures !', es: 'Un libro de cosas de verdad te enseña cómo es el mundo. ¡Lee las portadas!', pt: 'Um livro de verdade ensina coisas reais do mundo. Leia as capas!', it: 'Un libro di fatti insegna cose vere. Leggi le copertine!', nl: 'Zoek naar echte dingen, zoals het woord echt of alles over.' },
      /* es §A.13.54: `win` shows on EVERY round → anchored on «el libro», a fixed masculine noun
         ...
         IS the protection. */
      win: { en: 'Yes! You found the right kind of book. 🐛', de: 'Ja! Du hast das richtige Buch gefunden. 🐛', fr: 'Oui ! Tu as trouvé le bon type de livre. 🐛', es: '¡Sí! Encontraste el libro correcto. 🐛', pt: 'Isso! Você achou o livro certo. 🐛', it: 'Sì! Hai trovato il libro giusto. 🐛', nl: 'Ja! Je vond het juiste soort boek. 🐛' }
    },
    defaults: {},
