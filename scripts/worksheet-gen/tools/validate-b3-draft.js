#!/usr/bin/env node
/**
 * validate-b3-draft.js <locale> [path] [--no-probe] — the refuse-don't-guess gate for
 * a nt20-C native-panel draft (default path i18n/.draft-b3-<locale>.json).
 *
 * Draft shape (docs/worksheet-gen/b3-panel-brief.md):
 *   { locale,
 ...
 *              theme candidates of tools/gen-b3-waves.js) against the draft's blocks in a
 *              temp locales dir (B3_LOCALES_DIR) — a spec that refuses must be DECLARED in
 ...
 *
 * Poison-tested by tools/validate-b3-draft.test.js (an EN-derived draft PASSES; one
...
/** Writes the draft's blocks to a temp locales dir and runs tools/b3-probe-child.js there. */
function buildProbe(loc, draft) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'b3draft-'));
  for (const [b, block] of Object.entries(draft.banks || {})) fs.writeFileSync(path.join(dir, `${b}.${loc}.json`), JSON.stringify(block));
  const out = execFileSync(process.execPath, [path.join(__dirname, 'b3-probe-child.js'), loc], {
    env: { ...process.env, B3_LOCALES_DIR: dir }, encoding: 'utf8', maxBuffer: 64 * 1024 * 1024, cwd: WG,
  });
...
if (require.main === module) {
  ...
  const p = process.argv[3] && !process.argv[3].startsWith('--') ? process.argv[3] : path.join(WG, 'i18n', `.draft-b3-${loc}.json`);
  if (!loc) { console.error('usage: validate-b3-draft.js <locale> [path] [--no-probe]'); process.exit(2); }
