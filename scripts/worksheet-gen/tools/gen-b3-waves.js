#!/usr/bin/env node
/**
 * gen-b3-waves.js — nt20-C wave files, two per locale:
 *   waves/wave-b3-<loc>.json     the 20 base specs
 *   waves/wave-b3var-<loc>.json  the 100 variation faces
 *
 *   node tools/gen-b3-waves.js [--locales=en,de,…] [--dry-run]
 *
 ...
 * every apply-b3-locale (Phase 4) — until then those waves carry only what builds.
 */
'use strict';
const fs = require('fs');
...
const PREF = {
  'K-318': ['animals', 'around the house', 'clothing', 'At the Supermarket', 'zoo animals', 'toys'],
  'K-320': ['animals', 'zoo animals', 'farm animals', 'pets', 'vehicles', 'toys', 'fruits'],
  'K-324': ['animals', 'fruits', 'vehicles', 'toys', 'zoo animals', 'farm animals', 'clothing'],
  'G1-305': ['animals', 'zoo animals', 'around the house', 'clothing', 'At the Supermarket', 'toys', 'farm animals'],
  'G1-308': ['animals', 'zoo animals', 'farm animals', 'fruits', 'toys', 'vehicles'],
  'G2-318': ['forest creatures', 'farm animals', 'zoo animals', 'pets', 'animals'],
  'G2-319': ['pets', 'fruits', 'toys', 'vehicles', 'farm animals', 'zoo animals'],
  'G3-377': ['animals', 'fruits', 'zoo animals', 'toys', 'farm animals', 'vehicles'],
};
const HARD = {
  'G1-318': ['animals', 'zoo animals', 'farm animals', 'pets', 'vehicles', 'dinosaurs', 'birds 2', 'forest creatures'],  // racers
  'G1-313': ['around the house', 'animals', 'clothing', 'At the Supermarket'],   // tiers = texBoundary pool (en: house only)
  'G1-324': ['animals', 'zoo animals', 'farm animals'],                         // syllable cards: en/da pool exactly 8/9 on animals
  'G2-318': ['forest creatures'],                                              // base: exemplar hedgehog lives on forest creatures
  'G2-339': ['forest creatures'], 'G2-340': ['forest creatures'], 'G2-341': ['forest creatures'],
  'G2-342': ['forest creatures', 'farm animals', 'zoo animals'], 'G2-343': ['forest creatures', 'farm animals', 'zoo animals'],
  'G3-379': ['pets', 'toys', 'vehicles', 'farm animals'],                     // two-attr: attr2 = fruits (never fruits itself)
};
...
function writeWave(id, locale, m) {
  ...
  const plan = {
    ...
    _refused: m.refused,
    _note: 'nt20-C (generated by tools/gen-b3-waves.js — every themed type pinned by a MEASURED build; _refused = specs that build on no candidate in this locale, dropped from the wave and recorded, never padded)',
  };
...
if (bases.length !== 20 || faces.length !== 100) throw new Error(`expected 20 bases + 100 faces on disk, found ${bases.length} + ${faces.length}`);
