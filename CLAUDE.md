...
2. **deploy.sh — retain recent builds' chunks so the 404 never happens.** After the swap (replacing the old `rm -rf .next-old`), snapshot THIS build's pristine chunks to `.next/static-archive/<BUILD_ID>/` (inside `.next/` → gitignored, never served), merge the previous `KEEP-1` generations into the live `.next/standalone/.next/static/` with `cp -rn` (current build wins), prune to `KEEP=5`. Bounded (snapshot taken before merge; live rebuilt pristine each swap → no accumulation). Static is served from disk per request, so no extra pm2 restart is needed. A WARN-only post-deploy step (after blog cache warming, before the Google ping) curls `/en/auth/signin` + `/en`, extracts `/_next/static/*.{js,css}`, and reports any non-200 (never `exit 1` — `set -e` is active and the swap already happened).
