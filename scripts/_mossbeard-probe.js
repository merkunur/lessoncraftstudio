#!/usr/bin/env node
/* =====================================================================
   _mossbeard-probe.js — multi-state control-bottom probe for "Two Moons"
   (RL.1.9, clarity-first redesign of #68). Drives a same round + a diff round
   + a wrong-tap + resolved, and MEASURES the lowest control INCLUDING the
   shell `.lcs-activity-check` across 320·360·412·768·1024·1366, plus tap ≥44px
   on the statement cards, no overlap, no overflow.
   ===================================================================== */
'use strict';
...
(async () => {
  ...
  const solve = (ok) => page.evaluate((correct) => {
    const t = window.TwoTalesActivity, r = t._round, C = window.TwoTalesCore;
    const oi = C.oracle(r, t._stories), idx = correct ? oi : ((oi + 1) % r.cardTags.length);
    const b = Array.from(document.querySelectorAll('.tt-cand')).find(x => +x.getAttribute('data-ci') === idx); if (b) b.click();
  }, ok).then(() => sleep(50));

  const states = {
    'same': async () => { await force('tt-s1'); },
    'diff': async () => { await force('tt-d1'); },
    'wrong': async () => { await force('tt-s2'); await solve(false); },
    'resolved': async () => { await force('tt-s3'); await solve(true); }
  };

  async function controlBottom() {
    return await page.evaluate(() => {
      const sel = '.tt-root,.tt-tales,.tt-tale,.tt-say,.tt-strip,.tt-cand,.tt-hear,.tt-line-msg,.lcs-activity-check';
  ...
  async function checks() {
    return await page.evaluate(() => {
      const taps = Array.from(document.querySelectorAll('.tt-cand'));
  ...
  if (!(await page.evaluate(() => window.TwoTalesActivity._resolved))) fails.push('a correct tap did NOT resolve');
  await states.wrong();
  if (await page.evaluate(() => window.TwoTalesActivity._resolved)) fails.push('a wrong card resolved — must not advance');

  const order = ['same', 'diff', 'wrong', 'resolved'];
  ...
  console.log('');
  if (fails.length) { console.error(`MOSSBEARD-PROBE FAILED — ${fails.length}:`); fails.forEach(f => console.error('  • ' + f)); process.exit(1); }
  console.log(`MOSSBEARD-PROBE PASSED — same/diff/wrong/resolved: a correct tap resolves + a wrong card does not advance; lowest control (incl. shell Check) clears the fold by ≥${MARGIN}px across 320→1366; cards ≥${CTRL_MIN}px (off the 320 sub-floor), no overlap, no horizontal overflow.`);
  process.exit(0);
