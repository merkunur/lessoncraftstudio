...
(function (global) {
  ...
  function gooseSVG() {
    return '<svg class="gfs-goose-svg" viewBox="0 0 100 100" role="img" aria-label="' + (LANG === 'fr' ? 'Gabby l’oie' : LANG === 'de' ? 'Gabby die Gans' : LANG === 'es' ? 'Gabby la gansa' : LANG === 'pt' ? 'Gabby, a gansa' : LANG === 'it' ? 'Gabby, la papera' : LANG === 'nl' ? 'Gabby de gans' : 'Gabby the goose') + '">' +
  ...
  global.GabbySayingsActivity = {
    ...
    strings: {
      ...
      title: { en: "Gabby's Funny Sayings", de: 'Gabbys Redewendungen', fr: 'Les drôles d’expressions de Gabby', es: 'Las expresiones de Gabby', pt: 'As expressões da Gabby', it: 'I modi di dire di Gabby', nl: 'De grappige uitdrukkingen van Gabby' },
      ...
      prompt: { en: 'What does the saying really mean?', de: 'Was bedeutet die Redewendung wirklich?', fr: 'Que veut vraiment dire cette expression ?', es: '¿Qué quiere decir en realidad esta expresión?', pt: 'O que a expressão quer dizer de verdade?', it: 'Che cosa vuol dire davvero?', nl: 'Wat betekent de uitdrukking echt?' },
      gabbyIntro: { en: 'Sayings are tricky — what does this one REALLY mean?', de: 'Schnatter! Manche Sätze meinen etwas ganz anderes, als sie sagen.', fr: 'Coin coin ! Certaines expressions ne veulent pas dire ce qu’elles disent…', es: '¡Soy Gabby! Digo frases chistosas que no significan lo que parecen.', pt: 'Algumas frases dizem uma coisa, mas querem dizer outra!', it: 'I modi di dire sono buffi: che cosa vuol dire?', nl: 'Hoi, ik ben Gabby! Wat betekent deze zin echt?' },
      theAsk: { en: 'Tap the real meaning.', de: 'Tippe die echte Bedeutung an.', fr: 'Tape le vrai sens.', es: 'Toca el significado verdadero.', pt: 'Toque no sentido de verdade.', it: 'Tocca il significato vero.', nl: 'Tik de echte betekenis aan.' },
      hintPick: { en: 'It does not mean the words exactly — tap the real meaning!', de: 'Es meint nicht genau die Wörter – tippe die echte Bedeutung an!', fr: 'Ce n’est pas le sens des mots exactement — tape le vrai sens !', es: 'No significa las palabras exactas: ¡toca el significado verdadero!', pt: 'Não é bem o que as palavras dizem — toque no sentido de verdade!', it: 'Non vuol dire le parole esatte: tocca il significato vero!', nl: 'Het betekent niet precies de woorden — tik de echte betekenis aan!' },
      ...
      hintWrong: { en: "That is what the words say, but not what it means — think again.", de: 'Das sagen die Wörter, aber so ist es nicht gemeint – denk noch mal nach.', fr: 'Ça, c’est ce que disent les mots, mais pas ce que ça veut dire — réfléchis encore.', es: 'Eso es lo que dicen las palabras, pero no lo que significa. Piénsalo otra vez.', pt: 'É isso que as palavras dizem, mas não é o que significa — pense de novo.', it: 'Questo dicono le parole, ma non è il vero significato: prova ancora.', nl: 'Dat zeggen de woorden wel, maar zo is het niet bedoeld — denk nog eens na.' },
      win: { en: 'Yes! That is what it really means. 🪿', de: 'Ja! Genau das bedeutet es wirklich. 🪿', fr: 'Oui ! C’est bien ce que ça veut dire. 🪿', es: '¡Sí! Eso es lo que significa de verdad. 🪿', pt: 'Isso! É isso que quer dizer de verdade. 🪿', it: 'Sì! Vuol dire proprio questo. 🪿', nl: 'Ja! Dat betekent het echt. 🪿' }
    },
    defaults: {},
    ...
    render: function () {
      ...
      var sp = api.el('button', 'gfs-spk'); sp.type = 'button'; sp.setAttribute('aria-label', LANG === 'fr' ? 'écouter l’expression' : LANG === 'de' ? 'die Redewendung anhören' : LANG === 'es' ? 'escuchar la expresión' : LANG === 'pt' ? 'ouvir a expressão' : LANG === 'it' ? 'ascolta il modo di dire' : LANG === 'nl' ? 'de uitdrukking beluisteren' : 'hear the saying'); sp.textContent = '🔊';
