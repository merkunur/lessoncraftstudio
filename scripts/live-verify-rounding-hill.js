/* =====================================================================
   LIVE VERIFY — TOOL #52 on production
   =====================================================================
   ...
   Run: node scripts/live-verify-rounding-hill.js
   ===================================================================== */
'use strict';
const fs = require('fs');
...
const T = require(path.join(__dirname, '..', 'mini tools', 'rounding-hill.js'));
...
(async () => {
  ...
  /* ---- 0. the DEPLOYED BYTES are this build ---------------------- */
  {
    ...
    const r = await p.goto(BASE + '/mini-tools/rounding-hill.js?lv=' + Date.now(), { waitUntil: 'domcontentloaded' });
    ok(r.status() === 200, '0 the tool script is not 200 (' + r.status() + ')');
    const live = await r.text();
    const local = fs.readFileSync(path.join(__dirname, '..', 'mini tools', 'rounding-hill.js'), 'utf8');
    ok(live.replace(/\r\n/g, '\n') === local.replace(/\r\n/g, '\n'),
  ...
  const L = require(path.join(__dirname, '_rounding-hill-landing.js'));
  for (const loc of LOCALES) {
    ...
    ok(!!iframe && iframe.indexOf('rounding-hill') >= 0, '1 ' + loc + ' landing has no tool iframe');
    await p.close();
  }
  ...
  /* ---- 2. DRIVE THE APPARATUS on production --------------------- */
  {
    ...
    await p.goto(BASE + "/mini-tools/rounding-hill.html?lang=en&cb=" + Date.now(), { waitUntil: "domcontentloaded" });
    await new Promise(r => setTimeout(r, 1100));
    const read = () => {
      const ar = document.querySelector(".rnh-field") || document.querySelector(".rnh-arena");
      if (!ar) return null;
      ...
      const st = document.querySelector(".rnh-stone"), rg = document.querySelector(".rnh-ridge");
      return { x: fx(st), txt: st ? st.textContent : null,
        ...
        groundD: (document.querySelector(".rnh-ground") || {}).getAttribute
          ? document.querySelector(".rnh-ground").getAttribute("d").length : 0 };
    };
  ...
  {
    const p = await b.newPage();
    const r = await p.goto(BASE + '/mini-tools/tool-previews/rounding-hill.webp?cb=' + Date.now(), { waitUntil: 'domcontentloaded' });
