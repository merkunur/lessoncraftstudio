...
/* =====================================================================
   ...
   (CCSS L.1.1.c). Serves `mini tools/`; drives the real shell:
     • a round renders the sentence-with-blank + 3 verb cards (am/is/are);
     • tapping the agreeing verb → shell Check → celebrate (blank fills);
     • tapping a wrong verb → tryagain, NO card marked (no leak);
...
(async () => {
  ...
  const url = `http://127.0.0.1:${PORT}/vera-verb-match-activity.html?lang=en&activity=${ACTIVITY}&embed=1`;
  ...
  const correctId = () => page.evaluate(() => window.BeAgreementCore.oracle(window.VeraVerbMatchActivity.round));
  const wrongId = () => page.evaluate(() => { const o = window.BeAgreementCore.oracle(window.VeraVerbMatchActivity.round); return [0, 1, 2].filter(i => i !== o)[0]; });
  const tap = (id) => page.evaluate((x) => { const b = document.querySelector('.vvm-opt[data-id="' + x + '"]'); if (b) b.click(); }, id).then(() => sleep(40));
  ...
  try {
    ...
    note(title === "Vera's Verb Match", `header title "${title}"`);

    const Np = await page.evaluate(() => window.VeraVerbMatchActivity._pool.length);
    note(Np >= 8, `only ${Np} rounds (<8)`);
    const ids = await page.evaluate((count) => { const t = window.VeraVerbMatchActivity, out = []; for (let i = 0; i < count; i++) { const x = t.nextTask({ index: i }); out.push(x ? x.id : null); } return out; }, 2 * Np);
    ...
    note(await page.$$eval('.vvm-opt', els => els.length) === 3, 'did not render 3 verb cards');

    await tap(await correctId()); await check();
    note(await celebrated(), 'the agreeing verb did not celebrate');
    note(await page.$('.vvm-blank.vvm-filled') != null, 'the blank did not fill on select');

    await force('dog');
    await tap(await wrongId()); await check();
    note(await triedAgain(), 'a wrong verb did not show try-again');
    note(!(await celebrated()), 'a wrong verb celebrated (must not)');
    ...
    await tap(await correctId()); await check();
    note(await celebrated(), 'the agreeing verb did not celebrate after the wrong attempt');

    const firsts = [];
    for (const id of ['ready', 'dog', 'dogs', 'park', 'happy']) { await force(id); firsts.push(await firstCardText()); }
    ...
    console.log(`  ${fails.length ? 'FAIL' : 'ok  '} vera-verb-match/en — "${title}"`);
  } catch (e) {
    fails.push('vera-verb-match/en: ' + e.message);
    console.log(`  FAIL vera-verb-match/en — ${e.message}`);
  ...
  console.log('VERA-VERB-MATCH LOCAL TEST PASSED — sentence + 3 verb cards; the agreeing verb celebrates + fills the blank; wrong = try-again with NO card marked (no leak); cards shuffle; tap-to-deselect; ≥8 distinct + reshuffle; no overflow 280→768.');
  process.exit(0);
