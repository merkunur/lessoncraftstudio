...
(function (global) {
  ...
  function numWord(n) { return LANG === 'de' ? numWordDE(n) : LANG === 'fr' ? numWordFR(n) : LANG === 'es' ? numWordES(n) : enWord(n); }
  ...
  function boltSVG(mood) {
    ...
    return '<svg viewBox="0 0 100 100" role="img" aria-label="' + (LANG === 'de' ? 'Bündel-Bolt, der Roboter' : LANG === 'fr' ? 'Bolt le robot' : LANG === 'es' ? 'Bolt, el robot' : 'Bundle Bot') + '">'
  ...
  global.BundleBotActivity = {
    ...
    strings: {
      title: { en: 'Bundle Bot', de: 'Bündel-Bolt', fr: 'Bolt fait des dizaines', es: 'Bolt agrupa decenas' },
      ...
      prompt: { en: 'Bundle the tens!', de: 'Bündle die Zehner!', fr: 'Groupe les dizaines !', es: '¡Agrupa las decenas!' },
      make: { en: 'Make {n}!', de: 'Mach {n}!', fr: 'Fabrique {n} !', es: 'Forma {n}!' },
      tensLab: { en: 'Tens', de: 'Zehner', fr: 'Dizaines', es: 'Decenas' },
      trayLab: { en: 'Loose ones', de: 'Lose Einer', fr: 'Unités seules', es: 'Unidades sueltas' },
      feed: { en: 'Drop a one', de: 'Einer hineinlegen', fr: 'Mettre une unité', es: 'Suelta una unidad' },
      bundle: { en: 'Bundle ten! 🔧', de: 'Zehn bündeln! 🔧', fr: 'Grouper dix ! 🔧', es: '¡Agrupa diez! 🔧' },
      tidy: { en: 'Tidy ⊞', de: 'Ordnen ⊞', fr: 'Ranger ⊞', es: 'Ordenar ⊞' },
      untidy: { en: 'Scatter', de: 'Verstreuen', fr: 'Éparpiller', es: 'Desordenar' },
      qBuild: { en: 'Count ten ones, then bundle!', de: 'Zähle zehn Einer und bündle dann!', fr: 'Compte dix unités, puis groupe une dizaine !', es: '¡Cuenta diez unidades y agrúpalas!' },
      qUnbundle: { en: 'Show {n} as {t} tens and {o} ones — tap a ten to un-bundle!', de: 'Zeig {n} als {t} Zehner und {o} Einer — tippe einen Zehner an, um ihn zu entbündeln!', fr: 'Montre {n} avec {t} dizaines et {o} unités — touche une dizaine pour la défaire !', es: 'Muestra {n} como {t} decenas y {o} unidades: ¡toca una decena para desagrupar!' },
      qImpostor: { en: 'Is that ten? Count carefully!', de: 'Sind das wirklich zehn? Zähle genau!', fr: 'Ça fait vraiment dix ? Compte bien !', es: '¿Eso es diez? ¡Cuenta con cuidado!' },
      qDecade: { en: 'Make {n} — all tens, no loose ones!', de: 'Mach {n} — nur Zehner, keine losen Einer!', fr: 'Fabrique {n} — que des dizaines, aucune unité seule !', es: 'Forma {n}: ¡puras decenas, sin unidades sueltas!' },
      qReadState: { en: 'Bolt has {v} already — add ones to make {n}!', de: 'Bolt hat schon {v} — lege Einer dazu, um {n} zu machen!', fr: 'Bolt a déjà {v} — ajoute des unités pour faire {n} !', es: 'Bolt ya tiene {v}: ¡agrega unidades para formar {n}!' },
      qOverfill: { en: 'Drop ones and bundle each ten!', de: 'Lege Einer hinein und bündle jeden Zehner!', fr: 'Mets les unités et groupe chaque dizaine !', es: '¡Suelta unidades y agrupa cada decena!' },
      refuse: { en: "Not ten yet — keep counting!", de: 'Noch keine zehn — zähl weiter!', fr: 'Pas encore dix — continue de compter !', es: 'Todavía no son diez… ¡sigue contando!' },
      overfill: { en: "Whoa, more than ten — bundle a ten first!", de: 'Oha, mehr als zehn — bündle zuerst einen Zehner!', fr: 'Oh là, plus de dix — groupe d’abord une dizaine !', es: '¡Uy, son más de diez! Primero agrupa una decena.' },
      unbundled: { en: 'Un-bundled! Ten ones again.', de: 'Entbündelt! Wieder zehn Einer.', fr: 'Défait ! De nouveau dix unités seules.', es: '¡Desagrupada! Otra vez diez unidades.' },
      bundled: { en: 'Ten ones make one ten! 🔧', de: 'Zehn Einer ergeben einen Zehner! 🔧', fr: 'Dix unités font une dizaine ! 🔧', es: '¡Diez unidades hacen una decena! 🔧' },
      win: { en: '{w} — all bundled! 🧰', de: '{w} — alles gebündelt! 🧰', fr: '{w} — tout est groupé ! 🧰', es: '{w}: ¡todo agrupado! 🧰' },
      tapCheck: { en: 'Tap Check! ✓', de: 'Tippe auf Prüfen! ✓', fr: 'Touche Vérifier ! ✓', es: '¡Toca Comprobar! ✓' },
      ariaBar: { en: 'a ten — tap to un-bundle into ten ones', de: 'ein Zehner — tippe, um ihn in zehn Einer zu zerlegen', fr: 'une dizaine — touche pour la défaire en dix unités', es: 'una decena: toca para desagrupar en diez unidades' },
      ariaCube: { en: 'a loose one — tap to take it back', de: 'ein loser Einer — tippe, um ihn zurückzunehmen', fr: 'une unité seule — touche pour la reprendre', es: 'una unidad suelta: toca para quitarla' }
    },
    defaults: {},
    ...
    _pull: function () {
      ...
      if (r === 'refused') { this._mood = 'sheepish'; this.msg = this.api.t('refuse'); this.api.sound && this.api.sound(330); speak(LANG === 'de' ? 'noch nicht zehn' : LANG === 'fr' ? 'Pas encore dix' : LANG === 'es' ? 'todavía no son diez' : 'not ten yet'); this.render(); return; }
      ...
      this.msg = this.api.t('bundled'); speak(LANG === 'de' ? 'Zehn Einer sind ein Zehner' : LANG === 'fr' ? 'Dix unités font une dizaine' : LANG === 'es' ? 'Diez unidades hacen una decena' : 'Ten ones make one ten');
      if (Core.isSolved(this.cstate)) { this._win(); return; }
      this.render();
    },
    _unbundle: function () {
    ...
    _win: function () {
      ...
      var wd = numWord(whole); if (LANG === 'fr' || LANG === 'es') wd = wd.charAt(0).toUpperCase() + wd.slice(1);
      this.msg = api.t('win').replace('{w}', wd);
      this.api.sound && this.api.sound(940); this.render(); this.announce(this.msg);
      speak(wd + (LANG === 'de' ? ' — alles gebündelt' : LANG === 'fr' ? ' — tout est groupé' : LANG === 'es' ? ' — todo agrupado' : ' — all bundled'));
    },
