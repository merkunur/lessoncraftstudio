...
(function (global) {
  ...
  function numWord(n) { return (LANG === 'es' ? WORDS_ES : (LANG === 'de' ? WORDS_DE : (LANG === 'fr' ? WORDS_FR : (LANG === 'pt' ? WORDS_PT : (LANG === 'it' ? WORDS_IT : (LANG === 'nl' ? WORDS_NL : WORDS))))))[n | 0] || String(n); }
  ...
  var RESIDENT_FR = { Snail: 'l’escargot', Fox: 'le renard', Frog: 'la grenouille', Bun: 'le lapin', Wren: 'le roitelet', Mole: 'la taupe', Hen: 'la poule', Owl: 'la chouette', Cat: 'le chat', Duck: 'le canard', Bee: 'l’abeille', Ant: 'la fourmi', Newt: 'le triton', Toad: 'le crapaud', Pig: 'le cochon' };
  /* resident names → Spanish, definite article baked in (the win line „¡es {name}!" is gender-safe: „es" invariant + „¡Entregada!" agrees with la carta). */
  ...
  /* resident names → Dutch, definite article baked in (win line "dat is {name}" is gender-safe; diminutives are always HET). */
  var RESIDENT_NL = { Snail: 'de slak', Fox: 'de vos', Frog: 'de kikker', Bun: 'het konijntje', Wren: 'het winterkoninkje', Mole: 'de mol', Hen: 'de kip', Owl: 'de uil', Cat: 'het katje', Duck: 'de eend', Bee: 'de bij', Ant: 'de mier', Newt: 'de salamander', Toad: 'de pad', Pig: 'het varkentje' };
  ...
  function pipSVG(mood) {
    ...
    return '<svg viewBox="0 0 100 100" role="img" aria-label="' + (LANG === 'es' ? 'Pip el ratón cartero' : (LANG === 'de' ? 'Pip die Maus' : (LANG === 'fr' ? 'Pip la souris' : (LANG === 'pt' ? 'Pip, o ratinho carteiro' : (LANG === 'it' ? 'Pip, la topina postina' : (LANG === 'nl' ? 'Snor de muis' : 'Pip the mouse')))))) + '">'
  ...
  global.PipsRoundActivity = {
    ...
    strings: {
      title: { en: "Pip's Round", de: 'Pip bringt die Post', fr: 'La tournée de Pip', es: 'Pip trae el correo', pt: 'A Entrega do Pip', it: 'La posta di Pip', nl: 'Snor brengt de post' },
      instruction: { en: 'Read the number on the letter, then tap the matching house.', de: 'Lies die Zahl auf dem Brief und tippe dann auf das passende Haus.', fr: 'Lis le nombre sur la lettre, puis touche la maison qui correspond.', es: 'Lee el número de la carta y toca la casa que sea igual.', pt: 'Leia o número na carta e toque na casa certa.', it: 'Leggi il numero sulla lettera, poi tocca la casa giusta.', nl: 'Lees het getal op de brief en tik dan op het juiste huis.' },
      prompt: { en: 'Deliver the letter!', de: 'Bring den Brief!', fr: 'Distribue la lettre !', es: '¡Entrega la carta!', pt: 'Entregue a carta!', it: 'Consegna la lettera!', nl: 'Bezorg de brief!' },
      readIt: { en: 'A letter for…', de: 'Ein Brief für …', fr: 'Une lettre pour…', es: 'Una carta para…', pt: 'Uma carta para…', it: 'Una lettera per…', nl: 'Een brief voor…' },
      hearIt: { en: '🔊 Hear the number', de: '🔊 Hör die Zahl', fr: '🔊 Écoute le nombre', es: '🔊 Escucha el número', pt: '🔊 Ouvir o número', it: '🔊 Ascolta il numero', nl: '🔊 Hoor het getal' },
      qRead: { en: 'Read the number — tap that house!', de: 'Lies die Zahl – tippe auf das Haus!', fr: 'Lis le nombre — touche cette maison !', es: 'Lee el número y ¡toca la casa!', pt: 'Leia o número — toque nessa casa!', it: 'Leggi il numero — tocca quella casa!', nl: 'Lees het getal — tik op dat huis!' },
      qHear: { en: 'Listen, then tap that house!', de: 'Hör zu und tippe auf das Haus!', fr: 'Écoute, puis touche cette maison !', es: '¡Escucha y toca la casa!', pt: 'Escute e toque nessa casa!', it: 'Ascolta, poi tocca quella casa!', nl: 'Luister en tik op dat huis!' },
      hint: { en: 'Read the house numbers and find the match.', de: 'Lies die Hausnummern und finde die passende Zahl.', fr: 'Lis les numéros des maisons et trouve celui qui correspond.', es: 'Lee los números de las casas y encuentra el que sea igual.', pt: 'Leia os números das casas e ache o que combina.', it: 'Leggi i numeri delle case e trova quello giusto.', nl: 'Lees de huisnummers en zoek het juiste.' },
      wrong: { en: "That's house {r} — let's find {t}!", de: 'Das ist Haus {r} – wir suchen die {t}!', fr: 'C’est la maison {r} — cherchons le {t} !', es: 'Esa es la casa {r} — ¡buscamos el {t}!', pt: 'Essa é a casa {r} — vamos achar a {t}!', it: 'Questa è la casa {r} — cerchiamo il {t}!', nl: 'Dat is huis {r} — we zoeken de {t}!' },
      wrongNoRead: { en: "Not that one — let's find {t}!", de: 'Das ist es nicht – wir suchen die {t}!', fr: 'Pas celle-là — cherchons le {t} !', es: 'Esa no es — ¡buscamos el {t}!', pt: 'Essa não — vamos achar a {t}!', it: 'Non questa — cerchiamo il {t}!', nl: 'Dat is het niet — we zoeken de {t}!' },
      win: { en: '{w} — that\'s {name}! Delivered 📬', de: '{w} – das ist {name}! Zugestellt 📬', fr: '{w} — c’est {name} ! Distribué 📬', es: '{w} — ¡es {name}! ¡Entregada! 📬', pt: '{w} — é {name}! Entregue 📬', it: '{w} — ecco {name}! Consegnata 📬', nl: '{w} — dat is {name}! Bezorgd 📬' },
    ...
    render: function () {
      ...
      if (this.round.present === 'audio' && !this._spoken) { this._spoken = true; var self = this; setTimeout(function () { speak(LANG === 'es' ? ('Una carta para el número ' + numWord(self.round.targetValue)) : (LANG === 'de' ? ('Ein Brief für die ' + numWord(self.round.targetValue)) : (LANG === 'fr' ? ('Une lettre pour le numéro ' + numWord(self.round.targetValue)) : (LANG === 'pt' ? ('Uma carta para o número ' + numWord(self.round.targetValue)) : (LANG === 'it' ? ('Una lettera per il numero ' + numWord(self.round.targetValue)) : (LANG === 'nl' ? ('Een brief voor nummer ' + numWord(self.round.targetValue)) : ('A letter for number ' + numWord(self.round.targetValue)))))))); }, 260); }
    },
    ...
    _envelope: function () {
      ...
      if (this.round.present === 'audio') {
        ...
        hear.addEventListener('click', function () { speak(LANG === 'es' ? ('Una carta para el número ' + numWord(self.round.targetValue)) : (LANG === 'de' ? ('Ein Brief für die ' + numWord(self.round.targetValue)) : (LANG === 'fr' ? ('Une lettre pour le numéro ' + numWord(self.round.targetValue)) : (LANG === 'pt' ? ('Uma carta para o número ' + numWord(self.round.targetValue)) : (LANG === 'it' ? ('Una lettera per il numero ' + numWord(self.round.targetValue)) : (LANG === 'nl' ? ('Een brief voor nummer ' + numWord(self.round.targetValue)) : ('A letter for number ' + numWord(self.round.targetValue)))))))); });
    ...
    _map: function () {
      ...
      this.round.houses.forEach(function (h, i) {
        ...
        btn.setAttribute('data-hid', h.hid); btn.setAttribute('aria-label', (LANG === 'es' ? 'casa número ' : (LANG === 'de' ? 'Hausnummer ' : (LANG === 'fr' ? 'maison numéro ' : (LANG === 'pt' ? 'casa número ' : (LANG === 'it' ? 'casa numero ' : (LANG === 'nl' ? 'huisnummer ' : 'house number ')))))) + h.numeral);
    ...
    _deliver: function (hid, btn) {
      ...
      this.api.sound && this.api.sound(330); speak(LANG === 'es' ? ('busca el ' + numWord(t)) : (LANG === 'de' ? ('Such die ' + numWord(t)) : (LANG === 'fr' ? ('Cherche le ' + numWord(t)) : (LANG === 'pt' ? ('Vamos achar o número ' + numWord(t)) : (LANG === 'it' ? ('cerchiamo il ' + numWord(t)) : (LANG === 'nl' ? ('Zoek de ' + numWord(t)) : ("let's find " + numWord(t))))))));
      this.render();
    },
    _win: function (hid) {
      ...
      var nm = (LANG === 'es') ? (RESIDENT_ES[house.resident] || 'un amigo') : (LANG === 'de') ? (RESIDENT_DE[house.resident] || 'ein Freund') : (LANG === 'fr') ? (RESIDENT_FR[house.resident] || 'un ami') : (LANG === 'it') ? (RESIDENT_IT[house.resident] || 'un amico') : (LANG === 'pt') ? (RESIDENT_PT[house.resident] || 'um amigo') : (LANG === 'nl') ? (RESIDENT_NL[house.resident] || 'een vriend') : (house.resident || 'a friend');
      ...
      speak(numWord(n) + (LANG === 'es' ? ' — entregada' : (LANG === 'de' ? ' – zugestellt' : (LANG === 'fr' ? ' — distribué' : (LANG === 'pt' ? ' — entregue' : (LANG === 'it' ? ' — consegnata' : (LANG === 'nl' ? ' — bezorgd' : ' — delivered')))))));
    },
