/* =====================================================================
   register-unit-handle.js — wire TOOL #40 into the site
   ---------------------------------------------------------------------
   Run:  node scripts/register-unit-handle.js [--dry-run]
...
const PREV = 'lids';           /* the key this one is inserted after */
const E = require('./_unit-handle-content.js');
...
LOCALES.forEach((l) => {
  ...
  if (!e || !e.slug || !e.name || !Array.isArray(e.about) || e.about.length !== 4) {
    die(`_unit-handle-content.js is incomplete for "${l}" — run apply-unit-handle-locales.js first`);
  }
});
