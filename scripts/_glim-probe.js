#!/usr/bin/env node
/* =====================================================================
   _glim-probe.js — multi-state control-bottom probe for "Glim's Describing
   Words" (L.2.1.e, clarity-first redesign of #73). Drives an adjective +
   adverb round + a wrong-tap + resolved, and MEASURES the lowest control
   INCLUDING the shell `.lcs-activity-check` across 320·360·412·768·1024·1366,
   plus tap ≥44px on the form chips, no overlap, no overflow.
   ===================================================================== */
'use strict';
...
(async () => {
  ...
  const solve = (ok) => page.evaluate((correct) => {
    const t = window.WordclassActivity, r = t._round, C = window.WordclassCore;
    const ans = C.oracle(r), cls = correct ? ans : C.CLASSES.find(x => x !== ans);
    const b = Array.from(document.querySelectorAll('.wc-cand')).find(x => x.getAttribute('data-class') === cls); if (b) b.click();
  }, ok).then(() => sleep(50));

  const states = {
    'adjective': async () => { await force('wc-a1'); },
    'adverb': async () => { await force('wc-v1'); },
    'wrong': async () => { await force('wc-a2'); await solve(false); },
    'resolved': async () => { await force('wc-v2'); await solve(true); }
  };

  async function controlBottom() {
    return await page.evaluate(() => {
      const sel = '.wc-root,.wc-sentence,.wc-say,.wc-hear,.wc-strip,.wc-cand,.wc-line-msg,.lcs-activity-check';
  ...
  async function checks() {
    return await page.evaluate(() => {
      const taps = Array.from(document.querySelectorAll('.wc-cand'));
  ...
  if (!(await page.evaluate(() => window.WordclassActivity._resolved))) fails.push('a correct tap did NOT resolve');
  await states.wrong();
  if (await page.evaluate(() => window.WordclassActivity._resolved)) fails.push('a wrong form resolved — must not advance');

  const order = ['adjective', 'adverb', 'wrong', 'resolved'];
  ...
  console.log('');
  if (fails.length) { console.error(`GLIM-PROBE FAILED — ${fails.length}:`); fails.forEach(f => console.error('  • ' + f)); process.exit(1); }
  console.log(`GLIM-PROBE PASSED — adjective/adverb/wrong/resolved: a correct tap resolves + a wrong form does not advance; lowest control (incl. shell Check) clears the fold by ≥${MARGIN}px across 320→1366; chips ≥${CTRL_MIN}px (off the 320 sub-floor), no overlap, no horizontal overflow.`);
  process.exit(0);
