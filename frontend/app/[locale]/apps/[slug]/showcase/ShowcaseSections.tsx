  ...
  return (
    <section className="relative overflow-hidden py-16 md:py-20" style={{ background: gradient }}>
      ...
      <div className="absolute inset-0 pointer-events-none select-none" aria-hidden="true">
        <span className={`absolute text-6xl font-bold text-${accentColor}-300/30 top-[8%] left-[5%] rotate-12`}>{decorativeSymbol}</span>
        <span className={`absolute text-5xl font-bold text-${accentColor}-200/25 top-[60%] left-[12%] -rotate-6`}>{decorativeSymbol}</span>
        <span className={`absolute text-7xl font-bold text-${accentColor}-300/20 top-[15%] right-[8%] rotate-45`}>{decorativeSymbol}</span>
        <span className={`absolute text-4xl font-bold text-${accentColor}-200/30 bottom-[12%] right-[15%] -rotate-12`}>{decorativeSymbol}</span>
        <span className={`absolute text-5xl font-bold text-${accentColor}-300/25 bottom-[25%] left-[40%]`} style={{ transform: 'rotate(20deg)' }}>{decorativeSymbol}</span>
        {/* Sparkle dots */}
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className="text-center mb-12">
          <span className={`inline-block px-4 py-1.5 rounded-full text-xs font-bold tracking-widest uppercase bg-white/80 text-${accentColor}-700 shadow-sm mb-4`}>
            {badge}
          </span>
          <h2 className="text-3xl md:text-4xl font-extrabold text-gray-900 mb-3">
            {heading}
          </h2>
          <p className={`text-lg text-${accentColor}-900/70 max-w-xl mx-auto`}>
            {subheading}
          </p>
        </div>
        ...
        <div className="flex flex-col md:flex-row items-end justify-center gap-4 md:gap-6 max-w-4xl mx-auto">
          ...
          <div className="w-full md:w-1/3 md:self-end">
            <div className={`rounded-xl overflow-hidden shadow-xl bg-white border-2 border-${accentColor}-200 transform md:translate-y-8`}>
          ...
          <div className="w-full md:w-1/3">
            <div className={`rounded-xl overflow-hidden shadow-xl bg-white border-2 border-${accentColor}-200 transform md:translate-y-0`}>
        ...
        <div className="flex flex-wrap justify-center gap-3 mt-10">
          {pills.map((pill) => (
            <span
              key={pill.label}
              className={`inline-flex items-center gap-1.5 px-4 py-2 rounded-full text-sm font-semibold bg-white/90 text-${accentColor}-800 shadow-sm border border-${accentColor}-200/50`}
            >
    ...
    <section className="relative overflow-hidden py-16 md:py-20" style={{ background: gradient }}>
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className="text-center mb-10 mt-4">
          <h2 className={`text-4xl md:text-5xl font-extrabold text-${accentColor}-900 mb-3`}>
            {heading}
          </h2>
          <span className={`inline-block px-5 py-1.5 rounded-full text-sm font-bold bg-${accentColor}-600 text-white shadow-md`}>
            {tagline}
          </span>
        </div>
        ...
        <div className="flex flex-wrap justify-center gap-3 mt-8">
          {pills.map((text) => (
            <span
              key={text}
              className={`px-5 py-2 rounded-full text-sm font-semibold bg-white/80 text-${accentColor}-700 shadow-sm border border-${accentColor}-200/60`}
            >
              {text}
            </span>
          ))}
