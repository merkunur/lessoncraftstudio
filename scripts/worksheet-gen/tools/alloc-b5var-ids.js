#!/usr/bin/env node
/**
 * alloc-b5var-ids.js — the nt10-E variation-face id allocation (Phase 2).
 *
 ...
 * K-371+ · G1-381+ · G2-360+ · G3-392+ in file order. Deterministic, so twenty
 * builders working in parallel cannot collide. Writes
 * docs/worksheet-gen/b5-designs/_records/b5var-id-allocation.json and refuses if the
...
const DESIGNS = path.join(ROOT, '..', '..', 'docs', 'worksheet-gen', 'b5-designs');
const FAMILIES = ['K-368', 'K-369', 'K-370', 'G1-376', 'G1-377', 'G1-378', 'G1-379', 'G1-380', 'G2-358', 'G2-359'];
const NEXT = { K: 371, G1: 381, G2: 360, G3: 392 };
...
fs.writeFileSync(path.join(recDir, 'b5var-id-allocation.json'), JSON.stringify({ generated: new Date().toISOString().slice(0, 10), next: NEXT, faces: out }, null, 1) + '\n');
