...
(function (global) {
  ...
  global.TildyActivity = {
    ...
    strings: {
      title: { en: "Tildy's Tailor Shop", de: 'Tildys Schneiderei', fr: 'L’atelier de Tildy', es: 'El taller de Tildy' },
      instruction: { en: '', de: '', fr: '', es: '' },
      prompt: { en: 'Measure the strip.', de: 'Miss den Streifen.', fr: 'Mesure la bande.', es: 'Mide la tira.' },
      measureIt: { en: 'Measure it ✂️', de: 'Messen ✂️', fr: 'Mesurer ✂️', es: 'Medir ✂️' },
      sayWelcome: { en: 'Slide the ruler so 0 is right at the start of the strip!', de: 'Schieb das Lineal so, dass die 0 genau am Anfang des Streifens liegt!', fr: 'Fais glisser la règle pour que le 0 soit juste au début de la bande !', es: '¡Desliza la regla para que el 0 quede justo al inicio de la tira!' },
      sayWin: { en: 'Measured and wrapped! 🎀', de: 'Gemessen und eingepackt! 🎀', fr: 'Mesuré et emballé ! 🎀', es: '¡Medido y listo! 🎀' },
      sayOffShort: { en: 'The 0 wasn’t at the start — see, it reads one short. Slide it to the start!', de: 'Die 0 war nicht am Anfang — schau, jetzt misst du einen cm zu wenig. Schieb sie an den Anfang!', fr: 'Le 0 n’était pas au début — tu vois, ça mesure un cm de trop peu. Fais-le glisser jusqu’au début !', es: 'El 0 no estaba al inicio; mira, así mides un centímetro de menos. ¡Deslízala hasta el inicio!' },
      sayNotAligned: { en: 'The 0 isn’t at the start yet — line it up, then measure.', de: 'Die 0 ist noch nicht am Anfang — leg sie genau an, dann miss.', fr: 'Le 0 n’est pas encore au début — aligne-le, puis mesure.', es: 'El 0 todavía no está al inicio. Alinéalo justo ahí y luego mide.' },
      sayRead: { en: 'Great — now read the number at the far end!', de: 'Super — jetzt lies die Zahl am anderen Ende ab!', fr: 'Super — maintenant lis le nombre à l’autre bout !', es: '¡Muy bien! Ahora lee el número en el otro extremo.' },
      sayMisread: { en: 'Look again at the far end — which number?', de: 'Schau noch mal ans andere Ende — welche Zahl?', fr: 'Regarde encore l’autre bout — quel nombre ?', es: 'Mira otra vez el otro extremo, ¿qué número es?' },
      sayAgain: { en: 'Hmm — let’s look again.', de: 'Hmm — schauen wir noch mal.', fr: 'Hmm — regardons encore.', es: 'Mmm… vamos a mirar otra vez.' },
      howMany: { en: 'How many cm?', de: 'Wie viele cm?', fr: 'Combien de cm ?', es: '¿Cuántos cm?' },
      diagZero: { en: 'Started at 1', de: 'Bei 1 angefangen', fr: 'Commencé à 1', es: 'Empezó en el 1' }, diagShort: { en: 'Too short', de: 'Zu kurz', fr: 'Trop courte', es: 'Muy corta' }, diagEnd: { en: 'Wrong end', de: 'Falsches Ende', fr: 'Mauvais bout', es: 'Extremo equivocado' },
      hintCheck: { en: 'Line up 0 with the start, then read the far end.', de: 'Leg die 0 an den Anfang, dann lies das andere Ende ab.', fr: 'Aligne le 0 sur le début, puis lis l’autre bout.', es: 'Pon el 0 al inicio y luego lee el otro extremo.' }
    },
    defaults: {},
    ...
    _stageSvg: function (opts) {
      ...
      var sv = svg('svg', { viewBox: '0 0 ' + VW + ' ' + VH, class: 'td-svg', 'aria-label': (LANG === 'es' ? 'una regla y una tira para medir' : LANG === 'de' ? 'ein Lineal und ein Streifen zum Messen' : LANG === 'fr' ? 'une règle et une bande à mesurer' : 'a ruler and a strip to measure') });
    ...
    _renderSelectTool: function (root) {
      ...
      var sv = svg('svg', { viewBox: '0 0 ' + (cmX(TABLE) + MARGIN) + ' 26', class: 'td-svg td-svgstrip', 'aria-label': (LANG === 'es' ? 'una tira para medir' : LANG === 'de' ? 'ein Streifen zum Messen' : LANG === 'fr' ? 'une bande à mesurer' : 'a strip to measure') });
      ...
      (this._opts || []).forEach(function (L) {
        var b = api.el('button', 'td-opt td-optword'); b.type = 'button'; b.textContent = (LANG === 'es' ? ('regla de ' + L + ' cm') : LANG === 'de' ? ('Lineal ' + L + ' cm') : LANG === 'fr' ? ('règle de ' + L + ' cm') : (L + '-cm ruler'));
    ...
    _correct: function () {
      ...
      setTimeout(function () { speak(LANG === 'es' ? '¡Medido!' : LANG === 'de' ? 'Gemessen!' : LANG === 'fr' ? 'Mesuré !' : 'Measured!'); }, 160);
