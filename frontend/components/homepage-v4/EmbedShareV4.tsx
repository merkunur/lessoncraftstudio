...
import { Arrow, Pencil, Sparkle } from '../homepage-v3/DoodleAccents';
...
export default async function EmbedShareV4({ locale }: Props) {
  ...
  // v4: at the subscription flip, the free-embed promise names its scope
  // ("every worksheet in the CATALOG") so it can't be read as contradicting
  // the Teacher plan's own-creation share links. Pre-flip, original body.
  const t4 = await getTranslations({ locale, namespace: 'homepageV4.embed' });
  const bodyText = PRICING_PUBLIC ? t4('bodyCatalogScoped') : t('body');
