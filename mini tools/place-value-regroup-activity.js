...
(function (global) {
  ...
  function _pvrTitle(id) {
    id = id || '';
    if (/add-compose-hundred/.test(id)) return { title: { en: 'Tuck Makes a Hundred', de: 'Tuck bündelt einen Hunderter', fr: 'Tuck fait une centaine', es: 'Tuck forma una centena', pt: 'Tuck agrupa uma centena', it: 'Tuck forma un centinaio', nl: 'Tuck maakt een honderdtal' }, instruction: { en: 'Tap “Make a hundred” to bundle 10 tens, then type the total.', de: 'Tippe auf „Hunderter bündeln", um 10 Zehner zu bündeln, und tippe dann das Ergebnis ein.', fr: 'Appuie sur « Grouper une centaine » pour faire un paquet de 10 dizaines, puis écris la réponse.', es: 'Toca «Formar una centena» para agrupar 10 decenas y luego escribe el total.', pt: 'Toque em «Agrupar uma centena» para juntar 10 dezenas e depois escreva o total.', it: 'Tocca «Forma un centinaio» per unire 10 decine, poi scrivi il totale.', nl: 'Tik op «Maak een honderdtal» om 10 tientallen te bundelen en typ dan het totaal.' } };
    if (/subtract-decompose-hundred/.test(id)) return { title: { en: 'Tuck Breaks a Hundred', de: 'Tuck entbündelt einen Hunderter', fr: 'Tuck casse une centaine', es: 'Tuck desarma una centena', pt: 'Tuck desagrupa uma centena', it: 'Tuck scompone un centinaio', nl: 'Tuck splitst een honderdtal' }, instruction: { en: 'Break a hundred, then a ten — then take some away and type the answer.', de: 'Entbündle einen Hunderter, dann einen Zehner – nimm dann welche weg und tippe das Ergebnis ein.', fr: 'Casse une centaine, puis une dizaine — enlève, puis écris la réponse.', es: 'Desarma una centena y luego una decena. Después quita algunas unidades y escribe la respuesta.', pt: 'Desagrupe uma centena e depois uma dezena. Em seguida, tire algumas unidades e escreva a resposta.', it: 'C\'è uno zero: scomponi un centinaio, poi una decina, poi togli e scrivi la risposta.', nl: 'Splits een honderdtal en dan een tiental — haal er dan een paar weg en typ het antwoord.' } };
    if (/subtract/.test(id)) return { title: { en: 'Tuck Breaks a Ten', de: 'Tuck entbündelt einen Zehner', fr: 'Tuck casse une dizaine', es: 'Tuck desarma una decena', pt: 'Tuck desagrupa uma dezena', it: 'Tuck scompone una decina', nl: 'Tuck splitst een tiental' }, instruction: { en: 'Tap “Break a ten”, then take some away and type the answer.', de: 'Tippe auf „Zehner entbündeln", nimm dann welche weg und tippe das Ergebnis ein.', fr: 'Appuie sur « Casser une dizaine », enlève, puis écris la réponse.', es: 'Toca «Desarmar una decena», quita algunas unidades y escribe la respuesta.', pt: 'Toque em «Desagrupar uma dezena», tire algumas unidades e escreva a resposta.', it: 'Tocca «Scomponi una decina», poi togli alcune unità e scrivi la risposta.', nl: 'Tik op «Splits een tiental», haal er dan een paar weg en typ het antwoord.' } };
  ...
  var _PVR_DE = {
    title:        { en: "Tuck's Ten Bundles", de: 'Tuck bündelt einen Zehner', fr: 'Tuck fait une dizaine', es: 'Tuck forma una decena', pt: 'Tuck agrupa uma dezena', it: 'Tuck fa il cambio', nl: 'Tuck maakt een tiental' },
    instruction:  { en: 'Tap “Make a ten” to bundle 10 ones, then type the total.', de: 'Tippe auf „Zehner bündeln", um 10 Einer zu bündeln, und tippe dann das Ergebnis ein.', fr: 'Appuie sur « Grouper une dizaine » pour faire un paquet de 10 unités, puis écris la réponse.', es: 'Toca «Formar una decena» para agrupar 10 unidades y luego escribe el total.', pt: 'Toque em «Agrupar uma dezena» para juntar 10 unidades e depois escreva o total.', it: 'Tocca «Fai il cambio» per cambiare 10 unità con una decina, poi scrivi il totale.', nl: 'Tik op «Maak een tiental» om 10 eenheden te bundelen en typ dan het totaal.' },
    colHundreds:  { en: 'hundreds', de: 'Hunderter', fr: 'centaines', es: 'centenas', pt: 'centenas', it: 'centinaia', nl: 'honderdtallen' },
    colTens:      { en: 'tens', de: 'Zehner', fr: 'dizaines', es: 'decenas', pt: 'dezenas', it: 'decine', nl: 'tientallen' },
    colOnes:      { en: 'ones', de: 'Einer', fr: 'unités', es: 'unidades', pt: 'unidades', it: 'unità', nl: 'eenheden' },
    makeTen:      { en: '🔁 Make a ten', de: '🔁 Zehner bündeln', fr: '🔁 Grouper une dizaine', es: '🔁 Formar una decena', pt: '🔁 Agrupar uma dezena', it: '🔁 Fai il cambio', nl: '🔁 Maak een tiental' },
    makeHundred:  { en: '🔁 Make a hundred', de: '🔁 Hunderter bündeln', fr: '🔁 Grouper une centaine', es: '🔁 Formar una centena', pt: '🔁 Agrupar uma centena', it: '🔁 Forma un centinaio', nl: '🔁 Maak een honderdtal' },
    breakTen:     { en: '🔁 Break a ten', de: '🔁 Zehner entbündeln', fr: '🔁 Casser une dizaine', es: '🔁 Desarmar una decena', pt: '🔁 Desagrupar uma dezena', it: '🔁 Scomponi una decina', nl: '🔁 Splits een tiental' },
    breakHundred: { en: '🔁 Break a hundred', de: '🔁 Hunderter entbündeln', fr: '🔁 Casser une centaine', es: '🔁 Desarmar una centena', pt: '🔁 Desagrupar uma centena', it: '🔁 Scomponi un centinaio', nl: '🔁 Splits een honderdtal' },
    hintBundleFirst:       { en: 'First tap “Make a ten” to bundle 10 ones!', de: 'Tippe zuerst auf „Zehner bündeln", um 10 Einer zu bündeln!', fr: 'Appuie d\'abord sur « Grouper une dizaine » pour faire un paquet de 10 !', es: '¡Primero toca «Formar una decena» para agrupar 10 unidades!', pt: 'Primeiro toque em «Agrupar uma dezena» para juntar 10 unidades!', it: 'Prima tocca «Fai il cambio» per cambiare 10 unità con una decina!', nl: 'Tik eerst op «Maak een tiental» om 10 eenheden te bundelen!' },
    hintMakeHundredFirst:  { en: 'First tap “Make a hundred” to bundle 10 tens!', de: 'Tippe zuerst auf „Hunderter bündeln", um 10 Zehner zu bündeln!', fr: 'Appuie d\'abord sur « Grouper une centaine » pour faire un paquet de 10 dizaines !', es: '¡Primero toca «Formar una centena» para agrupar 10 decenas!', pt: 'Primeiro toque em «Agrupar uma centena» para juntar 10 dezenas!', it: 'Prima tocca «Forma un centinaio»: 10 decine formano un centinaio!', nl: 'Tik eerst op «Maak een honderdtal» om 10 tientallen te bundelen!' },
    hintBreakFirst:        { en: 'Break a ten first — there aren’t enough ones to take away.', de: 'Entbündle zuerst einen Zehner – es sind nicht genug Einer zum Wegnehmen da.', fr: 'Il n\'y a pas assez d\'unités pour enlever. Casse d\'abord une dizaine !', es: 'Primero desarma una decena: no hay suficientes unidades para quitar.', pt: 'Desagrupe uma dezena primeiro — não há unidades suficientes para tirar.', it: 'Prima scomponi una decina: non ci sono abbastanza unità da togliere.', nl: 'Splits eerst een tiental — er zijn niet genoeg eenheden om weg te halen.' },
    hintBreakHundredFirst: { en: 'No tens to break — tap “Break a hundred” first.', de: 'Hier gibt es keine Zehner – tippe zuerst auf „Hunderter entbündeln".', fr: 'Il n\'y a pas de dizaine à casser — appuie d\'abord sur « Casser une centaine ».', es: 'No hay decenas para desarmar; primero toca «Desarmar una centena».', pt: 'Não há dezenas para desagrupar; toque primeiro em «Desagrupar uma centena».', it: 'Non ci sono decine da scomporre: tocca prima «Scomponi un centinaio».', nl: 'Er zijn geen tientallen om te splitsen — tik eerst op «Splits een honderdtal».' },
    hintBreakTenNext:      { en: 'Now tap “Break a ten” to get enough ones.', de: 'Tippe jetzt auf „Zehner entbündeln", damit du genug Einer hast.', fr: 'Casse une dizaine pour avoir 10 unités de plus, puis enlève.', es: 'Ahora toca «Desarmar una decena» para tener suficientes unidades.', pt: 'Agora toque em «Desagrupar uma dezena» para ter unidades suficientes.', it: 'Ora tocca «Scomponi una decina» per avere abbastanza unità.', nl: 'Tik nu op «Splits een tiental» zodat je genoeg eenheden hebt.' },
    hintReadTotal:         { en: 'Now count the blocks and type the total.', de: 'Zähle jetzt alle Blöcke und tippe das Ergebnis ein.', fr: 'Maintenant, compte tous les blocs et écris le total.', es: 'Ahora cuenta los bloques y escribe el total.', pt: 'Agora conte os blocos e escreva o total.', it: 'Ora conta i blocchi e scrivi il totale.', nl: 'Tel nu alle blokken en typ het totaal.' },
    srMat:        { en: '{t} tens and {o} ones', de: '{t} Zehner und {o} Einer', fr: '{t} dizaines et {o} unités', es: '{t} decenas y {o} unidades', pt: '{t} dezenas e {o} unidades', it: '{t} decine e {o} unità', nl: '{t} tientallen en {o} eenheden' }
  };
  ...
  global.PlaceValueRegroupActivity = Object.assign({}, Core, {
    ...
    lockAndCaption: function () {
      ...
      if (global.LCSAudio && global.LCSAudio.speak) {
        var op = sub ? (loc === 'fr' ? ' moins ' : loc === 'pt' ? ' menos ' : loc === 'it' ? ' meno ' : ' minus ') : (loc === 'pt' ? ' mais ' : loc === 'it' ? ' più ' : ' plus ');   /* „plus"/„minus" read by de+en TTS; fr subtract → „moins"; pt „mais"/„menos"; it „più"/„meno" */
        var verb = loc === 'fr' ? ' font ' : loc === 'de' ? ' macht ' : loc === 'pt' ? ' dá ' : loc === 'it' ? ' fa ' : ' makes ';
