 ...
 * Retrieves all train template themes and their associated images from database
 */
async function getHandler(request: NextRequest, userId: string) {
  try {
    ...
    return NextResponse.json(
      { error: 'Failed to read train templates metadata', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
  }
}
 ...
 * Updates train template metadata (translations, display names) in database
 */
async function postHandler(request: NextRequest, userId: string) {
  try {
    ...
    return NextResponse.json({
      success: true,
      message: 'Train templates metadata updated successfully in database',
      themes: updatedThemes,
    });
  } catch (error) {
    console.error('Error updating train templates metadata:', error);
    return NextResponse.json(
      { error: 'Failed to update train templates metadata', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
  }
}
...
/**
 * DELETE /api/admin/train-templates/update
 * Deletes a train template theme
 */
