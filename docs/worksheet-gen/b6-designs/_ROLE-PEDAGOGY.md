...
**D. Data + gates.** The bank shape (fenced), which existing bank it reuses, the validator rules (numbered) and ≥10 poison cases (each must FAIL; the correct draft is the control), plus what `qa/verify-b6-<key>.js` asserts on the render (floors, uniqueness, no answer printed, the 722 stack).
