...
(function (global) {
  ...
  function bearSVG() {
    return '<svg class="bgd-bear-svg" viewBox="0 0 100 100" role="img" aria-label="' + (LANG === 'de' ? 'Booker, der Bär' : LANG === 'fr' ? 'Booker l’ours' : LANG === 'es' ? 'Booker el oso' : LANG === 'pt' ? 'Booker, o urso' : 'Booker the bear') + '">' +
  ...
  global.BookerGlossaryDeskActivity = {
    ...
    strings: {
      title: { en: "Booker's Glossary Desk", de: 'Bookers Wörterbuch-Pult', fr: 'Le dictionnaire de Booker', es: 'El diccionario de Booker', pt: 'O dicionário do Booker' },
      prompt: { en: 'Which word is on this page?', de: 'Tippe das Wort an, das im ABC zwischen den beiden Leitwörtern steht.', fr: 'Quel mot est sur cette page ?', es: 'Toca la palabra que va en orden alfabético entre las dos palabras guía.', pt: 'Toque na palavra que fica, na ordem alfabética, entre as duas palavras-guia.' },
      bookerIntro: { en: 'Use the guide words! Which word comes between them?', de: 'Hallo, ich bin Booker! Im Wörterbuch stehen alle Wörter in der ABC-Reihenfolge — so finde ich jedes Wort ganz schnell.', fr: 'Bonjour, je suis Booker ! Sers-toi des mots repères pour trouver le bon mot.', es: '¡Hola, soy Booker! En el diccionario todas las palabras están en orden alfabético, así encuentro cada palabra rapidísimo.', pt: 'Use as palavras-guia! Qual palavra fica entre elas?' },
      theAsk: { en: 'Tap the word that belongs on this page.', de: 'Welches Wort steht im ABC zwischen den beiden Leitwörtern?', fr: 'Touche le mot qui se trouve sur cette page.', es: '¿Cuál palabra va en orden alfabético entre las dos palabras guía?', pt: 'Qual palavra fica na ordem alfabética entre as duas palavras-guia?' },
      hintPick: { en: 'Tap the word that comes between the guide words in ABC order!', de: 'Schau dir die beiden Leitwörter ganz oben an. Welches Wort kommt im ABC dazwischen?', fr: 'Touche le mot qui vient entre les deux mots repères dans l’ordre alphabétique !', es: 'Mira las dos palabras guía de hasta arriba. ¿Cuál palabra va en el ABC entre ellas?', pt: 'Olhe as duas palavras-guia lá em cima. Qual fica no meio, na ordem alfabética?' },
      hintWrong: { en: "Not on this page — check the guide words again, letter by letter.", de: 'Fast! Vergleiche die Wörter Buchstabe für Buchstabe — erst der erste Buchstabe, dann der zweite. Welches liegt im ABC zwischen den Leitwörtern?', fr: 'Presque ! Compare les mots lettre par lettre — d’abord la première lettre, puis la deuxième.', es: '¡Casi! Compara las palabras letra por letra: primero la primera letra, luego la segunda. ¿Cuál queda entre las dos palabras guía?', pt: 'Quase! Compare as palavras letra por letra: primeiro a primeira, depois a segunda. Qual fica entre as palavras-guia?' },
      win: { en: 'Yes! That word is on this page. 📖', de: 'Ja, super! Dieses Wort steht im ABC genau zwischen den beiden Leitwörtern.', fr: 'Oui ! Ce mot est bien sur cette page. 📖', es: '¡Sí! Esa palabra va en orden alfabético justo entre las dos palabras guía.', pt: 'Isso! Essa palavra fica na ordem alfabética bem entre as duas palavras-guia. 📖' }
    },
    defaults: {},
    ...
    render: function () {
      ...
      if (!this._spoke) { this._spoke = true; setTimeout(function () { speak(v.g1 + (LANG === 'de' ? ' bis ' : LANG === 'fr' ? ' à ' : LANG === 'es' ? ' a ' : LANG === 'pt' ? ' a ' : ' to ') + v.g2); }, 320); }
    },
