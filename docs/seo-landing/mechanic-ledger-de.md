...
| code-addition / null, secret-word | **Kodierrätsel:** a legend maps each picture→a number; child decodes the pictures, adds the numbers, writes the SUM (null) OR looks the sum up in a 2nd legend (sum→letter) to reveal a secret WORD (secret-word) | logic/decode-puzzle — the arithmetic is a key-lookup step, NOT the taught skill | **2. Klasse** (ceiling read: maxCleanNumber=40, ≤100 & >20 → 2. Klasse; 0 breaches/removals) | **RULED readiness-style: NO CCSS standard, NO framework chip** (the EN 1.OA decode-puzzle standard does NOT transfer — claiming a German numeric standard fabricates a standard-fit, the DROP-honesty principle). strand *Logisches Denken – Rechenrätsel (Vorläuferfähigkeit)* ⚠FLAG: the literal '(Vorläuferfähigkeit)' suffix reads oddly on a 2.-Klasse page; operator may drop it. mode-true per §22 (null ends at "die Summe schreiben" / secret-word at "das Lösungswort enthüllen"). **Validity gate drops the 8 non-discrete-countable themes** (colors/emotions/body_parts/weather/spring/summer/winter/activities) → 79 of 95 (EN-consistent countability gate; FLAG: code-addition images are code-symbols not counted, so those themes could land via an override — operator's call). | code-addition.html (null / word_reveal_mode) |
