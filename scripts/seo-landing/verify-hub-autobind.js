...
//
// Asserts, en.json-driven (no per-wave hardcoding):
//   (a) the wave's coords auto-bind to /worksheets/ landings,
//   (b) LEAK = 0 — no rendered deck-PAGE link whose slug has a published landing is stuck at /decks/
...
//
// Usage:
//   node scripts/seo-landing/verify-hub-autobind.js [--type=addition] [--theme=animals] [--pages=/en/topic/foo,/en/topic/bar]
const path = require('path');
const puppeteer = require('puppeteer');
const data = require(path.resolve(__dirname, '../../frontend/content/seo-landing/en.json'));
...
const PAGES = (arg('pages', '') ? arg('pages', '').split(',') : [
  `/en/topic/${TYPE}`, `/en/topic/${TYPE}?page=2`, `/en/topic/${TYPE}?page=3`, `/en/topic/${THEME}`
]).filter(Boolean);
...
const deckPageSlug = h => (h.match(/\/en\/decks\/([^/]+)\/(?:[?#].*)?$/) || [])[1];
...
(async () => {
  ...
  for (const p of PAGES) {
    ...
    const w = hrefs.filter(h => /\/en\/worksheets\/[a-z0-9-]/.test(h));
    const d = hrefs.filter(h => /\/en\/decks\//.test(h));
  ...
  const modes = [...new Set(Object.values(repointed))].sort();
  console.log(`\n/worksheets/ bound (distinct, across rendered hubs), type='${TYPE}':`);
  for (const m of modes) console.log(`  ${TYPE}-${m}: ${works.filter(h => new RegExp(`/worksheets/${TYPE}-${m}`).test(h)).length}`);

  const leaks = [...allDeckHrefs].map(deckPageSlug).filter(Boolean).filter(s => repointed[s]);
  console.log(`\nLEAK CHECK (rendered deck-PAGE /decks/ that SHOULD be /worksheets/): ${leaks.length} ${leaks.length ? 'FAIL: ' + [...new Set(leaks)].slice(0, 10).join(', ') : 'OK (zero)'}`);
