/* =====================================================================
   register-baking-tray.js — wire TOOL #46 into the site
   ---------------------------------------------------------------------
   Run:  node scripts/register-baking-tray.js [--dry-run]
...
const KEY = 'baking-tray';
const PREV = 'exchange-machine';    /* the key this one is inserted after — #45 */
const E = require('./_baking-tray-content.js');
...
LOCALES.forEach((l) => {
  const e = E[l];
  if (!e) die(`_baking-tray-content.js has no entry for "${l}"`);
...
(function () {
  ...
  /* ⭐⭐ THE CATEGORY MUST BE A MEMBER OF THE UNION, READ OFF THE SOURCE.
     This one is MULTIPLICATION, so 'number' is right — but the
