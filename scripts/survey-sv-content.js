...
for (const f of files) {
  ...
  let meta = '';
  if (m) {
    meta = m[1].slice(1, -1).replace(/\\'/g, "'").replace(/\\"/g, '"').replace(/\\u([0-9a-fA-F]{4})/g, (_, h) => String.fromCharCode(parseInt(h, 16)));
  }
...
const svBlock = tmpl.match(/\s\ssv:\s*\{[\s\S]*?\},/);
if (svBlock) {
  console.log('startCreating: ' + (svBlock[0].match(/startCreating:\s*'([^']*)'/)?.[1] || ''));
  console.log('pairedToolPrompt present: ' + /pairedToolPrompt/.test(svBlock[0]));
}
