...
(function (global) {
  ...
  function groupPhrase(v) { return LANG === 'de' ? ('viele ' + v.plural) : LANG === 'fr' ? ('un groupe ' + elideDe(v.plural)) : LANG === 'es' ? ('un grupo de ' + v.plural) : LANG === 'pt' ? ('um grupo de ' + v.plural) : ('a group of ' + v.plural); }
  function speak(word) {
    try { if (global.LCSAudio && global.LCSAudio.speak) { global.LCSAudio.speak({ type: 'word', text: word, lang: (LANG === 'pt' ? 'pt-BR' : LANG), rate: 0.95 }); return; }
      if (global.speechSynthesis && global.SpeechSynthesisUtterance) { var u = new global.SpeechSynthesisUtterance(word); u.rate = 0.95; u.lang = (LANG === 'fr' ? 'fr-FR' : LANG === 'de' ? 'de-DE' : LANG === 'es' ? 'es-MX' : LANG === 'pt' ? 'pt-BR' : 'en-US'); global.speechSynthesis.cancel(); global.speechSynthesis.speak(u); } } catch (e) {}
  }
  function shuffle(arr) { var a = arr.slice(), i, j, t; for (i = a.length - 1; i > 0; i--) { j = Math.floor(Math.random() * (i + 1)); t = a[i]; a[i] = a[j]; a[j] = t; } return a; }

  function monkeySVG() {
    return '<svg class="mag-mon-svg" viewBox="0 0 100 100" role="img" aria-label="' + (LANG === 'fr' ? 'Mango le singe' : LANG === 'es' ? 'Mango el mono' : LANG === 'pt' ? 'Mango, o macaco' : 'Mango the monkey') + '">' +
  ...
  global.MangoAnimalGroupsActivity = {
    ...
    strings: {
      title: { en: "Mango's Animal Groups", de: 'Mango der Affe', fr: 'Mango le singe', es: 'Mango el mono', pt: 'Mango, o macaco' },
      prompt: { en: 'What do we call a group of them?', de: 'Wie heißt die Gruppe?', fr: 'Comment appelle-t-on ce groupe ?', es: '¿Cómo se llama su grupo?', pt: 'Como se chama um grupo deles?' },
      mangoIntro: { en: 'Lots of animals together make a group — what is it called?', de: 'Viele Tiere zusammen sind eine Gruppe – wie heißt sie?', fr: 'Plein d’animaux ensemble, ça forme un groupe — comment ça s’appelle ?', es: 'Muchos animales juntos forman un grupo. ¿Cómo se llama?', pt: 'Muitos animais juntos formam um grupo. Como ele se chama?' },
      askTpl: { en: 'A group of {plural} is a …', de: 'Wie nennt man viele {plural}?', fr: 'Comment appelle-t-on un groupe {dePlural} ?', es: '¿Cómo se llama un grupo de {plural}?', pt: 'Um grupo de {plural} se chama…' },
      theAsk: { en: 'Tap the word for the group.', de: 'Tipp den Sammelnamen.', fr: 'Tape le bon nom de groupe.', es: 'Toca el nombre del grupo.', pt: 'Toque na palavra que dá nome ao grupo.' },
      hintPick: { en: 'Each animal has its own group word — tap your pick!', de: 'Jedes Tier hat sein eigenes Gruppenwort – tipp deine Wahl!', fr: 'Chaque animal a son mot de groupe — tape ta réponse !', es: 'Cada animal tiene su palabra de grupo. ¡Toca la tuya!', pt: 'Cada animal tem a sua própria palavra de grupo — toque na sua escolha!' },
      hintWrong: { en: "Not that one — think of the special word for this group.", de: 'Nicht ganz – denk an das besondere Wort für diese Gruppe.', fr: 'Pas tout à fait — pense au mot juste pour ce groupe.', es: 'Esa no. Piensa en la palabra especial para este grupo.', pt: 'Essa não — pense na palavra especial para este grupo.' },
      win: { en: 'Yes! That is the group word. 🐒', de: 'Genau! Das ist der Sammelname. 🐒', fr: 'Oui ! C’est bien le nom du groupe. 🐒', es: '¡Sí! Esa es la palabra del grupo. 🐒', pt: 'Isso! Essa é a palavra do grupo. 🐒' }
    },
    defaults: {},
    ...
    render: function () {
      ...
      var pic = api.el('button', 'mag-pic'); pic.type = 'button'; pic.setAttribute('aria-label', LANG === 'fr' ? ('écouter : ' + v.plural) : LANG === 'de' ? ('anhören: ' + v.plural) : LANG === 'es' ? ('escuchar: ' + v.plural) : LANG === 'pt' ? ('ouvir: ' + v.plural) : ('hear ' + v.subject.noun));
