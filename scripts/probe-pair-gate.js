...
const READ = () => {
  ...
  const arch = document.querySelector(".pgt-arch"), bar = document.querySelector(".pgt-bar");
  ...
  const m = document.querySelector(".pgt-m");
  return {
  ...
  for (const c of [{ w: 360, h: 800 }, { w: 704, h: 900 }, { w: 768, h: 1024 }, { w: 1024, h: 900 }]) {
    ...
    while (guard++ < 25) {
      const moved = await p.evaluate(() => {
        const b = document.querySelector(".pgt-b-call");
        const before = document.querySelectorAll(".pgt-yard .pgt-m").length;
        b.click();
        return before;
      });
