...
**STATUS (2026-06-07):** Planning COMPLETE. **LIVE + verified: pilot + Wave 1 + Wave 1b + Wave 2 + Wave 3** — **423 landings** (addition 144 + chart-count 43 + subtraction 180 + code-addition 44 + math-puzzle 12; an earlier draft mis-stated this as 566 — corrected). **Wave 4 (Literacy × K, first cross-strand)** authored + shipping: word-guess 47 (L.K.2.d) + find-and-count 47 (RF.K.3.a) + prepositions 50 (L.K.1.e) + bingo 18 (readiness) = 162 → 585 total. **Wave 3 (Math → Grade 1, the first grade-boundary crossing)** shipped **142 Gr1 landings** at `fe8c1b9d` (math-puzzle/mixed 12 · subtraction find-subtrahend 41 + mixed 45 · code-addition null 29 + secret-word 15); per-entry level wiring + the ≤N quantity-ceiling instrument debuted here, K-stays-Kindergarten regression PASSED. This §22 fold records the Wave-3 doctrine dividends. **NEXT: Wave 4 (Literacy × K)** — opens on operator word with its own plan-mode ledger-lock (first cross-strand wave; find-and-count rides Wave 4 by true-skill; word-guess locks there).
...
**Sequence (plan P3.b):** EN waves 1→8 — **1 LIVE** addition·image-image·K (43) → **1b LIVE** +image-number+mixed (143) → **2 LIVE** subtraction (cross-out+image-number) + chart-count·K (137) → **3 LIVE** Math re-leveled Gr1 (142: math-puzzle + subtraction find-subtrahend/mixed + code-addition null/secret-word) → **4 NEXT** Literacy·K (find-and-count→K, prepositions-mc, bingo, word-guess) → **5** Logic·K (**+ picture-sort de-orphan rides here**) → **6** Spatial → **7** Readiness-class + Preschool → **8** Gr2 (crossword, cryptogram). Then full-market locales **de → es → nl → fr → it → pt → sv → da** (operator may reorder the middle eight on market priority) → **no, fi LAST** (sample-only; honesty-filter in as their decks publish). **The picture-sort de-orphan (global theme-hub query change) rides wave 5, NOT wave 1**, with its own verify + rollback.
