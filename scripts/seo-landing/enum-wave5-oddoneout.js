#!/usr/bin/env node
/* Wave 5 coupled enum (READ-ONLY): odd-one-out/same-theme + odd-one-out/cross-theme × EN K (readiness).
 * Group by (mode, theme); canonical = earliest publishedAt; siblings = rest. slugTheme theme-derived.
 * MUST run on Hetzner. Usage: node scripts/seo-landing/enum-wave5-oddoneout.js
 */
'use strict';
...
(async function () {
  ...
  var rows = await c.deck.findMany({
    where: { language: 'en', status: 'published', exerciseType: 'odd-one-out', exerciseMode: { in: ['same-theme', 'cross-theme'] } },
