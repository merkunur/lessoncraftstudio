...
const content: AppContent = {
  seo: {
    ...
    lsiKeywords: [
      'digital matching printables passive income',
      'commercial use matching worksheet generator',
      'printable matching activity business tools',
    ],
    titleTag: 'Matching Worksheet Generator | Create & Sell Matching Activities',
    metaDescription:
      'Create professional matching worksheets to sell on Etsy, Amazon KDP, and TPT. 4 matching modes, auto answer key, 104 themes, configurable pairs. Free to try \u2014 commercial license available.',
  },
    ...
    steps: [
      {
        ...
        description:
          'Open the Page Setup panel and choose a page size: Letter Portrait, Letter Landscape, A4 Portrait, A4 Landscape, Square (1200\u00d71200), or any custom dimension. Pick a page color using the color picker as a fallback background. Select a background theme and adjust its opacity (0\u20131 in 0.05 steps), then choose a border theme with its own independent opacity control. These layout choices frame your matching worksheet before you configure any content.',
      },
      {
    ...
    features: [
      {
        ...
        description:
          'One generator covers four distinct matching activities. Beginning Letter mode (letter) places images on the left and their first letters on the right \u2014 with three sub-modes for random themes, random images from a chosen theme, or manual image selection. Image+Word mode (imgname) shows labeled image pairs on both sides for vocabulary reinforcement. Image/Word Mixed mode (imgorfname) uses per-row dropdown selectors so each side can display an image or a word independently, creating varied difficulty within a single worksheet. Custom Word mode (custom) pairs images with your own typed words for spelling practice, vocabulary quizzes, or foreign language exercises. Each mode generates a different cognitive challenge from the same image library.',
      },
      {
      ...
      },
      {
        ...
        description:
          'Browse 104 themed image collections covering animals, food, vehicles, nature, professions, holidays, sports, seasons, and dozens more. Each theme provides a coordinated set of colorful illustrations that work together in matching activities. Filter by theme using the dropdown or search for specific images by keyword. Click any image to add it to your worksheet. The Commercial tier includes 10 colorful themes for getting started; Full Access unlocks all 104 themes for maximum creative variety across all matching modes.',
      },
      {
    ...
    cases: [
      {
        ...
        description:
          'Create themed matching activity packs using the 104 image collections \u2014 animal matching, food matching, vehicle matching, holiday matching, and dozens more. Each theme provides enough illustrations for multiple unique matching worksheets across different modes. Package 10\u201320 matching worksheets per theme with answer keys included, and sell at $3\u2013$7 per bundle. Mix modes within a single bundle for variety: Beginning Letter worksheets for letter recognition, Image+Word worksheets for vocabulary, and Custom Word worksheets for spelling practice. The auto-generated answer key eliminates the biggest time sink in matching worksheet production.',
  ...
  visuals: {
    ...
    videoTitle: 'How to Create Matching Worksheets with 4 Modes and Auto Answer Keys \u2014 Step-by-Step Tutorial',
  },
};
