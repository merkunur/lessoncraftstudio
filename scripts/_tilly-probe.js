#!/usr/bin/env node
/* =====================================================================
   _tilly-probe.js — multi-state control-bottom probe for "The Faraway Shelf"
   (1.MD.A.1, clarity-first redesign of #69). Drives a longest + shortest +
   samecord round + a wrong-tap + resolved, and MEASURES the lowest control
   INCLUDING the shell `.lcs-activity-check` across 320·360·412·768·1024·1366,
   plus tap ≥44px on the answer chips, no overlap, no overflow.
   ===================================================================== */
'use strict';
...
(async () => {
  ...
  const solve = (ok) => page.evaluate((correct) => {
    const t = window.SeriationActivity, r = t._round, C = window.SeriationCore;
    const oi = C.oracle(r), idx = correct ? oi : ((oi + 1) % r.ribbons.length);
    const b = Array.from(document.querySelectorAll('.sr-cand')).find(x => +x.getAttribute('data-ci') === idx); if (b) b.click();
  }, ok).then(() => sleep(50));

  const states = {
    'longest': async () => { await force('fs-l1'); },
    'shortest': async () => { await force('fs-s1'); },
    'samecord': async () => { await force('fs-c1'); },
    'wrong': async () => { await force('fs-l2'); await solve(false); },
    'resolved': async () => { await force('fs-s2'); await solve(true); }
  };

  async function controlBottom() {
    return await page.evaluate(() => {
      const sel = '.sr-root,.sr-scene,.sr-row,.sr-say,.sr-strip,.sr-cand,.sr-line-msg,.lcs-activity-check';
  ...
  async function checks() {
    return await page.evaluate(() => {
      const taps = Array.from(document.querySelectorAll('.sr-cand'));
  ...
  if (!(await page.evaluate(() => window.SeriationActivity._resolved))) fails.push('a correct tap did NOT resolve');
  await states.wrong();
  if (await page.evaluate(() => window.SeriationActivity._resolved)) fails.push('a wrong chip resolved — must not advance');

  const order = ['longest', 'shortest', 'samecord', 'wrong', 'resolved'];
  ...
  console.log('');
  if (fails.length) { console.error(`TILLY-PROBE FAILED — ${fails.length}:`); fails.forEach(f => console.error('  • ' + f)); process.exit(1); }
  console.log(`TILLY-PROBE PASSED — longest/shortest/samecord/wrong/resolved: a correct tap resolves + a wrong chip does not advance; lowest control (incl. shell Check) clears the fold by ≥${MARGIN}px across 320→1366; chips ≥${CTRL_MIN}px (off the 320 sub-floor), no overlap, no horizontal overflow.`);
  process.exit(0);
