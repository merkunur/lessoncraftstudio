#!/usr/bin/env node
/* =====================================================================
   apply-name-sticks-fanout.js — batch-apply the 10 native-agent
   deliverables (scratchpad name-sticks-<loc>.json) to:
     1. mini tools/name-sticks.js      — strings corrections (exactly-once
        single-line rebuild per key; optional trailing comma)
     2. frontend/messages/tool-content/<loc>.json — the ToolEntry
     3. frontend/lib/manipulatives.ts  — ONE new name-sticks entry from
...
const TOOL_FILE = path.join(REPO, 'mini tools', 'name-sticks.js');
...
const deliverables = {};
for (const L of LOCS) {
  const f = path.join(SCRATCH, `name-sticks-${L}.json`);
...
console.log('\n1. strings corrections (name-sticks.js)');
...
for (const L of LOCS) {
  ...
  for (const k of Object.keys(j)) {
    if (k === 'labels') out['name-sticks'] = { slug: te.slug, name: te.name, tagline: te.tagline, metaTitle: te.metaTitle, metaDescription: te.metaDescription, about: te.about, howToUse: te.howToUse, classroomIdeas: te.classroomIdeas };
    if (k !== 'name-sticks') out[k] = j[k];
  }
  tcWrites[file] = out;
...
console.log('\n3. manipulatives entry');
const manipEN = { /*name-sticks*/
  title: 'Name Sticks',
  tagline: 'The fair jar — every name gets a turn.',
  description: 'The class roster becomes a jar of illustrated name sticks: pull one with a wooden rattle, hear the name spoken right, and pulled sticks rest in a visible cup until everyone has had a turn. Shake mode deals the class into animal-team cups.',
};
...
if (manipSrc.includes('id: "name-sticks"')) OK('manipulatives entry already present — skipping');
else {
...
console.log('\nAPPLIED — name-sticks.js, 10 tool-content files, manipulatives.ts');
