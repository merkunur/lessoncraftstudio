/* =====================================================================
   FOLD BY REFERENCE — the ten panel files into `mini tools/the-queue.js`
   ---------------------------------------------------------------------
   ...
   Cloned from `apply-shape-stretcher-locales.js` (#57), whose two parsers
   were measured against THIS tool before a line was changed: the strings
   block regex matches, and `ENTRY_RE` (six-space indent) matches 26 of 26
   declared keys with none missed. The clone is therefore structural, not
   hopeful.
   ...
   its own copy of the field list is operating on a copy.
   ...
   `_gate()` never builds — survived two rounds of review. This tool has its
   own instance of that shape already dealt with: `ariaLanded` was declared,
   reachable from NO state, and deleted before the panels were briefed. The
   applier stops and names any key that comes back.
   ...
   module cache busted and each of the 11 × 26 values compared BYTE-EXACT
   against its panel file. Three live reasons, all bought on the shelf:
     - Dutch `Eén` (the numeral) vs `een` (the article) carry acute accents
       that are LOAD-BEARING; stripping them changes the sentence.
   ...
   RUN:    node scripts/apply-the-queue-locales.js
   POISON: node scripts/apply-the-queue-locales.js --poison
   ===================================================================== */
'use strict';
...
function loadPanels() {
  ...
  LOCALES.forEach(function (l) {
    var p = path.join(ROOT, 'scripts', '_the-queue-locale-' + l + '.js');
...
/* observed PASSING. Run against COPIES in memory; the real tool and the */
/* real panel files are never touched.                                  */
/* ------------------------------------------------------------------ */
...
function poison() {
  ...
  var cases = [
    ...
    ['a value containing an ASCII apostrophe', "Un'altra estremità"],
    ['a value containing a backslash', 'a\\b'],
    ['the Dutch numeral with its acute accents', 'Eén wacht nog op het perron.'],
    ['the French right single quote U+2019', 'l’autre bout du quai.'],
    ['Nordic and Finnish diacritics', 'Välj den andra änden — påhållplatsen, laiturilla, kø.']
  ];
  ...
  /* the unknown-key refusal, both directions, without touching disk.
     ⚠ `ariaLanded` is THIS tool's `gateClose`: it was declared, reachable
     from no state in `_paint`'s dispatch, and deleted before the panels
     were briefed. If it ever comes back, a panel authoring it ten times
     is ten wasted rebuilds — so its absence is asserted here. */
  var declared = readEntries(src).entries.map(function (e) { return e.key; });
  if (declared.indexOf('ariaLanded') !== -1) { ok = false; console.log('  !! the tool declares ariaLanded again — the dead string is back'); }
  else console.log('  PASSED  the tool does not declare ariaLanded, so a panel carrying it is fatal');

  ['sayPickEnd', 'sayStepped', 'sayLandedSame', 'sayBoarded', 'sayDealt', 'sayEndOfLine'].forEach(function (k) {
    if (declared.indexOf(k) === -1) { ok = false; console.log('  !! the tool does not declare ' + k); }
  });
  console.log('  PASSED  all six say* keys are declared');

  if (declared.indexOf('ariaSame') !== -1) { ok = false; console.log('  !! the old ariaSame name survives'); }
  else console.log('  PASSED  the self-same aria key is ariaLandedSame, not ariaSame');

  /* ⚠ the CONTROL for the placeholder rule: exactly one key carries {n},
     and nothing else carries anything. A parity check over a contract
     with no tokens at all would pass vacuously. */
  var tool = loadTool();
  var withTok = declared.filter(function (k) { return /\{\w+\}/.test(tool.strings[k].en); });
  if (withTok.length === 1 && withTok[0] === 'ariaPlatform' && toks(tool.strings.ariaPlatform.en) === '{n}') {
    console.log('  PASSED  exactly one key carries a token, and it is ariaPlatform {n}');
  } else {
    ok = false;
    console.log('  !! the token contract moved: ' + JSON.stringify(withTok) + ' — update the panels before folding');
  }
