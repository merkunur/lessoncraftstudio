#!/usr/bin/env node
/* =====================================================================
   _maple-probe.js — multi-state control-bottom probe for "Maple's Bakery"
   (3.OA.A.2 division, clarity-first). Drives share / pack (initial) + a
...
(async () => {
  ...
  const solve = (ok) => page.evaluate((correct) => {
    const t = window.MapleBakeryActivity, r = t._round, Core = window.MapleBakeryCore;
    const a = Core.answerValue(r), val = correct ? a : Core.choices(r).find(c => c !== a);
    const b = Array.from(document.querySelectorAll('.mb-cand')).find(x => Number(x.textContent) === val); if (b) b.click();
  }, ok).then(() => sleep(50));

  const states = {
    'share': async () => { await force('mb-share-20-5'); },
    'pack': async () => { await force('mb-pack-20-4'); },
    'wrong': async () => { await force('mb-share-12-3'); await solve(false); },
    'resolved': async () => { await force('mb-pack-18-6'); await solve(true); }
  };

  async function controlBottom() {
    return await page.evaluate(() => {
      const sel = '.mb-root,.mb-say,.mb-board,.mb-scene,.mb-row,.mb-cand,.mb-line-msg,.lcs-activity-check';
  ...
  async function checks() {
    return await page.evaluate(() => {
      const taps = Array.from(document.querySelectorAll('.mb-cand'));
  ...
  if (!(await page.evaluate(() => window.MapleBakeryActivity._resolved))) fails.push('a correct tap did NOT resolve');
  await states.wrong();
  if (await page.evaluate(() => window.MapleBakeryActivity._resolved)) fails.push('a wrong number resolved — must not advance');

  const order = ['share', 'pack', 'wrong', 'resolved'];
  ...
  console.log('');
  if (fails.length) { console.error(`MAPLE-PROBE FAILED — ${fails.length}:`); fails.forEach(f => console.error('  • ' + f)); process.exit(1); }
  console.log(`MAPLE-PROBE PASSED — share/pack/wrong/resolved: a correct tap resolves + a wrong one does not advance; lowest control (incl. shell Check) clears the fold by ≥${MARGIN}px across 320→1366; controls ≥${CTRL_MIN}px (off the 320 sub-floor), no overlap, no horizontal overflow.`);
  process.exit(0);
