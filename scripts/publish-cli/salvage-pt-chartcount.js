#!/usr/bin/env node
/**
 * salvage-pt-chartcount.js — fix the theme/slug defect on the ~47 published pt
 * chart-count decks (SEO-recovery 2026-06-25 Part 2; clone of salvage-es-chartcount.js).
 *
 * DEFECT: all carry a wrong manifest.theme (the default label 4th_of_july → slug
 * contagem-em-grafico-4-de-julho-XXXX) + matching wrong seo_trace.themeName,
 * while their actual images span the real themes. The §15.16 reconcile gate is
 ...
 *      .value = axes.theme.<correct>.name.pt; set variant_id per collision policy
 ...
 *   3. rename <old>-vN → <new>-vN ; rewrite deck.html /pt/decks/<old>/→/<new>/
 ...
 *   node scripts/publish-cli/salvage-pt-chartcount.js --dry-run
 *   node scripts/publish-cli/salvage-pt-chartcount.js --confirm
 */
'use strict';
...
var LOCALE = 'pt';
var PREFIX = 'contagem-em-grafico';
...
function applyThemeFix(manifest, correctTheme, keepVariantId) {
  ...
  if (nm && manifest.seo_trace) {
    ['title', 'description'].forEach(function (sec) {
      ...
      if (t && typeof t === 'object') { t.value = nm; t.source = 'salvage:taxonomy.axes.theme.' + correctTheme + '.name.pt'; }
    });
  }
