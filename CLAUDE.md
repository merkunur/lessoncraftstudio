...
Standing rules from the design phase: ONE fan mechanism for non-theme axes (`unitAxis`); themeless types draw from a cross-theme picture index; boundary-printing syllable faces use only TeX-agreed entries (EN approved boundaries are often rule-only); the grapheme `chunks` layer exists only in de/nl/sv/no; refusals lower the hub-gate expectation per locale explicitly, fillers never pad a page; the deck meta lead (`seo.words.free_printable`) conflicts with the "nothing is free" ruling catalogue-wide and must be surfaced before the first b3 publish.
