#!/usr/bin/env node
/* Wave 6 enum (READ-ONLY): pattern-worksheet/(null) × EN K — the ship-or-drop fork coordinate.
 * Group by theme; canonical = earliest publishedAt; siblings = rest.
 * MUST run on Hetzner. Usage: node scripts/seo-landing/enum-wave6-patternworksheet.js
 */
'use strict';
...
(async function () {
  ...
  var rows = await c.deck.findMany({
    where: { language: 'en', status: 'published', exerciseType: 'pattern-worksheet' },
  ...
  console.log(JSON.stringify({
    ...
    sampleSlugs: valid.slice(0, 4).map(function (x) { return x.canonical; }),
