#!/usr/bin/env node
/* NL picture-sort "-vs-" PAIR enumeration (READ-ONLY). Clone of enum-sv-picturesort.js with
 * language='nl'. subjectTags[0]="X-vs-Y" combined key. Group by pairKey; canonical = earliest
 * publishedAt; siblings = rest. Parse left/right themes.
 * MUST run on Hetzner (DB). Usage: node scripts/seo-landing/enum-nl-picturesort.js > scripts/seo-landing/nl-picture-sort-coordinates.json
 */
'use strict';
...
(async function () {
  ...
  var rows = await c.deck.findMany({
    where: { language: 'nl', status: 'published', exerciseType: 'picture-sort' },
  ...
  console.log(JSON.stringify({
    locale: 'nl', exerciseType: 'picture-sort', rawRowCount: rows.length, pairCount: coords.length,
