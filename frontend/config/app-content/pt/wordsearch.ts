import type { AppContent } from '../types';

const content: AppContent = {
  seo: {
    primaryKeyword: 'criar e vender fichas de caça-palavras',
    secondaryKeywords: [
      'fichas de caça-palavras imprimíveis para vendedores Etsy',
      'gerador de grelhas de palavras ocultas para Amazon KDP',
      'vender fichas de caça-palavras no Teachers Pay Teachers',
      'fichas de caça-palavras com gabarito código de cores licença comercial',
    ],
    lsiKeywords: [
      'produtos digitais de puzzles de palavras ocultas para empreendedores',
      'negócio de fichas de sopa de letras imprimíveis online',
      'gerador de fichas de caça-palavras uso comercial',
    ],
    titleTag: 'Gerador Fichas Caça-Palavras | Criar e Vender',
    metaDescription:
      'Crie fichas de caça-palavras para Etsy, KDP e TPT. Grelha 5×5 a 30×30, direções diagonais, letras adaptadas ao idioma. Teste grátis com marca d\'água.',
  },

  hero: {
    title: 'Gerador de Fichas de Caça-Palavras para Puzzles de Palavras Ocultas em Grelha',
    tagline: 'Grelhas ajustáveis de 5×5 a 30×30 com controlos de direção diagonal e invertida — construa o nível de dificuldade perfeito para cada audiência.',
    description:
      'Crie fichas profissionais de caça-palavras onde as palavras ocultas estão incorporadas numa grelha de letras de preenchimento aleatórias e os alunos percorrem linhas, colunas e diagonais para as encontrar — o formato de puzzle clássico que nunca sai de moda. Escolha entre três fontes de conteúdo: a Biblioteca de Imagens integrada com mais de 3100 ilustrações em 104 temas, o Modo Edição Manual onde seleciona imagens e personaliza os nomes de exibição, ou o modo Lista de Palavras Personalizada para puzzles de texto apenas com até 8 palavras que escreve diretamente. Ajuste a grelha de 5×5 para aquecimentos rápidos a 30×30 para desafios sérios, e ative ou desative as palavras em diagonal e as palavras invertidas (ao contrário) para afinar a dificuldade em quatro a oito direções de colocação possíveis. Três modos de exibição — palavras mais imagens, apenas imagens ou apenas palavras — controlam como as pistas aparecem ao lado da grelha. O Acesso Completo desbloqueia os 11 idiomas suportados onde tanto as palavras ocultas COMO as letras de preenchimento da grelha mudam conforme o idioma: as grelhas em alemão incluem ä, ö, ü com frequências naturais ponderadas, as grelhas em francês incluem é, è, ç, à, as grelhas em português incluem ã, õ, ç, â, á, é, í, ó, ú, e cada idioma usa o seu conjunto de caracteres autêntico. Um gabarito com código de cores destaca cada palavra encontrada numa de seis cores alternadas para verificação visual instantânea. Exporte PDFs prontos para impressão e JPEGs a mais de 400 DPI nos formatos Carta, A4 ou dimensões personalizadas. Quer venda pacotes de puzzles temáticos na Etsy, compile livros de caça-palavras para a Amazon KDP ou crie centros de vocabulário diferenciados para o TPT, o Gerador de Fichas de Caça-Palavras produz resultados profissionais em minutos — teste grátis com todas as funcionalidades, sem registo, sem cartão de crédito. Os downloads incluem uma marca d\'água; adquira uma licença para removê-la.',
  },

  howItWorks: {
    title: 'Como Criar Fichas de Caça-Palavras em 5 Passos',
    steps: [
      {
        title: 'Configure o layout da página',
        description:
          'Abra o painel de Configuração de Página e escolha um tamanho: Carta Vertical, Carta Horizontal, A4 Vertical, A4 Horizontal ou qualquer dimensão personalizada. Selecione uma cor de fundo, escolha um tema decorativo de fundo da biblioteca de imagens e ajuste a opacidade para manter a grelha legível. Adicione uma moldura de borda combinante com controlo de opacidade independente. Estas opções de layout dão às suas fichas de caça-palavras um aspeto profissional e polido antes de adicionar uma única palavra.',
      },
      {
        title: 'Escolha a fonte de conteúdo',
        description:
          'Abra o painel de Conteúdo e selecione uma das três fontes. O modo Biblioteca de Imagens permite-lhe explorar 104 temas e selecionar imagens — o nome de cada imagem torna-se uma palavra oculta na grelha. Use o Botão de Tema Aleatório para selecionar automaticamente um tema surpresa e gerar variedade rapidamente. O Modo Edição Manual permite-lhe selecionar imagens da biblioteca e depois editar os seus nomes de exibição antes de gerar, perfeito para vocabulário simplificado ou palavras focadas em fonética. O modo Lista de Palavras Personalizada elimina as imagens e permite-lhe escrever até 8 palavras diretamente, ideal para listas de vocabulário semanais, termos específicos de uma disciplina ou prática de idiomas estrangeiros.',
      },
      {
        title: 'Configure o tamanho da grelha e as direções das palavras',
        description:
          'Defina as dimensões da grelha ajustando linhas e colunas de forma independente de 5 a 30 (predefinição 12×12). Grelhas pequenas como 5×5 ou 6×6 funcionam bem para iniciantes; grelhas grandes como 20×20 ou 30×30 criam desafios sérios. Depois ative ou desative as palavras em diagonal e as palavras invertidas (ao contrário). Com ambas ativadas, as palavras podem ser colocadas em oito direções — horizontal, vertical, diagonal e as suas inversões. Desativar as diagonais limita a colocação apenas a horizontal e vertical, criando puzzles mais fáceis para alunos mais novos ou menos experientes.',
      },
      {
        title: 'Escolha o modo de exibição e adicione toques personalizados',
        description:
          'Escolha entre três modos de exibição de palavras/imagens: Mostrar Palavras e Imagens (predefinição), Mostrar Apenas Imagens ou Mostrar Apenas Palavras. Cada modo controla o que aparece ao lado da grelha como pistas. Use o painel de Ferramentas de Texto para adicionar um título de ficha, campos de nome e data do aluno, instruções ou qualquer texto personalizado. Escolha entre sete fontes, incluindo Lexend Deca, Baloo 2, Fredoka e Quicksand. Ajuste o tamanho da fonte, a cor e o contorno para máxima legibilidade. Use o canvas Fabric.js para arrastar, redimensionar e reposicionar cada elemento.',
      },
      {
        title: 'Gere o gabarito com código de cores e descarregue',
        description:
          'Vá ao separador de Gabarito e clique em Gerar Gabarito para criar uma versão resolvida onde cada palavra oculta é destacada numa de seis cores alternadas — rosa, azul, verde, dourado, coral e lavanda — tornando cada palavra visualmente distinta. Ative a exportação em escala de cinzentos para versões económicas em tinta. Depois abra o menu de Download e exporte tanto a ficha como o seu gabarito em JPEG de alta resolução ou PDF pronto para impressão. Cada exportação é renderizada a mais de 400 DPI para uma nitidez perfeita, quer imprima em casa, carregue na Etsy ou inclua num livro de caça-palavras na Amazon KDP.',
      },
    ],
  },

  keyFeatures: {
    title: 'Características Principais do Gerador de Fichas de Caça-Palavras',
    features: [
      {
        title: 'Puzzle de palavras ocultas em grelha com pistas de imagem',
        description:
          'Cada puzzle incorpora palavras ocultas dentro de uma grelha de letras de preenchimento aleatórias. Os alunos percorrem linhas, colunas e diagonais para encontrar cada palavra — o formato clássico de caça-palavras que estimula simultaneamente a perceção visual, o reconhecimento de padrões e a recordação de vocabulário. No modo Biblioteca de Imagens ou Edição Manual, as imagens aparecem ao lado da grelha como pistas visuais, ligando o reconhecimento de imagens à identificação de palavras. O modo Lista de Palavras Personalizada cria caça-palavras puramente textuais sem imagens, alargando o formato a qualquer tema ou conjunto de vocabulário.',
      },
      {
        title: 'Tamanho de grelha personalizável de 5×5 a 30×30',
        description:
          'Ajuste linhas e colunas de forma independente de 5 a 30, criando grelhas tão compactas como 5×5 (25 casas) ou tão expansivas como 30×30 (900 casas). Grelhas pequenas criam puzzles rápidos e acessíveis, ideais para alunos mais novos ou atividades de aquecimento. Grelhas grandes criam buscas densas e desafiantes que mantêm os resolvedores experientes ocupados durante mais tempo. A grelha predefinida de 12×12 equilibra a dificuldade e o espaço de página para a maioria das audiências. Esta gama permite-lhe criar pacotes com dificuldade progressiva — desde grelhas pequenas de iniciação até desafios massivos de nível especialista — tudo com a mesma ferramenta.',
      },
      {
        title: 'Controlos de direção das palavras: ativadores de diagonal e invertida',
        description:
          'Dois ativadores independentes controlam onde as palavras podem ser colocadas na grelha. Ative as palavras em diagonal para permitir a colocação ao longo de ambos os eixos diagonais além do horizontal e vertical. Ative as palavras invertidas para permitir a colocação ao contrário em cada direção habilitada. Com ambas ativadas, as palavras podem aparecer em oito direções possíveis; com ambas desativadas, apenas se usam as colocações horizontal da esquerda para a direita e vertical de cima para baixo. Estes controlos criam uma gama de dificuldade massiva, desde grelhas simples de duas direções até puzzles complexos de oito direções, dando-lhe um controlo preciso sobre o nível de desafio.',
      },
      {
        title: 'Três fontes de conteúdo: Biblioteca de Imagens, Edição Manual e Lista de Palavras Personalizada',
        description:
          'A fonte Biblioteca de Imagens obtém as palavras de coleções de imagens temáticas — selecione animais, veículos ou alimentos e a palavra oculta é o nome da imagem. O Modo Edição Manual adiciona uma camada de personalização: selecione imagens da biblioteca e depois edite os seus nomes de exibição antes de gerar, perfeito para ortografias simplificadas ou vocabulário alternativo. O modo Lista de Palavras Personalizada elimina as imagens e permite-lhe escrever até 8 palavras diretamente, possibilitando fichas de caça-palavras para qualquer disciplina, ditados ou lista de vocabulário sem ilustrações correspondentes.',
      },
      {
        title: 'Preenchimento alfabético adaptado ao idioma com caracteres específicos',
        description:
          'As letras de preenchimento da grelha adaptam-se automaticamente ao alfabeto e à frequência de caracteres do idioma selecionado. As grelhas em alemão incluem ä, ö, ü e ß com frequências ponderadas que refletem o texto alemão natural. As grelhas em francês incluem à, â, é, è, ê, ë, î, ï, ô, ù, û e ç. As grelhas em português incluem ã, õ, ç, â, á, é, í, ó e ú com frequências que refletem o texto português natural. Cada idioma suportado obtém letras de preenchimento autênticas, tornando os puzzles linguisticamente naturais em vez de obviamente gerados em inglês. Esta funcionalidade é única de Caça-Palavras — afeta não só as palavras ocultas mas todo o fundo da grelha.',
      },
      {
        title: 'Gabarito com código de cores e 6 cores de destaque',
        description:
          'O gabarito destaca cada palavra encontrada numa de seis cores alternadas: rosa, azul, verde, dourado, coral e lavanda. Cada palavra recebe uma cor distinta para identificação visual instantânea, facilitando a autoverificação pelos alunos ou a correção rápida pelos professores. O formato com código de cores é visualmente atrativo e profissional — um passo acima dos simples gabaritos de «circule a palavra». Ative a exportação em escala de cinzentos quando necessitar de versões económicas em tinta para impressão a preto e branco.',
      },
      {
        title: 'Exportação PDF e JPEG pronta para impressão a mais de 400 DPI',
        description:
          'Descarregue fichas e gabaritos como imagens JPEG de alta resolução ou documentos PDF prontos para impressão. O motor de exportação usa um multiplicador 6x, produzindo mais de 400 DPI em tamanhos de página padrão. Os formatos Carta Vertical, Carta Horizontal, A4 Vertical, A4 Horizontal e dimensões personalizadas são todos suportados. Ative a exportação em escala de cinzentos para versões económicas em tinta, ideais para impressão na sala de aula ou livros de puzzles a preto e branco. Cada exportação está pronta para produção: listagens da Etsy, interiores da Amazon KDP e ficheiros de produtos TPT.',
      },
      {
        title: 'Edição completa do canvas com ferramentas de texto e imagem',
        description:
          'O canvas Fabric.js integrado permite-lhe arrastar, redimensionar, rodar e reposicionar cada elemento da página — texto, grelhas de puzzle, imagens, bordas e fundos. Adicione texto personalizado com sete fontes, insira elementos decorativos e use os controlos de camadas para um empilhamento preciso. As ferramentas de alinhamento posicionam os elementos ao píxel, e o bloqueio/desbloqueio protege os elementos terminados enquanto edita outros. Desfazer e refazer ilimitados permitem-lhe experimentar livremente, oferecendo a flexibilidade de uma ferramenta de design gráfico dentro de um gerador de caça-palavras.',
      },
    ],
  },

  businessUseCases: {
    title: 'Como Vender Fichas de Caça-Palavras Online',
    cases: [
      {
        title: 'Pacotes temáticos de caça-palavras na Etsy',
        description:
          'Crie conjuntos de 10 a 20 fichas de caça-palavras agrupadas por tema e dificuldade — animais da quinta numa grelha pequena 8×8 sem diagonais, criaturas marinhas numa grelha 15×15 com diagonais ativadas, dinossauros numa grelha 20×20 com palavras invertidas — e ofereça-os como download instantâneo na Etsy. Inclua os gabaritos com código de cores como ficheiros extra. O formato de grelha visual com pistas de imagem destaca-se face às simples listas de palavras nos resultados de pesquisa da Etsy. Defina o preço de pacotes temáticos individuais entre $3–$5 e mega-pacotes multi-tema com dificuldade progressiva entre $12–$18.',
        platform: 'Etsy (etsy.com)',
      },
      {
        title: 'Livros de caça-palavras na Amazon KDP',
        description:
          'Compile de 50 a 100 puzzles de caça-palavras num livro de puzzles impresso em formato Amazon KDP. Use tamanhos de grelha progressivos como curva de dificuldade: comece com grelhas 6×6 apenas horizontal, avance através de grelhas 12×12 com diagonais e termine com grelhas 25×25 com palavras invertidas ativadas. Agrupe os capítulos por tema (animais, alimentos, veículos, natureza) para uma variedade navegável. Adicione uma capa, um guia de dificuldade e uma secção de gabaritos com código de cores no final. Os livros de caça-palavras vendem-se de forma constante durante todo o ano com mínima publicidade.',
        platform: 'Amazon KDP (kdp.amazon.com)',
      },
      {
        title: 'Centros de vocabulário diferenciados para TPT',
        description:
          'Construa conjuntos de prática de caça-palavras por níveis usando o tamanho da grelha e os controlos de direção como eixos de diferenciação: Nível 1 (8×8, apenas horizontal, 4 palavras), Nível 2 (12×12, horizontal e vertical, 6 palavras), Nível 3 (16×16, diagonais ativadas, 8 palavras) e Nível 4 (20×20, diagonais e invertidas, 8 palavras). Acompanhe cada nível com gabaritos com código de cores e uma folha de acompanhamento do progresso. Os compradores do Teachers Pay Teachers pagam preços premium por recursos de vocabulário diferenciados que podem usar imediatamente em múltiplos níveis de capacidade.',
        platform: 'Teachers Pay Teachers (teacherspayteachers.com)',
      },
      {
        title: 'Produtos de caça-palavras multilingues com Acesso Completo',
        description:
          'O Acesso Completo desbloqueia os 11 idiomas, e como Caça-Palavras é sensível ao idioma, as palavras ocultas E as letras de preenchimento da grelha mudam conforme o idioma. «Dog» em inglês torna-se «cão» em português numa grelha preenchida com caracteres portugueses como ã, õ, ç, â, á; «Hund» em alemão numa grelha com ä, ö, ü; «chien» em francês numa grelha com é, è, ç, à. Cada versão linguística é um produto genuinamente diferente com palavras diferentes, letras de preenchimento diferentes e uma experiência de resolução diferente. Crie séries temáticas idênticas em múltiplos idiomas e publique cada uma como um produto de Etsy ou um livro de KDP separado — onze fluxos de receita a partir de um único fluxo de trabalho.',
        platform: 'Etsy / Amazon KDP / TPT (multilingue)',
      },
      {
        title: 'Ferramenta de vocabulário para sala de aula e explicações',
        description:
          'Use o modo Lista de Palavras Personalizada para criar fichas de caça-palavras com qualquer lista de vocabulário — palavras do ditado semanal, termos de ciências, vocabulário de estudos sociais ou prática de idiomas estrangeiros. Escreva até 8 palavras diretamente sem precisar de imagens correspondentes. Ajuste o tamanho da grelha e os ativadores de direção conforme a capacidade de cada aluno: grelhas pequenas apenas horizontal para quem tem dificuldades na leitura, grelhas grandes com diagonais e inversões para os alunos avançados. Imprima séries personalizadas em minutos para grupos pequenos ou turmas inteiras. O gabarito com código de cores poupa tempo de correção, e o formato clássico de caça-palavras mantém os alunos motivados.',
        platform: 'Sala de aula / Explicações / Ensino doméstico',
      },
    ],
  },

  faq: [
    {
      question: 'Como funciona o puzzle de caça-palavras?',
      answer:
        'As palavras ocultas estão incorporadas dentro de uma grelha de letras de preenchimento aleatórias. Os alunos percorrem linhas, colunas e opcionalmente diagonais para encontrar cada palavra. Quando encontrada, os alunos rodeiam ou destacam a palavra na grelha. As letras de preenchimento são geradas aleatoriamente cada vez que gera, por isso cada puzzle é único mesmo com a mesma lista de palavras. Podem ser colocadas até 8 palavras numa única grelha.',
    },
    {
      question: 'Que tamanhos de grelha estão disponíveis?',
      answer:
        'Pode configurar linhas e colunas de forma independente de 5 a 30. A predefinição é 12×12. Uma grelha 5×5 cria um puzzle rápido e fácil com apenas 25 casas; uma grelha 30×30 cria uma busca densa e desafiante com 900 casas. Ajustar o tamanho da grelha é a forma principal de controlar a dificuldade, juntamente com os ajustes de direção das palavras.',
    },
    {
      question: 'O que fazem os ativadores de direção diagonal e invertida?',
      answer:
        'O ativador de diagonal permite que as palavras sejam colocadas ao longo de ambos os eixos diagonais além do horizontal e vertical. O ativador de invertida permite que as palavras sejam colocadas ao contrário em cada direção habilitada. Com ambos ativados, as palavras podem aparecer em oito direções possíveis. Com ambos desativados, as palavras só aparecem da esquerda para a direita horizontalmente e de cima para baixo verticalmente. Estes ativadores permitem-lhe afinar a dificuldade desde grelhas simples de duas direções até puzzles complexos de oito direções.',
    },
    {
      question: 'Quais são as três fontes de conteúdo?',
      answer:
        'O modo Biblioteca de Imagens usa ilustrações temáticas onde o nome da imagem se torna uma palavra oculta na grelha. O Modo Edição Manual permite-lhe selecionar imagens e depois personalizar os seus nomes de exibição antes de gerar — perfeito para vocabulário alternativo ou ortografias simplificadas. O modo Lista de Palavras Personalizada elimina as imagens por completo e permite-lhe escrever até 8 palavras diretamente, possibilitando fichas de caça-palavras para qualquer disciplina sem ilustrações correspondentes.',
    },
    {
      question: 'Quais são os três modos de exibição?',
      answer:
        'Mostrar Palavras e Imagens (predefinição) apresenta tanto as pistas de imagem como as etiquetas de palavras ao lado da grelha. Mostrar Apenas Imagens apresenta as pistas de imagem sem etiquetas de texto, exigindo que os alunos identifiquem a imagem e encontrem a palavra por si mesmos. Mostrar Apenas Palavras apresenta as etiquetas de texto sem imagens, criando uma experiência de caça-palavras puramente textual. Cada modo controla o que aparece como referência de pistas junto à grelha.',
    },
    {
      question: 'Como funciona o preenchimento alfabético adaptado ao idioma?',
      answer:
        'Quando seleciona um idioma, as letras de preenchimento da grelha mudam automaticamente para corresponder ao alfabeto desse idioma com distribuições de frequência ponderadas. As grelhas em alemão incluem ä, ö, ü e ß. As grelhas em francês incluem é, è, ç, à e outros caracteres acentuados. As grelhas em português incluem ã, õ, ç, â, á, é, í, ó e ú. Isto torna os puzzles linguisticamente autênticos — as letras de preenchimento parecem naturais em vez de obviamente geradas em inglês.',
    },
    {
      question: 'Como funciona o gabarito com código de cores?',
      answer:
        'O gabarito destaca cada palavra encontrada numa de seis cores alternadas: rosa, azul, verde, dourado, coral e lavanda. Cada palavra recebe uma cor distinta para ser visualmente distinguível mesmo quando as palavras se cruzam ou sobrepõem na grelha. Pode ativar a exportação em escala de cinzentos para versões de gabarito económicas em tinta. O gabarito é gerado como um ficheiro descarregável separado.',
    },
    {
      question: 'Qual é o número máximo de palavras por puzzle?',
      answer:
        'Pode colocar até 8 palavras ou imagens numa única grelha de caça-palavras. Este limite garante que as palavras encaixam corretamente na grelha sem sobreposição excessiva. Para listas de palavras mais longas, crie várias fichas e agrupe-as como conjunto — isto também aumenta o valor percecionado dos seus pacotes de produtos.',
    },
    {
      question: 'O gerador de Caça-Palavras é sensível ao idioma?',
      answer:
        'Sim. Como as palavras ocultas são os nomes das imagens no idioma selecionado, mudar o idioma altera completamente o conteúdo do puzzle. Além disso, as letras de preenchimento da grelha mudam para corresponder ao conjunto de caracteres do idioma-alvo com frequências ponderadas adequadas. «Dog» em inglês torna-se «cão» em português numa grelha com ã, õ, ç, â, á. A Licença Comercial é apenas em inglês; o Acesso Completo desbloqueia os 11 idiomas, fazendo de cada versão linguística um produto distinto com palavras diferentes e letras de preenchimento diferentes.',
    },
    {
      question: 'Existe um teste grátis?',
      answer:
        'Sim. Pode aceder a todas as funcionalidades — as três fontes de conteúdo, os três modos de exibição, a biblioteca completa de imagens, os tamanhos de grelha ajustáveis, os controlos de direção, os gabaritos com código de cores e todos os formatos de exportação — sem criar uma conta, inserir cartão de crédito ou instalar qualquer software. Os downloads do teste grátis incluem uma pequena marca d\'água. Uma licença comercial remove a marca d\'água e concede direitos completos de venda.',
    },
    {
      question: 'Posso vender fichas criadas com esta ferramenta na Etsy e Amazon KDP?',
      answer:
        'Sim. Com uma licença comercial, tem todos os direitos para vender as suas fichas de caça-palavras como downloads digitais na Etsy, como livros de puzzles impressos na Amazon KDP, como recursos educativos no TPT ou através de qualquer outro canal de venda. Os tamanhos de grelha ajustáveis, os controlos de direção e a capacidade multilingue oferecem-lhe uma forte diferenciação em categorias de mercado muito concorridas.',
    },
  ],

  internalLinks: [
    {
      pageType: 'app',
      slug: 'trem-alfabeto-fichas',
      anchorText: 'Gerador de Fichas de Trem do Alfabeto',
    },
    {
      pageType: 'app',
      slug: 'preposicoes-fichas',
      anchorText: 'Gerador de Fichas de Preposições',
    },
    {
      pageType: 'app',
      slug: 'adivinhar-palavras-fichas',
      anchorText: 'Gerador de Fichas de Adivinhar Palavras',
    },
    {
      pageType: 'app',
      slug: 'palavras-embaralhadas-fichas',
      anchorText: 'Gerador de Fichas de Palavras Embaralhadas',
    },
    {
      pageType: 'app',
      slug: 'criptograma-imagens-fichas',
      anchorText: 'Gerador de Fichas de Criptograma com Imagens',
    },
    {
      pageType: 'bundle',
      slug: 'pacote-leitura-linguagem',
      anchorText: 'Pacote Leitura e Linguagem — Todas as Aplicações de Letras num Pacote',
    },
    {
      pageType: 'tool',
      slug: 'gerador-caca-palavras',
      anchorText: 'Criador de Fichas de Caça-Palavras',
    },
    {
      pageType: 'guide',
      slug: 'vender-material-educativo-etsy',
      anchorText: 'Guia para Vender Material Educativo na Etsy',
    },
    {
      pageType: 'start',
      slug: 'guia-completo-negocio-imprimiveis',
      anchorText: 'O Guia Completo para Iniciar um Negócio de Imprimíveis',
    },
    {
      pageType: 'idea',
      slug: 'animais-fazenda-ideias-imprimiveis',
      anchorText: 'Ideias de Imprimíveis de Animais da Quinta',
    },
  ],

  visuals: {
    heroImages: {
      primary: '/samples/english/wordsearch/wordsearch portrait.webp',
      primaryAlt: 'Ficha de caça-palavras com palavras ocultas numa grelha de letras e pistas de imagem mostrando um puzzle de vocabulário',
    },
    sampleGallery: [
      {
        src: '/samples/english/wordsearch/wordsearch portrait.webp',
        alt: 'Ficha de caça-palavras em formato vertical com grelha 12×12 e pistas de imagem temáticas',
        caption: 'Formato vertical — grelha 12×12 com pistas de imagem temáticas e lista de palavras',
      },
      {
        src: '/samples/english/wordsearch/wordsearch landscape.webp',
        alt: 'Ficha de caça-palavras em formato horizontal com grelha maior e palavras em diagonal ativadas',
        caption: 'Formato horizontal — grelha mais ampla com palavras em diagonal e invertidas ativadas',
      },
      {
        src: '/samples/english/wordsearch/custom word list.webp',
        alt: 'Ficha de caça-palavras usando o modo lista de palavras personalizada sem imagens',
        caption: 'Modo Lista de Palavras Personalizada — caça-palavras de texto apenas sem pistas de imagem',
      },
    ],
    youtubeId: '36keBFzJbPo',
    videoTitle: 'Como Criar Fichas de Caça-Palavras com Palavras Ocultas em Grelha — Tutorial Passo a Passo',
  },
};

export default content;
