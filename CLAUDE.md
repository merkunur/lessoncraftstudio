...
**STATUS (2026-06-06):** Planning COMPLETE (all phases RULED). **EN pilot LIVE** (16 landings) + **Wave 1 LIVE + verified** (43 EN `addition/image-image/K`). **ON HOLD** — no Wave 1b until the operator rules Wave 1 proven (operator live-eyeball of `/en/topic/addition` + a sample of `/en/worksheets/addition-image-image-*`).
...
- **P1-enrichment:** ≥4 P1 skeletons per template-class (6–8 for broad ~100-theme classes); cluster-mean is **skeleton-count-driven, not theme-count-driven** (the scale-risk is retired, not deferred). **Similarity gate** (`scripts/seo-landing/gate.js`) = whole-page RAW word-3-gram Jaccard, **FAIL ≥0.80 / WARN 0.65–0.80**; slot-normalization is cross-class template-collision only. Every landing body ≥200 words.
...
**Sequence (plan P3.b):** EN waves 1→8 — **1** addition·image-image·K (43) → **1b** +image-number+mixed (→144) → **2** subtraction + chart-count·K → **3** Math re-leveled Gr1 → **4** Literacy·K (find-and-count→K, prepositions-mc, bingo, word-guess) → **5** Logic·K (**+ picture-sort de-orphan rides here**) → **6** Spatial → **7** Readiness-class + Preschool → **8** Gr2 (crossword, cryptogram). Then full-market locales **de → es → nl → fr → it → pt → sv → da** (operator may reorder the middle eight on market priority) → **no, fi LAST** (sample-only; honesty-filter in as their decks publish). **The picture-sort de-orphan (global theme-hub query change) rides wave 5, NOT wave 1**, with its own verify + rollback.

**Per-wave runbook:** (1) ledger-lock any new types (source-read; interrupt on surprise); (2) if a new locale, its native ensemble authors that locale's compat booleans + P1 skeletons + native keyword research first; (3) extend the `gen-wave1.js`-style generator → author + pass `gate.js` (≥200w, 0 FAIL) + `validity-gate.js` (0 invalid); (4) ship landings (new URLs, safe) + repoint the canonical decks' `deck.html`; (5) **the conditional repoint auto-applies — no code change after wave 1**; (6) verify the **rendered DOM** — the topic deck grid is hydration-gated, so **curl shows 0 deck links (§A.13.50); use the headless-browser `scripts/seo-landing/.verify-wave1-hub.js`**, not curl; (7) STOP for operator ruling + live eyeball.

**Key files:** route `frontend/app/[locale]/worksheets/[slug]/page.tsx` · lib `frontend/lib/seo/landing-content.ts` · content `frontend/content/seo-landing/en.json` · generator `scripts/seo-landing/gen-wave1.js` · gates `scripts/seo-landing/{gate.js,validity-gate.js}` · compat `frontend/content/seo-landing/theme-mechanic-compat.json` · preview hub `frontend/app/[locale]/preview/landing-hub/page.tsx` · ledger `docs/seo-landing/mechanic-ledger-mathK.md`. **0 protected-core lines** throughout (the 5 activity cores are never touched). Commits: pilot `32e9af1e`/`941c9501`/`8158fef9`; Part-2 `822dcc54`; Wave 1 `d2f12145` (content) → `51ae864c` (conditional repoint) → `67a8ae18` (carousel-thumbnail fix).
