...
(function (global) {
  ...
  var CREATURE_NL = { fox: 'een vos', owl: 'een uil', whale: 'een walvis', deer: 'een hert', bear: 'een beer', cat: 'een poes', bird: 'een vogeltje' };
  var LANG = 'en';
  function creaturePhrase(reveal) { return LANG === 'es' ? (CREATURE_ES[reveal] || 'una estrella') : LANG === 'de' ? (CREATURE_DE[reveal] || 'einen Stern') : LANG === 'fr' ? (CREATURE_FR[reveal] || 'une étoile') : LANG === 'pt' ? (CREATURE_PT[reveal] || 'uma estrela') : LANG === 'it' ? (CREATURE_IT[reveal] || 'una stella') : LANG === 'nl' ? (CREATURE_NL[reveal] || 'een sterretje') : (CREATURE[reveal] || ['⭐', 'star'])[1]; }
  ...
  function speak(text) {
    try { if (global.LCSAudio && global.LCSAudio.speak) { global.LCSAudio.speak({ type: 'number', text: String(text), lang: (LANG === 'es' ? 'es-MX' : LANG === 'pt' ? 'pt-BR' : LANG === 'it' ? 'it-IT' : LANG), rate: 0.95 }); return; }
      if (global.speechSynthesis && global.SpeechSynthesisUtterance) { var u = new global.SpeechSynthesisUtterance(String(text)); u.rate = .95; u.lang = (LANG === 'es' ? 'es-MX' : LANG === 'de' ? 'de-DE' : LANG === 'fr' ? 'fr-FR' : LANG === 'pt' ? 'pt-BR' : LANG === 'it' ? 'it-IT' : LANG === 'nl' ? 'nl-NL' : 'en-US'); global.speechSynthesis.cancel(); global.speechSynthesis.speak(u); } } catch (e) {}
  }
  ...
  global.StarStitcherActivity = {
    ...
    strings: {
      title: { en: 'Count the Stars Awake', de: 'Zähl die Sterne wach!', fr: 'Réveille les étoiles en comptant', es: '¡Cuenta las estrellas!', pt: 'Conte e acorde as estrelas!', it: 'Conta e accendi le stelle', nl: 'Tel de sterren wakker!' },
      prompt: { en: 'Count the firefly forward!', de: 'Zähl das Glühwürmchen weiter!', fr: 'Compte pour avancer !', es: '¡Haz que la luciérnaga siga contando!', pt: 'Conte para o vaga-lume avançar!', it: 'Fai contare la lucciola in avanti!', nl: 'Tel het vuurvliegje verder!' },
      countOn: { en: 'Count on', de: 'Weiterzählen', fr: 'Compte encore', es: 'Sigue contando', pt: 'Conte mais', it: 'Conta ancora', nl: 'Tel verder' },
      wake: { en: 'Wake it!', de: 'Wecken!', fr: 'Réveille !', es: '¡Despierta!', pt: 'Acorde!', it: 'Accendila!', nl: 'Maak wakker!' },
      countHint: { en: 'Tap Count on to fly her to the next star, then Wake it!', de: 'Tippe auf „Weiterzählen", damit es zum nächsten Stern fliegt – dann auf „Wecken"!', fr: 'Touche « Compte encore » pour avancer d’une étoile, puis « Réveille ! ».', es: 'Toca «Sigue contando» para que vuele a la siguiente estrella… ¡luego toca «Despierta»!', pt: 'Toque em Conte mais para ir à próxima estrela. Depois, Acorde!', it: 'Tocca Conta ancora, vola alla stella e accendila!', nl: 'Tik op ‘Tel verder’ tot de volgende ster, dan ‘Maak wakker!’' },
      pickHint: { en: 'Tap the glowing star to start counting from there.', de: 'Tippe auf den leuchtenden Stern, um von dort weiterzuzählen.', fr: 'Touche l’étoile qui brille pour compter à partir de là.', es: 'Toca la estrella que brilla para contar desde ahí.', pt: 'Toque na estrela acesa para começar a contar dali.', it: 'Tocca la stella accesa per iniziare a contare.', nl: 'Tik op de brandende ster om daar te beginnen met tellen.' },
      setHint: { en: 'Tap each dot to count them — that is where she starts!', de: 'Tippe jeden Punkt an und zähle mit – dort fängt es an!', fr: 'Touche chaque point pour les compter — c’est là qu’elle commence !', es: 'Toca cada punto y cuenta con él… ¡ahí empieza!', pt: 'Toque em cada ponto para contá-los — é daí que o vaga-lume começa!', it: 'Tocca ogni punto per contarli: da lì lei parte!', nl: 'Tik op elke stip om ze te tellen — daar begint ze!' },
      overshoot: { en: 'Not yet — count on again from the lit star.', de: 'Noch nicht – zähl vom leuchtenden Stern noch einmal weiter.', fr: 'Pas encore — recompte depuis l’étoile allumée.', es: 'Todavía no… cuenta otra vez desde la estrella que brilla.', pt: 'Ainda não — conte de novo a partir da estrela acesa.', it: 'Non ancora: conta di nuovo dalla stella accesa.', nl: 'Nog niet — tel verder vanaf de brandende ster.' },
      win: { en: 'You woke a {c}!', de: 'Du hast {c} geweckt!', fr: 'Tu as réveillé {c} !', es: '¡Despertaste a {c}!', pt: 'Você acordou {c}!', it: 'Hai svegliato {c}!', nl: 'Je hebt {c} wakker gemaakt!' },
      hintCheck: { en: 'Count the firefly to each star and Wake it.', de: 'Zähl das Glühwürmchen zu jedem Stern und wecke ihn.', fr: 'Compte la luciole jusqu’à chaque étoile et réveille-la.', es: 'Lleva a la luciérnaga contando a cada estrella y despiértala.', pt: 'Conte o vaga-lume até cada estrela e depois Acorde!', it: 'Fai contare la lucciola a ogni stella e accendila.', nl: 'Tel het vuurvliegje naar elke ster en maak hem wakker.' },
      startHere: { en: 'Start here', de: 'Hier starten', fr: 'Commence ici', es: 'Empieza aquí', pt: 'Começar aqui', it: 'Parti da qui', nl: 'Begin hier' },
      countSetGo: { en: "That's {n} — count on!", de: 'Das sind {n} – weiterzählen!', fr: 'Ça fait {n} — continue à compter !', es: 'Van {n}… ¡sigue contando!', pt: 'Deu {n} — conte mais!', it: 'Sono {n}: conta in avanti!', nl: 'Dat zijn er {n} — tel verder!' }
    },
    defaults: {},
