#!/usr/bin/env node
/* Generic FR readiness/landing generator — the fr-fan engine (clone of gen-da-readiness.js,
 * fr ledger-lock 2026-06-15). Config-driven: reads a per-type config (fr-readiness-<type>.js) +
 * fr-<type>-coordinates.json + fr-themes + fr-render (PURE SUBSTITUTION — French plural
 * contractions derived from plIndef + vowelInit; the Danish B4/B5 definiteness guards are no-ops in
 * fr-render so the inherited pre-pass/lint calls are inert). Readiness types carry no `standard`;
 * raw l.strand = the per-type Éducation Nationale label. Standard-bearing types pass cfg.standard
 * (the EN R3 CCSS code, the invisible machine anchor; on-page chip = "Éducation Nationale").
 *
 * Config shape (fr-readiness-<type>.js):
 ...
 *   modeKey 'null' = the null exerciseMode. level may be a string or (mk)=>levelKey (band-split-by-mode).
 *   Placeholders in templates: {N_PL} {LES_PL}/{N_PL_DEF} {DES_PL} {DE_PL} {AUX_PL} {GEN} {H1};
 *   P3 uses {NB1}/{NB2} (neighbor plIndef) + {GEN} (own plIndef).
 *
 * Usage: node scripts/seo-landing/gen-fr-readiness.js --type=grid-match [--only=<mode>]
 */
'use strict';
const fs = require('fs');
const { THEMES } = require('./fr-themes');
const { render, assertThemeTable, placeholdersRemain, singularSlotSuspects, badDefiniteForm, doubleDefinitenessInTemplate } = require('./fr-render');
...
const cfg = require('./fr-readiness-' + TYPE);
const FR = 'frontend/content/seo-landing/fr.json';
const COORDS = JSON.parse(fs.readFileSync('scripts/seo-landing/fr-' + TYPE + '-coordinates.json', 'utf8')).coordinates;

// ---- template pre-pass (inert in fr: doubleDefinitenessInTemplate is a no-op) ----
{
  const b5 = [];
  ...
  if (b5.length) { console.error('fr template pre-pass (' + b5.length + '):'); b5.forEach(f => console.error('  ' + f)); process.exit(1); }
}

const tableFails = assertThemeTable(THEMES);
if (tableFails.length) {
  console.error('fr-themes ASSERTION FAILED (' + tableFails.length + '):');
...
const levelFor = (mk) => (typeof cfg.level === 'function' ? cfg.level(mk) : (cfg.level || 'maternelle'));
...
function buildMode(mk){
  ...
  list.forEach((co,i)=>{
    ...
    const entry = {
      ...
      slotTokens: [d.plIndef, 'les ' + d.plIndef, d.h1Display, co.theme.replace(/_/g,' '), level, cfg.slotWord],
...
console.log('generated ' + generated.length + ' (' + TYPE + ', modes ' + MODES.join('+') + '); blocked ' + blockedTotal + '; fr.json total ' + merged.landings.length);
...
generated.forEach(e => {
  ...
  if (th) { const bad = badDefiniteForm(text, th); if (bad) { console.log('  B4 BAD-DEFINITE-FORM ' + e.slug + ' :: "' + bad + '"'); lintFail++; } }
});
...
if (lintFail) { console.error('fr lint: ' + lintFail + ' FAIL — halting.'); process.exit(1); }
console.log('fr lint: clean' + (short ? (' (but ' + short + ' SHORT)') : '') + '; all checks pass');
