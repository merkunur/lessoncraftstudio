...
import { importCharacterSheet, StudioMediaError } from '@/lib/studio/media';
import { checkAssetQuota, addAssetBytes } from '@/lib/studio/quota';
import { STUDIO_PLAY_API_PREFIX } from '@/lib/studio/config';

// Story Studio — ADMIN: TexturePacker JSON-Hash character-sheet import
// (multi-pose character, sharp re-bake to the normalized atlas contract).
// Body: { name, atlas, imageBase64, imageExt } — the same JSON envelope the
// studio client has always sent to the local server.
...
export async function POST(request: NextRequest, { params }: Params) {
  ...
  try {
    const saved = await importCharacterSheet(row.id, body);
