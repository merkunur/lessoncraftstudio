/* =====================================================================
   audit-the-gap-locale-layout.js — 11 locales × 6 viewports
   ---------------------------------------------------------------------
   Run:  node scripts/audit-the-gap-locale-layout.js [--shot]

   ⚠⚠ THE ENGLISH SWEEP IS NOT THE SWEEP, AND THIS TOOL PROVES IT TWICE.
   `probe-the-gap.js` measures geometry in English only, and
   `smoke-the-gap-locales.js` visits all eleven locales but ONLY at
   ...
   `.crt-row` is `flex-wrap`, so a button that does not fit simply wraps
   ...
   bounding box of `.crt-wrap`, which is the box a teacher sees.
   ...
   floor), which is what `.crt-btn{min-height:44px}` claims and what this
...
const OUT = path.join(__dirname, '..', 'docs', 'audit-results', 'the-gap', 'locales');
...
async function poison(browser) {
  ...
  await page.goto(`http://127.0.0.1:${PORT}/the-gap.html?lang=de&embed=1`, { waitUntil: 'domcontentloaded' });
  await page.waitForSelector('.crt-wrap', { timeout: 9000 });
  await new Promise((r) => setTimeout(r, 320));

  const probe = (mutate) => page.evaluate((src) => {
    const r = (e) => e.getBoundingClientRect();
    window.TheGap.api.settings.range = 'sixteen';
    window.TheGap.st = { n: 16, k: -9, m: 7, phase: 'after', tried: -9 };
    window.TheGap.render();
    /* eslint-disable no-eval */
    if (src) eval(src);
    const wrap = document.querySelector('.crt-wrap');
    const wb = r(wrap);
    let over = -Infinity, tap = 999;
    for (const b of document.querySelectorAll('.crt-btn')) {
      const bx = r(b);
  ...
  const p1 = await probe("document.querySelector('.crt-b-again .crt-label').textContent='X'.repeat(120);"
    + "document.querySelector('.crt-b-again').style.flex='0 0 auto';"
    + "document.querySelector('.crt-b-again').style.whiteSpace='nowrap';");
  ...
  const p2 = await probe("var b=document.querySelector('.crt-b-again');b.style.minHeight='20px';b.style.height='20px';b.style.padding='0';");
  ...
  const p3 = await page.evaluate(() => {
    const w = document.querySelector('.crt-wrap');
...
(async () => {
  ...
  for (const loc of LOCALES) {
    for (const W of WIDTHS) {
      ...
      await page.goto(`http://127.0.0.1:${PORT}/the-gap.html?lang=${loc}&embed=1`, { waitUntil: 'domcontentloaded' });
      await page.waitForSelector('.crt-wrap', { timeout: 9000 });
      await new Promise((r) => setTimeout(r, 320));

      const m = await page.evaluate(() => {
        const inst = window.TheGap;
        const doc = document.documentElement;
        const r = (e) => e.getBoundingClientRect();
        /* ⚠⚠ `.crt-wrap` IS RE-QUERIED INSIDE THE STATE LOOP, NEVER HOISTED.
        ...
        /* ⚠ BOTH SETTINGS ARE SWEPT, AND THE WIDEST STATE OF EACH. "up to
           sixteen" is not the default, but it is a shipped setting that
           puts SIXTEEN counters on the shelf and up to fifteen keys on
           the rail — the row is at its widest in a state the default
           never shows. The `after`-with-a-theory states are the ones
           that carry three numerals AND the rail at once. */
        const states = [
          { range: 'ten', st: { n: 5, k: 4, m: 9, phase: 'before', tried: null } },
          { range: 'ten', st: { n: 10, k: -8, m: 2, phase: 'after', tried: -8 } },
          { range: 'ten', st: { n: 3, k: 7, m: 10, phase: 'gap', tried: null } },
          { range: 'sixteen', st: { n: 16, k: -9, m: 7, phase: 'before', tried: null } },
          { range: 'sixteen', st: { n: 16, k: -9, m: 7, phase: 'after', tried: -9 } },
          { range: 'sixteen', st: { n: 3, k: 13, m: 16, phase: 'after', tried: 13 } }
        ];

        for (const s of states) {
          inst.api.settings.range = s.range;
          inst.st = s.st;
          inst.render();
          out.states++;
          const wrap = document.querySelector('.crt-wrap');   /* fresh, every state */
          ...
          for (const b of document.querySelectorAll('.crt-btn')) {
            ...
            out.btnSeen++;
            /* ⚠ measured against THE CARD. `.crt-row` is flex-wrap, so a
          ...
          const say = document.querySelector('.crt-say');
          if (say) out.sayOver = Math.max(out.sayOver, Math.round(r(say).right - wrapBox.right));
          const gate = document.querySelector('.crt-gate.is-on');
          if (gate) {
            const gb = r(gate);
      ...
      is(!m.noWrap, `${tag}: .crt-wrap was absent after a render — the gate cannot measure`);
      is(!m.badWrap, `${tag}: .crt-wrap measured ${m.badWrap}px wide — a degenerate reference box`);
