...
(function (global) {
  ...
  function speak(text) {
    try { if (global.LCSAudio && global.LCSAudio.speak) { global.LCSAudio.speak({ type: 'number', text: String(text), lang: (LANG === 'pt' ? 'pt-BR' : LANG), rate: 0.95 }); return; }
      if (global.speechSynthesis && global.SpeechSynthesisUtterance) { var u = new global.SpeechSynthesisUtterance(String(text)); u.rate = .95; u.lang = (LANG === 'fr' ? 'fr-FR' : LANG === 'de' ? 'de-DE' : LANG === 'es' ? 'es-MX' : LANG === 'pt' ? 'pt-BR' : LANG === 'it' ? 'it-IT' : LANG === 'nl' ? 'nl-NL' : 'en-US'); global.speechSynthesis.cancel(); global.speechSynthesis.speak(u); } } catch (e) {}
  }
  ...
  function spriteSVG(mood) {
    ...
    return '<svg class="tw-sprite-svg" viewBox="0 0 100 100" role="img" aria-label="' + (LANG === 'fr' ? 'lutin jumeau' : LANG === 'de' ? 'Zwillings-Wicht' : LANG === 'es' ? 'duende gemelo' : LANG === 'pt' ? 'duende gêmeo' : LANG === 'it' ? 'folletto gemello' : LANG === 'nl' ? 'tweelingkabouter' : 'Twin-Sprite') + '"><ellipse cx="50" cy="54" rx="30" ry="30" fill="#9ED2C6"/><ellipse cx="30" cy="32" rx="7" ry="10" fill="#9ED2C6"/><ellipse cx="70" cy="32" rx="7" ry="10" fill="#9ED2C6"/>' + eyes + mouth + '</svg>'; }
  ...
  global.TwinsiesActivity = {
    ...
    strings: {
      title: { en: 'Twinsies', de: 'Zwillinge — gleich viele!', fr: 'Les jumeaux — autant l’un que l’autre !', es: '¡Gemelos — la misma cantidad!', pt: 'Gêmeos — a mesma quantidade!', it: 'Gemelli — tanti quanti!', nl: 'Tweelingmaatjes' },
      prompt: { en: 'Make a twin with the same many!', de: 'Bau einen Zwilling mit gleich vielen!', fr: 'Fais un jumeau avec autant !', es: '¡Un gemelo con la misma cantidad!', pt: 'Faça um gêmeo com a mesma quantidade!', it: 'Crea un gemello con la stessa quantità!', nl: 'Maak een tweeling met evenveel!' },
      countHint: { en: 'Tap each one to count the Twin-Sprite\'s pile.', de: 'Tippe jede Eichel an und zähle den Haufen vom Zwillings-Wicht.', fr: 'Touche chaque gland pour compter le tas du lutin jumeau.', es: 'Toca cada bellota y cuenta el montón del duende gemelo.', pt: 'Toque em cada bolota e conte.', it: 'Tocca ogni ghianda e conta il mucchio del folletto.', nl: 'Tik elke eikel aan en tel het hoopje.' },
      buildHint: { en: 'Build a twin with the same number.', de: 'Bau einen Zwilling mit gleich vielen.', fr: 'Construis un jumeau avec le même nombre.', es: 'Arma un gemelo con la misma cantidad.', pt: 'Monte um gêmeo com a mesma quantidade.', it: 'Costruisci un gemello con lo stesso numero.', nl: 'Bouw een tweeling met evenveel.' },
      numeralHint: { en: 'Count out this many on your plate.', de: 'Zähle so viele auf deinen Teller.', fr: 'Compte autant de glands sur ton assiette.', es: 'Cuenta esa misma cantidad en tu plato.', pt: 'Conte a mesma quantidade no prato.', it: 'Conta questa quantità sul tuo piatto.', nl: 'Tel er zoveel op je bord.' },
      countOnHint: { en: 'Almost a twin — add one more!', de: 'Fast ein Zwilling — leg noch eine dazu!', fr: 'Presque un jumeau — ajoute-en un !', es: '¡Casi un gemelo — agrega uno más!', pt: 'Quase um gêmeo — coloque mais uma!', it: 'Quasi gemelli — aggiungi ancora uno!', nl: 'Bijna een tweeling — leg er nog één bij!' },
      commit: { en: 'I counted {n} — build the twin! →', de: 'Ich habe {n} gezählt — bau den Zwilling! →', fr: 'J’ai compté {n} — construis le jumeau ! →', es: 'Conté {n} — ¡arma el gemelo! →', pt: 'Contei {n} — monte o gêmeo! →', it: 'Ho contato {n} — costruisci il gemello! →', nl: 'Ik heb er {n} geteld — bouw de tweeling! →' },
      peek: { en: '👀 Peek', de: '👀 Gucken', fr: '👀 Coup d’œil', es: '👀 Espía', pt: '👀 Espiar', it: '👀 Sbircia', nl: '👀 Gluren' },
      twinsBtn: { en: "We're twins!", de: 'Wir sind Zwillinge!', fr: 'On est jumeaux !', es: '¡Somos gemelos!', pt: 'Somos gêmeos!', it: 'Siamo gemelli!', nl: 'We zijn een tweeling!' },
      win: { en: 'Twins! Same many!', de: 'Zwillinge! Gleich viele!', fr: 'Jumeaux ! Autant l’un que l’autre !', es: '¡Gemelos! ¡La misma cantidad!', pt: 'Gêmeos! A mesma quantidade!', it: 'Gemelli! Tanti quanti!', nl: 'Tweeling! Evenveel!' },
      notyet: { en: 'Not twins yet — let\'s count again.', de: 'Noch keine Zwillinge — zählen wir noch mal.', fr: 'Pas encore jumeaux — comptons encore.', es: 'Todavía no son gemelos — vamos a contar de nuevo.', pt: 'Ainda não são gêmeos — vamos contar de novo.', it: 'Non ancora gemelli — contiamo di nuovo.', nl: 'Nog geen tweeling — tel nog eens samen!' },
      hintCheck: { en: 'Make a matching twin, then tap We\'re twins!', de: 'Bau einen passenden Zwilling und tippe dann auf „Wir sind Zwillinge!“', fr: 'Fais un jumeau identique, puis touche On est jumeaux !', es: 'Arma un gemelo igual y luego toca «¡Somos gemelos!».', pt: 'Monte um gêmeo igualzinho e depois toque em “Somos gêmeos!”.', it: 'Crea un gemello uguale, poi tocca Siamo gemelli!', nl: 'Maak een gelijke tweeling, tik dan op "We zijn een tweeling!"' },
      readCounted: { en: 'Counted', de: 'Gezählt', fr: 'Comptés', es: 'Conté', pt: 'Contei', it: 'Contate', nl: 'Geteld' },
      readTwin: { en: 'Twin', de: 'Zwilling', fr: 'Jumeau', es: 'Gemelo', pt: 'Gêmeo', it: 'Gemello', nl: 'Tweeling' }
    },
    defaults: {},
    ...
    _declare: function () {
      ...
      if (res.twins) {
        ...
        var n = this._n(); api.announce && api.announce(api.t('win')); speak(LANG === 'fr' ? (n + ' et ' + n + ' — jumeaux !') : LANG === 'de' ? (n + ' und ' + n + ' — Zwillinge!') : LANG === 'es' ? (n + ' y ' + n + ' — ¡gemelos!') : LANG === 'pt' ? (n + ' e ' + n + ' — gêmeos!') : LANG === 'it' ? (n + ' e ' + n + ' — gemelli!') : LANG === 'nl' ? (n + ' en ' + n + ' — een tweeling!') : (n + ' and ' + n + ' — twins!'));
      } else {
    ...
    _renderDone: function (root) {
      ...
      var eq = api.el('div', 'tw-eq'); eq.textContent = (LANG === 'fr') ? (this._n() + ' & ' + this._n() + ' — jumeaux !') : (this._n() + ' & ' + this._n() + (LANG === 'de' ? ' — Zwillinge!' : LANG === 'es' ? ' — ¡gemelos!' : LANG === 'pt' ? ' — gêmeos!' : LANG === 'it' ? ' — gemelli!' : LANG === 'nl' ? ' — een tweeling!' : ' — twins!')); root.appendChild(eq);
    ...
    injectCSS: function () {
      ...
      var css = ''
        ...
        + '@media (max-height:820px),(max-width:480px){.tw-root{gap:4px;padding:7px;}.tw-sprite{width:clamp(36px,8vw,44px);}.tw-say{font-size:11.5px;max-width:100%;-webkit-line-clamp:2;line-clamp:2;}.tw-model[data-arr="scattered"]{height:clamp(120px,32vw,150px);}.tw-cell{width:44px;height:44px;}.tw-commit{min-height:42px;}.tw-declare{min-height:44px;}}'
        + '@media (max-height:680px){.tw-root{gap:2px;}.tw-main{gap:2px;}.tw-side{gap:3px;}.tw-numeral{font-size:24px;min-width:46px;padding:0 8px;}.tw-model[data-arr="scattered"]{height:104px;}.tw-cell{width:44px;height:44px;}.tw-declare{min-height:44px;}.tw-readout{padding:2px 8px;}.tw-readout-num{font-size:18px;}}'
