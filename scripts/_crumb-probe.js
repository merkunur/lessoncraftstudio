#!/usr/bin/env node
/* =====================================================================
   _crumb-probe.js — multi-state control-bottom probe for "Crumb's Same-Amount
   Bakery" (3.NF.A.3, clarity-first redesign of #86). Drives 2 rounds + a
   wrong-tap + resolved, and MEASURES the lowest control INCLUDING the shell
   `.lcs-activity-check` across 320·360·412·768·1024·1366, plus tap ≥44px on the
   candidate cards, no overlap, no overflow.
   ===================================================================== */
'use strict';
...
(async () => {
  ...
  const solve = (ok) => page.evaluate((correct) => {
    const t = window.FractionEquivActivity, r = t._round, C = window.FractionEquivCore;
    const oi = C.oracle(r);
    let target = oi;
    if (!correct) { for (let i = 0; i < r.candidates.length; i++) if (!C.isAnswer(r, i)) { target = i; break; } }
    const b = Array.from(document.querySelectorAll('.fe-cand')).find(x => +x.getAttribute('data-ci') === target); if (b) b.click();
  }, ok).then(() => sleep(50));

  const states = {
    'roundA': async () => { await force('fe-half-fourths'); },
    'roundB': async () => { await force('fe-twothirds-sixths'); },
    'wrong': async () => { await force('fe-half-eighths'); await solve(false); },
    'resolved': async () => { await force('fe-fourths-half'); await solve(true); }
  };

  async function controlBottom() {
    return await page.evaluate(() => {
      const sel = '.fe-root,.fe-ref,.fe-row,.fe-cand,.fe-say,.fe-hear,.fe-msg,.lcs-activity-check';
  ...
  async function checks() {
    return await page.evaluate(() => {
      const taps = Array.from(document.querySelectorAll('.fe-cand'));
  ...
  if (!(await page.evaluate(() => window.FractionEquivActivity._resolved))) fails.push('a correct tap did NOT resolve');
  await states.wrong();
  if (await page.evaluate(() => window.FractionEquivActivity._resolved)) fails.push('a wrong (non-equivalent) tap resolved — must not advance');
  ...
  for (const [w, h] of VIEWPORTS) {
    ...
    for (const label of order) {
      ...
      if (tt.minTap < CTRL_MIN && tt.minTap !== 999 && !subFloor) fails.push(`${label} @${w}×${h}: a candidate card is ${tt.minTap}px (<${CTRL_MIN}px)`);
  ...
  console.log('');
  if (fails.length) { console.error(`CRUMB-PROBE FAILED — ${fails.length}:`); fails.forEach(f => console.error('  • ' + f)); process.exit(1); }
  console.log(`CRUMB-PROBE PASSED — roundA/roundB/wrong/resolved: a correct tap resolves + a wrong (non-equivalent) tap does not advance; lowest control (incl. shell Check) clears the fold by ≥${MARGIN}px across 320→1366; candidate cards ≥${CTRL_MIN}px (off the 320 sub-floor), no overlap, no horizontal overflow.`);
  process.exit(0);
