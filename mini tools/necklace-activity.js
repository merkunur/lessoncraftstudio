...
(function (global) {
  ...
  function speak(text) {
    try {
      if (global.LCSAudio && global.LCSAudio.speak) { global.LCSAudio.speak({ type: 'number', text: String(text), lang: (LANG === 'es' ? 'es-MX' : LANG === 'pt' ? 'pt-BR' : LANG), rate: 0.95 }); return; }
      if (global.speechSynthesis && global.SpeechSynthesisUtterance) { var u = new global.SpeechSynthesisUtterance(String(text)); u.rate = .95; u.lang = (LANG === 'es' ? 'es-MX' : LANG === 'fr' ? 'fr-FR' : LANG === 'de' ? 'de-DE' : LANG === 'pt' ? 'pt-BR' : 'en-US'); global.speechSynthesis.cancel(); global.speechSynthesis.speak(u); }
    } catch (e) {}
  }
  ...
  function foxSVG(mood) {
    ...
    return '<svg class="nk-fox-svg" viewBox="0 0 100 100" role="img" aria-label="' + (LANG === 'es' ? 'zorro' : LANG === 'fr' ? 'renard' : LANG === 'de' ? 'Fuchs' : LANG === 'pt' ? 'raposa' : 'fox') + '">'
  ...
  global.NecklaceActivity = {
    ...
    strings: {
      title: { en: "The Necklace That Won't Hold Still", de: 'Die Kette, die nicht stillhält', fr: 'Le collier qui ne tient pas en place', es: 'El collar que no se queda quieto', pt: 'O colar que não para quieto' },
      prompt: { en: 'Count the beads — make the same many!', de: 'Zähl die Perlen – mach gleich viele!', fr: 'Compte les perles — fais-en autant !', es: 'Cuenta las perlas… ¡haz la misma cantidad!', pt: 'Conte as contas — faça a mesma quantidade!' },
      countHint: { en: "Tap each bead to count the fox's necklace.", de: 'Tippe jede Perle an und zähl die Kette vom Fuchs.', fr: 'Touche chaque perle pour compter le collier du renard.', es: 'Toca cada perla y cuenta el collar del zorro.', pt: 'Toque em cada conta pra contar o colar da raposa.' },
      countScatterHint: { en: 'Tap each bead — careful not to miss one!', de: 'Tippe jede Perle an – pass auf, dass du keine vergisst!', fr: 'Touche chaque perle — attention à n’en oublier aucune !', es: 'Toca cada perla… ¡con cuidado de no saltarte ninguna!', pt: 'Toque em cada conta — não deixe nenhuma passar!' },
      recountHint: { en: 'Count again — the fox shook it!', de: 'Zähl noch mal – der Fuchs hat geschüttelt!', fr: 'Compte encore — le renard l’a secoué !', es: 'Cuenta otra vez… ¡el zorro lo sacudió!', pt: 'Conte de novo — a raposa sacudiu!' },
      produceHint: { en: 'Thread your own — the same many!', de: 'Fädel deine eigene auf – gleich viele!', fr: 'Enfile les tiennes — autant que lui !', es: 'Ensarta las tuyas… ¡la misma cantidad!', pt: 'Enfie as suas — a mesma quantidade!' },
      heardHint: { en: 'The fox said a number — thread that many!', de: 'Der Fuchs hat eine Zahl gesagt – fädel so viele auf!', fr: 'Le renard a dit un nombre — enfile-en autant !', es: 'El zorro dijo un número… ¡ensarta esa cantidad!', pt: 'A raposa falou um número — enfie essa quantidade!' },
      confirmHint: { en: 'How many did you count?', de: 'Wie viele hast du gezählt?', fr: 'Combien en as-tu compté ?', es: '¿Cuántas contaste?', pt: 'Quantas você contou?' },
      fixSkipHint: { en: 'One bead too many — take one off!', de: 'Eine Perle zu viel – nimm eine weg!', fr: 'Une perle de trop — enlèves-en une !', es: 'Una perla de más… ¡quita una!', pt: 'Uma conta a mais — tire uma!' },
      fixRepairHint: { en: "The fox's necklace is wrong — fix it to match!", de: 'Die Kette vom Fuchs stimmt nicht – mach gleich viele!', fr: 'Le collier du renard n’est pas bon — corrige-le pour qu’il soit pareil !', es: 'El collar del zorro está mal… ¡haz la misma cantidad!', pt: 'O colar da raposa está errado — conserte pra ficar igual!' },
      shakeBtn: { en: 'Give it a shake! 🫳', de: 'Schüttel sie! 🫳', fr: 'Secoue-le ! 🫳', es: '¡Sacúdelo! 🫳', pt: 'Sacuda! 🫳' },
      commitHide: { en: 'I counted {n} — thread mine! →', de: 'Ich habe {n} gezählt – jetzt meine! →', fr: 'J’ai compté {n} — j’enfile les miennes ! →', es: 'Conté {n}… ¡ahora el mío! →', pt: 'Contei {n} — vou enfiar as minhas! →' },
      commitConfirm: { en: 'I counted {n} — how many? →', de: 'Ich habe {n} gezählt – wie viele? →', fr: 'J’ai compté {n} — combien ? →', es: 'Conté {n}… ¿cuántas? →', pt: 'Contei {n} — quantas são? →' },
      proceedAffirm: { en: "That's how many! ✓", de: 'So viele sind es! ✓', fr: 'C’est bien ça ! ✓', es: '¡Esa es la cantidad! ✓', pt: 'É essa a quantidade! ✓' },
      recountAffirm: { en: 'Still {n}! ✓', de: 'Immer noch {n}! ✓', fr: 'Toujours {n} ! ✓', es: '¡Sigue igual: {n}! ✓', pt: 'Ainda são {n}! ✓' },
      claspBtn: { en: 'Clasp it! 🔗', de: 'Zumachen! 🔗', fr: 'Ferme le collier ! 🔗', es: '¡Ciérralo! 🔗', pt: 'Feche o colar! 🔗' },
      doneBtn: { en: 'All fixed! ✓', de: 'Alles richtig! ✓', fr: 'Tout est réparé ! ✓', es: '¡Todo correcto! ✓', pt: 'Consertei! ✓' },
      hearAgain: { en: '🔊 Hear it again', de: '🔊 Nochmal hören', fr: '🔊 Écouter encore', es: '🔊 Escuchar otra vez', pt: '🔊 Ouvir de novo' },
      confirmChip: { en: "It's {n}!", de: 'Genau {n}!', fr: 'C’est {n} !', es: '¡Conté {n}!', pt: 'São {n}!' },
      peek: { en: '👀 Peek', de: '👀 Spicken', fr: '👀 Coup d’œil', es: '👀 Espiar', pt: '👀 Espiar' },
      notyet: { en: "Not yet — let's count again together.", de: 'Noch nicht – zählen wir noch mal zusammen.', fr: 'Pas encore — comptons encore ensemble.', es: 'Todavía no… contemos juntos otra vez.', pt: 'Ainda não — vamos contar de novo juntos.' },
      win: { en: 'Clasp! The same many! 🦊🦊', de: 'Zugemacht! Gleich viele! 🦊🦊', fr: 'Fermé ! Autant l’un que l’autre ! 🦊🦊', es: '¡Cerrado! ¡La misma cantidad! 🦊🦊', pt: 'Fechou! A mesma quantidade! 🦊🦊' },
      hintCheck: { en: 'Make the same many, then clasp it!', de: 'Fädel gleich viele auf und mach sie dann zu!', fr: 'Fais-en autant, puis ferme le collier !', es: 'Ensarta la misma cantidad y… ¡ciérralo!', pt: 'Faça a mesma quantidade e feche o colar!' }
    },
    defaults: {},
    ...
    _readout: function (val, kind) {
      ...
      var lab = api.el('span', 'nk-readout-lab'); lab.textContent = (LANG === 'es') ? (kind === 'counted' ? 'Contadas' : 'Las tuyas') : (LANG === 'fr') ? (kind === 'counted' ? 'Comptées' : 'Les tiennes') : (LANG === 'de') ? (kind === 'counted' ? 'Gezählt' : 'Deine') : (LANG === 'pt') ? (kind === 'counted' ? 'Contadas' : 'As suas') : (kind === 'counted' ? 'Counted' : 'Yours');
    ...
    _winClasp: function () {
      ...
      var n = this._target(); this.announce('win'); speak(LANG === 'es' ? (n + ' y ' + n + '… ¡a cerrar!') : LANG === 'fr' ? (n + ' et ' + n + ' — fermé !') : LANG === 'de' ? (n + ' und ' + n + ' — zugemacht!') : LANG === 'pt' ? (n + ' e ' + n + ' — fechou!') : (n + ' and ' + n + ' — clasp!'));
    },
    ...
    _renderDone: function (root) {
      ...
      var eq = api.el('div', 'nk-eq'); eq.textContent = (LANG === 'es') ? (n + ' y ' + n + '… ¡la misma cantidad!') : (LANG === 'fr') ? (n + ' & ' + n + ' — autant l’un que l’autre !') : (LANG === 'de') ? (n + ' & ' + n + ' — gleich viele!') : (LANG === 'pt') ? (n + ' e ' + n + ' — a mesma quantidade!') : (n + ' & ' + n + ' — the same many!'); root.appendChild(eq);
