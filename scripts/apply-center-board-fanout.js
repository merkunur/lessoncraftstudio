#!/usr/bin/env node
/* =====================================================================
   apply-center-board-fanout.js — batch-apply the 10 native-agent
   deliverables (scratchpad center-board-<loc>.json) to:
     1. mini tools/center-board.js      — strings corrections (exactly-once
        single-line rebuild per key; optional trailing comma)
     2. frontend/messages/tool-content/<loc>.json — the ToolEntry
     3. frontend/lib/manipulatives.ts  — ONE new center-board entry from
...
const TOOL_FILE = path.join(REPO, 'mini tools', 'center-board.js');
...
const deliverables = {};
for (const L of LOCS) {
  const f = path.join(SCRATCH, `center-board-${L}.json`);
...
console.log('\n1. strings corrections (center-board.js)');
let stringsApplied = 0;
for (const L of LOCS) {
  const corr = deliverables[L].stringsCorrections || {};
  for (const key of Object.keys(corr)) {
    if (/^team(Fox|Turtle|Bee|Rabbit|Whale|Owl)$/.test(key)) { console.log('  ⚠ ' + L + '.' + key + ': team keys are byte-locked — skipped'); continue; }
    ...
    let phBad = false;
    for (const ph of ['{n}', '{m}']) {
      if (obj.en.includes(ph) && !val.includes(ph)) { E(`${L}.${key}: drops the ${ph} placeholder`); phBad = true; }
    }
    if (phBad) continue;
    obj[L] = val;
...
for (const L of LOCS) {
  ...
  for (const k of Object.keys(j)) {
    if (k === 'labels') out['center-board'] = { slug: te.slug, name: te.name, tagline: te.tagline, metaTitle: te.metaTitle, metaDescription: te.metaDescription, about: te.about, howToUse: te.howToUse, classroomIdeas: te.classroomIdeas };
    if (k !== 'center-board') out[k] = j[k];
  }
  tcWrites[file] = out;
...
console.log('\n3. manipulatives entry');
const manipEN = { /*center-board*/
  title: 'Center Board',
  tagline: 'Your rotation chart, running itself calmly.',
  description: 'A projector rotation board for centers: station cards with animal team cards docked under them — tap Rotate and the cards slide one station over with a chime. A calm round timer, per-station notes for substitutes, and QR links for the tablet station.',
};
...
if (manipSrc.includes('id: "center-board"')) OK('manipulatives entry already present — skipping');
else {
...
console.log('\nAPPLIED — center-board.js, 10 tool-content files, manipulatives.ts');
