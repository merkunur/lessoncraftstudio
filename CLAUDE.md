...
**Position (2026-07-31).** `#36 number-sieve` **LIVE** · `#37 arrow-strip` **LIVE** → **NEXT = C1 The Draw Bag**, then A2 Splat Mat; then wave 2 (B3 Unit Handle · B4 Unrolling Tape · A5 Comparison Tape · B7 Cold Line). Full wave order in the companion.

**Constants for the next build:** header ordinal **#38** · `PREV = 'arrow-strip'` · `TOOL_WRAPPER_VERSION` **7.50 → 7.51** · it becomes `TOOL_KEYS` position **41**. ⚠ **The header ordinal is a LEDGER, not the array index — it runs 3 behind** (`class-timer`/`letter-studio` both claim #25, `wodb`/`syllable-splitter` both claim #22, three early tools carry none). Take the next ordinal from the last shipped tool's header. **Do not renumber anything.**
