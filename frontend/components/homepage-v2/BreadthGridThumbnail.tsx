...
/**
 * Client-component wrapper for a single BreadthGrid deck thumbnail.
 *
 * Extracted from BreadthGrid.tsx (server component) so the click handler
 * can call the quota gate via React hook. Per CLAUDE.md §7 free-tier 2/day
 * cap: clicking a homepage deck thumbnail counts as one quota-gated 'play'
 * action.
 *
 * Middle-click / ctrl-click bypass the onClick (browser opens href directly
 * in new tab); the deck.href is the catalog deck URL which is SEO-public per
 * §7 locked design.
 */
  ...
  return (
    <>
      <a
        href={deckUrl}
        onClick={e =>
          gatedClick(e, deckUrl, 'play', deckId ?? `${language}-${slug}`, 'self')
        }
