...
(function (global) {
  ...
  function speak(text) {
    try { if (global.LCSAudio && global.LCSAudio.speak) { global.LCSAudio.speak({ type: 'number', text: String(text), lang: LANG, rate: 0.95 }); return; }
      if (global.speechSynthesis && global.SpeechSynthesisUtterance) { var u = new global.SpeechSynthesisUtterance(String(text)); u.rate = .95; u.lang = (LANG === 'fr' ? 'fr-FR' : LANG === 'de' ? 'de-DE' : LANG === 'es' ? 'es-MX' : 'en-US'); global.speechSynthesis.cancel(); global.speechSynthesis.speak(u); } } catch (e) {}
  }
  ...
  function spriteSVG(mood) {
    ...
    return '<svg class="tw-sprite-svg" viewBox="0 0 100 100" role="img" aria-label="' + (LANG === 'fr' ? 'lutin jumeau' : LANG === 'de' ? 'Zwillings-Wicht' : LANG === 'es' ? 'duende gemelo' : 'Twin-Sprite') + '"><ellipse cx="50" cy="54" rx="30" ry="30" fill="#9ED2C6"/><ellipse cx="30" cy="32" rx="7" ry="10" fill="#9ED2C6"/><ellipse cx="70" cy="32" rx="7" ry="10" fill="#9ED2C6"/>' + eyes + mouth + '</svg>'; }
  ...
  global.TwinsiesActivity = {
    ...
    strings: {
      title: { en: 'Twinsies', de: 'Zwillinge — gleich viele!', fr: 'Les jumeaux — autant l’un que l’autre !', es: '¡Gemelos — la misma cantidad!' },
      prompt: { en: 'Make a twin with the same many!', de: 'Bau einen Zwilling mit gleich vielen!', fr: 'Fais un jumeau avec autant !', es: '¡Un gemelo con la misma cantidad!' },
      countHint: { en: 'Tap each one to count the Twin-Sprite\'s pile.', de: 'Tippe jede Eichel an und zähle den Haufen vom Zwillings-Wicht.', fr: 'Touche chaque gland pour compter le tas du lutin jumeau.', es: 'Toca cada bellota y cuenta el montón del duende gemelo.' },
      buildHint: { en: 'Build a twin with the same number.', de: 'Bau einen Zwilling mit gleich vielen.', fr: 'Construis un jumeau avec le même nombre.', es: 'Arma un gemelo con la misma cantidad.' },
      numeralHint: { en: 'Count out this many on your plate.', de: 'Zähle so viele auf deinen Teller.', fr: 'Compte autant de glands sur ton assiette.', es: 'Cuenta esa misma cantidad en tu plato.' },
      countOnHint: { en: 'Almost a twin — add one more!', de: 'Fast ein Zwilling — leg noch eine dazu!', fr: 'Presque un jumeau — ajoute-en un !', es: '¡Casi un gemelo — agrega uno más!' },
      commit: { en: 'I counted {n} — build the twin! →', de: 'Ich habe {n} gezählt — bau den Zwilling! →', fr: 'J’ai compté {n} — construis le jumeau ! →', es: 'Conté {n} — ¡arma el gemelo! →' },
      peek: { en: '👀 Peek', de: '👀 Gucken', fr: '👀 Coup d’œil', es: '👀 Espía' },
      twinsBtn: { en: "We're twins!", de: 'Wir sind Zwillinge!', fr: 'On est jumeaux !', es: '¡Somos gemelos!' },
      win: { en: 'Twins! Same many!', de: 'Zwillinge! Gleich viele!', fr: 'Jumeaux ! Autant l’un que l’autre !', es: '¡Gemelos! ¡La misma cantidad!' },
      notyet: { en: 'Not twins yet — let\'s count again.', de: 'Noch keine Zwillinge — zählen wir noch mal.', fr: 'Pas encore jumeaux — comptons encore.', es: 'Todavía no son gemelos — vamos a contar de nuevo.' },
      hintCheck: { en: 'Make a matching twin, then tap We\'re twins!', de: 'Bau einen passenden Zwilling und tippe dann auf „Wir sind Zwillinge!“', fr: 'Fais un jumeau identique, puis touche On est jumeaux !', es: 'Arma un gemelo igual y luego toca «¡Somos gemelos!».' },
      readCounted: { en: 'Counted', de: 'Gezählt', fr: 'Comptés', es: 'Conté' },
      readTwin: { en: 'Twin', de: 'Zwilling', fr: 'Jumeau', es: 'Gemelo' }
    },
    defaults: {},
    ...
    _declare: function () {
      ...
      if (res.twins) {
        ...
        var n = this._n(); api.announce && api.announce(api.t('win')); speak(LANG === 'fr' ? (n + ' et ' + n + ' — jumeaux !') : LANG === 'de' ? (n + ' und ' + n + ' — Zwillinge!') : LANG === 'es' ? (n + ' y ' + n + ' — ¡gemelos!') : (n + ' and ' + n + ' — twins!'));
      } else {
    ...
    _renderDone: function (root) {
      ...
      var eq = api.el('div', 'tw-eq'); eq.textContent = (LANG === 'fr') ? (this._n() + ' & ' + this._n() + ' — jumeaux !') : (this._n() + ' & ' + this._n() + (LANG === 'de' ? ' — Zwillinge!' : LANG === 'es' ? ' — ¡gemelos!' : ' — twins!')); root.appendChild(eq);
