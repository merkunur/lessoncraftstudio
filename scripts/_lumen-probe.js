#!/usr/bin/env node
/* =====================================================================
   _lumen-probe.js — multi-state control-bottom probe for "Lumen's Lighthouse
   Windows" (RL.1.6, clarity-first redesign of #66). Drives a high round + a
   low round (initial) + a wrong-scaffold + resolved, and MEASURES the lowest
   control INCLUDING the shell `.lcs-activity-check` across 320·360·412·768·
   1024·1366, plus tap ≥44px on the window-cards, no overlap, no overflow.
   ===================================================================== */
'use strict';
...
(async () => {
  ...
  const solve = (ok) => page.evaluate((correct) => {
    const t = window.PointOfViewActivity, r = t._round, C = window.PointOfViewCore;
    const oi = C.oracle(r), idx = correct ? oi : ((oi + 1) % r.chars.length);
    const b = Array.from(document.querySelectorAll('.lw-cand')).find(x => +x.getAttribute('data-ci') === idx); if (b) b.click();
  }, ok).then(() => sleep(50));

  const states = {
    'high': async () => { await force('pv-boat'); },
    'low': async () => { await force('pv-wave'); },
    'wrong': async () => { await force('pv-fish'); await solve(false); },
    'resolved': async () => { await force('pv-town'); await solve(true); }
  };

  async function controlBottom() {
    return await page.evaluate(() => {
      const sel = '.lw-root,.lw-say,.lw-row,.lw-cand,.lw-win,.lw-line-msg,.lcs-activity-check';
  ...
  async function checks() {
    return await page.evaluate(() => {
      const taps = Array.from(document.querySelectorAll('.lw-cand'));
  ...
  if (!(await page.evaluate(() => window.PointOfViewActivity._resolved))) fails.push('a correct tap did NOT resolve');
  await states.wrong();
  if (await page.evaluate(() => window.PointOfViewActivity._resolved)) fails.push('a wrong-height creature resolved — must not advance');

  const order = ['high', 'low', 'wrong', 'resolved'];
  for (const [w, h] of VIEWPORTS) {
    ...
    for (const label of order) {
      ...
      if (tt.minTap < CTRL_MIN && tt.minTap !== 999 && !subFloor) fails.push(`${label} @${w}×${h}: a window-card is ${tt.minTap}px (<${CTRL_MIN}px)`);
  ...
  console.log('');
  if (fails.length) { console.error(`LUMEN-PROBE FAILED — ${fails.length}:`); fails.forEach(f => console.error('  • ' + f)); process.exit(1); }
  console.log(`LUMEN-PROBE PASSED — high/low/wrong/resolved: a correct tap resolves + a wrong-height creature does not advance; lowest control (incl. shell Check) clears the fold by ≥${MARGIN}px across 320→1366; window-cards ≥${CTRL_MIN}px (off the 320 sub-floor), no overlap, no horizontal overflow.`);
  process.exit(0);
