...
## 26. nt10-D — the LAST batch of 10 NEW printable worksheet types × (1 base + 5 variations) × 11 locales = 660 (DESIGN COMPLETE 2026-09-21; BUILD IN PROGRESS 2026-09-21 — Phases 0-2 DONE: 10 EN bases + 50 EN faces committed, baseline 30,030; NEXT = Phase 3 EN waves)
...
**BUILD STATUS (2026-09-21):** Phase 0 (b4 tooling: registrars → taxonomy 115 → 125 keys, Science strand row, the b4 spine, `gen-b4-landings.js` with the 60-id STANDARD map) · Phase 1 (10 EN bases) · Phase 2 (50 EN variation faces, `_FACE-BRIEF.md`) are DONE and committed per family; the release baseline (`out/b3-baseline.json`) holds all 60 ids with 0 drift. Build rulings that bind the rest of the build: the family key is `weather-symbols`; **SPARSE is a defect** (top-anchor the stage, size zones to the task, every gate asserts a stage floor + slack ≤180 — six faces were sent back for it); **a title that lists chips lists exactly the d2 page's chips**; **open every picture** (the design-excluded `neck` portrait reached a face pool); per-locale refusals stand by NAME in the banks (recycling en F4, cloze da F3, pronouns es + fi F3 → 656). **NEXT = Phase 3** (`gen-b4-waves.js --locales=en` → generate → readiness), then the 10 native panels (`b4-panel-brief.md`), per-locale waves, landings, publish, hub gate hard. Resume pointer + per-family commits + panel items: memory `project_nt10d_worksheet_types.md`; plan `resilient-spinning-goblet.md`.
