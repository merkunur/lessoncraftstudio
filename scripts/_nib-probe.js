#!/usr/bin/env node
/* =====================================================================
   _nib-probe.js — multi-state control-bottom probe for "Nib's Apostrophe Seat"
   (L.2.2.c, clarity-first redesign of #77). Drives a regular + an irregular
   ...
   on the contraction chips, no overlap, no overflow.
   ===================================================================== */
'use strict';
...
(async () => {
  ...
  const solve = (ok) => page.evaluate((correct) => {
    const t = window.ContractionActivity, r = t._round, C = window.ContractionCore;
    const ans = C.deriveCorrect(r), pick = correct ? ans : C.chipStrings(r).find(s => s !== ans);
    const b = Array.from(document.querySelectorAll('.ct-cand')).find(x => x.getAttribute('data-str') === pick); if (b) b.click();
  }, ok).then(() => sleep(50));

  const states = {
    'regular': async () => { await force('ct-dont'); },
    'irregular': async () => { await force('ct-wont'); },
    'wrong': async () => { await force('ct-isnt'); await solve(false); },
    'resolved': async () => { await force('ct-youre'); await solve(true); }
  };

  async function controlBottom() {
    return await page.evaluate(() => {
      const sel = '.ct-root,.ct-words,.ct-say,.ct-hear,.ct-strip,.ct-cand,.ct-line-msg,.lcs-activity-check';
  ...
  async function checks() {
    return await page.evaluate(() => {
      const taps = Array.from(document.querySelectorAll('.ct-cand'));
  ...
  if (!(await page.evaluate(() => window.ContractionActivity._resolved))) fails.push('a correct tap did NOT resolve');
  await states.wrong();
  if (await page.evaluate(() => window.ContractionActivity._resolved)) fails.push('a wrong chip resolved — must not advance');

  const order = ['regular', 'irregular', 'wrong', 'resolved'];
  ...
  console.log('');
  if (fails.length) { console.error(`NIB-PROBE FAILED — ${fails.length}:`); fails.forEach(f => console.error('  • ' + f)); process.exit(1); }
  console.log(`NIB-PROBE PASSED — regular/irregular/wrong/resolved: a correct tap resolves + a wrong chip does not advance; lowest control (incl. shell Check) clears the fold by ≥${MARGIN}px across 320→1366; chips ≥${CTRL_MIN}px (off the 320 sub-floor), no overlap, no horizontal overflow.`);
  process.exit(0);
