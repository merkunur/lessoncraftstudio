#!/usr/bin/env node
/**
 * Swedish SEO Validation Script — Part 300
 *
 * Validates ALL file-based Swedish metadata for:
 *   - Title length (target 50–60 chars)
 *   - Description length (target 150–160 chars)
 *   - Uniqueness (no duplicate titles or descriptions)
 *   - Language quality (no English leakage, Swedish markers present)
 *
 * Page types checked:
 *   - 33 product pages
 *   - 50 theme hubs
 ...
 *
 * Usage: node scripts/validate-sv-all-seo.js
 */
...
/** Check if string has Swedish markers (å, ä, ö, or common Swedish words) */
function hasSwedishMarkers(str) {
  if (!str) return false;
  // Check for Swedish characters (å, ä, ö and uppercase)
  if (/[\u00e5\u00e4\u00f6\u00c5\u00c4\u00d6]/.test(str)) return true;
  // Check for common Swedish words
  const swedishWords = [
    'och', 'till', 'barn', 'arbetsblad', 'gratis', 'utskrivbar',
    'klass', 'matematik', '\u00f6vningar', 'f\u00f6rskola',
    'f\u00f6rskoleklass', 'f\u00e4rgl\u00e4ggning', 'generatorer',
    'skriv ut', 'teckningar', 'pussel', 'bokst\u00e4ver', 'utbildning',
  ];
  const lower = str.toLowerCase();
  return swedishWords.some(w => lower.includes(w));
}
...
function validateEntry(pageType, pageName, rawTitle, rawDesc) {
  ...
  // Swedish markers
  if (!hasSwedishMarkers(title) && !hasSwedishMarkers(desc)) {
    pageOk = false;
    pageIssues.push('No Swedish markers');
    issues.push({ page: `${pageType}/${pageName}`, field: 'both', value: '', len: 0, issue: 'No Swedish markers' });
  }
...
function validateThemeHubs() {
  ...
  const themeDirs = fs.readdirSync(baseDir).filter(d => {
    const svPath = path.join(baseDir, d, 'sv.ts');
    return fs.existsSync(svPath) && fs.statSync(path.join(baseDir, d)).isDirectory();
  }).sort();
  ...
  for (const theme of themeDirs) {
    const content = fs.readFileSync(path.join(baseDir, theme, 'sv.ts'), 'utf8');
...
function validateCategoryHubs() {
  ...
  for (const cat of categories) {
    ...
    let title = '', desc = '';
    if (catMatch) {
      const svBlock = catMatch[1];
      const titleMatch = svBlock.match(/title:\s*'([^']*(?:\\.[^']*)*)'/);
      const descMatch = svBlock.match(/description:\s*'([^']*(?:\\.[^']*)*)'/);
      if (titleMatch) title = titleMatch[1];
      if (descMatch) desc = descMatch[1];
    }
...
function validateNavigationPages() {
  ...
  // 1. Homepage — frontend/app/[locale]/page.tsx
  {
    ...
    const content = fs.readFileSync(filePath, 'utf8');
    const svBlock = content.match(/sv:\s*\{([^}]*title:[^}]*description:[^}]*)\}/);
    let title = '', desc = '';
    if (svBlock) {
      const tm = svBlock[1].match(/title:\s*'([^']*)'/);
      const dm = svBlock[1].match(/description:\s*'([^']*)'/);
      if (tm) title = tm[1];
      if (dm) desc = dm[1];
    }
  ...
  // 3. Worksheets page — separate pageTitle / pageDescription Records
  {
    ...
    const content = fs.readFileSync(filePath, 'utf8');
    const titleMatch = content.match(/pageTitle[\s\S]*?sv:\s*'([^']*)'/);
    const descMatch = content.match(/pageDescription[\s\S]*?sv:\s*'([^']*)'/);
    const title = titleMatch ? titleMatch[1] : '';
    const desc = descMatch ? descMatch[1] : '';
    navPages.push({ name: 'worksheets', title, desc });
  }

  // 4. Pricing page — sv: { title: '...', description: '...' }
  {
  ...
  // 5. Blog listing page — blogPageMeta[locale]
  {
    ...
    const content = fs.readFileSync(filePath, 'utf8');
    const svBlock = content.match(/sv:\s*\{\s*\n?\s*title:\s*'([^']*)'\s*,\s*\n?\s*description:\s*'([^']*)'/);
    let title = '', desc = '';
    if (svBlock) {
      title = svBlock[1];
      desc = svBlock[2];
...
function checkLanguageQuality() {
  ...
  const noSwedish = issues.filter(i => i.issue === 'No Swedish markers');
  ...
  if (noSwedish.length === 0) {
    console.log('  Missing Swedish markers: 0 pages');
  } else {
    console.log(`  Missing Swedish markers: ${noSwedish.length} pages`);
    for (const { page } of noSwedish) {
      console.log(`    ${page}`);
    }
  }

  return { englishLeakages: englishLeakages.length, noSwedish: noSwedish.length };
}

// ─── Main ──────────────────────────────────────────────────────────────────

function main() {
  console.log('=== SWEDISH SEO VALIDATION REPORT ===');
  ...
  console.log(`  Missing Swedish:        ${language.noSwedish}`);
