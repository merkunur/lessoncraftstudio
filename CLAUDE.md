...
> **[STATUS 2026-07-31 — THE CURRENT ACTIVE THRUST IS §23, the Premium Tools v4 program.]** Two v4 instruments are live (`#36 number-sieve`, `#37 arrow-strip`) and the next is **C1 The Draw Bag**. **"Build the next tool" resolves to §23.0, not to anything in this section.** Everything below is a **PAUSED BACKLOG**, resumable whenever the operator names it — nothing here is abandoned and nothing here is the default "continue". **⚠ EXCEPTION (added 2026-08-18): "continue" / "switch to plan mode + continue" (in plan mode) resolves to the [ACTIVE — pt-BR SECOND-BATCH FAN-OUT] block immediately below** — that is a live "continue" loop, NOT a paused backlog. (Two distinct triggers: "build the next tool" → §23; "continue" → the pt-BR fan-out.)

**[ACTIVE 2026-08-18 — pt-BR SECOND-BATCH FAN-OUT · the current "continue" loop.]** The ~123-activity second-batch fan-out (the games-build-walk EN activities, fanned to each locale in the SAME #1..#123 order) is now on **Brazilian Portuguese**: de PAUSED (~30 of ~123 remain, below) · **es COMPLETE · fr COMPLETE (175 live)** · **pt-BR ACTIVE — #1–#47 shipped (commit/push, NO deploy), NEXT = #48 = `vet-diagnosis`** (1.OA.A.1 word problems ≤20, de commit `f39af3cb`). **Recipe per "continue":** ONE activity per plan-mode session → EnterPlanMode → find the next in order (`git rev-list --all --children | grep ^<prev-de-hash>` → child de commit → `git show --stat`) + survey the de commit/engine → **3-agent native BR ensemble** (expert pedagogue + content creator + linguist) → ExitPlanMode approval → build **activity-layer ONLY** (0 core/shell/Direction-A lines) → full **§A.13.62 LOCAL DoD** (parity self-check via a WRITTEN scratchpad script + `verify-activity-content-pt.js` + `preflight-activity-routes.js` + `tsc --noEmit` + `visual-qa-activity.js` pt+en sweep 320·360·412·768·1024·1366 + personal Read 768/1024/360) → **commit + push, NO deploy** → append the record to `project_pt_secondbatch_fanout.md` + refresh the MEMORY.md pointer. **Load-bearing pt doctrine (recurring traps):** rebuild-not-translate, NEVER skip; BNCC framework NAME only (no habilidade/CCSS code in pt-facing text, §20.10); GRADE + STRAND are per-activity ensemble calls — **strand cheat-sheet: MATH OA (Operations & Algebraic Thinking) auto-maps to the WRONG "Álgebra" → pt `STRAND_OVERRIDE` "Números" is REQUIRED** (⚠ exactly the #48 case); NBT / Measurement-&-Data / grammar-literacy auto-map correctly (no override); vocab-literacy → route override "Ampliação do vocabulário". **GRADE by SKILL not number-ceiling**, and genuine forks are PEDAGOGUE-decisive — e.g. #47 `bos-berry-pantry` (valor posicional) was ruled **2º ano AGAINST** the es/fr grade-1 harmonization (pt added a `pt:'2'` override; es/fr introduce dizaines/tens-and-ones at grade 1 by their own curricula, BNCC defers to 2º); that ruling also **flagged the earlier `bundle-bot` 1º-ano ruling as possibly mis-graded** — a SEPARATE operator item, not acted on. **roundsL10n** = copy id/band/numbers/options VERBATIM from es, translate ONLY the text/story field, numbers LOCKED to the on-screen tiles/equation. **Per-locale number-word engines** add a `numWord<LANG>()` sibling (pt uses the ` e ` joiner; masculine-default cardinals safe unless a value ends in 1/2 near a feminine noun). ⚠ `frontend/public/mini-tools/` is a **gitignored STALE copy — NEVER edit; source is `mini tools/`**. Deploy is **batch-end only** (cp `mini tools/` BEFORE `deploy.sh` build per §20.4; `npx tsc --noEmit` after any route edit). Character RENAME when the EN name is opaque for a BR kid = an AskUserQuestion fork (pt-only divergence). **SoT = `memory/project_pt_secondbatch_fanout.md`** (recipe + full per-#N trap log + strand cheat-sheet + all standing rules) **+ the out-of-tree plan file `C:\Users\rkgen\.claude\plans\you-have-already-built-playful-karp.md`** (per-activity). READ the topic file each session.
