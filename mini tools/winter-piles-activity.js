...
(function (global) {
  ...
  function numWord(n) { if (LANG === 'de') return ONES_DE[n] || String(n); if (LANG === 'fr') return ONES_FR[n] || String(n); if (LANG === 'es') return ONES_ES[n] || String(n); if (LANG === 'pt') return ONES_PT[n] || String(n); if (LANG === 'it') return ONES_IT[n] || String(n); if (n < 20) return ONES[n]; var t = ['', '', 'twenty', 'thirty', 'forty', 'fifty']; return (t[Math.floor(n / 10)] || n) + (n % 10 ? '-' + ONES[n % 10] : ''); }
  ...
  function speak(text) {
    try { if (global.LCSAudio && global.LCSAudio.speak) { global.LCSAudio.speak({ type: 'number', text: text, lang: (LANG === 'es' ? 'es-MX' : LANG === 'pt' ? 'pt-BR' : LANG === 'it' ? 'it-IT' : LANG), rate: 0.92 }); return; }
      if (global.speechSynthesis && global.SpeechSynthesisUtterance) { var u = new global.SpeechSynthesisUtterance(text); u.rate = .92; u.lang = (LANG === 'es' ? 'es-MX' : LANG === 'de' ? 'de-DE' : LANG === 'fr' ? 'fr-FR' : LANG === 'pt' ? 'pt-BR' : LANG === 'it' ? 'it-IT' : 'en-US'); global.speechSynthesis.cancel(); global.speechSynthesis.speak(u); } } catch (e) {}
  }
  ...
  function squirrelSVG(mood) {
    ...
    return '<svg class="wp-sq-svg" viewBox="0 0 100 100" role="img" aria-label="' + (LANG === 'de' ? 'Eichhörnchen' : LANG === 'fr' ? 'écureuil' : LANG === 'es' ? 'ardilla' : LANG === 'pt' ? 'esquilo' : LANG === 'it' ? 'scoiattolo' : 'squirrel') + '">' +
  ...
  global.WinterPilesActivity = {
    ...
    strings: {
      title: { en: "Squirrel's Fair Winter Piles", de: 'Flos faire Winterhäufchen', fr: 'Les tas égaux de Flo', es: 'Los montoncitos justos de Flo', pt: 'Os montinhos iguais de Flo', it: 'I mucchietti invernali di Flo' },
      prompt: { en: 'Make fair piles!', de: 'Mach faire Häufchen!', fr: 'Fais des tas égaux !', es: '¡Haz montoncitos iguales!', pt: 'Faça montinhos iguais!', it: 'Fai mucchietti uguali!' },
      ...
      promptN: { en: 'Make exactly {n} fair piles.', de: 'Mach genau {n} gleich große Häufchen.', fr: 'Fais exactement {n} tas égaux.', es: 'Haz exactamente {n} montoncitos iguales.', pt: 'Faça exatamente {n} montinhos iguais.', it: 'Fai esattamente {n} mucchietti uguali.' },
      promptFix: { en: 'These piles are unfair — re-cut them so they match!', de: 'Diese Häufchen sind nicht gleich groß – schneide noch mal, damit sie zusammenpassen!', fr: 'Ces tas ne sont pas égaux — recoupe-les pour qu’ils correspondent !', es: 'Estos montoncitos no son iguales. ¡Corta otra vez para que queden iguales!', pt: 'Esses montinhos não estão iguais. Corte de novo!', it: 'Mucchietti diversi! Taglia di nuovo per renderli uguali.' },
      promptMatch: { en: 'Cut and read to make {eq}.', de: 'Schneide und lies, um {eq} zu bauen.', fr: 'Coupe et lis pour faire {eq}.', es: 'Corta y lee para formar {eq}.', pt: 'Corte e leia para formar {eq}.', it: 'Taglia e leggi per fare {eq}.' },
      readHint: { en: 'Read each pile and tap its number in.', de: 'Lies jedes Häufchen und tippe seine Zahl ein.', fr: 'Lis chaque tas et entre son nombre.', es: 'Lee cada montoncito y escribe su número.', pt: 'Leia cada montinho e marque o número dele.', it: 'Leggi ogni mucchietto e tocca il suo numero.' },
      lockIt: { en: 'Lock it in!', de: 'Einrasten!', fr: 'Valide !', es: '¡Fíjalos!', pt: 'Prontinho!', it: 'Conferma!' },
      win: { en: 'Fair piles! They sparkle as one!', de: 'Faire Häufchen! Sie funkeln wie eins!', fr: 'Des tas égaux ! Ils brillent ensemble !', es: '¡Montoncitos iguales! ¡Brillan como uno solo!', pt: 'Montinhos iguais! Eles brilham como um só!', it: 'Mucchietti uguali! Brillano tutti insieme!' },
      unfair: { en: "Those piles aren't equal yet — make them all the same.", de: 'Die Häufchen sind noch nicht gleich groß – mach sie alle gleich.', fr: 'Ces tas ne sont pas encore égaux — fais-les tous pareils.', es: 'Los montoncitos todavía no son iguales. Haz que todos queden del mismo tamaño.', pt: 'Ainda não estão iguais. Deixe todos do mesmo tamanho.', it: 'Questi mucchietti non sono ancora uguali: falli tutti della stessa grandezza.' },
      hintCheck: { en: 'Make fair piles + read each one, then lock it in.', de: 'Mach gleich große Häufchen und lies jedes, dann raste sie ein.', fr: 'Fais des tas égaux et lis chacun, puis valide.', es: 'Haz montoncitos iguales y lee cada uno; luego fíjalos.', pt: 'Faça montinhos iguais, leia cada um e confirme.', it: 'Fai mucchietti uguali, leggi ognuno e poi conferma.' }
    },
    defaults: {},
    ...
    _onBecomeValid: function () {
      ...
      var piles = Core.derivePiles(this.round, this.cuts), eq = piles.map(numWord).join(LANG === 'es' ? ' más ' : LANG === 'pt' ? ' mais ' : LANG === 'it' ? ' più ' : ' plus ') + (LANG === 'de' ? ' ist gleich ' : LANG === 'fr' ? ' égale ' : LANG === 'es' ? ' es igual a ' : LANG === 'pt' ? ' é igual a ' : LANG === 'it' ? ' fa ' : ' equals ') + numWord(this._R() * this._Cc());
