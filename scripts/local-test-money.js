...
const SHOT = has('shot');
/* both money activities — count-out (#1) + make-change (#2) */
const ACTIVITIES = [
  { id: 'money.count-out.2-md-c-8', change: false },
  { id: 'money.make-change.2-md-c-8', change: true }
];
...
const rowById = (id) => manifest.find((r) => r.id === id);
...
(async () => {
  ...
  for (const ACT of ACTIVITIES) {
    const byLocale = ((rowById(ACT.id) || {}).params || {}).byLocale || {};
    const pfx = ACT.change ? 'make-change' : 'count-out';
   for (const loc of LOCALES) {
    ...
    const tag = `${pfx}/${loc}`;
    const url = `${BASE}/money-activity.html?lang=${loc}&activity=${ACT.id}&embed=1`;
    try {
      if (!L) { note(false, `${tag}: no byLocale.${loc} in manifest (currency not authored)`); console.log(`  FAIL ${tag} — no byLocale.${loc}`); await page.close(); continue; }
      ...
      const expectTitle = ACT.change ? ((STR.titleChange && STR.titleChange[loc]) || null) : ((STR.title && STR.title[loc]) || null);
      ...
      /* make-change shows the purchase context; count-out must not */
      const hasCtx = await page.evaluate(() => !!document.querySelector('.mn-context'));
      note(hasCtx === ACT.change, `${tag}: .mn-context ${hasCtx ? 'present' : 'absent'} (expected ${ACT.change})`);
      ...
      /* interaction: reload round 0, read target (count-out: amount; make-change: change), tender */
      ...
      const round = ACT.change
        ? ((L.rounds || []).find(r => (r.paid - r.cost) === target) || L.rounds[0])
        : ((L.rounds || []).find(r => r.target === target) || L.rounds[0]);
      ...
      note(!(await celebrating(page)), `${tag}: an under-tender still celebrated`);
      ...
      note(ok.correct, `${tag}: tendering the solution (${solution.join('+')}=${target}) did not satisfy isCorrect()`);
      note(ok.c, `${tag}: correct tender did not celebrate`);
      ...
      console.log(`  ${okLoc ? 'ok  ' : 'FAIL'} ${tag} — "${title}" | "${prompt}" | ${new Set(p1).size} distinct`);
    } catch (e) {
  ...
  console.log(`MONEY LOCAL TEST PASSED — ${ACTIVITIES.length} activities × ${LOCALES.length} locale(s): render + localized title/prompt + make-change context + tap-to-tender (correct celebrates, under-tender doesn't) + ≥7-round reshuffle + no mobile overflow.`);
  process.exit(0);
