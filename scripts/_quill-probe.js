#!/usr/bin/env node
/* =====================================================================
   _quill-probe.js — the multi-state control-bottom probe for "Quill's Mirror
   Market" (K.L.5.b · antonyms). Drives the pick / generate / balance / verb /
   route / oddpair / scene / wrong-nudge / resolved states and MEASURES the
   ...
   overflow. Code-drawn glyphs (no library images). (Unique name — grepped clear.)
   ===================================================================== */
'use strict';
...
(async () => {
  ...
  const clickWord = (w) => page.evaluate((word) => {
    const els = Array.from(document.querySelectorAll('.op-card,.op-pair'));
    const hit = els.find(e => (e.getAttribute('aria-label') || '').toLowerCase().indexOf(word.toLowerCase()) === 0);
    if (hit) hit.click();
  }, w).then(() => sleep(30));
  const clickBtn = (t) => page.evaluate((tx) => { const b = Array.from(document.querySelectorAll('.op-btn')).find(x => x.textContent.trim() === tx); if (b && !b.disabled) b.click(); }, t).then(() => sleep(30));

  const states = {
    'pick': async () => { await force('opposites.pick-black'); },
    'generate': async () => { await force('opposites.generate-left'); },
    'balance': async () => { await force('opposites.balance-white'); },
    'verb': async () => { await force('opposites.verb-push'); },
    'route': async () => { await force('opposites.route-black'); },
    'oddpair': async () => { await force('opposites.oddpair-1'); },
    'scene': async () => { await force('opposites.scene-bedtime'); },
    'wrong-nudge': async () => { await force('opposites.pick-black'); await clickWord('red'); await clickBtn('Trade it!'); },
    'resolved': async () => { await force('opposites.balance-white'); await clickWord('black'); }
  };

  async function controlBottom() {
    return await page.evaluate(() => {
      const sel = '.op-root,.op-grid,.op-row,.op-card,.op-pair,.op-door,.op-btn,.op-target,.op-prompt,.lcs-activity-check';
  ...
  async function checks() {
    return await page.evaluate(() => {
      const taps = Array.from(document.querySelectorAll('.op-card,.op-pair,.op-door,.op-btn'));
      ...
      let bleed = 0;
      document.querySelectorAll('.op-line,.op-prompt,.op-w,.op-btn,.op-door').forEach(el => { if (el.scrollWidth > el.clientWidth + 1) bleed++; });
      const boxes = Array.from(document.querySelectorAll('.op-card,.op-pair,.op-btn')).map(e => e.getBoundingClientRect());
      let overlap = 0;
  ...
  const order = ['pick', 'generate', 'balance', 'verb', 'route', 'oddpair', 'scene', 'wrong-nudge', 'resolved'];
  ...
  console.log('');
  if (fails.length) { console.error(`QUILL-PROBE FAILED — ${fails.length}:`); fails.forEach(f => console.error('  • ' + f)); process.exit(1); }
  console.log(`QUILL-PROBE PASSED — pick/generate/balance/verb/route/oddpair/scene/wrong-nudge/resolved: lowest control (incl. shell Check) clears the fold by ≥${MARGIN}px across 320→1366; controls ≥${CTRL_MIN}px (off the 320 sub-floor), no text overflow, no overlap, no horizontal overflow.`);
  process.exit(0);
