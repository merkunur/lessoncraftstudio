#!/usr/bin/env node
/**
 * gen-b5-faces-table.js <locale> [--sweep=<dir>]
 *
 * Writes out/b5-faces.<loc>.json — the face table a landing panel briefs FROM:
 * one row per nt10-E type the locale SHIPS (20 bases + the faces its panel did not
 ...
 *   node tools/gen-b5-probe-jobs.js waves/wave-b5-<loc>.json  --out=/tmp/a.json
 *   node render/batch.js /tmp/a.json --out out/b5-sweep/<loc>     (same for b5var)
...
if (!locale) { console.error('usage: gen-b5-faces-table.js <locale> [--sweep=<dir>]'); process.exit(2); }
const sweep = argOf('sweep') || path.join(ROOT, 'out', 'b5-sweep', locale);

const wave = (k) => JSON.parse(fs.readFileSync(path.join(ROOT, 'waves', `wave-${k}-${locale}.json`), 'utf8'));
const WB = wave('b5'), WV = wave('b5var');
const strings = JSON.parse(fs.readFileSync(path.join(ROOT, 'i18n', `strings.${locale}.json`), 'utf8'));
const refPath = path.join(ROOT, '..', '..', 'docs', 'worksheet-gen', 'b5-designs', '_records', `refusals.${locale}.json`);
...
const out = path.join(ROOT, 'out', `b5-faces.${locale}.json`);
