#!/usr/bin/env node
/* =====================================================================
   _bramble-probe.js — the multi-state control-bottom probe for "Bramble's
   Berry-Juice Stand" (K.MD.A.2). The uncapped visual-qa sweep covers first
   paint; this drives the predict / predict-fit / pouring / verified-overflow
   / verified-same / describe / resolved states and MEASURES the lowest control
   INCLUDING the shell `.lcs-activity-check` <= vh-4 across
   320·360·412·768·1024·1366, plus text-containment (Bramble's line + choice
   labels), tap >= 44px (the predict buttons; the SVG is exempt), no overlap,
   no horizontal overflow. Runs reduced-motion for deterministic states.
...
(async () => {
  ...
  const predict = (opt) => page.evaluate((o) => window.BrambleActivity._onPredict(o), opt).then(() => sleep(50));

  const states = {
    'predict': async () => { await force('wide-wins-1'); },
    'predict-fit': async () => { await force('fit-1'); },
    'resolved': async () => { await force('tall-less-1'); await predict('A_MORE'); },        /* miss → verified/resolved, Check shown */
    'verified-same': async () => { await force('same-1'); await predict('A_MORE'); },
    'describe': async () => { await force('describe-1'); await predict('A_MORE'); await sleep(60); }
  };

  async function controlBottom() {
    return await page.evaluate(() => {
      const sel = '.bb-root,.bb-line,.bb-choices,.bb-stage-svg,.bb-choice,.lcs-activity-check';
  ...
  async function checks() {
    return await page.evaluate(() => {
      const taps = Array.from(document.querySelectorAll('.bb-choice'));
      ...
      let bleed = 0;
      document.querySelectorAll('.bb-line,.bb-choice').forEach(el => { if (el.scrollWidth > el.clientWidth + 1) bleed++; });
      const boxes = Array.from(document.querySelectorAll('.bb-choice')).map(e => e.getBoundingClientRect());
      let overlap = 0;
  ...
  const order = ['predict', 'predict-fit', 'resolved', 'verified-same', 'describe'];
  ...
  for (const [w, h] of VIEWPORTS) {
    ...
    for (const label of order) {
      ...
      if (tt.minTap < CTRL_MIN && tt.minTap !== 999 && !subFloor) fails.push(`${label} @${w}×${h}: a predict button is ${tt.minTap}px (<${CTRL_MIN}px)`);
  ...
  console.log('');
  if (fails.length) { console.error(`BRAMBLE-PROBE FAILED — ${fails.length}:`); fails.forEach(f => console.error('  • ' + f)); process.exit(1); }
  console.log(`BRAMBLE-PROBE PASSED — predict/predict-fit/resolved/verified-same/describe: lowest control (incl. shell Check) clears the fold by ≥${MARGIN}px across 320→1366; predict buttons ≥${CTRL_MIN}px (off the 320 sub-floor), no line/label overflow, no button overlap, no horizontal overflow.`);
  process.exit(0);
