/* =====================================================================
   register-counting-cups.js — the seven registration points for #48.
   Run:  node scripts/register-counting-cups.js
...
const KEY = 'counting-cups';
const PREV = 'folding-wall';
const L = require('./_counting-cups-landing.js');
...
/* ---- 2. tool-content.ts × 4 -------------------------------------- */
{
  ...
  let s = rd(f), before = s;
  if (s.indexOf(`'${KEY}'`) === -1) {
    s = s.replace(new RegExp(`('${PREV}')(\\s*\\] as const)`), `$1, '${KEY}'$2`);
    s = s.replace(new RegExp(`('${PREV}':\\s*'[^']*',)`), `$1\n  '${KEY}': '/mini-tools/${KEY}.html',`);
    s = s.replace(new RegExp(`('${PREV}':\\s*'[^']*',)(?![\\s\\S]*'${PREV}':)`), `$1`);
  }
  if (s !== before) { wr(f, s); step('tool-content.ts (keys, mini url, prefix, interface)', true); }
  else step('tool-content.ts (keys, mini url, prefix, interface)', false);
}
...
/* ---- 3. wrapper version — the one non-idempotent step ------------ */
{
  ...
  if (registered && m && m[1] === '7.82') {
    s = s.replace(`const TOOL_WRAPPER_VERSION = '${m[1]}'`, `const TOOL_WRAPPER_VERSION = '7.83'`);
