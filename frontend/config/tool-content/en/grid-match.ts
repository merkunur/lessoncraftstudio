...
const content: ToolContent = {
    ...
    steps: [
      {
        ...
        description: 'Click \u201cTry Free Now\u201d to launch the grid matching worksheet generator in your browser. The tool loads instantly with a settings sidebar on the left and a dual-tab canvas on the right \u2014 one tab for the worksheet, one for the answer key. No account creation, no software download, no installation required \u2014 start building grid matching puzzles immediately.',
      },
      {
        title: 'Configure the Grid Size',
        description: 'Open the Grid Options panel and set the number of rows (2\u20134, default 3) and columns (2\u20134, default 3) independently. A 2\u00d72 grid creates 4 tiles for simple introductory puzzles. A 3\u00d73 grid produces 9 tiles for balanced difficulty. A 4\u00d74 grid delivers 16 tiles for advanced spatial reasoning challenges. You can also mix rows and columns \u2014 a 2\u00d74 grid creates a wide rectangular puzzle with 8 tiles, while a 4\u00d72 produces a tall narrow layout. The grid size directly determines puzzle complexity and visual density.',
      },
      {
      ...
      },
      {
        ...
        description: 'In the Page Setup section, select your page size: Letter Portrait, Letter Landscape, A4 Portrait, A4 Landscape, or enter a custom dimension. Grid Match does not support Square page size. Choose a page background color. Select a decorative background theme and a border theme from the built-in library, each with an independent opacity slider (0\u20131, step 0.05). Background and border themes work independently, so you can pair a subtle pattern background with a bold decorative border or any combination that fits your product style.',
      },
      {
        title: 'Generate the Grid Match Puzzle',
        description: 'Click Generate to create the grid matching worksheet. The app divides your selected image into the configured grid, reveals the clue cells with actual image tiles visible, and marks remaining cells with \u201c?\u201d placeholders. All hidden tiles are shuffled using Fisher-Yates randomization and displayed as a numbered palette. The responsive layout adapts automatically: portrait pages place the grid on top with the palette below, landscape pages position the grid on the left with the palette on the right. A styled header with cyan background (#00BCD4), deep purple title (#6A1B9A), and orange border frame (#FF8C42) displays \u201cGrid Match\u201d and instructions.',
      },
      {
        title: 'Review the Auto-Generated Answer Key',
        description: 'Click the Answer Key tab to see the auto-generated solution. The answer key displays the complete uncut image with numbered circles overlaid on each grid cell \u2014 yellow background (#ffffe0) circles with black outlines and Fredoka font numbers showing which palette tile belongs in each position. Switch between Worksheet and Answer Key tabs to compare. The answer key generates simultaneously with the worksheet \u2014 no manual creation step, no separate design process, no possibility of mismatched answers. This dual-canvas system is your biggest time-saver when creating grid puzzle bundles.',
      },
      {
  ...
  whatYouCanCreate: [
    {
      title: 'Themed Grid Puzzle Bundles by Difficulty Level',
      description: 'Create grid matching puzzle packs organized by theme and difficulty using the 104 image collections. Each theme produces multiple difficulty tiers from a single set of images: easy puzzles (2\u00d72 grid with 3 clues, just 1 tile to match), medium puzzles (3\u00d73 with 2 clues, 7 tiles to match), and hard puzzles (4\u00d74 with 1 clue, 15 tiles to match). Package 15\u201320 grid puzzles per difficulty tier per theme with auto-generated answer keys included. Fisher-Yates randomization means every generation produces different tile order, so you can create multiple unique puzzles from the same image simply by regenerating \u2014 no need for different source images to fill a bundle.',
    },
    {
  ...
  businessIdeas: [
    {
      title: 'Themed Grid Puzzle Shop on Etsy',
      description: 'Open an Etsy shop specializing in grid matching puzzle bundles organized by theme using the 104 image collections. Animals, vehicles, nature, food, holidays, and professions each become separate listings with easy, medium, and hard difficulty versions. Every puzzle includes the auto-generated answer key with numbered circle overlays \u2014 a critical selling point that differentiates your listings from competitors selling puzzles without solutions. Fisher-Yates randomization ensures every worksheet is unique. Price individual theme packs at $3\u2013$5 for 15\u201320 puzzles with answer keys and premium multi-difficulty bundles at $8\u2013$12.',
      platform: 'Etsy',
    },
    {
      title: 'Amazon KDP Visual Perception Workbook Series',
      description: 'Compile 60\u201380 grid matching puzzles into themed workbooks for Amazon KDP. Structure a series by difficulty progression: \u201cEasy Grid Puzzles\u201d uses 2\u00d72 grids with 3 clues, \u201cMedium Grid Puzzles\u201d uses 3\u00d73 grids with 2 clues, and \u201cAdvanced Grid Puzzles\u201d uses 4\u00d74 grids with 1 clue. Include answer key pages at the back with numbered circle overlays on the complete image. Toggle grayscale for ink-friendly output that prints perfectly in black-and-white. The purely visual format publishes identically across all international KDP marketplaces without translation \u2014 one interior serves every country.',
      platform: 'Amazon KDP',
    },
    {
    ...
    {
      title: 'Gumroad Complete Grid Puzzle Toolkit',
      description: 'Bundle grid matching puzzles across all 104 themes and all difficulty levels into a comprehensive toolkit on Gumroad. Include 300+ puzzles spanning easy, medium, and hard configurations, each with its auto-generated answer key \u2014 600+ total files. The three-tier difficulty system (2\u00d72, 3\u00d73, 4\u00d74 grids) with variable clue counts provides enormous variety. The toolkit format justifies premium pricing because buyers get a complete puzzle library rather than individual packs. Add new seasonal collections quarterly to drive repeat purchases.',
      platform: 'Gumroad',
    },
    {
  ...
  proTips: [
    ...
    },
    {
      ...
      description: 'Every click of Generate shuffles the tile palette using Fisher-Yates randomization, producing a different numbered tile order. This means a single image with fixed grid settings generates multiple unique puzzle worksheets, each with different tile numbers and clue positions. Use this to rapidly fill large bundles without needing different source images for every page. Generate 10\u201315 unique puzzles per image, then multiply across 104 themes for massive product catalogs with minimal effort.',
    },
    {
    ...
    },
    {
      ...
      description: 'The independent background and border theme system with separate opacity sliders lets you create a consistent visual identity across your grid puzzle bundles. Set a subtle background theme at 15\u201325% opacity for visual warmth without distracting from the puzzle content. Layer a decorative border at 80\u2013100% opacity for a polished frame. Apply the same background and border combination across every puzzle in a bundle for a cohesive product look that buyers associate with quality and professionalism.',
    },
    {
  ...
  faq: [
    ...
    },
    {
      ...
      answer: 'When you generate a worksheet, the app simultaneously creates a matching answer key on a separate canvas tab. The answer key displays the complete uncut image with numbered circles overlaid on each grid cell \u2014 yellow background (#ffffe0) circles with black outlines and Fredoka font numbers showing which palette tile belongs in each position. Switch between Worksheet and Answer Key tabs to compare. Download each version independently \u2014 worksheet JPEG, worksheet PDF, answer key JPEG, and answer key PDF \u2014 giving you four production-ready files from a single generation. The automatic answer key eliminates manual creation and ensures perfect accuracy.',
    },
    {
    ...
    },
    {
      ...
      answer: 'Yes. Every time you click Generate, the app shuffles tiles using Fisher-Yates randomization, producing a different numbered tile order. Clue cell positions also change between generations. This means you can create multiple distinct puzzle worksheets from a single image without changing any settings \u2014 each will have different tile numbers and clue positions, making them unique puzzle experiences. This is essential for building large product bundles efficiently.',
    },
    {
    ...
    },
    {
      ...
      answer: 'The generator automatically detects your page orientation and repositions elements. Portrait pages (height > width) place the grid on top using 45% of available height with the numbered palette below and a full-width header (100px height, 15px radius). Landscape pages (width > height) position the grid on the left (48% of available width) with the palette on the right and a compact header (70px height, 35px radius). This automatic repositioning ensures grid puzzles look balanced and professional in either orientation without manual layout adjustments.',
    },
    {
  ...
  visuals: {
    heroImages: {
      primary: '/samples/english/grid-match/grid_match_worksheet.jpeg',
    ...
    sampleGallery: [
      {
        src: '/samples/english/grid-match/grid_match_3x3.jpeg',
      ...
      {
        src: '/samples/english/grid-match/grid_match_4x4.jpeg',
      ...
      {
        src: '/samples/english/grid-match/grid_match_answer_key.jpeg',
    ...
    videoTitle: 'How to Create Grid Matching Puzzles with Configurable Grid Sizes, Adjustable Clue Cells, and Auto Answer Keys \u2014 Step-by-Step Tutorial',
  },
};
