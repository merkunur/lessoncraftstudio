...
srv.listen(5901,async()=>{
 ...
 await p.goto('http://localhost:5901/the-queue.html',{waitUntil:'networkidle2'});
 await new Promise(r=>setTimeout(r,700));
 const r=await p.evaluate(()=>({errs:0,marks:document.querySelectorAll('.que-member').length,
   btns:document.querySelectorAll('.que-btn').length,nums:document.querySelectorAll('.que-walker').length,
   say:(document.querySelector('.que-say')||{}).textContent,html:document.body.innerText.slice(0,90)}));
 console.log('pageerrors:',errs.length?errs:'none');
 console.log('marks='+r.marks,'buttons='+r.btns,'numerals='+r.nums);
 console.log('say:',JSON.stringify(r.say));
 await p.screenshot({path:'docs/audit-results/the-queue-mount.png',fullPage:true});
