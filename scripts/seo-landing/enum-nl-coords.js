#!/usr/bin/env node
/* Generic NL coordinate enumeration (READ-ONLY) — the nl fan (STEP 2), any non-picture-sort type.
 * Clone of enum-sv-coords.js with language='nl'. Dumps published nl decks for an exercise-type,
 ...
 * "-vs-" grouping is a separate sub-slice — use enum-nl-picturesort.js).
 *
 * Emits `level` per coordinate ONLY when --level=<key> is passed (readiness types: --level=kleuters).
 * Numeric/banded types (addition/subtraction/code-addition/math-puzzle) omit level here and get a
 * per-coordinate re-grade pass (by child-seen quantity, split-at-20: ≤20 groep-3 / 21-100 groep-4)
 * before gen — DO NOT default their level.
 *
 * MUST run on Hetzner (DATABASE_URL -> local production Postgres).
 * Usage: node scripts/seo-landing/enum-nl-coords.js --type=grid-match --level=kleuters > scripts/seo-landing/nl-grid-match-coordinates.json
 */
'use strict';
...
(async function () {
  ...
  var rows = await c.deck.findMany({
    where: { language: 'nl', status: 'published', exerciseType: TYPE },
