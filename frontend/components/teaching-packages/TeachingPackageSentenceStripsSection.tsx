...
const CDN_BASE = 'https://www.lessoncraftstudio.com/materials/sentence-strips';

export default async function TeachingPackageSentenceStripsSection({ locale, packageSlug }: Props) {
  const t = await getTranslations({ locale, namespace: 'teachingPackagePage.sentenceStrips' });
  const printUrl = `${CDN_BASE}/${locale}/${packageSlug}/print-sentence-strips.pdf`;
  return (
    <section id="sentence-strips" className="mb-10 scroll-mt-6">
