#!/usr/bin/env node
/* FR picture-sort "-vs-" PAIR enumeration (READ-ONLY). subjectTags[0]="X-vs-Y" combined key.
 * Group by pairKey; canonical = earliest publishedAt; siblings = rest. Parse left/right themes.
 * MUST run on Hetzner (DB). Usage: node scripts/seo-landing/enum-fr-picturesort.js > scripts/seo-landing/fr-picture-sort-coordinates.json
 */
'use strict';
...
(async function () {
  ...
  var rows = await c.deck.findMany({
    where: { language: 'fr', status: 'published', exerciseType: 'picture-sort' },
  ...
  console.log(JSON.stringify({
    locale: 'fr', exerciseType: 'picture-sort', rawRowCount: rows.length, pairCount: coords.length,
