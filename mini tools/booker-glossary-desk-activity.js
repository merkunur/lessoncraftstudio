...
(function (global) {
  ...
  function bearSVG() {
    return '<svg class="bgd-bear-svg" viewBox="0 0 100 100" role="img" aria-label="' + (LANG === 'de' ? 'Booker, der Bär' : LANG === 'fr' ? 'Booker l’ours' : LANG === 'es' ? 'Booker el oso' : LANG === 'pt' ? 'Booker, o urso' : LANG === 'it' ? 'Booker, un orso' : 'Booker the bear') + '">' +
  ...
  global.BookerGlossaryDeskActivity = {
    ...
    strings: {
      title: { en: "Booker's Glossary Desk", de: 'Bookers Wörterbuch-Pult', fr: 'Le dictionnaire de Booker', es: 'El diccionario de Booker', pt: 'O dicionário do Booker', it: 'Il banco del dizionario di Booker' },
      prompt: { en: 'Which word is on this page?', de: 'Tippe das Wort an, das im ABC zwischen den beiden Leitwörtern steht.', fr: 'Quel mot est sur cette page ?', es: 'Toca la palabra que va en orden alfabético entre las dos palabras guía.', pt: 'Toque na palavra que fica, na ordem alfabética, entre as duas palavras-guia.', it: 'Quale parola si trova in questa pagina?' },
      ...
      hintWrong: { en: "Not on this page — check the guide words again, letter by letter.", de: 'Fast! Vergleiche die Wörter Buchstabe für Buchstabe — erst der erste Buchstabe, dann der zweite. Welches liegt im ABC zwischen den Leitwörtern?', fr: 'Presque ! Compare les mots lettre par lettre — d’abord la première lettre, puis la deuxième.', es: '¡Casi! Compara las palabras letra por letra: primero la primera letra, luego la segunda. ¿Cuál queda entre las dos palabras guía?', pt: 'Quase! Compare as palavras letra por letra: primeiro a primeira, depois a segunda. Qual fica entre as palavras-guia?', it: 'Quasi! Confronta le parole lettera per lettera: prima la prima lettera, poi la seconda. Quale sta tra le parole guida?' },
      win: { en: 'Yes! That word is on this page. 📖', de: 'Ja, super! Dieses Wort steht im ABC genau zwischen den beiden Leitwörtern.', fr: 'Oui ! Ce mot est bien sur cette page. 📖', es: '¡Sí! Esa palabra va en orden alfabético justo entre las dos palabras guía.', pt: 'Isso! Essa palavra fica na ordem alfabética bem entre as duas palavras-guia. 📖', it: 'Sì! Quella parola si trova in questa pagina. 📖' }
    },
    defaults: {},
    ...
    render: function () {
      ...
      if (!this._spoke) { this._spoke = true; setTimeout(function () { speak(v.g1 + (LANG === 'de' ? ' bis ' : LANG === 'fr' ? ' à ' : LANG === 'es' ? ' a ' : LANG === 'pt' ? ' a ' : LANG === 'it' ? ' a ' : ' to ') + v.g2); }, 320); }
    },
