#!/usr/bin/env node
/* Generic FR coordinate enumeration (READ-ONLY) — the fr fan, any non-picture-sort type.
 * Clone of enum-da-coords.js with language='fr'. Dumps published fr decks for an exercise-type,
 ...
 * picture-sort (use enum-fr-picturesort.js).
 *
 * Emits `level` per coordinate ONLY when --level=<key> is passed (readiness types: --level=maternelle).
 * Banded arithmetic (addition/subtraction/code-addition/math-puzzle/math-worksheet) omit level here and
 * get a per-coordinate re-grade pass (by child-seen quantity, ≤20 cp / 21-100 ce1) before gen.
 *
 * MUST run on Hetzner. Usage: node scripts/seo-landing/enum-fr-coords.js --type=grid-match --level=maternelle > scripts/seo-landing/fr-grid-match-coordinates.json
 */
'use strict';
...
(async function () {
  ...
  var rows = await c.deck.findMany({
    where: { language: 'fr', status: 'published', exerciseType: TYPE },
