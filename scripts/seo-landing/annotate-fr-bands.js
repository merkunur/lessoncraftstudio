#!/usr/bin/env node
/* Per-coordinate band annotation for the band-split fr arithmetic types (math-puzzle, code-addition,
 * math-worksheet). Reads each coord's deck quantities (manifest operands/results OR the deck.html
 * sum-slot), assigns `level` (cp/ce1) + `standard` by the child-seen max, and writes them back into
 * the coordinate JSON. Coords whose max > 100 are DROPPED (above the K-2 ceiling). Clone of
 * annotate-no-bands.js (cp/ce1 instead of 2-/3-trinn). MUST run on Hetzner (reads /var/www/lcs-media/decks/fr/).
 *
 * Band map (fr ledger, go-luminous-moon.md Wave C):
 *   math-puzzle / math-worksheet: ≤10 → cp K.OA.A.2 · 11-20 → cp 1.OA.C.6 · 21-100 → ce1 2.NBT.B.5
 *   code-addition:                ≤20 → cp 1.OA.C.6 · 21-100 → ce1 2.NBT.B.5
 * Usage: node annotate-fr-bands.js <coords.json> --type=math-puzzle [--locale=fr] [--source=deck-html]
 */
'use strict';
const fs = require('fs');
...
const locale = (argv.find(a => a.indexOf('--locale=') === 0) || '--locale=fr').split('=')[1];
...
function bandFor(type, max) {
  if (type === 'math-puzzle' || type === 'math-worksheet') {
    if (max <= 10) return { level: 'cp', standard: 'K.OA.A.2' };
    if (max <= 20) return { level: 'cp', standard: '1.OA.C.6' };
    if (max <= 100) return { level: 'ce1', standard: '2.NBT.B.5' };
    return null;
  }
