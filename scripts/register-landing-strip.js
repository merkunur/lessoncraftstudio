/* =====================================================================
   register-landing-strip.js — the seven registration points for #51.
   Run:  node scripts/register-landing-strip.js
...
const KEY = 'landing-strip';
const PREV = 'number-drum';
const L = require('./_landing-strip-landing.js');
...
/* ---- 2. tool-content.ts x 4, EACH ANCHORED EXPLICITLY --------------
   ⚠ On #51 a single clever regex silently performed 2 of these 4 and
...
/* ---- 3. wrapper version — the one non-idempotent step ------------ */
{
  ...
  if (registered && m && m[1] === '7.85') {
    s = s.replace(`const TOOL_WRAPPER_VERSION = '${m[1]}'`, `const TOOL_WRAPPER_VERSION = '7.86'`);
    wr(f, s); step(`TOOL_WRAPPER_VERSION ${m[1]} → 7.86`, true);  /* ⚠ #40: the log must report the value ACTUALLY WRITTEN */
...
/* ---- 4. manipulatives.ts hub card -------------------------------- */
{
  ...
  if (s.indexOf(`id: "${KEY}"`) === -1) {
    const card = fs.readFileSync(path.join(__dirname, '_landing-strip-card.txt'), 'utf8');
