#!/usr/bin/env node
/* =====================================================================
   apply-heart-words-fanout.js — applies the nine native ensembles'
   output for TOOL #21 Heart Words.

   The per-locale BANKS (mini tools/heart-words-<loc>.json) are written
   directly by each ensemble and validated by scripts/verify-heart-words.js;
   this script applies everything else:
     ...
     2. strings — surgical single-line replace inside mini tools/heart-words.js
   ...
   Usage:
     node scripts/apply-heart-words-fanout.js --input=<file> [--apply]
...
const TOOL_JS = path.join(TOOLS_DIR, 'heart-words.js');
...
/* Heart Words ships in TEN locales — no fi. */
const LOCALES = ['de', 'fr', 'it', 'es', 'pt', 'nl', 'sv', 'da', 'no'];
...
console.log('STEP 1 — slug collision gate');
{
  ...
  for (const loc of LOCALES) {
    ...
    if (owner && !/tool-content\/.*:heart-words/.test(owner)) {
...
for (const loc of LOCALES) {
  ...
  if (j['heart-words']) { console.log(`  ${loc}: already present — skipped`); continue; }
  const out = {};
  for (const k of Object.keys(j)) {
    if (k === 'labels') out['heart-words'] = te;   /* always immediately before labels */
    out[k] = j[k];
  }
  if (!out['heart-words']) die(`${loc}.json has no trailing "labels" key to anchor against`);
...
console.log('\nSTEP 4 — manipulatives copy');
{
  ...
  const before = src;
  const start = src.indexOf('    id: "heart-words",');
  if (start < 0) die('heart-words entry not found in manipulatives.ts');
...
console.log('Next: node scripts/verify-heart-words.js   &&   node scripts/smoke-heart-words-locales.js');
