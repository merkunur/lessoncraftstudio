...
**Sequence (plan P3.b):** EN waves 1→8 — **1 LIVE** addition·image-image·K (43) → **1b LIVE** +image-number+mixed (143) → **2 LIVE** subtraction (cross-out+image-number) + chart-count·K (137) → **3 LIVE** Math re-leveled Gr1 (142) → **4 LIVE** Literacy·K (find-and-count + prepositions-mc + bingo + word-guess; 162) → **5 LIVE** Logic·K (sudoku + picture-sort + odd-one-out + find-objects + matching; 423 — the largest wave; **+ picture-sort de-orphan DONE**) → **6 LIVE** Spatial·K (pattern-train + pattern-worksheet + picture-trail + shadow-match; 392 — all-readiness; NEW "Patterning (readiness)" label; pattern-worksheet conditional-SHIP) → **7 NEXT** Readiness-class + Preschool (first sub-K grade band) → **8** Gr2 (crossword, cryptogram). Then full-market locales **de → es → nl → fr → it → pt → sv → da** (operator may reorder the middle eight on market priority) → **no, fi LAST** (sample-only; honesty-filter in as their decks publish). **The picture-sort de-orphan (the program's first live-query hub change) shipped in Wave 5** via `themeSubjectTagsWhere` `hasSome`-over-exact-`-vs-`-component-keys (§22.1), with its own verify + rollback.
