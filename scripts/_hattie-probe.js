#!/usr/bin/env node
/* =====================================================================
   _hattie-probe.js — multi-state control-bottom probe for "The Borrowed Hat"
   (L.1.1.d, clarity-first redesign of #84). Drives a subject + object +
   possessive round + a wrong-tap + resolved, and MEASURES the lowest control
   INCLUDING the shell `.lcs-activity-check` across 320·360·412·768·1024·1366,
   plus tap ≥44px on the pronoun chips, no overlap, no overflow.
   ===================================================================== */
'use strict';
...
(async () => {
  ...
  const solve = (ok) => page.evaluate((correct) => {
    const t = window.PronounActivity, r = t._round, C = window.PronounCore;
    const ans = C.deriveCorrect(r), pick = correct ? ans : C.deriveWrong(r);
    const b = Array.from(document.querySelectorAll('.pn-cand')).find(x => x.getAttribute('data-str') === pick); if (b) b.click();
  }, ok).then(() => sleep(50));

  const states = {
    'subject': async () => { await force('pn-he-subj'); },
    'object': async () => { await force('pn-he-obj'); },
    'possessive': async () => { await force('pn-his'); },
    'wrong': async () => { await force('pn-i-subj'); await solve(false); },
    'resolved': async () => { await force('pn-their'); await solve(true); }
  };

  async function controlBottom() {
    return await page.evaluate(() => {
      const sel = '.pn-root,.pn-sentence,.pn-say,.pn-hear,.pn-strip,.pn-cand,.pn-line-msg,.lcs-activity-check';
  ...
  async function checks() {
    return await page.evaluate(() => {
      const taps = Array.from(document.querySelectorAll('.pn-cand'));
  ...
  if (!(await page.evaluate(() => window.PronounActivity._resolved))) fails.push('a correct tap did NOT resolve');
  await states.wrong();
  if (await page.evaluate(() => window.PronounActivity._resolved)) fails.push('a wrong chip resolved — must not advance');

  const order = ['subject', 'object', 'possessive', 'wrong', 'resolved'];
  ...
  console.log('');
  if (fails.length) { console.error(`HATTIE-PROBE FAILED — ${fails.length}:`); fails.forEach(f => console.error('  • ' + f)); process.exit(1); }
  console.log(`HATTIE-PROBE PASSED — subject/object/possessive/wrong/resolved: a correct tap resolves + a wrong chip does not advance; lowest control (incl. shell Check) clears the fold by ≥${MARGIN}px across 320→1366; chips ≥${CTRL_MIN}px (off the 320 sub-floor), no overlap, no horizontal overflow.`);
  process.exit(0);
