#!/usr/bin/env node
/* =====================================================================
   _elapsed-probe.js — control-bottom probe for "Sprocket's Clock — Elapsed
   Time". Drives 2 rounds + wrong + resolved, MEASURES the lowest control incl.
   the shell Check across 320·360·412·768·1024·1366, plus tap ≥44 on the digital
...
(async () => {
  ...
  const solve = (ok) => page.evaluate((correct) => {
    const t = window.ClockElapsedActivity, r = t._round, C = window.ClockElapsedCore;
    const oi = C.oracle(r);
    let target = oi;
    if (!correct) { for (let i = 0; i < r.options.length; i++) if (!C.isAnswer(r, i)) { target = i; break; } }
    const b = Array.from(document.querySelectorAll('.ce-choice')).find(x => +x.getAttribute('data-oi') === target); if (b) b.click();
  }, ok).then(() => sleep(50));
  ...
  async function controlBottom() {
    return await page.evaluate(() => {
      const sel = '.ce-root,.ce-clock,.ce-duration,.ce-row,.ce-choice,.ce-say,.ce-msg,.lcs-activity-check';
  ...
  async function checks() {
    return await page.evaluate(() => {
      const taps = Array.from(document.querySelectorAll('.ce-choice'));
  ...
  if (!(await page.evaluate(() => window.ClockElapsedActivity._resolved))) fails.push('a correct tap did NOT resolve');
  await states.wrong();
  if (await page.evaluate(() => window.ClockElapsedActivity._resolved)) fails.push('a wrong tap resolved — must not advance');
  ...
  console.log('');
  if (fails.length) { console.error(`ELAPSED-PROBE FAILED — ${fails.length}:`); fails.forEach(f => console.error('  • ' + f)); process.exit(1); }
  console.log(`ELAPSED-PROBE PASSED — roundA/roundB/wrong/resolved: a correct tap resolves + a wrong tap does not advance; lowest control (incl. Check) clears the fold by ≥${MARGIN}px across 320→1366; answer cards ≥${CTRL_MIN}px (off the 320 sub-floor), no overlap, no overflow.`);
  process.exit(0);
