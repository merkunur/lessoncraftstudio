...
(function (global) {
  ...
  /* Italian shape names — ALL masculine (il cerchio / il quadrato / il rombo), so "un {n}"
     + past-participle agreement (girato/riconosciuto) work universally, no gender-branching;
     no elision trap (un cerchio / un ovale / un esagono all take plain "un"). rombo (NOT the
     heraldic "losanga"); cerchio (the round FIGURE, NOT "circonferenza" = the boundary line). */
  var SHAPE_IT = { circle: 'cerchio', triangle: 'triangolo', square: 'quadrato', rectangle: 'rettangolo', hexagon: 'esagono', rhombus: 'rombo', oval: 'ovale', mystery: 'Sala dei misteri' };
  var SHAPE_IT_PL = { circle: 'cerchi', triangle: 'triangoli', square: 'quadrati', rectangle: 'rettangoli', hexagon: 'esagoni', rhombus: 'rombi', oval: 'ovali' };
  /* Dutch shape names (§A.13.61 formal groep-3 terms). Gender varies (de cirkel/driehoek/
     rechthoek/zeshoek/ruit vs HET vierkant/ovaal) but the hint frames are article-free
     (bare {n}) or use gender-neutral "een" → no gender-branching. ruit NEVER "rombus". */
  var SHAPE_NL = { circle: 'cirkel', triangle: 'driehoek', square: 'vierkant', rectangle: 'rechthoek', hexagon: 'zeshoek', rhombus: 'ruit', oval: 'ovaal', mystery: 'Raadselgalerij' };
  var SHAPE_NL_PL = { circle: 'cirkels', triangle: 'driehoeken', square: 'vierkanten', rectangle: 'rechthoeken', hexagon: 'zeshoeken', rhombus: 'ruiten', oval: 'ovalen' };
  function gname(t) { return LANG === 'es' ? (SHAPE_ES[t] || t) : LANG === 'de' ? (GNAME[t] || t) : LANG === 'fr' ? (SHAPE_FR[t] || t) : LANG === 'pt' ? (SHAPE_PT[t] || t) : LANG === 'it' ? (SHAPE_IT[t] || t) : LANG === 'nl' ? (SHAPE_NL[t] || t) : (NAME[t] || t); }
  function gnamePl(t) { return LANG === 'es' ? (SHAPE_ES_PL[t] || SHAPE_ES[t] || t) : LANG === 'de' ? (GNAME_PL[t] || GNAME[t] || t) : LANG === 'fr' ? (SHAPE_FR_PL[t] || SHAPE_FR[t] || t) : LANG === 'pt' ? (SHAPE_PT_PL[t] || SHAPE_PT[t] || t) : LANG === 'it' ? (SHAPE_IT_PL[t] || SHAPE_IT[t] || t) : LANG === 'nl' ? (SHAPE_NL_PL[t] || SHAPE_NL[t] || t) : ((NAME[t] || t) + 's'); }
  ...
  global.PipMuseumActivity = {
    ...
    strings: {
      title: { en: "Professor Pip's Museum", de: 'Professor Pips Museum', fr: 'Le musée du professeur Pip', es: 'El museo de Pip', pt: 'O Museu do Professor Pip', it: 'Il museo del professor Pip', nl: 'Professor Tobbes museum' },
      prompt: { en: 'Name each treasure — any way it turns!', de: 'Benenne jeden Schatz — egal, wie er sich dreht!', fr: 'Nomme chaque trésor, même tourné !', es: '¡Nombra cada tesoro, no importa cómo esté girado!', pt: 'Dê o nome de cada tesouro, mesmo girado!', it: 'Dai un nome a ogni tesoro, comunque sia girato!', nl: 'Geef elke schat een naam — hoe hij ook draait!' },
      routeHint: { en: 'What is this? Tap its pedestal to name it.', de: 'Was ist das? Tippe auf den richtigen Sockel.', fr: 'Qu’est-ce que c’est ? Touche le bon socle pour le nommer.', es: '¿Qué es? Toca el pedestal correcto.', pt: 'O que é isto? Toque no pedestal para dar o nome.', it: 'Che forma è? Tocca il piedistallo giusto.', nl: 'Wat is dit? Tik op het juiste voetstuk om het te benoemen.' },
      excludeHint: { en: 'Name it — or send it to the Mystery Gallery!', de: 'Benenne es — oder schick es in die Rätsel-Galerie!', fr: 'Nomme-le, ou envoie-le au Mystère !', es: '¡Nómbralo… o mándalo a la sala de misterio!', pt: 'Dê o nome — ou leve aos Mistérios!', it: 'Dagli un nome o mandalo nella Sala dei misteri!', nl: 'Geef het een naam — of stuur het naar de Raadselgalerij!' },
      fineHint: { en: 'Square or rectangle? Look closely!', de: 'Quadrat oder Rechteck? Schau genau hin!', fr: 'Carré ou rectangle ? Regarde bien !', es: '¿Cuadrado o rectángulo? ¡Míralo bien!', pt: 'Quadrado ou retângulo? Olhe bem!', it: 'Quadrato o rettangolo? Guarda bene!', nl: 'Vierkant of rechthoek? Kijk goed!' },
      nameHint: { en: 'Pip wants a {n}! Tap the {n}.', de: 'Pip sucht diese Form: {n}. Tippe darauf!', fr: 'Pip cherche un {n} ! Touche-le !', es: 'Pip quiere esta figura: un {n}. ¡Tócala!', pt: 'O Pip quer um {n}! Toque no {n}.', it: 'Pip vuole un {n}! Tocca il {n}.', nl: 'Tobbe zoekt deze vorm: {n}. Tik erop!' },
      matchHint: { en: 'Find the TWO {n}s — even if they look different!', de: 'Finde die ZWEI {np} — auch wenn sie anders aussehen!', fr: 'Trouve les DEUX {np} — même s’ils ont l’air différents !', es: 'Encuentra los DOS {np}… ¡aunque se vean distintos!', pt: 'Ache os DOIS {np} — mesmo que pareçam diferentes!', it: 'Trova i due {np}, anche se sembrano diversi!', nl: 'Vind de TWEE {np} — ook al zien ze er anders uit!' },
      confirmHint: { en: 'Pip says it is a {n}. Is that right?', de: 'Pip nennt es {n}. Stimmt das?', fr: 'Pip dit que c’est un {n}. C’est vrai ?', es: 'Pip dice que es un {n}. ¿Es cierto?', pt: 'O Pip diz que é um {n}. Isso está certo?', it: 'Pip dice che è un {n}. È giusto?', nl: 'Tobbe noemt het {n}. Klopt dat?' },
      capstoneHint: { en: 'Name every treasure and open the hall!', de: 'Benenne jeden Schatz und öffne den Saal!', fr: 'Nomme chaque trésor et ouvre la salle !', es: '¡Nombra cada tesoro y abre la sala!', pt: 'Dê o nome de tudo e abra o salão!', it: 'Dai un nome a ogni tesoro e apri la sala!', nl: 'Geef elke schat een naam en open de zaal!' },
      mystery: { en: 'Mystery Gallery', de: 'Rätsel-Galerie', fr: 'Mystère', es: 'Sala de misterio', pt: 'Galeria dos Mistérios', it: 'Sala dei misteri', nl: 'Raadselgalerij' },
      agree: { en: 'Yes!', de: 'Ja!', fr: 'Oui !', es: '¡Sí!', pt: 'Sim!', it: 'Sì!', nl: 'Ja!' },
      disagree: { en: 'No!', de: 'Nein!', fr: 'Non !', es: '¡No!', pt: 'Não!', it: 'No!', nl: 'Nee!' },
      win: { en: 'The hall is open! 🎀', de: 'Der Saal ist offen! 🎀', fr: 'La salle est ouverte ! 🎀', es: '¡La sala ya está abierta! 🎀', pt: 'O salão está aberto! 🎀', it: 'La sala è aperta! 🎀', nl: 'De zaal is open! 🎀' },
      revealTilted: { en: '{n}! Even turned — you knew.', de: '{n}! Auch gedreht — du hast’s gewusst.', fr: '{n} ! Même tourné, tu l’as reconnu.', es: '¡Un {n}! Aunque esté girado, lo supiste.', pt: '{n}! Mesmo girado, você sabia!', it: '{n}! Anche girato, lo sapevi!', nl: '{n}! Ook gedraaid — je wist het.' },
      misroute: { en: "Hmm — that's a {n}. It goes over here.", de: 'Hmm — {n} gehört dorthin!', fr: 'Hmm — c’est un {n}. Il va par ici.', es: 'Mmm… ¡el {n} va allá!', pt: 'Hmm — isso é um {n}. Ele vai aqui.', it: 'Ehm, questo è un {n}. Va qui.', nl: 'Hmm — dat is een {n}. Die hoort hierheen.' },
      hintCheck: { en: 'Name each treasure by its pedestal.', de: 'Benenne jeden Schatz an seinem Sockel.', fr: 'Nomme chaque trésor à son socle.', es: 'Nombra cada tesoro en su pedestal.', pt: 'Dê o nome de cada tesouro pelo pedestal.', it: 'Dai un nome a ogni tesoro sul piedistallo.', nl: 'Geef elke schat een naam bij zijn voetstuk.' }
    },
    defaults: {},
    ...
    _hint: function () {
      ...
      if (this.facet === 'match-pair') { var mh = api.t('matchHint'); return (LANG === 'de' || LANG === 'fr' || LANG === 'es' || LANG === 'pt' || LANG === 'it' || LANG === 'nl') ? mh.replace('{np}', gnamePl(this.matchName)) : mh.replace('{n}', this.matchName); }
    ...
    _route: function (type) {
      ...
      speak(type === 'mystery' ? (LANG === 'es' ? 'figura misteriosa' : LANG === 'de' ? 'Rätselform' : LANG === 'fr' ? 'forme mystère' : LANG === 'pt' ? 'Mistério' : LANG === 'it' ? 'Forma misteriosa' : LANG === 'nl' ? 'Raadselvorm' : 'Mystery') : gname(type));
      if (truth === type) {
        ...
        if (type !== 'mystery') { var nm = gname(truth); this.announce(this.api.t('revealTilted').replace('{n}', nm)); setTimeout(function () { speak(LANG === 'es' ? (nm + '! Aunque esté girado, lo supiste.') : LANG === 'de' ? (nm + '! Auch gedreht hast du es gewusst.') : LANG === 'fr' ? (nm + ' ! Même tourné, tu l’as reconnu.') : LANG === 'pt' ? (nm + '! Mesmo girado, você sabia.') : LANG === 'it' ? (nm + '! Anche girato, lo sapevi!') : LANG === 'nl' ? (nm + '! Ook gedraaid wist je het.') : (nm + '! Even turned, you knew')); }, 180); }
        ...
        var nm2 = truth === 'mystery' ? (LANG === 'es' ? 'figura misteriosa' : LANG === 'de' ? 'Rätselform' : LANG === 'fr' ? 'forme mystère' : LANG === 'pt' ? 'forma misteriosa' : LANG === 'it' ? 'mistero' : LANG === 'nl' ? 'raadselvorm' : 'mystery shape') : gname(truth);
    ...
    _pickNamed: function (i, card) {
      ...
      else { this.msg = this.api.t('misroute').replace('{n}', truth === 'mystery' ? (LANG === 'es' ? 'figura misteriosa' : LANG === 'de' ? 'Rätselform' : LANG === 'fr' ? 'forme mystère' : LANG === 'pt' ? 'forma misteriosa' : LANG === 'it' ? 'mistero' : LANG === 'nl' ? 'raadselvorm' : 'mystery shape') : gname(truth)); speak(LANG === 'es' ? ('Era un ' + gname(truth)) : LANG === 'de' ? ('Richtig wäre ' + gname(truth)) : LANG === 'fr' ? ('C’est plutôt un ' + gname(truth)) : LANG === 'pt' ? ('Isso é um ' + gname(truth)) : LANG === 'it' ? ('Era un ' + gname(truth)) : LANG === 'nl' ? ('Dat is een ' + gname(truth)) : ("That's a " + truth)); this.api.sound && this.api.sound(330); this.render(); }
    },
    ...
    _pickMatch: function (i) {
      ...
      if (this.selected.length === 2) {
        ...
        if (ok) { this.api.sound && this.api.sound(880); speak(LANG === 'es' ? ('¡Los dos son ' + gnamePl(this.matchName) + '!') : LANG === 'de' ? ('Beide sind ' + gnamePl(this.matchName) + '!') : LANG === 'fr' ? ('Ce sont deux ' + gnamePl(this.matchName) + ' !') : LANG === 'pt' ? ('Os dois são ' + gnamePl(this.matchName) + '!') : LANG === 'it' ? ('Sono due ' + gnamePl(this.matchName) + '!') : LANG === 'nl' ? ('Allebei ' + gnamePl(this.matchName) + '!') : ('Both ' + this.matchName + 's!')); setTimeout(function () { self._win(); }, 300); return; }
        this.msg = (LANG === 'es') ? ('Esos no son los dos ' + gnamePl(this.matchName) + '… ¡mira otra vez!') : (LANG === 'de') ? ('Das sind nicht beide ' + gnamePl(this.matchName) + ' — schau nochmal!') : LANG === 'fr' ? ('Ce ne sont pas deux ' + gnamePl(this.matchName) + ' — regarde encore !') : LANG === 'pt' ? ('Não são dois ' + gnamePl(this.matchName) + ' — olhe de novo!') : LANG === 'it' ? ('Non sono due ' + gnamePl(this.matchName) + ', guarda ancora!') : LANG === 'nl' ? ('Niet allebei ' + gnamePl(this.matchName) + ' — kijk nog eens!') : ("Not both " + this.matchName + "s — look again!"); this.selected = []; this.api.sound && this.api.sound(330);
      }
      this.render();
    },
    ...
    _judge: function (agree) {
      ...
      if (agree === claimTrue) {
        this.api.sound && this.api.sound(820); speak(truth === 'mystery' ? (LANG === 'es' ? 'figura misteriosa' : LANG === 'de' ? 'Rätselform' : LANG === 'fr' ? 'forme mystère' : LANG === 'pt' ? 'forma misteriosa' : LANG === 'it' ? 'Forma misteriosa' : LANG === 'nl' ? 'Een raadselvorm' : 'A mystery shape') : gname(truth));
        this.claimIdx++; this.msg = null;
        if (this.claimIdx >= this.claims.length) { setTimeout(function () { self._win(); }, 200); } else this.render();
      } else { this.msg = (LANG === 'es') ? 'Mira otra vez… ¿qué figura es?' : (LANG === 'de') ? 'Schau nochmal — welche Form ist das?' : LANG === 'fr' ? 'Regarde encore — quelle forme est-ce ?' : LANG === 'pt' ? 'Olhe de novo — que forma é essa?' : LANG === 'it' ? 'Guarda ancora: che forma è?' : LANG === 'nl' ? 'Kijk nog eens — welke vorm is het?' : "Look again — what shape IS it?"; this.api.sound && this.api.sound(330); this.render(); }
    },

    _win: function () {
      this.solved = true; this.solvedCount = Math.min(this.solvedCount + 1, (this._pool && this._pool.length) || 8);
      this.api.sound && this.api.sound(920); this.render(); this.announce(this.api.t('win')); speak(LANG === 'es' ? 'La sala ya está abierta.' : LANG === 'de' ? 'Der Saal ist offen.' : LANG === 'fr' ? 'La salle est ouverte.' : LANG === 'pt' ? 'O salão está aberto.' : LANG === 'it' ? 'La sala è aperta.' : 'The hall is open');
    },
