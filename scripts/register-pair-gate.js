/* =====================================================================
   register-pair-gate.js — the seven registration points for #53.
   Run:  node scripts/register-pair-gate.js
...
const KEY = 'pair-gate';
const PREV = 'rounding-hill';
const L = require('./_pair-gate-landing.js');
...
/* ---- 2. tool-content.ts x 4, EACH ANCHORED EXPLICITLY --------------
   ⚠ On #53 a single clever regex silently performed 2 of these 4 and
...
/* ---- 3. wrapper version — the one non-idempotent step ------------ */
{
  ...
  if (registered && m && m[1] === '7.87') {
    s = s.replace(`const TOOL_WRAPPER_VERSION = '${m[1]}'`, `const TOOL_WRAPPER_VERSION = '7.88'`);
    wr(f, s); step(`TOOL_WRAPPER_VERSION ${m[1]} → 7.88`, true);  /* ⚠ #40: the log must report the value ACTUALLY WRITTEN */
...
/* ---- 4. manipulatives.ts hub card -------------------------------- */
{
  ...
  if (s.indexOf(`id: "${KEY}"`) === -1) {
    const card = fs.readFileSync(path.join(__dirname, '_pair-gate-card.txt'), 'utf8');
