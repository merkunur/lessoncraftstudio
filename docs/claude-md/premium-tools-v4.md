...
| **The Pairing Rail / The Pair Gate** (leftovers from two odd collections pair with *each other*) | ⚠⚠ **ROW AMENDED 2026-08-10 AT A PEDAGOGY PANEL'S FORMAL REQUEST — the original text stated the WEAKEST available ground and that cost a whole build.** The pedagogy is right and research-backed (Strachota 2023: first graders shift from **pattern-based** (last-digit) to **structure-based** (pairing) definitions — this proposal's thesis, validated; Blanton 2024: sums-of-parities arguments by the **end of Grade 1**), and no competitor animates pairing (Toy Theater 71 tools = 0; Polypad = 0; Topmarks 4 of 5 are classification quizzes). **Cut on GATE 1: the apparatus is shipped three times over** — `ten-frame` `order:'pairs'` fills two-abreast in 11 locales and `twentypair` stages the theorem; printable **K-016** is the pairing sheet with `data-lcs-leftover`; **`lids`** is the N-abreast gate with an honest remainder (`MIN_LIDS 2`/`MAX_LIDS 4`, floor division); `estimation-jar` is the march-with-leftover; `compare-balance-core` is child-made 1:1 pairing whose leftover IS the answer. **Also GATE 4** — parity has no degrees of freedom; nothing the child chooses changes the answer. **Also GATE 2, as now MEASURED** rather than asserted: KMK Bildungsstandards **zero** occurrences of *gerade/ungerade*; Lgr22 **zero** *udda/jämn* across åk 1-9; absent from the Dutch kerndoelen; Finland exactly one clause; **only Norway names it** (LK20 Year-2 aim, *generelle eigenskapar ved partal og oddetal*). CCSS 2.OA.C.3 is the ONLY parity standard in K-5 and **caps the repertoire at 20 in the standard's own text**. ⚠ **AND THE 2026 REMAINDER RESCUE WAS ITSELF ALREADY SHIPPED.** Told only "a unit, not a year", a 2026 proposer tried to rescue it with more hours — an N-abreast gate for division-with-remainder — which is `lids` with a different skin, and which lands at **Grade 4** (4.OA.A.3, 4.NBT.B.6), above the K-3 ceiling. **A rejected-list row that states the weak reason invites exactly that rescue.** **→ SHIP AS A `ten-frame` PARITY REPERTOIRE, hooked to Norway's LK20** — zero new apparatus, zero new locale authoring, zero new gates. And `lids` `MAX_LIDS 4 → 5` with quotitive setups. ⚠⚠ **ROW AMENDED AGAIN 2026-08-11 — the tool was BUILT anyway (operator's call), shipped weak, and then REBUILT to answer these objections instead of restating them.** The rebuild's answers: GATE 4 — the parade is now CHOSEN from a 1-20 numeral strip and the prediction is a committed NUMERAL (0..k-1 left standing), so the child's choice changes what happens and the commit is a falsifiable computed claim (Strachota's structure-based definition as the literal question); the inverse problem ("find a parade that leaves exactly 2 at three abreast") exists. The old build's second parade was silently rigged away from multiples of k — an UNFALSIFIABLE theorem; it is now any size, evens included, and the fizzle is a reachable, honestly-announced outcome. GATE 1 — the moat is the GRAMMAR, not the pairing: choose → commit → reveal, three times per run (parade, second parade, sill) — ten-frame/K-016/lids display a pairing after the fact; only here does the class commit a numeral claim about the remainder BEFORE the structure reveals it. The → SHIP-AS note above is SUPERSEDED by the shipped standalone rebuild. |
