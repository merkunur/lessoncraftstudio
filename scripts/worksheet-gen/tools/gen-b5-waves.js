#!/usr/bin/env node
/**
 * gen-b5-waves.js — nt10-E wave files, two per locale:
 *   waves/wave-b5-<loc>.json     the 20 base specs
 *   waves/wave-b5var-<loc>.json  the 100 variation faces
 *
 *   node tools/gen-b5-waves.js [--locales=en,de,…] [--dry-run]
 *
 ...
 * every apply-b5-locale (Phase 4) — until then those waves carry only what builds.
 */
'use strict';
const fs = require('fs');
...
function writeWave(id, locale, m) {
  ...
  const plan = {
    ...
    _refused: m.refused,
    _note: 'nt10-E (generated by tools/gen-b5-waves.js — every themed type pinned by a MEASURED build; _refused = specs that build on no candidate in this locale, dropped from the wave and recorded, never padded)',
  };
...
for (const loc of LOCALES) {
  ...
  const wb = writeWave('wave-b5-' + loc, loc, mb), wf = writeWave('wave-b5var-' + loc, loc, mf);
