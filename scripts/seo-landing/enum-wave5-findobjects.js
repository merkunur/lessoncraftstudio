#!/usr/bin/env node
/* Wave 5 coupled enum (READ-ONLY): find-objects/i-spy + find-objects/find-odd × EN K (readiness).
 * Group by (mode, theme); canonical = earliest publishedAt; siblings = rest. slug carries a hash suffix.
 * MUST run on Hetzner. Usage: node scripts/seo-landing/enum-wave5-findobjects.js
 */
'use strict';
...
(async function () {
  ...
  var rows = await c.deck.findMany({
    where: { language: 'en', status: 'published', exerciseType: 'find-objects', exerciseMode: { in: ['i-spy', 'find-odd'] } },
