#!/usr/bin/env node
/**
 * generate-numeral-cards.ts — Pillar 5 Phase 1 Sub-Phase 1.2 entry point.
 *
 * Generates text-only numeral-card PDFs per teaching package + locale.
 * Variant of generate-picture-cards.ts; no image-library resolution; renders
 * numerals 1-N with optional localized number-words via NUMBER_WORDS resource.
 *
 * Output per (package, locale):
 *   /var/www/lcs-media/materials/numeral-cards/<locale>/<package>/print-{N}up.pdf
 *
 * USAGE
 *   # Single-package render
 *   npx tsx frontend/scripts/generate-numeral-cards.ts \
 *     --package count-objects-1-to-10 --locale en
 *
 *   # Mass-run all locales
 *   npx tsx frontend/scripts/generate-numeral-cards.ts \
 *     --package count-objects-1-to-10 \
...
import {
  loadAllNumeralCardsPackages,
  loadNumeralCardsPackagesBySlugs,
  parseNumeralRange,
  NumeralCardsPackage,
} from './lib/numeral-cards-package-loader';
import { closeBrowser, writePrintPdf } from './lib/numeral-cards-render';

const REPO_ROOT = path.resolve(__dirname, '..', '..');
const DEFAULT_OUTPUT = path.join(REPO_ROOT, 'frontend', '.scratch', 'numeral-cards');
...
function printHelp() {
  console.log(`generate-numeral-cards.ts — Pillar 5 Phase 1 Sub-Phase 1.2

Usage:
  npx tsx frontend/scripts/generate-numeral-cards.ts --package <slug> --locale <loc>
  npx tsx frontend/scripts/generate-numeral-cards.ts --all-packages --locales en,de

Flags:
  --package <slug>     Render single package
  --packages <list>    Comma-separated package slugs
  --all-packages       Iterate all packages with numeral-cards material
...
async function runPackageTask(task: PackageTask, args: CliArgs): Promise<PackageResult> {
  ...
  const numerals = parseNumeralRange(pkg.numeralRange);

  try {
    await writePrintPdf(
      numerals,
      {
        cardsPerPage: pkg.cardsPerPage,
        locale,
        showNumberWord: pkg.showNumberWord,
        borderStyle: pkg.borderStyle,
        roundedCorners: pkg.roundedCorners,
      },
      printPath
    );
    return {
...
async function main() {
  ...
  let packages: Map<string, NumeralCardsPackage>;
  if (args.allPackages) {
    packages = loadAllNumeralCardsPackages();
  } else if (args.packages.length > 0) {
    packages = loadNumeralCardsPackagesBySlugs(args.packages);
  } else {
