/* =====================================================================
   register-exchange-machine.js — wire TOOL #45 into the site
   ---------------------------------------------------------------------
   Run:  node scripts/register-exchange-machine.js [--dry-run]
...
const KEY = 'exchange-machine';
const PREV = 'build-plan';          /* the key this one is inserted after — #44 */
const E = require('./_exchange-machine-content.js');
...
LOCALES.forEach((l) => {
  const e = E[l];
  if (!e) die(`_exchange-machine-content.js has no entry for "${l}"`);
...
(function () {
  ...
  /* ⭐⭐ THE CATEGORY MUST BE A MEMBER OF THE UNION, READ OFF THE SOURCE.
     This one is column ARITHMETIC, so 'number' is right — but the
     guard still parses the union and refuses anything outside it,
     because #38 proved the silent version of this defect is the one
     that ships: toolCategory() falls back rather than erroring, so a
     wrong category mis-buckets the tool on the hub forever. */
