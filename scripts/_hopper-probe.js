#!/usr/bin/env node
/* =====================================================================
   _hopper-probe.js — multi-state control-bottom probe for "Hopper's Lily
   Hops" (2.NBT.A.2 skip-counting, clarity-first). Drives fill / fill-back /
   whichstep (initial) + a wrong-scaffold + resolved, and MEASURES the lowest
...
(async () => {
  ...
  const solve = (ok) => page.evaluate((correct) => {
    const t = window.SkipCountActivity, r = t._round, Core = window.SkipCountCore;
    const a = Core.answerValue(r), val = correct ? a : Core.choices(r).find(c => c !== a);
    const b = Array.from(document.querySelectorAll('.sc-cand')).find(x => Number(x.textContent.replace(/[^0-9]/g, '')) === val); if (b) b.click();
  }, ok).then(() => sleep(50));

  const states = {
    'fill5': async () => { await force('sc-fill-5-mid'); },
    'fillback': async () => { await force('sc-fill-10-back'); },
    'step': async () => { await force('sc-step-10'); },
    'wrong': async () => { await force('sc-fill-10-mid'); await solve(false); },
    'resolved': async () => { await force('sc-fill-10-cross'); await solve(true); }
  };

  async function controlBottom() {
    return await page.evaluate(() => {
      const sel = '.sc-root,.sc-say,.sc-board,.sc-scene,.sc-row,.sc-cand,.sc-line-msg,.lcs-activity-check';
  ...
  async function checks() {
    return await page.evaluate(() => {
      const taps = Array.from(document.querySelectorAll('.sc-cand'));
  ...
  if (!(await page.evaluate(() => window.SkipCountActivity._resolved))) fails.push('a correct tap did NOT resolve');
  await states.wrong();
  if (await page.evaluate(() => window.SkipCountActivity._resolved)) fails.push('a wrong number resolved — must not advance');

  const order = ['fill5', 'fillback', 'step', 'wrong', 'resolved'];
  ...
  console.log('');
  if (fails.length) { console.error(`HOPPER-PROBE FAILED — ${fails.length}:`); fails.forEach(f => console.error('  • ' + f)); process.exit(1); }
  console.log(`HOPPER-PROBE PASSED — fill/fillback/step/wrong/resolved: a correct tap resolves + a wrong one does not advance; lowest control (incl. shell Check) clears the fold by ≥${MARGIN}px across 320→1366; controls ≥${CTRL_MIN}px (off the 320 sub-floor), no overlap, no horizontal overflow.`);
  process.exit(0);
