...
## 22. SEO / Landing-Page Program — tier-3 deck landing pages (2026-06; IN PROGRESS)
...
**Sequence (plan P3.b):** EN waves 1→8 — **1 LIVE** addition·image-image·K (43) → **1b LIVE** +image-number+mixed (143) → **2 LIVE** subtraction (cross-out+image-number) + chart-count·K (137) → **3 LIVE** Math re-leveled Gr1 (142) → **4 LIVE** Literacy·K (find-and-count + prepositions-mc + bingo + word-guess; 162) → **5 LIVE** Logic·K (sudoku + picture-sort + odd-one-out + find-objects + matching; 423 — the largest wave; **+ picture-sort de-orphan DONE**) → **6 LIVE** Spatial·K (pattern-train + pattern-worksheet + picture-trail + shadow-match; 392 — all-readiness; NEW "Patterning (readiness)" label; pattern-worksheet conditional-SHIP) → **7 LIVE** Readiness-class + Preschool (alphabet-train + big-small + grid-match + missing-pieces + more-less; 285 — FIRST sub-K band; 5 new readiness labels; Preschool level-wiring) → **8 NEXT** Gr2 (crossword, cryptogram — first Gr2 band, EN waves' last). Then full-market locales **de → es → nl → fr → it → pt → sv → da** (operator may reorder the middle eight on market priority) → **no, fi LAST** (sample-only; honesty-filter in as their decks publish). **The picture-sort de-orphan (the program's first live-query hub change) shipped in Wave 5** via `themeSubjectTagsWhere` `hasSome`-over-exact-`-vs-`-component-keys (§22.1), with its own verify + rollback.
...
**Carry-forward — three distinct DEFER classes (do not conflate; each resolves differently):** (a) **grade-deferred** — the type's grade is outside the current wave: **Gr1** (wordsearch, word-scramble), **Gr2** (the 71 math-puzzle/code-addition breachers [out-of-tree `wave3-mathpuzzle-gr2-breachers.md`] + treasure-hunt cardinal) — these get their wave when the matching grade band ships. (b) **breadth-non-viable** — the skill is real but the type cannot fill a themed-landing wave: **cryptogram** (5-7 tag but **themeless, 0 themes / 100 decks**) + **treasure-hunt's ≤3-theme** modes — deferred-non-viable-by-breadth, **NOT a removal** (revisit only if a non-themed landing model is ever built). (c) **browse-hub defect** — the `/en/topic/sudoku` 404 above (a registered axis-key whose browse hub 404s; not a landing-program task).
