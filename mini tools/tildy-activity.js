...
(function (global) {
  ...
  global.TildyActivity = {
    ...
    strings: {
      title: { en: "Tildy's Tailor Shop", de: 'Tildys Schneiderei', fr: 'L’atelier de Tildy', es: 'El taller de Tildy', pt: 'O Ateliê da Tildy', it: 'La sartoria di Tildy' },
      instruction: { en: '', de: '', fr: '', es: '', pt: '', it: '' },
      prompt: { en: 'Measure the strip.', de: 'Miss den Streifen.', fr: 'Mesure la bande.', es: 'Mide la tira.', pt: 'Meça a tira.', it: 'Misura la striscia.' },
      measureIt: { en: 'Measure it ✂️', de: 'Messen ✂️', fr: 'Mesurer ✂️', es: 'Medir ✂️', pt: 'Medir ✂️', it: 'Misura ✂️' },
      sayWelcome: { en: 'Slide the ruler so 0 is right at the start of the strip!', de: 'Schieb das Lineal so, dass die 0 genau am Anfang des Streifens liegt!', fr: 'Fais glisser la règle pour que le 0 soit juste au début de la bande !', es: '¡Desliza la regla para que el 0 quede justo al inicio de la tira!', pt: 'Deslize a régua até o 0 encostar bem no começo da tira!', it: 'Fai scorrere il righello: metti lo 0 dove comincia la striscia!' },
      sayWin: { en: 'Measured and wrapped! 🎀', de: 'Gemessen und eingepackt! 🎀', fr: 'Mesuré et emballé ! 🎀', es: '¡Medido y listo! 🎀', pt: 'Medido e embrulhado! 🎀', it: 'Misurata e impacchettata! 🎀' },
      sayOffShort: { en: 'The 0 wasn’t at the start — see, it reads one short. Slide it to the start!', de: 'Die 0 war nicht am Anfang — schau, jetzt misst du einen cm zu wenig. Schieb sie an den Anfang!', fr: 'Le 0 n’était pas au début — tu vois, ça mesure un cm de trop peu. Fais-le glisser jusqu’au début !', es: 'El 0 no estaba al inicio; mira, así mides un centímetro de menos. ¡Deslízala hasta el inicio!', pt: 'O 0 não estava no começo — deu um centímetro a menos. Deslize até encostar no começo!', it: 'Lo 0 non era dove comincia la striscia: vedi, misura uno in meno. Portalo dove comincia!' },
      sayNotAligned: { en: 'The 0 isn’t at the start yet — line it up, then measure.', de: 'Die 0 ist noch nicht am Anfang — leg sie genau an, dann miss.', fr: 'Le 0 n’est pas encore au début — aligne-le, puis mesure.', es: 'El 0 todavía no está al inicio. Alinéalo justo ahí y luego mide.', pt: 'O 0 ainda não está no começo — encoste ele no começo e meça.', it: 'Lo 0 non sta ancora dove comincia la striscia: allinealo, poi misura.' },
      sayRead: { en: 'Great — now read the number at the far end!', de: 'Super — jetzt lies die Zahl am anderen Ende ab!', fr: 'Super — maintenant lis le nombre à l’autre bout !', es: '¡Muy bien! Ahora lee el número en el otro extremo.', pt: 'Isso! Agora leia o número na outra ponta!', it: 'Bene: ora leggi il numero in fondo!' },
      sayMisread: { en: 'Look again at the far end — which number?', de: 'Schau noch mal ans andere Ende — welche Zahl?', fr: 'Regarde encore l’autre bout — quel nombre ?', es: 'Mira otra vez el otro extremo, ¿qué número es?', pt: 'Olhe de novo na outra ponta — qual número?', it: 'Guarda ancora in fondo: quale numero?' },
      sayAgain: { en: 'Hmm — let’s look again.', de: 'Hmm — schauen wir noch mal.', fr: 'Hmm — regardons encore.', es: 'Mmm… vamos a mirar otra vez.', pt: 'Hmm — vamos olhar de novo.', it: 'Hmm... guardiamo ancora.' },
      howMany: { en: 'How many cm?', de: 'Wie viele cm?', fr: 'Combien de cm ?', es: '¿Cuántos cm?', pt: 'Quantos cm?', it: 'Quanti cm?' },
      diagZero: { en: 'Started at 1', de: 'Bei 1 angefangen', fr: 'Commencé à 1', es: 'Empezó en el 1', pt: 'Começou no 1', it: 'Partito da 1' }, diagShort: { en: 'Too short', de: 'Zu kurz', fr: 'Trop courte', es: 'Muy corta', pt: 'Régua curta', it: 'Troppo corto' }, diagEnd: { en: 'Wrong end', de: 'Falsches Ende', fr: 'Mauvais bout', es: 'Extremo equivocado', pt: 'Ponta errada', it: 'Lato sbagliato' },
    ...
    _renderSelectTool: function (root) {
      ...
      var sv = svg('svg', { viewBox: '0 0 ' + (cmX(TABLE) + MARGIN) + ' 26', class: 'td-svg td-svgstrip', 'aria-label': (LANG === 'es' ? 'una tira para medir' : LANG === 'de' ? 'ein Streifen zum Messen' : LANG === 'fr' ? 'une bande à mesurer' : LANG === 'pt' ? 'uma tira para medir' : LANG === 'it' ? 'una striscia da misurare' : 'a strip to measure') });
      ...
      (this._opts || []).forEach(function (L) {
        var b = api.el('button', 'td-opt td-optword'); b.type = 'button'; b.textContent = (LANG === 'es' ? ('regla de ' + L + ' cm') : LANG === 'de' ? ('Lineal ' + L + ' cm') : LANG === 'fr' ? ('règle de ' + L + ' cm') : LANG === 'pt' ? ('régua de ' + L + ' cm') : LANG === 'it' ? ('righello da ' + L + ' cm') : (L + '-cm ruler'));
    ...
    _correct: function () {
      ...
      setTimeout(function () { speak(LANG === 'es' ? '¡Medido!' : LANG === 'de' ? 'Gemessen!' : LANG === 'fr' ? 'Mesuré !' : LANG === 'pt' ? 'Medido!' : LANG === 'it' ? 'Misurata!' : 'Measured!'); }, 160);
