...
## 🏁 NSR COMPLETE — all 10 non-EN locales reviewed by native linguists (2026-06-14, commits `8333a4f9` source)
**The §17.5.1 NSR-flag is CLEARED.** 10 expert native-linguist agents (one per locale) reviewed BOTH verticals' authored text (15 lit + 21 sci titles/instructions, 8 family names/slugs/skill-sentences, science bin labels, category + vowel/consonant labels). **52 corrections applied; es + fr passed clean** (native-confident). Notable: **no (Norwegian) science display strings had æ/ø/å wrongly ASCII-stripped** (hoerer/gronnsaker/kjaeledyr…) — restored across all 21 science strings + 6 bin labels + family name + 1 slug; de 5 (skill-sentence grammar), da 5, fi 5, sv 3, pt 2, nl 1, it 1.
- **53 (type,locale) pairs regenerated** (titles/instructions/bin-labels are baked into the worksheet JPEG → regen required) → **257 decks republished** (batch-20260614011629: 256 in-place UPDATE via `--updates-manifest` slug-stable + 1 INSERT for the 1 slug change) + OG (513) + hreflang (7630 rewrites) + deploy.sh (hub names/slug). 0 fail.
- **1 slug fix** (no science-sequence ø-fold: rekkefolge→**rekkefoelge**): new slug live (200), old slug unpublished/archived (404, ~1-day-old unlinked → no redirect).
- **Verified live:** no decks clean native Norwegian (0 stripped artifacts), de/da/fi/sv/pt/nl/it fixes applied, slug change 200/404.
- Tooling (committed): `i18n/build-nsr-bundle.js` + `apply-nsr-corrections.js`; review files `.nsr-*` out-of-tree. **No NSR-flag remains on either vertical.**
...
### EN CATALOG COMPLETE (commit `54158810`) — 15 types, STOP-before-fan per operator
3 new factory modes (Gate-2 round-2 APPROVED): lit-letter-knowledge `find-letter-grid` + `vowel-consonant`; lit-word-build `build-the-word`. 1 new family key **phonological-awareness** ("Sounds & Syllables"). 10 new specs K-226..K-235 (K-228 sort-by-first / K-234 syllable-sort / K-235 category REUSE science-category-sort; K-232 rhyming REUSE science-pair-match — 0 new render code). Full list by family key: beginning-sounds {K-221,K-228} · phonological-awareness {K-226 ending, K-227 middle, K-232 rhyming, K-233 syllable-count, K-234 syllable-sort} · letter-knowledge {K-222,K-223,K-229 find-letter,K-230 vowel/consonant} · word-building {K-224,K-231 build-word} · picture-vocabulary {K-225,K-235 category}. **HONEST DROP: opposites** (asset-thin: day/night/big/small not single-noun-imageable, turtle off-pool → 3-4 clean pairs only; logged, revisitable). VERIFIED: 15 ZIPs 0 fail, 15 distinct titles, descs 120-170, reuse-factory types render correct, **cannibalization gate ALL 15 PASS (worst 0.047)**. Data generated correct-by-construction (every noun approved+cached). NEXT = native 11-locale fan (deferred per operator; EN now complete) → scoped publish.
