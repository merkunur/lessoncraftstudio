#!/usr/bin/env node
/* Wave 5 lead coordinate enumeration (READ-ONLY): sudoku/null + sudoku/medium × EN K (readiness).
 * Group by (mode, theme); canonical = earliest publishedAt; siblings = rest. slugTheme theme-derived.
 * MUST run on Hetzner. Usage: node scripts/seo-landing/enum-wave5-sudoku.js
 */
'use strict';
...
(async function () {
  ...
  var rows = await c.deck.findMany({
    where: { language: 'en', status: 'published', exerciseType: 'sudoku' },
