...
(function (global) {
  ...
  /* nl cardinals 0-12, STANDALONE form (the reveal speaks the bare number → 1 = „één" met accenten, het TELWOORD, niet het lidwoord „een"). */
  var WORDS_NL = { 0: 'nul', 1: 'één', 2: 'twee', 3: 'drie', 4: 'vier', 5: 'vijf', 6: 'zes', 7: 'zeven', 8: 'acht', 9: 'negen', 10: 'tien', 11: 'elf', 12: 'twaalf' };
  var WORDS_SV = { 0: 'noll', 1: 'ett', 2: 'två', 3: 'tre', 4: 'fyra', 5: 'fem', 6: 'sex', 7: 'sju', 8: 'åtta', 9: 'nio', 10: 'tio', 11: 'elva', 12: 'tolv' };
  var LANG = 'en';
  function numWord(n) { var w = (LANG === 'de' ? WORDS_DE : LANG === 'fr' ? WORDS_FR : LANG === 'es' ? WORDS_ES : LANG === 'pt' ? WORDS_PT : LANG === 'it' ? WORDS_IT : LANG === 'nl' ? WORDS_NL : LANG === 'sv' ? WORDS_SV : WORDS)[n]; return (w != null) ? w : String(n); }
  ...
  global.SharingJarActivity = {
    ...
    /* nl (SLO-kerndoelen, Groep 3): Pim & Bo BEHOUDEN (cross-locale; #7 "Pip" = ander spel, nooit samen op één
       scherm). kralen; pot/potje (NIET glas/bokaal); scheppen; evenveel; het verschil (reveal) / hoeveel meer
       (vraag); eerlijk (story) vs gelijk (fairBridge). ⚠ "morsen" VERMEDEN (= vloeistof) → kralen "zijn eruit
       gerold" / pot "is omgevallen". één = TELWOORD (accenten). Apostrof VERMEDEN → "de pot van Bo" (nooit
       "Bo's"). GRADE = Groep 3 (grade '1' → Groep 3 auto, GEEN override, = de Duitse Klasse 1). STRAND override
       nl:'Getallen en bewerkingen' (auto "Bewerkingen" = geen SLO-domein; OA-relatie → volgt numbers-court #10,
       niet tally-squirrel #15 "Getallen"). */
    strings: {
      title: { en: 'The Sharing Jar', de: 'Das faire Perlenglas', fr: 'Le pot de perles juste', es: 'El frasco justo', pt: 'O pote justo', it: 'Il barattolo giusto', nl: 'De eerlijke kralenpot', sv: 'Den rättvisa pärlburken' },
      instruction: { en: 'Make it fair — say the kind number.', de: 'Mach es fair – sag die richtige Zahl.', fr: 'Rends le partage juste : dis le bon nombre.', es: 'Hazlo parejo: di el número correcto.', pt: 'Vamos deixar justo — diga o número gentil.', it: 'Rendi giusto: dì il numero gentile.', nl: 'Maak het eerlijk – zeg het juiste getal.', sv: 'Gör det rättvist: räkna ut talet och tryck på det.' },
      prompt: { en: 'Make it fair!', de: 'Mach es fair!', fr: 'Rends le partage juste !', es: '¡Hazlo parejo!', pt: 'Deixe justo!', it: 'Rendilo giusto!', nl: 'Maak het eerlijk!', sv: 'Gör det rättvist!' },
      qEqualize: { en: 'How many MORE beads does Pim need to match Bo?', de: 'Wie viele Perlen braucht Pim NOCH, um genauso viele wie Bo zu haben?', fr: 'Combien de perles Pim doit-il ajouter pour en avoir autant que Bo ?', es: '¿Cuántas cuentas MÁS necesita Pim para igualar a Bo?', pt: 'Quantas contas a mais o Pim precisa para ficar igual ao Bo?', it: 'Quante perline in PIÙ servono a Pim per pareggiare Bo?', nl: 'Hoeveel kralen heeft Pim NOG nodig om evenveel als Bo te hebben?', sv: 'Hur många pärlor TILL behöver Pim för att ha lika många som Bo?' },
      qCompare: { en: "How many MORE beads does Bo's jar have?", de: 'Wie viele Perlen hat Bos Glas MEHR?', fr: 'Combien de perles le pot de Bo a-t-il de plus ?', es: '¿Cuántas cuentas MÁS tiene el frasco de Bo?', pt: 'Quantas contas a mais tem o pote do Bo?', it: 'Quante perline in PIÙ ha il barattolo di Bo?', nl: 'Hoeveel kralen heeft de pot van Bo MEER?', sv: 'Hur många FLER pärlor har Bos burk än Pims?' },
      qRestore: { en: 'Some got knocked into the spill! How many to give Pim back?', de: 'Ein paar sind verschüttet! Wie viele bekommt Pim zurück?', fr: 'Quelques perles sont tombées ! Combien Pim en récupère-t-il ?', es: '¡Algunas se cayeron! ¿Cuántas hay que devolverle a Pim?', pt: 'Algumas contas caíram! Quantas devolver para o Pim?', it: 'Alcune sono cadute! Quante ne ridai a Pim?', nl: 'Een paar kralen zijn eruit gerold! Hoeveel krijgt Pim er terug?', sv: 'Några pärlor ramlade ut! Hur många ska Pim få tillbaka?' },
      qReduce: { en: 'Pim scooped too many! How many to put back so each has {T}?', de: 'Pim hat zu viele Perlen genommen! Wie viele müssen zurück, damit jeder {T} hat?', fr: 'Pim a pris trop de perles ! Combien doit-il en remettre pour que chacun en ait {T} ?', es: '¡Pim juntó de más! ¿Cuántas hay que regresar para que cada quien tenga {T}?', pt: 'O Pim pegou demais! Quantas devolver para cada um ficar com {T}?', it: 'Pim ne ha prese troppe! Quante ne rimetti così ognuno ne ha {T}?', nl: 'Pim heeft te veel kralen gepakt! Hoeveel moeten er terug zodat ze allebei {T} hebben?', sv: 'Pim tog för många! Hur många ska tillbaka så att Pim har {T}?' },
      qStart: { en: 'Bo gave {k} away and now has {r}. How many did Bo START with?', de: 'Bo hat {k} verschenkt und hat jetzt {r}. Wie viele hatte Bo am ANFANG?', fr: 'Bo a donné {k} perles et il en a {r} maintenant. Combien Bo en avait-il au DÉPART ?', es: 'Bo regaló {k} y ahora tiene {r}. ¿Con cuántas EMPEZÓ Bo?', pt: 'O Bo deu {k} e agora tem {r}. Com quantas o Bo começou?', it: 'Bo ha regalato {k} perline e ora ne ha {r}. Quante ne aveva Bo all\'INIZIO?', nl: 'Bo heeft er {k} weggegeven en heeft nu {r}. Hoeveel had Bo in het BEGIN?', sv: 'Bo gav bort {k} och har nu {r}. Hur många hade Bo från BÖRJAN?' },
      qZero: { en: 'How many more does Pim need to match Bo?', de: 'Wie viele braucht Pim noch, um genauso viele wie Bo zu haben?', fr: 'Combien de perles Pim doit-il ajouter pour en avoir autant que Bo ?', es: '¿Cuántas más necesita Pim para igualar a Bo?', pt: 'Quantas contas a mais o Pim precisa para ficar igual ao Bo?', it: 'Quante ne servono a Pim per pareggiare Bo?', nl: 'Hoeveel heeft Pim er nog nodig om evenveel als Bo te hebben?', sv: 'Hur många pärlor TILL behöver Pim för att ha lika många som Bo?' },
      hint: { en: 'Work it out, then tap that number.', de: 'Rechne es aus und tippe dann auf die Zahl.', fr: 'Réfléchis, puis touche le bon nombre.', es: 'Piénsalo y toca ese número.', pt: 'Descubra e toque nesse número.', it: 'Pensaci, poi tocca quel numero.', nl: 'Reken het uit en tik dan op het getal.', sv: 'Räkna ut det och tryck sedan på talet.' },
      hintZero: { en: 'Look closely — maybe it is already fair!', de: 'Schau genau hin – vielleicht ist es schon fair!', fr: 'Regarde bien… c’est peut-être déjà juste !', es: 'Fíjate bien: ¡a lo mejor ya está parejo!', pt: 'Olhe bem — talvez já esteja justo!', it: 'Guarda bene: forse è già pari!', nl: 'Kijk goed – misschien is het al eerlijk!', sv: 'Titta noga på båda burkarna och räkna.' },
      had: { en: 'had {n}', de: 'hatte {n}', fr: 'en avait {n}', es: 'tenía {n}', pt: 'tinha {n}', it: 'aveva {n}', nl: 'had {n}', sv: 'hade {n}' },
      fairIs: { en: 'fair = {n}', de: 'fair = {n}', fr: 'juste = {n}', es: 'parejo = {n}', pt: 'justo = {n}', it: 'giusto = {n}', nl: 'eerlijk = {n}', sv: 'ska bli {n}' },
      gaveAway: { en: 'gave {k} away', de: '{k} verschenkt', fr: 'a donné {k}', es: 'regaló {k}', pt: 'deu {k}', it: 'ha regalato {k}', nl: '{k} weggegeven', sv: 'gav bort {k}' },
      lookAgain: { en: 'Look again — how many more does Pim need?', de: 'Schau noch mal – wie viele braucht Pim noch?', fr: 'Regarde encore : combien Pim doit-il en ajouter ?', es: 'Mira otra vez: ¿cuántas más necesita Pim?', pt: 'Olhe de novo — quantas a mais o Pim precisa?', it: 'Guarda di nuovo: quante ne servono ancora a Pim?', nl: 'Kijk nog eens – hoeveel heeft Pim er nog nodig?', sv: 'Titta en gång till och räkna ut det igen.' },
      revealEqualize: { en: 'Yes! Now both have {n} — fair! 💛', de: 'Ja! Jetzt haben beide {n} – fair! 💛', fr: 'Oui ! Maintenant chacun en a {n} — c’est juste ! 💛', es: '¡Sí! Ahora los dos frascos tienen {n}: ¡parejo! 💛', pt: 'Isso! Agora os dois têm {n} — justo! 💛', it: 'Sì! Ora tutti e due ne hanno {n} — giusto! 💛', nl: 'Ja! Nu hebben ze allebei {n} – eerlijk! 💛', sv: 'Ja! Båda har {n} pärlor nu. Det är rättvist! 💛' },
      revealCompare: { en: "Right! Bo's jar has {u} more. That's the gap.", de: 'Richtig! Bos Glas hat {u} mehr. Das ist der Unterschied.', fr: 'Bravo ! Le pot de Bo en a {u} de plus. C’est l’écart.', es: '¡Correcto! El frasco de Bo tiene {u} más. Esa es la diferencia.', pt: 'Certo! O pote do Bo tem {u} a mais. Essa é a diferença.', it: 'Esatto! Il barattolo di Bo ne ha {u} in più. Ecco la differenza.', nl: 'Goed zo! De pot van Bo heeft er {u} meer. Dat is het verschil.', sv: 'Precis! Bos burk har {u} fler. Det är skillnaden.' },
      revealRestore: { en: 'Restored to {n} — Pim is happy again! 💛', de: 'Wieder bei {n} – Pim freut sich! 💛', fr: 'De retour à {n} — Pim est content ! 💛', es: '¡Devueltas hasta {n}! Pim ya está feliz otra vez. 💛', pt: 'De volta a {n} — o Pim ficou feliz de novo! 💛', it: 'Di nuovo {n} perline — Pim è felice! 💛', nl: 'Weer terug op {n} – Pim is blij! 💛', sv: 'Tillbaka på {n} pärlor. Pim är glad igen! 💛' },
      revealReduce: { en: 'Now each can have {n} — fair! 💛', de: 'Jetzt hat jeder {n} – fair! 💛', fr: 'Maintenant chacun en a {n} — c’est juste ! 💛', es: 'Ahora cada quien puede tener {n}: ¡parejo! 💛', pt: 'Agora cada um pode ter {n} — justo! 💛', it: 'Ora ognuno può averne {n} — giusto! 💛', nl: 'Nu hebben ze allebei {n} – eerlijk! 💛', sv: 'Nu har Pim {n} pärlor. Det är rättvist! 💛' },
      revealStart: { en: 'Yes! Bo started with {u} beads.', de: 'Ja! Bo hatte am Anfang {u} Perlen.', fr: 'Oui ! Bo en avait {u} au départ.', es: '¡Sí! Bo empezó con {u} cuentas.', pt: 'Isso! O Bo começou com {u} contas.', it: 'Sì! All\'inizio Bo aveva {u} perline.', nl: 'Ja! Bo had in het begin {u} kralen.', sv: 'Ja! Från början hade Bo {u} pärlor.' },
      revealZero: { en: "They're already the SAME — that's fair! Give none. 💛", de: 'Sie haben schon GLEICH viele – das ist fair! Gib keine. 💛', fr: 'Ils en ont déjà AUTANT — c’est juste ! N’en ajoute aucune. 💛', es: '¡Ya están IGUALES: parejo! No des ninguna. 💛', pt: 'Já estão IGUAIS — isso é justo! Não precisa dar nenhuma. 💛', it: 'Sono già UGUALI — è giusto! Non darne nessuna. 💛', nl: 'Ze hebben al EVENVEEL – dat is eerlijk! Geef er geen. 💛', sv: 'De har redan lika många. Det är rättvist! Ge inga alls. 💛' },
      ...
      pim: { en: 'Pim', de: 'Pim', fr: 'Pim', es: 'Pim', pt: 'Pim', it: 'Pim', nl: 'Pim', sv: 'Pim' }, bo: { en: 'Bo', de: 'Bo', fr: 'Bo', es: 'Bo', pt: 'Bo', it: 'Bo', nl: 'Bo', sv: 'Bo' }, fairBridge: { en: 'fair', de: 'fair', fr: 'juste', es: 'parejo', pt: 'justo', it: 'giusto', nl: 'gelijk', sv: 'lika' }
    },
    defaults: {},
    ...
    _produce: function (n) {
      ...
      else { this.api.sound && this.api.sound(330); this.msg = this.api.t('lookAgain'); speak(LANG === 'de' ? 'schau noch mal' : LANG === 'fr' ? 'regarde encore' : LANG === 'es' ? 'mira otra vez' : LANG === 'pt' ? 'olhe de novo' : LANG === 'it' ? 'guarda di nuovo' : LANG === 'nl' ? 'kijk nog eens' : LANG === 'sv' ? 'titta en gång till' : 'look again'); this.render(); }
    },
    _reconcile: function () {
    ...
    injectCSS: function () {
      ...
      var css = ''
        ...
        + '.sj-badge{font:700 clamp(12px,3vw,15px)/1.15 "Nunito",sans-serif;color:#2F6D62;background:#FFF7E8;border-radius:8px;padding:3px 8px;}'
        + '.sj-bridge{align-self:center;font:800 clamp(20px,5.5vw,28px)/1 "Baloo 2",sans-serif;color:#6E6353;min-width:24px;text-align:center;margin-bottom:18px;}.sj-bridge-on{color:' + C.T + ';}'
        /* tile row */
