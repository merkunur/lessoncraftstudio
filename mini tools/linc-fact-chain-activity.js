...
(function (global) {
  ...
  function lizardSVG() {
    return '<svg class="lfc-liz-svg" viewBox="0 0 100 100" role="img" aria-label="' + (LANG === 'de' ? 'Ketti, die Eidechse' : LANG === 'fr' ? 'Linc le lézard' : LANG === 'es' ? 'Linc la lagartija' : LANG === 'pt' ? 'Elo, o lagarto' : LANG === 'it' ? 'Lia la lucertola' : 'Linc the lizard') + '">' +
  ...
  global.LincFactChainActivity = {
    ...
    strings: {
      title: { en: "Linc's Fact Chain", de: 'Kettis Faktenkette', fr: 'La chaîne de faits de Linc', es: 'La cadena de hechos de Linc', pt: 'A corrente de fatos do Elo', it: 'La catena di fatti di Lia' },
      ...
      prompt: { en: 'Read it, then tap the fact that connects.', de: 'Lies den Fakt. Tippe dann auf das, was dazu passt.', fr: 'Lis le fait, puis touche ce qui va avec.', es: 'Léelo y toca el hecho que se conecta.', pt: 'Leia e toque no fato que se conecta.', it: 'Leggilo e tocca il fatto che si collega.' },
      lincIntro: { en: 'Facts link together like a chain — find the next link!', de: 'Fakten hängen zusammen wie eine Kette – finde das nächste Glied!', fr: 'Les faits s’enchaînent comme une chaîne — trouve le maillon suivant !', es: 'Los hechos se unen como una cadena. ¡Encuentra el siguiente eslabón!', pt: 'Os fatos se ligam como uma corrente — ache o próximo elo!', it: 'I fatti si uniscono come una catena — trova il prossimo anello!' },
      hintPick: { en: 'Think: what would really happen after the first fact?', de: 'Überlege: Was passiert nach dem ersten Fakt wirklich?', fr: 'Réfléchis : que se passerait-il vraiment après le premier fait ?', es: 'Piensa: ¿qué pasa de verdad después del primer hecho?', pt: 'Pense: o que acontece de verdade depois do primeiro fato?', it: 'Pensa: che cosa succede davvero dopo il primo fatto?' },
      hintWrong: { en: 'That one does not fit. What follows from the first fact?', de: 'Der passt nicht. Was folgt aus dem ersten Fakt?', fr: 'Celui-là ne va pas. Que se passe-t-il après le premier fait ?', es: 'Ese no va. ¿Qué sigue después del primer hecho?', pt: 'Esse não se liga. O que vem depois do primeiro fato?', it: 'Questo non si collega. Che cosa viene dopo il primo fatto?' },
      win: { en: 'Yes! Those two facts connect. 🔗', de: 'Ja! Diese zwei Fakten gehören zusammen. 🔗', fr: 'Oui ! Ces deux faits vont ensemble. 🔗', es: '¡Sí! Esos dos hechos se conectan. 🔗', pt: 'Isso! Esses dois fatos se ligam. 🔗', it: 'Sì! Questi due fatti si collegano. 🔗' }
    },
    defaults: {},
    ...
    render: function () {
      ...
      var stem = api.el('button', 'lfc-stem'); stem.type = 'button'; stem.setAttribute('aria-label', LANG === 'de' ? 'den ersten Fakt anhören' : LANG === 'fr' ? 'écouter le premier fait' : LANG === 'es' ? 'escucha el primer hecho' : LANG === 'pt' ? 'ouvir o primeiro fato' : LANG === 'it' ? 'ascolta il primo fatto' : 'hear the first fact');
