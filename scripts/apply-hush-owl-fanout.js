#!/usr/bin/env node
/* =====================================================================
   apply-hush-owl-fanout.js — batch-apply the 10 native-agent
   deliverables (scratchpad hush-owl-<loc>.json) to:
     1. mini tools/hush-owl.js         — strings corrections (exactly-once
        single-line rebuild per key; optional trailing comma)
     2. frontend/messages/tool-content/<loc>.json — the ToolEntry
     3. frontend/lib/manipulatives.ts  — ONE new hush-owl entry from
...
const TOOL_FILE = path.join(REPO, 'mini tools', 'hush-owl.js');
...
const deliverables = {};
for (const L of LOCS) {
  const f = path.join(SCRATCH, `hush-owl-${L}.json`);
...
console.log('\n1. strings corrections (hush-owl.js)');
...
for (const L of LOCS) {
  ...
  for (const key of Object.keys(corr)) {
    ...
    /* the privacy promise cannot shrink to a fragment */
    if (key === 'privacy' && (val.length < 80 || !/—/.test(val))) { E(`${L}.privacy: correction breaks the triple-verb shape (len ${val.length})`); continue; }
...
for (const L of LOCS) {
  ...
  if (/\bdB\b|[Dd]ecibel|[Dd]ezibel|[Dd]écibel|desibel|decibelli/.test(JSON.stringify(te))) E(`${L}: toolEntry mentions decibels (numbers-free is pedagogy-locked)`);
  const file = path.join(REPO, 'frontend', 'messages', 'tool-content', `${L}.json`);
  const j = JSON.parse(fs.readFileSync(file, 'utf8'));
  const out = {};
  for (const k of Object.keys(j)) {
    if (k === 'labels') out['hush-owl'] = { slug: te.slug, name: te.name, tagline: te.tagline, metaTitle: te.metaTitle, metaDescription: te.metaDescription, about: te.about, howToUse: te.howToUse, classroomIdeas: te.classroomIdeas };
    if (k !== 'hush-owl') out[k] = j[k];
  }
  tcWrites[file] = out;
...
const manipEN = {
  title: 'Hush Owl',
  tagline: 'Let the owl sleep — a calm picture of the room’s voices.',
  description: 'A gentle noise companion for the projector: a sleepy owl dozes while the class stays within today’s voice level, and sustained loudness slowly wakes him — no sirens, no red zones, no numbers. Sound is measured on your device only; nothing is ever recorded, saved, or sent.',
};
...
if (manipSrc.includes('id: "hush-owl"')) OK('manipulatives entry already present — skipping');
else {
...
console.log('\nAPPLIED — hush-owl.js, 10 tool-content files, manipulatives.ts');
