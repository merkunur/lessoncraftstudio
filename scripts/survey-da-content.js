...
const daBlock = tmpl.match(/\s\sda:\s*\{[\s\S]*?\},/);
if (daBlock) {
  console.log('startCreating: ' + (daBlock[0].match(/startCreating:\s*'([^']*)'/)?.[1] || ''));
  console.log('pairedToolPrompt present: ' + /pairedToolPrompt/.test(daBlock[0]));
}
