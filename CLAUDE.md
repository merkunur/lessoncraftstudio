...
Deck → `audit-deck-html.js` (auto via publish-wave) + live curl spot-check. Activity → `audit-activity-pages.js` + `audit-activity-mobile.js` (§A.13.55) + `audit-activity-variety.js` (§A.13.60 — ≥7 originals + post-pass reshuffle). Tool → `audit-tool-pages.js` (33/33). Mind Cloudflare 5-min TTL (§15.8) before the edge reflects new bytes. Full table → docs/claude-md/misc-detail.md.
