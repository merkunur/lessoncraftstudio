...
**5 Primitives / components.** **Reused (exact names + file).** **NOT used** (and why). **NEW** in `templates/components-b5/<key>.js` and, if genuinely new drawing, `primitives/<name>.js` — with the exact geometry (viewBox, anchor points, part ids, minimum size, stroke, palette tokens) so the engineer draws what you designed.
