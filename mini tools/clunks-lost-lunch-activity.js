...
(function (global) {
  ...
  global.ClunksLostLunchActivity = {
    ...
    strings: {
      title: { en: "Clunk's Lost Lunch" },
      instruction: { en: 'Make Clunk’s number with snacks, then feed him.' },
      prompt: { en: 'Make his number!' },
      needs: { en: 'Clunk needs' },
      crunch: { en: 'crunch-units' },
      qFree: { en: 'Pick snacks that make {n} — any way you like!' },
      qTwoWays: { en: 'Make {n} — then a WHOLE new way!' },
      qNewWay: { en: 'Yum! Now make {n} a different way!' },
      qMissing: { en: 'Clunk already holds {g}. Add the snack that makes {n}!' },
      qMulti: { en: 'Clunk holds {g}. Finish to {n} with exactly TWO snacks!' },
      qExactN: { en: 'Make {n} using exactly {k} snacks!' },
      qConstrained: { en: 'Make {n} — only these snacks today!' },
      qReduce: { en: 'Too full! Take snacks back until it makes {n}.' },
      hintFeed: { en: 'Tap snacks into the lunchbox, then feed Clunk!' },
      hintReduce: { en: 'Tap a lunchbox snack to put it back.' },
      lever: { en: 'Feed Clunk! 🍴' },
      lunchboxLab: { en: 'Lunchbox' },
      emptyBox: { en: 'tap snacks to add them' },
      wrong: { en: "Hmm — not quite my size! Let's pack a fresh one." },
      repeat: { en: "That's the same way — try a different one!" },
      nextWay: { en: 'Yum! Now make it a WHOLE new way!' },
      win: { en: '{combo} makes {n} — yum! 🍱' },
      winTwo: { en: 'Two ways to make {n} — Clunk is full! 🍱' },
    ...
    _wall: function () {
      ...
      costs.forEach(function (cost) {
        ...
        b.setAttribute('aria-label', 'a ' + cost + ' snack — tap to add');
    ...
    _tray: function () {
      ...
      s.tray.forEach(function (c) {
        ...
        chip.setAttribute('aria-label', (c.locked ? 'Clunk holds a ' : 'a ') + c.cost + ' snack' + (c.locked ? '' : ' — tap to take it back'));
    ...
    _feed: function () {
      ...
      else if (r === 'next-way') { this.api.sound && this.api.sound(760); this.msg = api.t('nextWay'); speak('now a new way'); this.render(); }
      else if (r === 'repeat') { this.api.sound && this.api.sound(440); this.msg = api.t('repeat'); this.render(); }
      else { this.api.sound && this.api.sound(330); this.msg = api.t('wrong'); speak('not quite my size'); this.render(); }   // wrong — non-directional, snacks returned
    },
    _win: function () {
      var api = this.api, n = this._T();
      this.solved = true; this.solvedCount = Math.min(this.solvedCount + 1, (this._pool && this._pool.length) || 9);
      var combo = (this.winCombo || []).map(function (c) { return WORDS[c] || c; }).join(' and ');
      ...
      speak((this.winCombo || []).map(function (c) { return WORDS[c] || c; }).join(', ') + ' makes ' + (WORDS[n] || n));
    },
