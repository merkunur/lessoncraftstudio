#!/usr/bin/env node
/* Generic IT coordinate enumeration (READ-ONLY) — the it fan, any non-picture-sort type.
 ...
 * picture-sort.
 *
 * Emits `level` per coordinate ONLY when --level=<key> is passed (readiness types: --level=infanzia).
 ...
 *
 * MUST run on Hetzner. Usage: node scripts/seo-landing/enum-it-coords.js --type=treasure-hunt --level=infanzia > scripts/seo-landing/it-treasure-hunt-coordinates.json
 */
'use strict';
...
(async function () {
  ...
  var rows = await c.deck.findMany({
    where: { language: 'it', status: 'published', exerciseType: TYPE },
