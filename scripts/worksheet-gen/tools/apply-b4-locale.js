#!/usr/bin/env node
/**
 * apply-b4-locale.js <locale|all> [--dry-run]
 *
 * Merges a validated nt10-D native-panel draft (i18n/.draft-b4-<loc>.json) into every
 ...
 *   data/b4/locales/<bank>.<loc>.json         the locale block per bank (GENERATED; merged
 *                                             by lib/b4-common.js bankModule — the module keeps
 ...
 *   docs/worksheet-gen/b4-designs/_records/refusals.<loc>.json
 *                                             the declared refusals (consumed by gen-b4-landings
 *                                             + export-hub-expectations)
 *
 * Runs tools/validate-b4-draft.js FIRST (with the build probe) and aborts on any error,
 * so a bad draft cannot leave half the surfaces patched. Validates ALL requested locales
 * before writing ANY. Idempotent. Then run: node tools/gen-b4-waves.js --locales=<loc>.
 */
'use strict';
const fs = require('fs');
const path = require('path');
const { validate, ALL_IDS, KEYS, bankName, LOCALES } = require('./validate-b4-draft.js');

const WG = path.join(__dirname, '..');
const ROOT = path.join(WG, '..', '..');
const [, , which, flag] = process.argv;
const DRY = flag === '--dry-run';
if (!which) { console.error('usage: apply-b4-locale.js <locale|all> [--dry-run]'); process.exit(2); }
const WANT = which === 'all' ? LOCALES : [which];
const writeJson = (p, o) => fs.writeFileSync(p, JSON.stringify(o, null, 2) + '\n');

const drafts = {};
for (const loc of WANT) {
  const file = path.join(WG, 'i18n', `.draft-b4-${loc}.json`);
...
for (const loc of WANT) {
  ...
  for (const k of KEYS) {
    ...
    if (!e) throw new Error(`taxonomy has no exercise-type ${k} — run tools/register-b4-taxonomy.js first`);
    e.slug[loc] = d.families[k].slug; e.name[loc] = d.families[k].name;
  }
  // bank locale blocks
  const dir = path.join(WG, 'data', 'b4', 'locales'); fs.mkdirSync(dir, { recursive: true });
  let nb = 0;
  for (const k of KEYS) {
    const b = bankName(k);
  ...
  for (const [strand, text] of Object.entries(d.strandNames || {})) {
    ...
    const insert = `${m[1]}  ${loc}: ${JSON.stringify(text).replace(/^"|"$/g, "'")}, // nt10-D panel (tools/apply-b4-locale.js)\n`;
...
console.log(`applied ${WANT.length} locale(s)${strandAdded ? `, ${strandAdded} strand-name row(s) added` : ''} — next: node tools/gen-b4-waves.js --locales=${WANT.join(',')} && node i18n/lint-locale.js <loc>`);
