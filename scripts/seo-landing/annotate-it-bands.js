#!/usr/bin/env node
/* Per-coordinate band annotation for the band-split it arithmetic types (math-worksheet, math-puzzle,
 * code-addition). Reads each coord's deck quantities (manifest operands/results OR the deck.html
 * sum-slot), assigns `level` (classe-prima/classe-seconda) + `standard` by the child-seen max, and
 * writes them back into the coordinate JSON. Coords whose max > 100 are DROPPED (above the K-2 ceiling).
 * Clone of annotate-fr-bands.js (classe-prima/classe-seconda instead of cp/ce1). MUST run on Hetzner
 * (reads /var/www/lcs-media/decks/it/).
 *
 * Band map (it ledger; concrete-vs-symbolic → classe-prima FLOOR, NO infanzia for symbolic calc):
 *   math-worksheet / math-puzzle: ≤10 → classe-prima K.OA.A.2 · 11-20 → classe-prima 1.OA.C.6 · 21-100 → classe-seconda 2.NBT.B.5
 *   code-addition:                ≤20 → classe-prima 1.OA.C.6 · 21-100 → classe-seconda 2.NBT.B.5
 * Usage: node annotate-it-bands.js <coords.json> --type=math-worksheet [--locale=it] [--source=deck-html]
 */
'use strict';
const fs = require('fs');
...
const locale = (argv.find(a => a.indexOf('--locale=') === 0) || '--locale=it').split('=')[1];
...
function bandFor(type, max) {
  if (type === 'math-worksheet' || type === 'math-puzzle') {
    if (max <= 10) return { level: 'classe-prima', standard: 'K.OA.A.2' };
    if (max <= 20) return { level: 'classe-prima', standard: '1.OA.C.6' };
    if (max <= 100) return { level: 'classe-seconda', standard: '2.NBT.B.5' };
    return null;
  }
