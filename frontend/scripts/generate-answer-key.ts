#!/usr/bin/env node
/**
 * generate-answer-key.ts — Pillar 5 Phase 2 Sub-Phase 2.1 entry point.
 *
 * Generates teacher-reference PDFs per teaching package + locale. Structured-
 * prose render aggregating package metadata into sections.
 *
 * Output per (package, locale):
 *   /var/www/lcs-media/materials/answer-key/<locale>/<package>/print-answer-key.pdf
 */

import * as fs from 'fs';
import * as path from 'path';
import {
  loadAllAnswerKeyPackages,
  loadAnswerKeyPackagesBySlugs,
  AnswerKeyPackage,
} from './lib/answer-key-package-loader';
import { closeBrowser, writePrintPdf } from './lib/answer-key-render';

const REPO_ROOT = path.resolve(__dirname, '..', '..');
const DEFAULT_OUTPUT = path.join(REPO_ROOT, 'frontend', '.scratch', 'answer-key');
...
function printHelp() {
  console.log(`generate-answer-key.ts — Pillar 5 Phase 2 Sub-Phase 2.1

Usage:
  npx tsx frontend/scripts/generate-answer-key.ts --package <slug> --locale <loc>
...
async function runPackageTask(task: PackageTask, args: CliArgs): Promise<PackageResult> {
  ...
  const printPath = path.join(outDir, 'print-answer-key.pdf');

  if (args.resume && fs.existsSync(printPath)) {
    return { packageSlug: pkg.slug, locale, printOk: true, skipped: true };
  }

  fs.mkdirSync(outDir, { recursive: true });

  try {
    await writePrintPdf(pkg, locale, printPath);
    return { packageSlug: pkg.slug, locale, printOk: true, skipped: false };
  } catch (e: any) {
    console.error(`FAIL print ${locale}/${pkg.slug}: ${e.message}`);
    return { packageSlug: pkg.slug, locale, printOk: false, skipped: false, error: e.message };
  }
}
...
async function main() {
  ...
  let packages: Map<string, AnswerKeyPackage>;
  if (args.allPackages) {
    packages = loadAllAnswerKeyPackages();
  } else if (args.packages.length > 0) {
    packages = loadAnswerKeyPackagesBySlugs(args.packages);
  } else {
