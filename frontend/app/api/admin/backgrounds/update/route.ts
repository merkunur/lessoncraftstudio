...
/**
 * GET /api/admin/backgrounds/update
 * Retrieves all background themes and their associated images from database
 */
async function getHandler(request: NextRequest, userId: string) {
  try {
    ...
    return NextResponse.json(
      { error: 'Failed to read backgrounds metadata', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
  }
}
...
/**
 * POST /api/admin/backgrounds/update
 * Updates background metadata (translations, display names) in database
 */
async function postHandler(request: NextRequest, userId: string) {
  try {
    ...
    return NextResponse.json({
      success: true,
      message: 'Backgrounds metadata updated successfully in database',
      themes: updatedThemes,
    });
  } catch (error) {
    console.error('Error updating backgrounds metadata:', error);
    return NextResponse.json(
      { error: 'Failed to update backgrounds metadata', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
  }
}
...
/**
 * DELETE /api/admin/backgrounds/update
 * Deletes a background theme
 */
