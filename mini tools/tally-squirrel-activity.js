...
(function (global) {
  ...
  function squirrelSVG() {
    return '<svg class="tsq-sq-svg" viewBox="0 0 100 100" role="img" aria-label="' + (LANG === 'de' ? 'Tally, das Eichhörnchen' : LANG === 'fr' ? 'Tally l’écureuil' : LANG === 'es' ? 'Tally, la ardilla' : LANG === 'pt' ? 'Tally, o esquilo' : LANG === 'it' ? 'Tally, lo scoiattolo' : 'Tally the squirrel') + '">' +
  ...
  global.TallySquirrelActivity = {
    ...
    strings: {
      title: { en: "Tally the Squirrel", de: 'Tallys Eichelkörbe', fr: 'Les paniers de glands de Tally', es: 'Tally la ardilla', pt: 'As cestas de bolotas do Tally', it: 'I cestini di ghiande di Tally' },
      instruction: { en: 'Tally fills the baskets with acorns. Add them all up and type the total.', de: 'Tally füllt die Körbe mit Eicheln. Zähle alle zusammen und tippe die Summe ein.', fr: 'Tally remplit ses paniers de glands. Additionne-les tous et écris le total.', es: 'Tally llena las canastas con bellotas. Súmalas todas y escribe el total.', pt: 'O Tally enche as cestas com bolotas. Some todas e digite o total.', it: 'Tally riempie i cestini di ghiande. Sommale tutte e scrivi il totale.' },
      prompt: { en: 'How many acorns in all?', de: 'Wie viele Eicheln sind es zusammen?', fr: 'Combien de glands en tout ?', es: '¿Cuántas bellotas hay en total?', pt: 'Quantas bolotas no total?', it: 'Quante ghiande in tutto?' },
      hint: { en: 'Add the baskets one at a time — tens with tens, ones with ones.', de: 'Zähle erst alle Zehner zusammen, dann alle Einer — und dann beides.', fr: 'Additionne d’abord toutes les dizaines, puis toutes les unités — et enfin rassemble le tout.', es: 'Suma las canastas de una en una: las decenas con las decenas y las unidades con las unidades.', pt: 'Some as cestas uma de cada vez — primeiro as dezenas, depois as unidades, e junte tudo.', it: 'Somma un cestino alla volta: le decine con le decine, le unità con le unità.' },
      readAria: { en: 'hear the baskets', de: 'die Körbe anhören', fr: 'écouter les paniers', es: 'escuchar las canastas', pt: 'ouvir as cestas', it: 'ascolta i cestini' }
    },
    defaults: {},
    ...
    render: function () {
      ...
      var sayLine = v.addends.join(LANG === 'it' ? ' più ' : LANG === 'es' ? ' más ' : LANG === 'pt' ? ' mais ' : ' plus ') + (LANG === 'de' ? '. Wie viele sind das zusammen?' : LANG === 'fr' ? '. Combien ça fait en tout ?' : LANG === 'es' ? '. ¿Cuántas son en total?' : LANG === 'pt' ? '. Quantas são no total?' : LANG === 'it' ? '. Quante sono in tutto?' : '. How many in all?');
