#!/usr/bin/env node
/* =====================================================================
   _bram-probe.js — multi-state control-bottom probe for "The Wobble Museum"
   (W.K.2, clarity-first redesign of #85). Drives 2 rooms + a wrong-tap +
   ...
   sentence rows, no overlap, no overflow.
   ===================================================================== */
'use strict';
...
(async () => {
  ...
  const solve = (ok) => page.evaluate((correct) => {
    const t = window.WobbleMuseumActivity, r = t._round, C = window.WobbleMuseumCore;
    const oi = C.oracle(r);
    let target = oi;
    if (!correct) { for (let i = 0; i < r.sentences.length; i++) if (!C.isDrift(r, i)) { target = i; break; } }
    const b = Array.from(document.querySelectorAll('.wm-line')).find(x => +x.getAttribute('data-oi') === target); if (b) b.click();
  }, ok).then(() => sleep(50));

  const states = {
    'roomA': async () => { await force('wm-frogs'); },
    'roomB': async () => { await force('wm-moon'); },
    'wrong': async () => { await force('wm-dogs'); await solve(false); },
    'resolved': async () => { await force('wm-bees'); await solve(true); }
  };

  async function controlBottom() {
    return await page.evaluate(() => {
      const sel = '.wm-root,.wm-sign,.wm-lines,.wm-line,.wm-say,.wm-hear,.wm-line-msg,.lcs-activity-check';
  ...
  async function checks() {
    return await page.evaluate(() => {
      const taps = Array.from(document.querySelectorAll('.wm-line'));
  ...
  if (!(await page.evaluate(() => window.WobbleMuseumActivity._resolved))) fails.push('a correct tap did NOT resolve');
  await states.wrong();
  if (await page.evaluate(() => window.WobbleMuseumActivity._resolved)) fails.push('a wrong (on-topic) tap resolved — must not advance');

  const order = ['roomA', 'roomB', 'wrong', 'resolved'];
  for (const [w, h] of VIEWPORTS) {
    ...
    for (const label of order) {
      ...
      if (tt.minTap < CTRL_MIN && tt.minTap !== 999 && !subFloor) fails.push(`${label} @${w}×${h}: a sentence row is ${tt.minTap}px (<${CTRL_MIN}px)`);
      if (tt.overlap > 0) fails.push(`${label} @${w}×${h}: ${tt.overlap} row pair(s) overlap`);
  ...
  console.log('');
  if (fails.length) { console.error(`BRAM-PROBE FAILED — ${fails.length}:`); fails.forEach(f => console.error('  • ' + f)); process.exit(1); }
  console.log(`BRAM-PROBE PASSED — roomA/roomB/wrong/resolved: a correct tap resolves + a wrong (on-topic) tap does not advance; lowest control (incl. shell Check) clears the fold by ≥${MARGIN}px across 320→1366; sentence rows ≥${CTRL_MIN}px (off the 320 sub-floor), no overlap, no horizontal overflow.`);
  process.exit(0);
