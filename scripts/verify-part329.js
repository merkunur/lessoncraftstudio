#!/usr/bin/env node
/**
 * Verify SEO Part 329: NL Second-Grade Enrichment — Themes 39-50
 *
 * Checks that all 10 enrichment fields exist in the second-grade block
 * of each NL theme file (sports through zoo).
 */
...
const themes = [
  { dir: 'sports', name: 'Sport' },
  { dir: 'spring', name: 'Lente' },
  { dir: 'summer', name: 'Zomer' },
  { dir: 'superheroes', name: 'Superhelden' },
  { dir: 'toys', name: 'Speelgoed' },
  { dir: 'transportation', name: 'Vervoer' },
  { dir: 'travel', name: 'Reizen' },
  { dir: 'vegetables', name: 'Groenten' },
  { dir: 'weather', name: 'Weer' },
  { dir: 'winter', name: 'Winter' },
  { dir: 'xmas', name: 'Kerst' },
  { dir: 'zoo', name: 'Dierentuin' },
];
...
for (const theme of themes) {
  ...
  if (secondGradeIdx === -1 || thirdGradeIdx === -1) {
    console.log(`FAIL: ${theme.dir}/nl.ts — missing grade blocks`);
    failCount++;
    continue;
  }

  const block = content.substring(secondGradeIdx, thirdGradeIdx);
  ...
  // Check seoTitle contains "Groep 4" and "LessonCraftStudio"
  const titleMatch = block.match(/seoTitle:\s*'([^']+)'/);
  if (titleMatch) {
    const title = titleMatch[1];
    if (!title.includes('Groep 4')) errors.push('seoTitle missing "Groep 4"');
