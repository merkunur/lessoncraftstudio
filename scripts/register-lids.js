/* =====================================================================
   register-lids.js — wire TOOL #39 into the site
   ---------------------------------------------------------------------
   Run:  node scripts/register-lids.js [--dry-run]
...
const KEY = 'lids';
const PREV = 'draw-bag';           /* the key this one is inserted after */
const E = require('./_lids-content.js');
...
LOCALES.forEach((l) => {
  ...
  if (!e || !e.slug || !e.name || !Array.isArray(e.about) || e.about.length !== 4) {
    die(`_lids-content.js is incomplete for "${l}" — run apply-lids-locales.js first`);
  }
});
...
(function () {
  ...
  s = s.slice(0, eol + 1) + "  '" + KEY + "': 'number'," + String.fromCharCode(10) + s.slice(eol + 1);
  write(rel, s);
  did(`${rel}: category "number"`);
}());
