...
(function (global) {
  ...
  function speak(text) {
    try {
      ...
      if (global.speechSynthesis && global.SpeechSynthesisUtterance) { var u = new global.SpeechSynthesisUtterance(text); u.rate = .92; u.lang = (LANG === 'de' ? 'de-DE' : LANG === 'fr' ? 'fr-FR' : LANG === 'es' ? 'es-MX' : LANG === 'pt' ? 'pt-BR' : LANG === 'it' ? 'it-IT' : 'en-US'); global.speechSynthesis.cancel(); global.speechSynthesis.speak(u); }
    } catch (e) {}
  }
  ...
  function brambleSVG() {
    return '<svg class="pt-bramble-svg" viewBox="0 0 120 120" role="img" aria-label="' + (LANG === 'de' ? 'Benno der Biber' : LANG === 'fr' ? 'Bramble le castor' : LANG === 'es' ? 'Beto el castor' : LANG === 'pt' ? 'o castor Beto' : LANG === 'it' ? 'Bruno, il castoro' : 'Bramble the beaver') + '">' +
  ...
  global.ParkingTowerActivity = {
    ...
    strings: {
      title:        { en: "Bramble's Parking Tower", de: 'Bennos Parkturm', fr: 'Le parking à étages de Bramble', es: 'El estacionamiento de Beto', pt: 'O estacionamento do Beto', it: 'La torre-parcheggio di Bruno' },
      instruction:  { en: 'Help Bramble park each vehicle in the right spot. Tap Check when it is parked.', de: 'Hilf Benno, jedes Fahrzeug am richtigen Platz zu parken. Tippe auf Prüfen, wenn es geparkt ist.', fr: 'Aide Bramble à garer chaque véhicule au bon endroit. Touche Vérifier quand il est garé.', es: 'Ayuda a Beto a estacionar cada vehículo en su lugar. Toca Comprobar cuando ya esté estacionado.', pt: 'Ajude o castor Beto a estacionar cada veículo no lugar certo. Toque em Verificar quando ele estiver estacionado.', it: 'Aiuta Bruno a parcheggiare ogni veicolo al posto giusto. Tocca Controlla quando è parcheggiato.' },
      promptPlace:  { en: 'Park the {mover} {rel} the {landmark}.', de: 'Parke {mover} {rel} {landmark}.', fr: 'Gare {mover} {rel} {landmark}.', es: 'Estaciona {mover} {rel} {landmark}.', pt: 'Estacione {mover} {rel} {landmark}.', it: 'Parcheggia {mover} {rel} {landmark}.' },
      promptBetween:{ en: 'Park the {mover} between the {lm1} and the {lm2}.', de: 'Parke {mover} zwischen {lm1} und {lm2}.', fr: 'Gare {mover} entre {lm1} et {lm2}.', es: 'Estaciona {mover} entre {lm1} y {lm2}.', pt: 'Estacione {mover} entre {lm1} e {lm2}.', it: 'Parcheggia {mover} tra {lm1} e {lm2}.' },
      promptReverse:{ en: 'Bramble parked {rel} the {landmark}. Tap his truck.', de: 'Benno hat {rel} {landmark} geparkt. Tippe auf seinen Lastwagen.', fr: 'Bramble s’est garé {rel} {landmark}. Touche son camion.', es: 'Beto estacionó {rel} {landmark}. Toca su tráiler.', pt: 'O Beto estacionou {rel} {landmark}. Toque no caminhão dele.', it: 'Bruno ha parcheggiato {rel} {landmark}. Tocca il suo camion.' },
      promptTwoTruck:{ en: 'Park the {mover} between your two trucks.', de: 'Parke {mover} zwischen deinen beiden Lastwagen.', fr: 'Gare {mover} entre tes deux camions.', es: 'Estaciona {mover} entre tus dos tráileres.', pt: 'Estacione {mover} entre os dois caminhões.', it: 'Parcheggia {mover} tra i tuoi due camion.' },
      reteachAbove: { en: 'Above means one level higher up — try again!', de: 'Über bedeutet eine Ebene höher — versuch es noch einmal!', fr: 'Au-dessus, ça veut dire un étage plus haut — essaie encore !', es: 'Encima quiere decir un piso más arriba… ¡inténtalo otra vez!', pt: 'Em cima quer dizer um andar acima… tente de novo!', it: 'Sopra vuol dire un piano più in alto — riprova!' },
      reteachBelow: { en: 'Below means one level lower down — try again!', de: 'Unter bedeutet eine Ebene tiefer — versuch es noch einmal!', fr: 'En dessous, ça veut dire un étage plus bas — essaie encore !', es: 'Debajo quiere decir un piso más abajo… ¡inténtalo otra vez!', pt: 'Embaixo quer dizer um andar abaixo… tente de novo!', it: 'Sotto vuol dire un piano più in basso — riprova!' },
      reteachNext:  { en: 'Next to means right beside it, same level — try again!', de: 'Neben bedeutet direkt daneben, auf derselben Ebene — versuch es noch einmal!', fr: 'À côté, ça veut dire juste à côté, au même étage — essaie encore !', es: 'Junto quiere decir en el mismo piso, al lado… ¡inténtalo otra vez!', pt: 'Ao lado quer dizer bem do lado, no mesmo andar… tente de novo!', it: 'Accanto vuol dire proprio di fianco, sullo stesso piano — riprova!' },
      reteachBetween:{ en: 'Between means in the middle of the two — try again!', de: 'Zwischen bedeutet in der Mitte zwischen den beiden — versuch es noch einmal!', fr: 'Entre, ça veut dire au milieu des deux — essaie encore !', es: 'Entre quiere decir en medio de los dos… ¡inténtalo otra vez!', pt: 'Entre quer dizer no meio dos dois… tente de novo!', it: 'Tra vuol dire in mezzo ai due — riprova!' },
      reteach2:     { en: 'That spot is {sib}, not {rel}. Listen again.', de: 'Dieser Platz ist {sib}, nicht {rel}. Hör noch einmal zu.', fr: 'Cette place est {sib}, pas {rel}. Écoute encore.', es: 'Ese lugar está {sib}, no {rel}. Escucha otra vez.', pt: 'Esse lugar está {sib}, não {rel}. Escute de novo.', it: 'Quel posto è {sib}, non {rel}. Ascolta di nuovo.' },
      parked:       { en: 'Yes! Parked just right.', de: 'Ja! Genau richtig geparkt.', fr: 'Oui ! Bien garé.', es: '¡Sí! Quedó justo en su lugar.', pt: 'Isso! Estacionou certinho.', it: 'Sì! Parcheggiato proprio bene.' },
      hintCheck:    { en: 'Park the vehicle in the right spot, then tap Check.', de: 'Parke das Fahrzeug am richtigen Platz und tippe dann auf Prüfen.', fr: 'Gare le véhicule au bon endroit, puis touche Vérifier.', es: 'Estaciona el vehículo en el lugar correcto y luego toca Comprobar.', pt: 'Estacione o veículo no lugar certo e depois toque em Verificar.', it: 'Parcheggia il veicolo al posto giusto, poi tocca Controlla.' }
    },
    ...
    _renderTower: function () {
      ...
      s.landmarks.forEach(function (l) { var c = cellEl(l.col, l.level); c.classList.add('pt-occupied'); var lbl = (LANG === 'de' && VEHICLE_DE[l.label]) ? VEHICLE_DE[l.label].bare : (LANG === 'fr' && VEHICLE_FR[l.label]) ? VEHICLE_FR[l.label].bare : (LANG === 'es' && VEHICLE_ES[l.label]) ? VEHICLE_ES[l.label].bare : (LANG === 'pt' && VEHICLE_PT[l.label]) ? VEHICLE_PT[l.label].bare : (LANG === 'it' && VEHICLE_IT[l.label]) ? VEHICLE_IT[l.label].bare : l.label; c.innerHTML = '<span class="pt-veh pt-landmark">' + vehicleSVG(l.vehicle) + '</span><span class="pt-label">' + esc(lbl) + '</span>'; tower.appendChild(c); });
  ...
  function makeTasks(rounds) {
    return rounds.map(function (round) {
      var s = round.scene;
      var de = (LANG === 'de'), fr = (LANG === 'fr'), es = (LANG === 'es'), pt = (LANG === 'pt'), it = (LANG === 'it');
      ...
      var lmVal = function (id) { var k = lm(id); return pt ? ptFused(k) : es ? esFused(round.relation, k) : it ? itFused(round.relation, k) : de ? ((VEHICLE_DE[k] && VEHICLE_DE[k].dat) || k) : fr ? ((VEHICLE_FR[k] && VEHICLE_FR[k].de) || k) : k; };
      ...
      var lmDef = function (id) { var k = lm(id); return pt ? ((VEHICLE_PT[k] && VEHICLE_PT[k].def) || k) : es ? ((VEHICLE_ES[k] && VEHICLE_ES[k].def) || k) : it ? ((VEHICLE_IT[k] && VEHICLE_IT[k].def) || k) : de ? ((VEHICLE_DE[k] && VEHICLE_DE[k].dat) || k) : fr ? ((VEHICLE_FR[k] && VEHICLE_FR[k].def) || k) : k; };
      var moverKey = s.mover ? s.mover.vehicle : 'truck';
      var moverVal = pt ? ((VEHICLE_PT[moverKey] && VEHICLE_PT[moverKey].def) || moverKey) : es ? ((VEHICLE_ES[moverKey] && VEHICLE_ES[moverKey].def) || moverKey) : it ? ((VEHICLE_IT[moverKey] && VEHICLE_IT[moverKey].def) || moverKey) : de ? ((VEHICLE_DE[moverKey] && VEHICLE_DE[moverKey].akk) || moverKey) : fr ? ((VEHICLE_FR[moverKey] && VEHICLE_FR[moverKey].def) || moverKey) : (s.mover ? s.mover.label : 'truck');
      var relVal = pt ? (PREP_PT[round.relation] || round.relation) : es ? (PREP_ES[round.relation] || round.relation) : it ? (PREP_IT[round.relation] || round.relation) : de ? (PREP_DE[round.relation] || round.relation) : fr ? (PREP_FR[round.relation] || round.relation) : relWord(round.relation);
      var pk, pa = {};
