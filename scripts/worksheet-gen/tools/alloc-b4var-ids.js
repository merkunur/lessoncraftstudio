#!/usr/bin/env node
/**
 * alloc-b4var-ids.js — the nt10-D variation-face id allocation (Phase 2).
 *
 ...
 * K-358+ · G1-354+ · G2-347+ · G3-385+ in file order. Deterministic, so twenty
 * builders working in parallel cannot collide. Writes
 * docs/worksheet-gen/b4-designs/_records/b4var-id-allocation.json and refuses if the
...
const DESIGNS = path.join(ROOT, '..', '..', 'docs', 'worksheet-gen', 'b4-designs');
const FAMILIES = ['K-353', 'K-354', 'K-355', 'K-356', 'K-357', 'G1-350', 'G1-351', 'G1-352', 'G1-353', 'G2-346'];
const NEXT = { K: 358, G1: 354, G2: 347, G3: 385 };
...
fs.writeFileSync(path.join(recDir, 'b4var-id-allocation.json'), JSON.stringify({ generated: new Date().toISOString().slice(0, 10), next: NEXT, faces: out }, null, 1) + '\n');
