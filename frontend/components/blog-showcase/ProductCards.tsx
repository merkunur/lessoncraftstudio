 ...
 * Used by Category 1 (product-guide) and Category 3 (niche-seasonal) posts.
 */

import type { ProductCardItem } from '@/config/blog-visual-sections/types';
import { imgUrl } from '@/config/showcase-i18n';
import { productCardAltText, appDisplayName } from '@/config/blog-visual-sections/alt-text';
...
const sectionHeading: Record<string, string> = {
  en: 'What Your Listings Could Look Like',
  de: 'So könnten Ihre Angebote aussehen',
  fr: 'Voici vos futures fiches produit',
  es: 'Así podrían verse tus listados',
  pt: 'Como seus anúncios poderiam ficar',
  it: 'Ecco come potrebbero apparire i tuoi annunci',
  nl: 'Zo kunnen uw advertenties eruitzien',
  sv: 'Så här kan dina annonser se ut',
  da: 'Sådan kunne dine opslag se ud',
  no: 'Slik kan annonsene dine se ut',
    ...
    <section className="py-10 md:py-14 bg-gray-50">
      <div className="container mx-auto px-4">
        ...
        <div className={`grid gap-5 max-w-4xl mx-auto ${items.length <= 2 ? 'grid-cols-1 sm:grid-cols-2' : 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-3'}`}>
          {items.map((item, i) => {
            const appName = appDisplayName(item.image.appFolder);
            return (
              <div key={i} className="bg-white rounded-xl shadow-md overflow-hidden border border-gray-100 hover:shadow-lg transition-shadow">
                ...
                <div className="relative">
                  <img
                    src={imgUrl(item.image.appFolder, item.image.filename, locale)}
                    alt={productCardAltText(locale, appName, item.productName)}
                    width={400}
                    height={566}
                ...
                <div className="p-4">
                  ...
                  <div className="mt-2">
                    <span className="inline-block px-3 py-1 rounded-full text-sm font-bold bg-emerald-600 text-white">
                      {item.price}
                    </span>
                  </div>
                </div>
