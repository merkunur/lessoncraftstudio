/* =====================================================================
   register-draw-bag.js — wire TOOL #38 into the site
   ---------------------------------------------------------------------
   Run:  node scripts/register-draw-bag.js [--dry-run]
...
const KEY = 'draw-bag';
const PREV = 'arrow-strip';           /* the key this one is inserted after */
const E = require('./_draw-bag-content.js');
...
LOCALES.forEach((l) => {
  ...
  if (!e || !e.slug || !e.name || !Array.isArray(e.about) || e.about.length !== 4) {
    die(`_draw-bag-content.js is incomplete for "${l}" — run apply-draw-bag-locales.js first`);
  }
});
