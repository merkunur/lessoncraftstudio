#!/usr/bin/env node
/**
 * check-b6-string-parity.js [--fix-list]
 *
 * WHY THIS EXISTS. A b6 family keeps its per-face title + instruction in TWO places:
 *   · `data/b6/locales/<family>.<loc>.json` -> `strings.<mode>.{title,instruction}`
 *     — what every `qa/verify-b6-<family>.js` reads and asserts on, and
 *   · `i18n/strings.<loc>.json` -> `<ID>.{title,instruction}`
 *     — what the generator actually PRINTS on the page and stamps into the manifest.
 *
 * `tools/apply-b6-locale.js` writes both from one draft, so they agree at apply time. A later
...
const BANK_DIR = path.join(ROOT, 'data/b6/locales');
