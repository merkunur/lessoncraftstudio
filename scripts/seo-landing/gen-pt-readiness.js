#!/usr/bin/env node
/* Generic PT readiness/other-types landing generator — PT-B bulk engine.
 * Config-driven clone of gen-de-readiness.js, adapted to pt-render (pure substitution,
 * render(tpl, themeObj)) + pt-themes + the agreement-safe placeholders {N_PL} (only in
 * a preposition/"immagini di" slot) / {GEN_ART} {GEN} (stored pair) / {H1}.
 *
 * Config shape (pt-readiness-<type>.js):
 ...
 *
 * Usage: node scripts/seo-landing/gen-pt-readiness.js --type=subtraction [--only=<mode>]
 */
'use strict';
const fs = require('fs');
const THEMES = require('./pt-themes');
const { render, assertThemeTable } = require('./pt-render');
...
const cfg = require('./pt-readiness-' + TYPE);
const IT = 'frontend/content/seo-landing/pt.json';
const COORDS = JSON.parse(fs.readFileSync('scripts/seo-landing/pt-' + TYPE + '-coordinates.json', 'utf8')).coordinates;
...
let cur = { _note: 'PT landing copy — PART PT.', landings: [] };
...
console.log('generated ' + generated.length + ' (' + TYPE + ', modes ' + MODES.join('+') + '); blocked ' + blockedTotal + '; pt.json total ' + merged.landings.length);
