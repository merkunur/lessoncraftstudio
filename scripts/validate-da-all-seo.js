#!/usr/bin/env node
/**
 * Danish SEO Validation Script — Part 234
 *
 * Validates ALL file-based Danish metadata for:
 *   - Title length (target 50–60 chars)
 *   - Description length (target 150–160 chars)
 *   - Uniqueness (no duplicate titles or descriptions)
 *   - Language quality (no English leakage, Danish markers present)
 *
 * Page types checked:
 *   - 33 product pages
 *   - 50 theme hubs
 ...
 *
 * Usage: node scripts/validate-da-all-seo.js
 */
...
/** Check if string has Danish markers (æ, ø, å, or common Danish words) */
function hasDanishMarkers(str) {
  if (!str) return false;
  // Check for Danish characters
  if (/[\u00e6\u00f8\u00e5\u00c6\u00d8\u00c5]/.test(str)) return true;
  // Check for common Danish words
  const danishWords = [
    'og', 'til', 'b\u00f8rn', 'arbejdsark', 'gratis', 'printbar',
    'klasse', 'matematik', 'opgaver', '\u00f8velser', 'f\u00f8rskole',
    'b\u00f8rnehaveklasse', 'farvel\u00e6gning', 'generatorer',
    'udskriv', 'tegninger', 'puslespil', 'bogstaver', 'uddannelse',
  ];
  const lower = str.toLowerCase();
  return danishWords.some(w => lower.includes(w));
}
...
function validateEntry(pageType, pageName, rawTitle, rawDesc) {
  ...
  // Danish markers
  if (!hasDanishMarkers(title) && !hasDanishMarkers(desc)) {
    pageOk = false;
    pageIssues.push('No Danish markers');
    issues.push({ page: `${pageType}/${pageName}`, field: 'both', value: '', len: 0, issue: 'No Danish markers' });
  }
...
function validateThemeHubs() {
  ...
  const themeDirs = fs.readdirSync(baseDir).filter(d => {
    const daPath = path.join(baseDir, d, 'da.ts');
    return fs.existsSync(daPath) && fs.statSync(path.join(baseDir, d)).isDirectory();
  }).sort();
  ...
  for (const theme of themeDirs) {
    const content = fs.readFileSync(path.join(baseDir, theme, 'da.ts'), 'utf8');
...
function validateCategoryHubs() {
  ...
  for (const cat of categories) {
    ...
    let title = '', desc = '';
    if (catMatch) {
      const daBlock = catMatch[1];
      const titleMatch = daBlock.match(/title:\s*'([^']*(?:\\.[^']*)*)'/);
      const descMatch = daBlock.match(/description:\s*'([^']*(?:\\.[^']*)*)'/);
      if (titleMatch) title = titleMatch[1];
      if (descMatch) desc = descMatch[1];
    }
...
function validateNavigationPages() {
  ...
  // 1. Homepage — frontend/app/[locale]/page.tsx
  {
    ...
    const content = fs.readFileSync(filePath, 'utf8');
    const daBlock = content.match(/da:\s*\{([^}]*title:[^}]*description:[^}]*)\}/);
    let title = '', desc = '';
    if (daBlock) {
      const tm = daBlock[1].match(/title:\s*'([^']*)'/);
      const dm = daBlock[1].match(/description:\s*'([^']*)'/);
      if (tm) title = tm[1];
      if (dm) desc = dm[1];
    }
  ...
  // 3. Worksheets page — separate pageTitle / pageDescription Records
  {
    ...
    const content = fs.readFileSync(filePath, 'utf8');
    const titleMatch = content.match(/pageTitle[\s\S]*?da:\s*'([^']*)'/);
    const descMatch = content.match(/pageDescription[\s\S]*?da:\s*'([^']*)'/);
    const title = titleMatch ? titleMatch[1] : '';
    const desc = descMatch ? descMatch[1] : '';
    navPages.push({ name: 'worksheets', title, desc });
  }

  // 4. Pricing page — da: { title: '...', description: '...' }
  {
  ...
  // 5. Blog listing page — blogPageMeta[locale]
  {
    ...
    const content = fs.readFileSync(filePath, 'utf8');
    const daBlock = content.match(/da:\s*\{\s*\n?\s*title:\s*'([^']*)'\s*,\s*\n?\s*description:\s*'([^']*)'/);
    let title = '', desc = '';
    if (daBlock) {
      title = daBlock[1];
      desc = daBlock[2];
...
function checkLanguageQuality() {
  ...
  const noDanish = issues.filter(i => i.issue === 'No Danish markers');
  ...
  if (noDanish.length === 0) {
    console.log('  Missing Danish markers: 0 pages');
  } else {
    console.log(`  Missing Danish markers: ${noDanish.length} pages`);
    for (const { page } of noDanish) {
      console.log(`    ${page}`);
    }
  }

  return { englishLeakages: englishLeakages.length, noDanish: noDanish.length };
}

// ─── Main ──────────────────────────────────────────────────────────────────

function main() {
  console.log('=== DANISH SEO VALIDATION REPORT ===');
  ...
  console.log(`  Missing Danish:         ${language.noDanish}`);
