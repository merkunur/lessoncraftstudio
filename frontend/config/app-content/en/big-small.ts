...
const content: AppContent = {
  seo: {
    ...
    metaDescription: 'Generate big and small comparison worksheets for KDP toddler activity books. Themed images, visual learning, auto answer keys. Commercial license. Try free.',
  },

  hero: {
    title: 'Big & Small Worksheet Generator — Create Printables to Sell on Etsy & KDP',
  ...
  ctaHeading: 'Start creating big-vs-small worksheets',
  ...
  howItWorks: {
    ...
    steps: [
      ...
      },
      {
        ...
        description:
          'Select one of five question types from the Content panel. Circle the Big One prompts identifying the largest image. Circle the Small One targets the smallest image. Circle the Medium One challenges finding the middle-sized image (requires 3 images per exercise). Number 1–2–3 Small to Big prompts ordering images from smallest to largest. Number 1–2–3 Big to Small reverses the order from largest to smallest. Each question type targets a different size discrimination skill — identification vs. sequencing.',
      },
      {
    ...
    features: [
      {
        ...
        description:
          'Five distinct question types cover every size discrimination skill. Circle the Big One and Circle the Small One target single-item identification — find the largest or smallest image. Circle the Medium One adds complexity by requiring identification of the middle-sized image, skipping both extremes. Number 1–2–3 Small to Big and Number 1–2–3 Big to Small shift from identification to sequencing, requiring ordering all images by size. This range of question types creates a natural difficulty progression from simple identification to multi-step ordering, giving sellers a built-in differentiation axis for leveled product bundles.',
      },
      {
        title: 'Identical vs. Different Image Comparison Modes',
        description:
          'Two image modes create fundamentally different exercise types from the same tool. Identical Images mode shows the same picture displayed at different sizes, isolating pure size perception — comparing scale without any confusing variables. Different Images mode shows distinct objects at different sizes, requiring relative size reasoning where unlike items must be compared. The identical mode is ideal for beginner-level products building basic size awareness, while the different mode targets advanced-level products with more abstract comparison skills. Switching between modes doubles your product variety without changing any other settings.',
      },
      {
      ...
      },
      {
        ...
        description:
          'Every worksheet contains only images and visual indicators — no text appears on the generated output. This makes Big & Small worksheets universally usable regardless of language, reading level, or literacy status. The pre-reading niche, language learners, and the inclusive content niche can all be served with exercises that require zero reading. For sellers, zero-text output means the same worksheet sells worldwide without translation — one product listing serves every market. This is a significant competitive advantage over text-heavy worksheet generators.',
      },
      {
  ...
  businessUseCases: {
    ...
    cases: [
      ...
      {
        title: 'Tiered Visual Activity Packs on Gumroad',
        description:
          'Build leveled size comparison practice sets using question type as the differentiation axis: Level 1 (Circle the Big One, 2 images, identical mode), Level 2 (Circle the Small One, 2 images, different mode), Level 3 (Circle the Medium, 3 images), and Level 4 (Number 1–2–3 ordering). Each level targets a different cognitive skill — from simple identification to multi-step sequencing. Package each level with themed exercises and answer keys. The zero-text visual format works in any language without modification, making these products valuable for inclusive and multilingual markets.',
