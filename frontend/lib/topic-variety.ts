...
/**
 * Homepage scale-copy values. Server-fetched honest numbers from current
 * catalog state — no hardcoded aspirational figures per CLAUDE.md §16.6.
 *
 * Returns:
 *   deckCount         — total published decks across all locales
 *   exerciseTypeCount — distinct exercise-types in production
 *   languageCount     — distinct locales with at least one published deck
 *
 * Used by the BreadthGrid intro string at homepage Section 2.
 */
export interface CatalogScaleNumbers {
  deckCount: number;
  exerciseTypeCount: number;
  languageCount: number;
}

export async function fetchCatalogScaleNumbers(): Promise<CatalogScaleNumbers> {
  try {
    const decks = await prisma.deck.findMany({
      where: { status: 'published' },
      select: { exerciseType: true, language: true },
    });
    return {
      deckCount: decks.length,
      exerciseTypeCount: new Set(decks.map(d => d.exerciseType)).size,
      languageCount: new Set(decks.map(d => d.language)).size,
    };
  } catch (err) {
    // Build-time DB unreachability is tolerated per project_unblock_recon.md
    // pattern (matches generateStaticParams fallback in topic page). ISR fills
    // accurate numbers on first hit; static fallback shows zeros (rendered as
    // honesty-aligned skipped block at the calling component).
    console.warn('[fetchCatalogScaleNumbers] DB unreachable; returning zeros:', (err as Error).message);
    return { deckCount: 0, exerciseTypeCount: 0, languageCount: 0 };
  }
}
