#!/usr/bin/env node
/* Generic DE coordinate enumeration (READ-ONLY) — the de readiness bulk (STEP 2) + any de type.
 * Dumps published DE decks for an exercise-type, grouped by (exerciseMode, subjectTags[0]) — the
 * language-AGNOSTIC theme axis-key. exerciseMode may be null (e.g. pattern-train "ab", sudoku/grid-match
 * single-mode) — null is kept as its own group. canonical = earliest publishedAt; siblings = the rest.
 * Drops 4th_of_july (STEP-0 locale-validity exclusion). NOT for picture-sort (its "-vs-" combined-key
 * grouping is handled at the picture-sort sub-slice).
 *
 * MUST run on Hetzner (DATABASE_URL -> local production Postgres).
 * Usage: node scripts/seo-landing/enum-coords-de.js --type=pattern-train > scripts/seo-landing/de-pattern-train-coordinates.json
 */
'use strict';
...
(async function () {
  ...
  var rows = await c.deck.findMany({
    where: { language: 'de', status: 'published', exerciseType: TYPE },
