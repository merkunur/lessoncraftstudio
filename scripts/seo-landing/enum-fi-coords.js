#!/usr/bin/env node
/* Generic FI coordinate enumeration (READ-ONLY) — the fi fan, any non-picture-sort type.
 * Clone of enum-pt-coords.js with language='fi'. Dumps published fi decks for an exercise-type,
 ...
 * picture-sort.
 *
 * Emits `level` per coordinate ONLY when --level=<key> is passed (readiness types: --level=esikoulu).
 * Banded arithmetic (math-worksheet/math-puzzle/code-addition) omit level here and get a
 * per-coordinate re-grade pass (annotate-fi-bands.js, by child-seen quantity) before gen.
 *
 * MUST run on Hetzner. Usage: node scripts/seo-landing/enum-fi-coords.js --type=treasure-hunt --level=esikoulu > scripts/seo-landing/fi-treasure-hunt-coordinates.json
 */
'use strict';
...
(async function () {
  ...
  var rows = await c.deck.findMany({
    where: { language: 'fi', status: 'published', exerciseType: TYPE },
  ...
  var coords = Object.keys(groups).sort().map(function (key) {
    ...
    var co = {
      ...
      n: members.length,
      canonicalDeckSlug: members[0].r.slug, // fi gen-fi-readiness reads `canonicalDeckSlug`
