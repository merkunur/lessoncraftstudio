/* =====================================================================
   register-number-drum.js — the seven registration points for #50.
   Run:  node scripts/register-number-drum.js
...
const KEY = 'number-drum';
const PREV = 'number-hotel';
const L = require('./_number-drum-landing.js');
...
/* ---- 2. tool-content.ts x 4, EACH ANCHORED EXPLICITLY --------------
   ⚠ On #50 a single clever regex silently performed 2 of these 4 and
...
/* ---- 3. wrapper version — the one non-idempotent step ------------ */
{
  ...
  if (registered && m && m[1] === '7.84') {
    s = s.replace(`const TOOL_WRAPPER_VERSION = '${m[1]}'`, `const TOOL_WRAPPER_VERSION = '7.85'`);
    wr(f, s); step(`TOOL_WRAPPER_VERSION ${m[1]} → 7.85`, true);  /* ⚠ #40: the log must report the value ACTUALLY WRITTEN */
...
/* ---- 4. manipulatives.ts hub card -------------------------------- */
{
  ...
  if (s.indexOf(`id: "${KEY}"`) === -1) {
    const card = fs.readFileSync(path.join(__dirname, '_number-drum-card.txt'), 'utf8');
