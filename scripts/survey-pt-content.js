...
console.log('');
console.log('=== uiStrings.pt state ===');
const tmpl = fs.readFileSync(path.join(__dirname, '..', 'frontend', 'app', '[locale]', 'apps', '[slug]', 'page.tsx'), 'utf8');
const ptBlock = tmpl.match(/\s\spt:\s*\{[\s\S]*?\},/);
if (ptBlock) {
  const hasPaired = /pairedToolPrompt/.test(ptBlock[0]);
  const sc = ptBlock[0].match(/startCreating:\s*'([^']*)'/)?.[1];
