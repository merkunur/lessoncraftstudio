...
**Version:** 3.19 (§28 nt5-F added — 5 printable types × 6 faces × 11 locales, BUILT + LIVE 2026-09-25: 318 decks + 318 landings, repointed, IndexNow, hub gate HARD, G1-204 republished; §14.10 count 135 → 140; 3.18 §27 nt10-E added — 10 MORE printable types × 6 faces × 11 locales, BUILT + LIVE 2026-09-23: 624 decks + 624 landings, repointed, IndexNow, hub gate HARD; §14.10 count 125 → 135; 3.17 §26 nt10-D marked **BUILT + LIVE 2026-09-23** — 652 decks + 652 landings in 11 locales, every deck repointed, hub gate HARD in deploy.sh, §14.10 count 115 → 125; 3.16 §26 BUILD STATUS — nt10-D Phases 0-2 DONE 2026-09-21: 10 EN bases + 50 EN faces, baseline 30,030, NEXT Phase 3; 3.15 §26 added — nt10-D, the LAST 10-type batch × 6 faces × 11 locales = 660, DESIGN COMPLETE 2026-09-21: 10 design files, hub matrix 656/660, build trigger "build the last batch of 10 worksheet types and their expansion"; 3.14 §25 nt20-C marked BUILT + LIVE 2026-09-20 — 1,295 decks + 1,295 landings, hub gate hard in deploy.sh, §14.10 count 95 → 115; 3.13 added §25 — nt20-C, the LAST 20-type batch × 6 faces × 11 locales, DESIGN-phase SoT + the measured hub-sidebar root cause; 3.12 adds §A.14.12 disk-hygiene rule — every file class a script creates has an expiry; 3.11 (repoints the "continue" trigger to the Swedish fan-out; 3.10 retired the local AI from the website AND the games — top AMENDMENT 2026-09-05; 3.9 added §24 — the 200 K-3 games BUILD program, trigger "build the next game"; 3.11 repoints the §20.9 "continue" loop from pt-BR (COMPLETE) to the **SWEDISH** second-batch fan-out — #1–#35 shipped, #36 = `wren-question-window` next; §23 Premium Tools v4 remains the "build the next tool" thrust) **Last updated:** 2026-09-25
...
> still reads "the active next major thrust (APPROVED)" and the §20.9 RESUME TRIGGER still names
> "let's start building the games" — **both are DEAD**. Do NOT open, plan, or offer the games
> program; do NOT treat the catalog/strategy artefacts as live scope. The Master Roadmap rev.2
> (2026-07-03, `memory/project-business-strategy-2026-07.md`) is the strategy SoT and supersedes
> this. Historical text follows.

### (historical) Premium games program — the then-active next major thrust (APPROVED, NOT yet started)

The disciplined-set activities phase is COMPLETE (~64 distinct activities). The next major thrust is the **premium games program** — the platform's FIRST paid product. Strategy + catalog + the premium-product vision are DONE + operator-approved; the BUILD has NOT started (operator has other tasks first). Full working record: [[project-premium-games-program]] + MEMORY.md + the plan/catalog/strategy files below.

**Model (operator-approved north star):** FREE = today's catalog (decks, 29 worksheet-gen apps, ~64 activities, mini-tools, PDFs, embedding) — the acquisition funnel, stays free forever. **PREMIUM (paid) = 3 reinforcing layers:** (1) **ACCESS** — 300+ exclusive simple K-3 games; (2) **ASSEMBLY** — games + relevant FREE content ordered per teaching point ("everything for addition-to-10 German kindergarten, sequenced, in my language/framework"); (3) **PREMIUM TEACHING MATERIALS** — an ONGOING pipeline of NEW printable worksheets + activities filling each curriculum area's coverage gap (the free ~64 activities cover only PART of each area). Moat over all three = 11 languages × national-framework alignment (Lehrplan/OPS 2014/Lgr22/SLO Kerndoelen/BNCC — NOT "Common Core translated"); every major competitor (education.com/ABCmouse/SplashLearn/Starfall) is English-only. **NO progress tracking** (operator-locked; education.com itself retired theirs). No-account teacher usage (projection / one shared link-or-class-code / take-home). Tiers: parent ~$45-49 / teacher ~$69-79 (the $69 dormant Lemon Squeezy anchor) / school-license (transparent ~$300/$700/$1,200). The prior premium (teaching-packages/lesson-plans/themed-bundles, nuked `920aebbc`) died on GENERIC-FILLER quality — the bar now is genuine quality, no filler.

**Locked build-workflow rules (the games):** (1) LOCAL PREVIEW before every deploy (self-contained double-click file; deploys are 8+ min, games iterate hard); (2) asset-stubbed FIRST (programmatic placeholders → surfaces the CA5 asset list the operator must render); (3) CC builds the sprite-sheets from the operator's CA5 image sequences (proven `sharp` pipeline); (4) simple / low-animation; (5) expert-ensemble design per game. Architecture = the **Rank-1 web shell** (DOM + CSS sprite-sheets; NOT Godot — payload + CC-self-verify + rebuild disqualified). SEO/locale lives in the SSR wrapper page, NOT the iframe (render-tech is SEO-neutral). 5 protected cores 0-line; no-competition lock (no timer/score/streak).
...
**Deck asset-URL columns are slug-derived for ALL live reads (SEO-recovery 2026-06-25).** The stored `pdf_url` / `answer_key_url` / `thumbnail_url` (also `html_url` / `manifest_url`) columns can **lag a re-slug** and MUST NOT be read for live rendering. Every browse surface (deck cards, variety strips, homepage tiles, collections, sitemap shards) derives the URL from `Deck.slug` via `canonicalDeckAssets` / `deckAssets` in `frontend/lib/seo/landing-content.ts` (the on-disk tree is always coherent with the slug). Root cause of the original drift: re-slug scripts (`migrate-native-mode-slugs.js`, `salvage-*-chartcount.js`, `rename-pdf-files.js`) update `Deck.slug`/files but NOT the URL columns. **Forward rule: after ANY re-slug op, run `scripts/publish-cli/fix-deck-url-columns.js --locales=<loc> --apply`** (host-preserving, verify-before-point, idempotent). The publish FORWARD path (`publish.js` → `db.insertDeck`) already writes canonical columns. Gates: `check-deck-url-drift.js` (fast DB-only, runs WARN-only in `deploy.sh`) + `audit-slug-fs-db-consistency.js` (deep FS+manifest, HARD=0).
...
### 14.3b Shared grammar engine (`window.LCSGrammar`) — the never-ungrammatical sentence layer
`REFERENCE TRANSLATIONS/lcs-grammar.js` (synced via master-sync; served at `js/lcs-grammar.js?v=1`; Node UMD for tests). Consumers: **find-and-count** (instructions + `targets[].localizedPlural`/`ariaHowMany`) + **prepositions** (`prepSentence` → `{prefix, expected, suffix}` drives worksheet ≡ answer key ≡ deck grading + `prepDistractorLabels` frame-valid options), both 2026-08-18 (`256571ac`, bundleVersions 19.4.0/12.4.0; 11 native-panel-audited). Design rules: **indefinite-article frames** eliminate contraction/definite-suffix morphology across de/nl/sv/da/no/fr/es/pt/it (stored vocab forms drop in unchanged; exceptions = de dative weak nouns via the curated 41-entry n-Deklination table + de pre-nominal adjective phrases, both refuse-guarded); fi = postposition+genitive via the in-file 1,166-entry `FI_GENITIVES` table; it uovo-class rule; **unresolvable input REFUSES** (per-locale static grammatical fallback / substitute-default-shape) — naive suffix pluralizers are unreachable in non-en paths. Gotchas: vocab non-noun tell = `entry.de` gender missing; fi `pt:true` doubles as the cross-locale plurale-tantum proxy; the two apps' `STRINGS_ALL` blobs are hand-edit-only (`inline-all-locales-strings.js` SKIPS already-baked apps). Tests: `node scripts/lcs-grammar.test.js` (517 asserts, poison-tested). Extending to more apps (more-less/chart-count share the vocab-insertion class; `translations-shared.js` `runtimeScore` fr/it/pt agreement) = separate commissions.
...
**Printable worksheet-gen family keys (2026-06, NOT apps).** The headless generator at `scripts/worksheet-gen/` publishes printable-only decks under 22 collapsed exercise-type family keys (counting-pictures, counting-frames, tally-counting, number-charts, comparing-numbers, comparing-groups, comparing-sizes, number-lines, base-ten, telling-time, fractions, graphing-data, arrays-multiplication, geometry, measurement, visual-logic, visual-discrimination, visual-matching, patterns, position-words, sorting-categories, picture-arithmetic). These are taxonomy-registered `axes.exercise-type` keys + `apps.*` default entries (`generator.app` = family key; worksheet type id rides as `variant_id`; `manifest.printable_only: true`, schema 1.1, no answer key) — they are NOT interactive apps and never join the §14.10 list. en-only taxonomy entries until the non-EN follow-up commission (picture-trail precedent). **[COUNT UPDATE 2026-09-02]** the "22" above is the June seed; the family-key set has since grown through the nt20 (2026-09-01) and nt20-B (2026-09-02: `word-tracing`, `dot-to-dot`, `grid-copy`, `singular-plural`, `articles`, `read-and-color`, `number-of-the-day`, `write-the-word`, `alphabetical-order`, `number-walls`, `doubles-halves`, `sentence-building`, `capitals-punctuation`, `word-classes`, `calendar`, `picture-writing`, `grid-coordinates`) batches — `topics-taxonomy.json` now carries 140 `axes.exercise-type` keys (29 apps + 111 printable families — nt20-C added 20 on 2026-09-20, nt10-D 10 on 2026-09-23, nt10-E 10 more the same day, §27, nt5-F added 5 on 2026-09-25, §28), all 11 locales authored per batch. Count from the taxonomy, never from this sentence (memory: `project_nt20b_worksheet_types.md`).
...
**[REVERSED 2026-06-20 — PDFs are now INDEXABLE; the noindex below is OFF.]** The §17.8.20 noindex was the PRIMARY cause of the 2026-05-31 click crash (80→15/day): the printable/answer-key PDFs ranked + were clicked directly for "printable … pdf" intent, and `noindex` removed them with nothing recapturing those queries. Reversed via `scripts/publish-cli/unpatch-nginx-pdf-noindex.py` (removes the nginx block → PDFs serve with NO `X-Robots-Tag`; og-image/thumbnail still indexable). **Do NOT re-run `patch-nginx-pdf-noindex.py`.** Companion: dead "make-whole"/legacy-mode PDF filenames (Google-crawled before the 2026-06-20 file rename) 301→canonical via `scripts/publish-cli/patch-nginx-pdf-makewhole-301.py` (safe `if (!-f)` idiom). The original locked policy below is HISTORICAL.
...
Engines NOT yet built: E5/E6 tracing, E9 sight-word. (**E18 number-bond is now OPENED** — NEW `number-bond-core` (clean sibling, 0 protected-core lines), first activity "Make 10" (K.OA.A.4) 11/11 LIVE: a part-part-whole bond (whole 10 + two parts; one part given) — the child TAPS the empty part to add counter-dots and a "−" to remove, until both parts make 10. answerType:'state'; `isCorrect = given + filled === whole` (single targeted answer = 10 − given); 8 distinct rounds given∈1..9 + `nextTask` reshuffle. build-gate `verify-number-bond-core.js` (parts-sum-to-whole + discriminating grade, MEASURED) + `local-test-number-bond.js` (11 locales tap-fill + 280px mobile). K.OA.A.3 (decompose-multiple-ways) is the fast-follow on the same core. **The activities index was also reorganized** (`/[locale]/activities/`) into grade-band → localized-strand → cards (localized headings via `seo.educational_level.*` + `localizeStrand`; auto-slots new activities from `alignment.{grade,strand}`; 0 CC-leak; verifier `local-test-activities-index.js`). **E10 clock/telling-time is OPENED — 2 activities** on NEW `clock-core` (clean sibling, 0 lines to the OTHER cores): #1 "Set the Clock" (1.MD.B.3, hours+half-hours) + #2 "Tell Time to the Nearest 5 Minutes" (2.MD.C.7, minutes {0,5,…,55}) — both 11/11 LIVE. A NEW RADIAL-DRAG verb: drag the analog hands (atan2 angle + snap, discrete setHour∈1..12/setMinute, coupled render 30H+0.5M / 6M), digital readout, answerType:'state'. #2 = an additive `minuteStep` config (30→#1 / 5→#2; setupTask+_snapMinute+keyboard only; #1 re-verified 11/11). build-gate `verify-clock-core.js` proves exact angles + discriminating grade per-coordinate scope; the **time idiom** is native-owned per locale — half-hour (toward-next-hour de/nl/sv/da/no/fi "halb/half/halv/puoli 8"=7:30; current-hour+half es/pt/fr/it) AND the harder 5-min (quarter-past/quarter-to, half-relative nl/Nordic "tien voor half tien"=9:20, Romance subtractive "ocho menos cuarto"=7:45, fi case-correct "kymmentä vaille puoli kymmentä"). a.m./p.m. deferred (not representable on a 12h analog SET). **E13 array/multiplication-readiness is OPENED** — NEW `array-core` (clean sibling, 0 protected-core lines), first activity "Build the Array" (2.OA.C.4) 11/11 LIVE: tap-to-fill an R×C grid → repeated-addition strip → keypad total; build-gate `verify-array-core.js` measures total=R×C + equal-addends; see [[project-activities-live-inventory]]. **E3 sort** is LIVE (2.G.A.1 + 1.G.A.1). E8 is now LIVE in all 5 facade locales es/it/pt/fr/fi; E4 match-pairs [5 activities ×11] + E12 place-value [3 activities ×11 = 33] engines are BUILT + live; **E14 fractions recognition family is BUILT on the E2 choice-board engine** — #1 "Equal Halves and Fourths" (1.G.A.3) + #2 "Same Size, Different Shape" (2.G.A.3 non-congruence), each **11/11 LIVE** (#2 closing `c2bb562c`); anchor-and-match + FRACTION_FIGURES wrapper layer, **0 protected-core lines**; `choice-board-activity.js?v=62`/`ACTIVITY_WRAPPER_VERSION='8.63'`; the active-partition facet **#3 (tap-to-partition) is DEFERRED — needs a new `fractions-core.js`**; full record [[project-e14-fractions-series]] + the per-locale fan-out doctrine §A.13.59 — see above + [[project-activities-live-inventory]].) See [[project-activities-master-queue]] for leverage ranking + queue triage.
...
> 🛑 **VISUAL-QA DEFINITION-OF-DONE (operator-locked 2026-06-23, FOREVER — "create a workflow so visual discrepancies NEVER appear again"). An activity is NOT done until ALL FOUR hold, in order:** (1) **`node scripts/visual-qa-activity.js --activity=<id>` is GREEN across the FULL viewport sweep `320·360·412·768·1024·1366` × EVERY round** — measured gates **FITS** (lowest control bottom ≤ viewport → no cut-off, **at DESKTOP 768/1024 too, not just phone**), **no overflow**, **not tiny** (answer content ≥14px), **not sparse** (answer-card content ≥0.32 area / ≥0.45 width), **tap ≥44px**; (2) **the visual-critic agent reads the FULL screenshot sweep** (every width incl. desktop **768 AND 1024**) against the §A.13.55/14-point rubric — clean; (3) **I personally Read the desktop (768+1024) AND phone (360) renders myself** and confirm; (4) functional gates green + `git diff` = **0 protected-core / lcs-shell**. **ROOT CAUSE fixed:** defects shipped because I verified at ONE viewport (phone 360) while the operator views DESKTOP (~768), and the old gates only caught horizontal overflow — never cut-off-at-desktop / sparse / tiny. **Desktop is now a first-class verified viewport; sparse/tiny/cut-off are MEASURED, not eyeballed. NEVER move a gate threshold to pass** — fix the layout (or fix WHAT the gate measures if it catches decorative/chrome nodes, never the threshold). See §A.13.62, [[feedback-visual-qa-container-containment]].

See [[feedback-activities-approval-cadence]]. **The bar is NOT "tests pass + HTTP 200."** The bar is: **live render + the full-viewport-sweep screenshots (phone→desktop) + operator eyeball.** Empirical bugs that passed tests but were visually broken: blank pages, MIME 307→404 redirect chains, layout breaks at narrow viewports, COUNT readout showing the answer, badge overlapping the board, oversized blank card area beneath short activities. **No timer / score / SmartScore / streak / countdown anywhere — operator-locked.** Warm K-3 no-shame tone. **Mobile-layout gate is MANDATORY: every new/changed activity MUST pass `scripts/audit-activity-mobile.js` (all phone widths 280→768, empty+filled, 0 hard fails) before ship — see §A.13.55** (this replaces ad-hoc single-width spot checks for the "layout breaks at narrow viewports" + "oversized blank card" classes above). **Variety+shuffle gate is MANDATORY (the 5th workflow rule): every activity ships with ≥7 ORIGINAL distinct exercises + a post-pass ORDER-ONLY reshuffle (child never gets the same sequence twice) — default-on, OUT OF SPEC without it even when unmentioned; gate `scripts/audit-activity-variety.js`, see §A.13.60.**
...
> **[STATUS 2026-07-31 — THE CURRENT ACTIVE THRUST IS §23, the Premium Tools v4 program.]** Two v4 instruments are live (`#36 number-sieve`, `#37 arrow-strip`) and the next is **C1 The Draw Bag**. **"Build the next tool" resolves to §23.0, not to anything in this section.** Everything below is a **PAUSED BACKLOG**, resumable whenever the operator names it — nothing here is abandoned and nothing here is the default "continue". **⚠ EXCEPTION: "continue" / "switch to plan mode + continue" / "continue building the second batch of the Swedish activities" (in plan mode) resolves to the [ACTIVE — SWEDISH SECOND-BATCH FAN-OUT] block immediately below** — that is the live "continue" loop, NOT a paused backlog. (Two distinct triggers: "build the next tool" → §23; "continue" → the **Swedish** fan-out. ⚠ pt-BR / es / fr / nl / it are COMPLETE and de is PAUSED — none of them is the default "continue" any more.)

**[ACTIVE — SWEDISH SECOND-BATCH FAN-OUT · the current "continue" loop.]** The ~123-activity second-batch fan-out (the games-build-walk EN activities, fanned to each locale in the SAME #1..#123 order) is now on **Swedish**: nl / it / es / fr / pt-BR **COMPLETE** · de **PAUSED** (source chain exhausted at `fb423d5b`) · **sv ACTIVE — #1–#35 shipped (commit + push, NO deploy), sv prose 95, NEXT = #36 = `wren-question-window`** (question words L.K.1.d, de commit `3696f277`, which added a per-activity grade override).

**RECIPE per "continue":** ONE activity per plan-mode session → EnterPlanMode → find the next in order (`git rev-list --all --children | grep ^<prev-de-hash>` → child de commit → `git show --stat`) → **survey the engine, its protected core, its data pools and its three gate scripts FIRST** → convene a **2-agent native Swedish panel IN PARALLEL** (lågstadielärare / Lgr22 pedagogue + linguist / lexicographer) → write the plan file → ExitPlanMode → build **activity-layer ONLY** → full **§A.13.62 LOCAL DoD** → **commit + push, NO deploy** → append the record to `project_sv_secondbatch_fanout.md` and refresh the MEMORY.md pointer.

**⭐ WRITE THE PANEL BRIEF FROM THE ARTEFACT, NEVER FROM MEMORY OR A PLAN-FILE PARAPHRASE.** At sv #33 three of the panels’ headline "source defects" convicted English strings that do not exist in the file. Quote the real `strings` block, the real round data and the real measured numbers. ⚠⚠ **Panels file false catastrophes too — reproduce before acting:** at sv #34 one reported five locales 100% solvable by card position (it had read the core’s ordered `childView` and stopped one layer short of the activity’s `shuffle`), and at sv #35 a claimed `nl` string defect was simply not in the file. Equally, **a panel can be right where no gate can** — they have caught two-right-answer rounds, a standard that is one-third false in Swedish, and a picture that was not what its filename said.

**LOAD-BEARING SWEDISH DOCTRINE (each bought by a real defect):** rebuild-not-translate, **NEVER skip** — find the Swedish teaching point even for English-specific literacy. ⚠ **Lgr22 states *centralt innehåll* ONCE for the whole åk 1–3 band, so it CANNOT place a deck in a year** — argue from the shipped Swedish ladder, and never put a verbatim Lgr22 quotation in metadata (framework NAME only, §20.10). **STRAND:** vocabulary decks take `sv: 'Ord och begrepp'` (three shipped precedents — `sage-root-garden`, `olive-kind-of`, `roary-roar-meter`); grammar decks keep the default «Språkliga strukturer och normer» with **no override**. **GRADE is a per-activity panel call and may go BELOW a sibling** — sv #35 sits at åk 1 *beneath* the shipped åk-2 vocabulary deck because it demands less reading, not more. ⚠⚠ **«grupp» is UNUSABLE in Swedish copy** (it is the counting word in ≥5 shipped sv maths decks) — use «hör ihop» / «passar ihop» / «av samma slag», and ⚠ not «hör ihop» as a *headline* phrase, which is `sage-root-garden`’s own title. ⚠ **`\b`, `\w`, `[A-Za-z]` and `/^[A-Z]/` are ASCII-only** and silently wrong on å/ä/ö — use `(?<!\p{L})…(?!\p{L})` and `/^\p{Lu}/u`.

**⭐⭐ EVERY ENGINE THIS LOOP HAS TOUCHED SHIPPED WITH THE SAME FOUR HOLES — expect them and budget for them:** (1) the per-engine `verify-*-core.js` reads only `params.rounds`, the ENGLISH pool, leaving ~48 of 56 rounds ungated; (2) `local-test-*.js` hardcodes `?lang=en` and its `force()` silently no-ops on a miss (`if (at > 0)`), so a typo’d id re-tests round 0 and the run still reports PASS; (3) there is **no `scripts/visual-qa-phases/` driver**, so only the OPENING frame has ever been photographed, in one language; (4) `.<prefix>-sel` is `#F2784B`, byte-identical to `--lcs-accent` (the shell’s `.tryagain` ink), with **no post-Check marking at all**, so chosen / wrong / right are ONE colour. Retrofit all four as part of the build — they are the reason a "continue" session is a day’s work, not an hour’s.

**⭐ MEASUREMENT DISCIPLINE.** Build a **cue/bot suite** and run it over EVERY locale pool before accepting a deck — five decks in this loop were **94–100% solvable without reading the language**. ⚠ **A number identical across seven locales means the instrument is not measuring the locale** (it was reading the English filename). ⚠ **A gate that measures a conjunction one conjunct at a time is not measuring it** — at sv #34 two halves passed at 0.38 and 0.25 while their combination scored 68.8%. ⚠ **`positionBot` is an ARTEFACT wherever the activity shuffles in `setupTask`**; assert the RENDERED order in the local test instead. **Ratchets come from `--measure`, never invented** (wrong three times, always in the direction that fails a CORRECT deck), may only SHRINK, and **freeze WHICH locales may carry a row** — otherwise adding a row launders a regression. **Poison every assertion in BOTH directions with a control**, and expect a new gate to accuse correct data first.

**⚠⚠ PICTURE DECKS: OPEN EVERY IMAGE.** At sv #35 three of three opened were not what the filename said — `christmas/tree` was a fully decorated Christmas tree filed as the *plant* exemplar (live in 7 locales), `tree/oak` a generic broadleaf, and `fruits/plum` **a red apple**. Assert on disk that every `<themeDir>/<noun>@2x.webp` exists (a broken tile is hidden by an `onerror` and renders as a word over blank space) and that no B&W theme is referenced — ⚠ the B&W marker is **localized** (`BW`/`SW`/`BN`/`NB`/`ZW`/`SH`/`PB`/`MV`/`SV`), so matching the literal "bw" leaks B&W art into ten locales. Labels come from `image-vocabulary.js` (§20.5), lower-cased for Swedish.

**SCOPE, every time:** activity layer ONLY — **0 lines** to any `*-core.js` or `lcs-shell.{js,css}`, with `git diff --name-only` as the proof. ⚠ `frontend/public/mini-tools/` is a **gitignored STALE mirror — NEVER edit it; the source is `mini tools/`**. Never `git add .` — stage explicitly. Bump the html `?v=N` **and** `ACTIVITY_WRAPPER_VERSION` in the same commit (§A.13.42), and add the sv prose entry (`verify-activity-content-sv.js` counts it). **Commit + push, NO deploy** — the Swedish batch deploys only when every activity is complete. ⚠ But a "commit-no-deploy" batch ships on any later unrelated `deploy.sh`, so verify live state rather than trusting a deferred-deploy note.

**SoT = `memory/project_sv_secondbatch_fanout.md`** — the per-#N trap log; **READ IT EACH SESSION** — **plus the out-of-tree plan file `C:\Users\rkgen\.claude\plans\you-were-building-second-purrfect-snowflake.md`**, rewritten per activity.

**[PAUSED 2026-07-31 · was the active loop through 2026-07-01 — the GERMAN SECOND-BATCH FAN-OUT, ~30 of ~123 remaining.]** The active loop fans the ~123 EN-only lcs-shell activities (the games-build-walk output, deployed `12327ce5`) out to **German, ONE per plan-mode session** — operator says "continue" / "switch to plan mode + continue" → EnterPlanMode → 3-agent native German ensemble (linguist + Grundschule educator + SEO) → ExitPlanMode approval → build (activity-layer only) → full **§A.13.62 LOCAL DoD** → show local screenshots → operator approves → **commit + push, NO deploy**. **Through #93, ~30 remain.** ⚠ **The "deploy-once-at-end" batch is ALREADY OUT — verified live 2026-07-30, so do NOT plan a pending German batch deploy.** The #2–#93 commits went to production incidentally via the tool deploys' `git pull` + `deploy.sh`: sampled German activity pages return **200**, **1,260 `/de/activities/` URLs are in `/sitemap/8.xml`**, and a full md5 comparison of `mini tools/` against `/var/www/lcs-media/mini-tools/` showed **0 content differences across 667 files** (so no §20.4 cp step was stranded). Lesson: a "commit, NO deploy" batch does NOT stay undeployed — **any later `deploy.sh` for unrelated work carries the whole branch**, so verify live state rather than trusting a deferred-deploy note. **rebuild-not-translate + NEVER skip** (find the German teaching point for every activity, incl. English-specific literacy — the operator-locked rule). **Deploy-once-at-end:** all #2–#93 commits sit on `pivot/printable-business-toolkit` awaiting ONE batch `git pull` + cp + `deploy.sh` (§20.4). Reusable route `GRADE_OVERRIDE`/`STRAND_OVERRIDE` (K-cluster K-label: grade-K school-content → Klasse 1 unless content drives higher) + additive `strand-names.ts` entries per new CCSS strand + 0-core `kind`/`role`-graded engines. **SoT = `memory/project_german_secondbatch_fanout.md` (recipe + accumulated doctrine) + the out-of-tree plan file `C:\Users\rkgen\.claude\plans\the-previous-session-you-groovy-pie.md` (the #1–#93 blow-by-blow + per-activity ensemble rulings).**

**[STATUS 2026-06-25 — PRIOR loop (its designed INDEX is exhausted → SUPERSEDED by the German fan-out above; await an explicit prompt to resume).]** The (then-)ACTIVE loop built the designed games down **`game-designs/INDEX.md`** as **lcs-shell FREE activities** (the proven Games 47-53 pattern: a new `<name>-core.js`+`<name>-activity.js`+html+manifest, the only tracked edit = the `activities.ts MANIFEST_FILES` row, EN-only pilot). **The operator phrase "Good. Continue with the next activity" = build the LOWEST-numbered un-built (`approved`, not `BUILT`) INDEX entry**, one per trigger, each in its own plan-mode session: EnterPlanMode → gate-check the CCSS code is UNOWNED → read its `game-designs/<spec>.md` (already approved, all critic fixes baked) → plan → ExitPlanMode → build → the full **§A.13.62 DoD** (verify-core MEASURED + visual-qa 6-viewport sweep + probe + local-test + variety + personal Read 360/768/1024 + visual-critic CLEAN + git-diff 0 lcs-shell/shared-core) → close out (append character to `docs/character-art-spec.md`, mark BUILT in INDEX). **No deploy** (operator batches). **#47-53 BUILT; NEXT = #54 "Sprout's Patchwork Meadow" (3.MD.C.6 · area)** → `game-designs/area-01.md`. NOTE the `game-designs/*.md` specs read "premium `game-shell.js` / DESIGN-ONLY" — IGNORE that on this loop; the operator builds each LIVE as a lcs-shell activity. (The PAID premium-games program is a SEPARATE thrust, trigger "let's start building the games" — top AMENDMENT 2026-06-19.) Working-recall SoT = `memory/project_games_build_walk.md` + MEMORY.md. **Older standing backlog (NOT the active loop; await an explicit prompt):** the worksheet-hub initiative; E14 active-partition #3 (needs `fractions-core.js`); the E8/E9/E2/E12/E4 queue; the deferred-by-decision residuals below.
...
4. **Alt-text** — `rewrite-deck-html-alt-text.js --confirm --locales=<wave>` (worksheet `alt` + app `aria-label` + deckend-thumb alts; the 29 apps emit empty body alt — this is the "alt-text SEO commission 2026-05-27" retrofit, never folded into the apps). Idempotent; preserves the hreflang block. ⚠ Writes a `deck.html.bak.alt-text-retrofit` beside every deck it touches — expired after 14 d by `housekeeping.sh` (§A.14.12); 52 GB of them had accumulated by 2026-09-11.
...
6c/6d. **Site chrome + deck actions** — `inject-deck-site-chrome.js --locale=<each>` (header/footer, SoT `scripts/lib/site-chrome.js`) then `inject-deck-actions.js --locale=<each>` (2026-09-20: the localized **[Download PDF] [Answer key] [Make your own]** strip under every interactive deck's title bar; SoT `scripts/lib/deck-actions.js`, strings `deckActions.*` ×11; PDF/answer-key via the metered `/api/quota/dl` proxy, "Make your own" → `/<loc>/tools/<maker-slug>`; answer-key only when the file exists, maker only when the locale has the slug; printable-only decks skipped; hidden in print / embed / landscape-compact). Both are the FORWARD path — no emitter in `catalog-export.js`. Gates: `deck-actions.test.js` (deploy.sh) + `verify-deck-actions.js --poison`.
...
Deck → `audit-deck-html.js` (auto via publish-wave) + live curl spot-check. Activity → **`visual-qa-activity.js` (§A.13.62 — the full-viewport-sweep DESKTOP-first visual DoD, MANDATORY)** + `audit-activity-pages.js` + `audit-activity-mobile.js` (§A.13.55) + `audit-activity-variety.js` (§A.13.60 — ≥7 originals + post-pass reshuffle). Tool → `audit-tool-pages.js` (33/33). Mind Cloudflare 5-min TTL (§15.8) before the edge reflects new bytes. Full table → docs/claude-md/misc-detail.md.

⭐ **SETTINGS — `audit-settings-efficacy.js`, whenever a surface with a settings drawer is touched.** The `--contract-only` half is browser-free and wired into `deploy.sh`; the runtime half (`--all`, `--all-activities`, `--tool=`, `--activity=`) is a dev run. ⚠⚠ **A CHIP THAT HIGHLIGHTS ITSELF PASSES EVERY OTHER GATE WE HAVE**: `buildDrawer()` appends the drawer INSIDE `.lcs-app` (`lcs-shell.js:587`), so `audit-tool-control-liveness`’s `.lcs-app`-scoped signature scores a dead option as “the world changed” — demonstrated mechanically by `--poison=drawer-scope`, which flips ten-frame’s two genuinely dead settings to LIVE. A static “declared key never referenced” scan is equally blind: **0 findings across all 53 settings-bearing files against a live defect**, because the key IS referenced, inside a function an override shadows. ⚠ Three channels would let such a gate mark its own homework and are excluded by construction — the drawer subtree; `localStorage` (~40 of 53 tools `_saveStore()` from `onSettings`, so storage moves on EVERY commit whatever the consequence); and `tool.render()`, which the shell calls unconditionally, held off by a **null transition** control that re-commits the value a field already has and requires two identical runs before crediting anything. Verdicts LIVE / DEAD / **REVERTED** (it applied, then the next task overwrote it) / UNPROVEN. ⚠ Expect it to accuse correct tools first — it did so seven times, and every one was fixed by changing WHAT is measured, never the rule. Full record: [[project-settings-efficacy-gate]].
### 21.5 Hand-maintained sync points (the only places adding content touches code)
Hreflang map + `LIVE_TOOL_SLUGS` are auto-derived. Remaining hand-maintained: **new theme** → `topics-taxonomy.json` (§16.5.1); **new noun** → `image-vocabulary.js` (operator-approved §10.3); **new activity** → a row in `mini-tools/*-activities.json`; **new activity ENGINE** → register its `<engine>-activities.json` in `frontend/lib/activities.ts MANIFEST_FILES` (the hardcoded loader list); **new locale** → §21.3 native-ensemble + every shared table. Detail → docs/claude-md/misc-detail.md.

**New TOOL = 5 points, all inside the build commit** (§23.5; the earlier one-line form of this entry named only two of them and omitted the one that breaks production):
1. `frontend/config/live-tool-slugs.ts` → `TOOL_KEYS` — ⚠ **miss this and ALL ELEVEN LOCALES RETURN 410**, because `/tools/*` is a seller-era teardown and `LIVE_TOOL_SLUGS` is the carve-out. This is the single most dangerous omission in the whole registration.
2. `frontend/lib/seo/tool-content.ts` → 4 edits (`TOOL_KEYS`, `TOOL_MINI_URL`, `TOOL_ACTIVITY_PREFIX`, the `ToolContentFile` interface member).
3. `frontend/app/[locale]/tools/[tool]/page.tsx` → `TOOL_WRAPPER_VERSION` bump — ⚠ **the one non-idempotent step; guard it on a state that IS stable** or a second `register-` run double-bumps it.
4. `frontend/lib/manipulatives.ts` → the hub card (11-locale title / tagline / description).
5. 11 × `frontend/messages/tool-content/<locale>.json` → one `ToolEntry` before `labels`, native slug matching `/^[a-z0-9-]+$/` (⚠ da folds `ø`→`oe`, no folds `ø`→`o`, so the two never collide).

6. `frontend/lib/tool-categories.ts` → `TOOL_CATEGORY` — ⚠ **omission does not error.** `toolCategory()` falls back to `'number'`, so the tool renders in the WRONG hub section forever. `draw-bag` (a chance instrument) sat under "Numbers & Counting".
7. **the hub thumbnail** — `node scripts/generate-tool-previews.js --only=<key>` (add a `SEEDS` entry FIRST if the board is empty at rest, so the card shows the apparatus in use), then **scp the webp to `/var/www/lcs-media/mini-tools/tool-previews/` BEFORE `deploy.sh`** (§20.4). ⚠ **Omission does not error either** — the card silently falls back to a generic "plus over minus" glyph. This is what the operator saw on #38.

Write a `scripts/register-<tool>.js` that does points 1-5 and is **idempotent** — a second run must report every point as already done. Points 6-7 are hand steps (6 is one line; 7 produces a gitignored artifact that travels by scp, not git).

⭐ **All seven are enforced by `scripts/preflight-tool-registration.js`, wired into `deploy.sh`** — 7 checks × every tool key, each poison-tested on every run, with the preview check self-disabling where no previews exist. Points 6 and 7 were missing from this list until #38 shipped without them, and **both degrade silently**, which is exactly why the gate exists rather than a longer checklist. Its `KNOWN_GAPS` set is a ratchet that may only shrink (currently one entry: `heart-words` has no `fi` landing copy — 41 tool entries in `fi.json` against 42 in `en.json`).
...
~2026-09-01 without explicit operator sign-off.** The forensic audit
(`docs/audit-results/seo-forensic-audit-2026-07-10.md`) established that every full-corpus
rewrite since 2026-05-26 (two title overhauls, canonical mass-repoint, 3-6× deck.html churn)
restarted Google's re-evaluation clock and deepened/extended the traffic crash. Recovery
requires SIGNAL STABILITY. Additive content (new decks/landings/activities/hubs) is fine;
mutating the identity or metadata of EXISTING indexed pages is not. The 2026-07-10
remediation scripts (`reconcile-deck-canonicals.js`, `strip-deck-hreflang.js`) are the
sanctioned exception (they REPAIR broken signals to a stable state).
...
**🏁 3 LOCALES COMPLETE + KEYWORD-RE-KEYED (CLOSED 2026-06-11) — es/en/de done on BOTH controllable axes.** Every deck type landed AND every title+meta keyword-re-keyed to harvested demand: **es 1927 (`c0eb3650`) / en 2005 (`d3cff15f`) / de 1972 (`f1bd2550`) = ~5,904 pages, 0 dup-titles per locale.** The **reopened-5** (deck types deferred during the wave build) closed per locale: math-worksheet (per-deck band re-grade), word-scramble, wordsearch, treasure-hunt LANDED; **cryptogram = proven themeless wall (UNLANDED)**. treasure-hunt app-bug (v32 wrote one theme for all decks) recovered by reading rendered thumbnails for es+en; de manifest was clean. The **keyword re-key** (locked locale patterns, honest-fit-gated qualifiers, the `rekey-{es,en,de}-titles.js` engines) = **§22.4**. de fan-out machinery/doctrine (3-band re-grade, picture-domain→readiness classifier, locale-agnostic de-orphan, config-driven `gen-de-readiness`) proven on the complete de locale — §22.3.

**🏁 8-LOCALE ROLLOUT HAS BEGUN — `sv` COMPLETE (the lead Nordic locale; 27 types / 2478 landings live; 2026-06-12).** The remaining-8 fan-out (§22.5) is no longer "operator's call—don't auto-open"; the operator ruled the sequence + ledger-locked + greenlit sv, which shipped in full (cryptogram proven-unlandable; math-worksheet removed). **NEXT = `nl`** (its own ledger-lock + autonomous fan, per §22.5). Both novel render surfaces sv flushed (Nordic definiteness engine `sv-render.js`; CARRIES range-led/band-split) are proven live + reusable. The per-locale fan playbook + the sv ledger-lock rulings + the deploy gotchas = **§22.5**.
**NEXT (operator-greenlit sequence; §22.5):** `nl` is the active next locale. Other open forks (still operator's call): **tracing/handwriting deck-PRODUCTION** (grafomotricidad / Schwungübungen / tracing — 3-locale-validated demand; a deck-creation product decision, NOT a landing task — the writing/drawing-lines apps are PDF-only) + the **fi deck-catalog gap** (fi = 29 seed decks; needs a ~3,000-deck `publish-wave` before any fi landing fan — escalated, not sequenced). **Carry-forward (browse-hub, NOT a landing-program task):** `/en/topic/sudoku` 404 (§22.2; confirmed per-axis-key, not systemic).
...
Per-locale-durable doctrine (3-band re-grade-by-mechanic; the picture-domain→readiness / grapheme-text-domain→CARRIES framework classifier; R3 band-vs-code keeping the EN CCSS code as `targetName`; R5 per-locale compat authority — never cross-applied; investigation-then-rule for held slices; the locale-agnostic de-orphan; config-driven `gen-de-readiness.js`) → [[project-de-landing-fanout-investigation]] + the de plan file `plan-mode-commission-de-enchanted-lagoon.md`. de = 1779 landings (vorschule 1057 / 1-klasse 548 / 2-klasse 174) at fan-out close; +193 reopened-5 → 1972 final (§22.4).

### 22.4 Reopened-5 completion + keyword re-key (es/en/de, CLOSED 2026-06-11) → [[project-seo-landing-page-program]]
After the per-locale landing arcs, two finishing passes brought es/en/de to **complete on both controllable axes** (every deck type landed AND keyword-targeted): es 1927 / en 2005 / de 1972 = ~5,904 pages.
- **Reopened-5 (the deferred DON'T-LAND types — operator rule: each LANDS or is proven-unlandable on RENDERED content, never the deferral label).** math-worksheet LANDED (the "Gr3-6 algebra" removal premise was WRONG — rendered = symbol-substitution multi-term add/sub, ≤9-20 operands, ≤49 results, 0 over 100; re-graded PER-DECK via ceiling+subtraction sweep: add-only ≤20→primer/1.OA.A.2, sub-or->20→segundo/2.OA.A.1). word-scramble + wordsearch LANDED (literacy; L.K.2.d / RF.K.3.c). treasure-hunt LANDED via **thumbnail recovery** — the v32 app wrote ONE theme into every deck's manifest+slug+title (es=accessories×96, en=4th_of_july×98); the real per-deck theme survives ONLY in the rendered JPEG → 6 agents read all ~99 thumbnails (the Read tool sees images) → ~46-48 real themes/locale, clean theme-slug + `canonicalDeckSlug`→the mis-slugged deck; **de treasure-hunt manifest was CLEAN** (48 themes, direct build); the deck-side app theme-emit fix for es+en is a separate §10.3 task. cryptogram = **proven themeless WALL** (decode a random sentence, 0 images) → UNLANDED. **LESSON: a theme field reading empty/uniform = the extraction is wrong (decks always have a theme); the rendered thumbnail is ground truth.**
- **Keyword re-key (DATA-ONLY title+meta; H1/p1-p3/coordinate untouched).** Engines `scripts/seo-landing/rekey-{es,en,de}-titles.js` (CLI `--type`/`--all`/`--dry-run`; two-pass resolve→ordinal-disambiguation→build; op-as-function for multi-mode; length-fallback suffix ladder, qualifier NEVER dropped; themeDisplay "-vs-"-split + null guard). The route renders `<title>=l.title||l.h1`, meta `=l.metaDescription||first-sentence-of-p1`, so re-keying the SERP title+meta keeps the on-page H1 theme-warm. **Locked patterns:** es/en GRADE-LED (`[op] para/for [Grado] [qual] – [Tema] | …PDF`), **de RANGE-LED** (`Arbeitsblätter [op] bis [N] – [Thema] | zum Ausdrucken PDF kostenlos`; non-numeric drops bis-N; meta uses em-dash before theme to avoid the "mit Tiere"→"mit Tieren" dative slip). **honest-fit gating is the load-bearing rule** — the numeric qualifier appears ONLY where content supports it: "sin llevar"/"without regrouping·borrowing"/"ohne Zehnerübergang" gated to within-10 no-carry (K.OA.A.2); de **bis-N gated to the deck's band** (K.OA.A.2→bis 10, 1.OA→bis 20, 2.NBT→bis 100; non-numeric types carry NO false range); grade gated to the ledger band. **Per-mode distinguishers + ordinal dedup** prevented an 828-deck multi-mode dup-collapse (the gold titles had them) and fixed the 47 pre-existing en addition h1-dups for free → 0 dup-titles per locale. **Length trade (standing):** keyword lead within Google's ~60-char display, full title indexed → honest-distinguisher overflow accepted (don't truncate the keyword base). Lightweight deploy (title+meta only — no deck.html/og/canonical/repoint).
- **NEXT (operator's call, do NOT auto-open):** remaining 8 langs (nl/fr/it/pt/sv/da/no/fi) — EACH needs its own ~20-min autocomplete harvest BEFORE build (born keyword-correct, no retrofit); OR tracing/handwriting deck-PRODUCTION (3-locale-validated demand; a deck-creation decision, not a landing task — the writing/drawing-lines apps are PDF-only, outside the 29-app catalog).

### 22.5 8-locale rollout — `sv` COMPLETE (lead Nordic); `nl` NEXT + the per-locale fan playbook → [[project-8locale-landing-rollout]]
The remaining-8 fan-out of the landing program is underway. **SoT = the plan file `C:\Users\rkgen\.claude\plans\commission-to-cc-swirling-bengio.md`** (the 8-locale investigation report + the sv ledger-lock + the full build tracker + the close) + its sibling agent reports (`...-agent-*.md`: it/nl/Scandinavian/fi curriculum frames + the sv render spec `ae16a8b8…`). **Read the plan file first when resuming.**

- **Sequence (operator-ruled):** **sv → nl → da → no → fr → it → pt**, with **fi ESCALATED separately** (fi = 29 seed decks, no real catalog → needs a ~3,000-deck `publish-wave` first; a deck-production decision upstream of the landing program). All 7 of sv/nl/da/no/fr/it/pt are catalog-ready (live DB 2026-06-11: each ~3,000 published). **`nl` = the active NEXT locale.** Novel-first reasoning front-loads the new render engines/band shapes (sv Nordic definiteness, nl groep system, no grade-offset) before the de-proven Romance bulk (fr/it/pt).
- **`sv` = COMPLETE (2026-06-12): 27 types / 2478 landings live + gated + deployed + repointed** (final commit `24e9a105`; per-type `bb84ebf5`→`24e9a105`). **cryptogram PROVEN-UNLANDABLE** (themeless wall: 0 subject_tags, 0 images, generic `Bildkryptogram` — DON'T-LAND, surfaced not folded). **math-worksheet REMOVED** (off-ceiling, per §22.1). Per-type live count + the close report are in the plan file.
- **sv ledger-lock rulings (operator-ruled; binding precedents for the Nordic family + the fan):** (1) **within-10 SYMBOLIC arithmetic = åk-1 CARRIES** (the förskola=no-standard invariant; strand `Taluppfattning och tals användning`; EN R3 anchor K.OA.A.2 direct / K.OA.A.4 find-addend; range 0–10; `utan växling` honest-fit, direct-modes-only) — covers addition/subtraction/math-puzzle-within-10/code-addition. **Guardrail:** a genuinely *concrete* count-the-pictures coordinate (no symbolic sum) = readiness (förskola), not åk-1 → surface any concrete-vs-symbolic borderline. (2) **chart-count = åk-1 CARRIES K.MD.B.3** (strand `Sannolikhet och statistik`; the bar-chart is a data abstraction, not a tally) + the **honest-count Quiz node** (`practiceProblems: Hur många <plural>? → N` where N = rendered bar = prose count, agree by construction). (3) **two readiness buckets by MECHANIC** (förskola, no standard, dashed "no standard" chip): math-flavored reasoning (pattern-train/pattern-worksheet/big-small/more-less) → `Matematiska resonemang och uttrycksformer`; pure visual-spatial/logic (grid-match/shadow-match/missing-pieces/find-objects/bingo/picture-path/odd-one-out/sudoku/picture-sort/treasure-hunt) → `Visuell perception och logiskt tänkande`. The es-calque `Matematiska begrepp och mönster` is REJECTED → must appear in NO authored sv field (gate assertion: grep = 0). (4) **3-band +1-year-Nordic-shift spine** (level keys `forskola`/`ak-1`/`ak-2`): `förskola` (EN Preschool+Kindergarten+all readiness, ages 3–6) → `åk 1` (EN Grade 1, age 7, within-20) → `åk 2` (EN Grade 2, age 8, within-100); `åk 3` (age 9) = OUT. EN-Kindergarten content → förskoleklass *readiness*, NOT a formal grade. (5) **locked keyword pattern (RANGE-LED):** `Arbetsblad [op] 0–[N] [utan växling] [qual] – [Tema] | skriva ut PDF gratis` — the format-trio (`skriva ut` + `gratis` + `PDF`) is protected; on overflow, tighten other quals never the trio.
- **Nordic definiteness render engine (the novel surface — de-risks da/no):** `scripts/seo-landing/sv-render.js` is **PURE SUBSTITUTION, ZERO morphology** — Swedish has NO case (the de `datN` dative axis disappears); the agreement axis is DEFINITENESS (species). Each theme stores `plIndef`+`plDef` as **hand-verified literals** (`sv-themes.js`: `{gender, decl, plIndef, plDef, h1Display}`; gender `t`=ett/neuter, `n`=en/common per §A.13.58 per-locale-gender-authority — NOT de m/f/n); templates pick the form by placeholder (`{N_PL}` indefinite / `{N_PL_DEF}` definite / `{H1}` display) so no code path ever computes a suffix → the `djur→djuren` never `*djurna` error is **structurally impossible**. A generation-time `assertThemeTable` FAIL-halts on a wrong definite plural (the A1 `*djurna` guard via `expectedPlDef` per declension). **PROVEN live** (grid-match `djur` artifact: `djuren`×6, zero genuine `*djurna`). **da/no reuse this engine; only the per-locale definite-suffix rule swaps. fi needs a genuinely new `fi-render.js` (agglutination + case).**
- **Per-locale machinery (the playbook — clone for nl/da/no/…):** `scripts/seo-landing/`: `sv-render.js` (engine+assertion) · `sv-themes.js` (50 themes, all pass the assertion) · `gen-sv-readiness.js` (config-driven engine cloned from `gen-de-readiness.js`; runs `assertThemeTable` first then per-landing definiteness-lint + ≥200-word check, both FAIL-halt; reads per-coord `co.standard` for band-split) · `sv-readiness-<type>.js` configs + `sv-<type>-coordinates.json` (native prose; fed to the engine) · `rekey-sv-titles.js` (the locked RANGE-LED pattern, `utan växling` honest-fit gate, ordinal dedup) · `enum-sv-coords.js` (generic enumerator, runs on Hetzner) · **dedicated gens** `gen-sv-chartcount.js` (honest-count Quiz; reads a per-deck `_sv-cc-facts.json` counts dump) + `gen-sv-picturesort.js` (`-vs-` pairs, both themes via `render2`, 16×12=192 cell-space) + `enum-sv-picturesort.js`. **Shared-map edits per locale:** `frontend/lib/seo/landing-content.ts` (FILES import) + `frontend/app/[locale]/worksheets/[slug]/page.tsx` (LEVELS / FRAMEWORK_BY_LOCALE / UI_STRINGS) + `scripts/seo-landing/verify-deorphan.js` (per-locale anchorHub/goodRe/badRe). **Reused as-is (locale-agnostic):** `validity-gate.js`, `gate.js` (thresholds), `check-sum-ceiling.js`, `gen-de-readiness`-style engine pattern, the `topic-decks.ts` de-orphan, carousel pass.
- **Doctrines proven this run (apply to every locale):** (a) **divergence** — each new prose config must mechanic-true-diverge from already-shipped siblings AND same-theme pairs, or they cannibalize on the similarity gate (fixed at subtraction-vs-addition 0.87→0.39, pattern-train cross-mode, missing-pieces/treasure-hunt 2-mode, math-puzzle template-collision 0.926→0.353); **never move the gate gauge — fix the content.** (b) **gate BOTH sections** — the gate has §4.B lint (≥200 words / no banned phrases / **theme-noun in P1**) AND similarity; picture-path shipped 10 lint fails when only the similarity verdict was grepped. (c) **cell-space scales** — picture-sort's 151 `-vs-` pairs needed 16×12=192 cells (8×7=56 collided at 0.94); cells must strictly exceed breadth with the coprime scatter (§22.1). (d) **band by mechanic+quantity** — `check-sum-ceiling.js --locale=<loc>` per-coordinate; per-coord `standard` in the coordinates JSON drives band-split (math-puzzle 78 åk-1 / 43 åk-2).
- **Deploy gotchas:** (i) never `pscp` a script that is also git-committed — the untracked server copy blocks `git pull` ("would be overwritten by merge"); `rm` it on Hetzner before `deploy.sh`, or commit-then-pull only (the committed enum scripts are already on the server post-deploy — just run them). (ii) **no Chrome on Hetzner** → the puppeteer `verify-deorphan.js` can't run there; the de-orphan is locale-agnostic + unchanged + proven across 3 locales → applies by construction (the sv config is committed for when Chrome is available). (iii) per-type flow = gen → rekey → `gate.js`(both sections) → commit → `deploy.sh` → `repoint-deck-canonical.js --types=<t> --locale=sv` → live-verify. Landings use THIS flow, **NOT** the deck `publish-wave`.
- **`nl` NEXT (its novelty):** framework SLO-kerndoelen; the **groep band system is the one genuinely-new band SHAPE** (non-collapse: kleuters / groep 3 / groep 4, where **groep-3 = the formal-math boundary** — math coordinates anchor groep 3+, readiness anchors kleuters/groep 1-2); no-carry `zonder tienoverschrijding`; range-led `tot N`; preschool `kleuters`; Dutch gender (de/het); math strand `Getallen en bewerkingen`, readiness `Voorbereidend rekenen`. nl's full curriculum frame + the other 6 locales' frames + per-locale calque flags are in the plan file (Deliverable 1) + the sibling agent reports. nl runs as its own plan-mode ledger-lock (3-agent native ensemble per §A.13.48) → operator rules → autonomous fan on green pre-flight.
---

## 23. Premium Tools v4 — the twenty language-light instruments (CURRENT ACTIVE THRUST)

The platform's **paid-product line**: 20 K-3 *curriculum instruments* — the apparatus a teacher stands at the front and teaches **from**, in a named routine, repeatedly, all year. Distinct from the 33 worksheet-generator apps (operator tooling), the deck catalog, and the ~204 free activities. Terse forward-rules below; **the catalog itself, the fence, the closed strands, the rejected list and the full per-tool recipe live in `docs/claude-md/premium-tools-v4.md`.** Working record: [[project_premium_tools_v4_catalog]] · [[project_arrow_strip_tool]] · [[feedback_next_tool_build_recipe]].

### 23.0 ⭐ THE RESUME TRIGGER — what "build the next tool" means
...
### 23.2 ⭐ THE DESIGN LAW — NO WORDS ON THE APPARATUS
The stage carries **numerals, icons and the material — nothing else**; authored language is chrome (title, settings, the paid chip), never content. Forced by three independent things: the 11-locale authoring cost; ⚠ **`LCSAudio` never calls `getVoices()` and silently substitutes a missing voice — TTS is reliable in only 5 of 11 locales, so six get silence, and no v4 tool may depend on hearing**; and pre-reader/EAL pedagogy. **Acceptance test: the tool is legible with the sound off.** The six v3 gates stand unchanged (apparatus · named repeated routine · visible abstraction · the material pushes back · **manufactures a conversation** · whitespace).

### 23.3 The fence — MECHANIC and VERB, four surfaces
K-2 CCSS is nearly saturated here and tools are free-play (no `tasks`/`nextTask` → no `educationalAlignment` → **no code collision is possible**), so *"no activity claims this code"* is the WRONG test. Check all four: **38+ tools** (`mini tools/*.js`) · **~204 activities** (`mini tools/*-activities.json`) · ⚠ **240 printable types** (`scripts/worksheet-gen/types/{k,g1,g2,g3}/` — the surface three consecutive fences missed) · **33 apps** (`REFERENCE APPS/`). ⭐ **When the fence comes back partially occupied, do not negotiate the overlap — subtract it and ship only the remainder.**

### 23.4 The gate suite — in this order, none skippable
`verify-<tool>.js` (exhaustive where the model is finite; ⚠ **the gate implements its OWN ground truth** — reading the expectation off the tool means it marks its own homework, which let 19 of 51 mutations survive once) → `mutate-<tool>.js` **every mutation killed**, `timeout: 30000` → `local-test-<tool>.js` (real pointer drags; sweep `320·360·412·768·1024·1366`; **two tap floors named separately** — controls ≥44px, canvas cells ≥34px) → `smoke-<tool>-locales.js` ×11 **fresh browser per locale**, printing the whole authored string set → `audit-<tool>-locale-layout.js` 11×6 → `audit-tool-control-liveness.js` **all three entitlement states** → **I read the 360/768/1024 renders myself** → `live-verify-<tool>.js` **driving the main control on production**, never "it mounts". **0 lines to `lcs-shell.{js,css}` or any protected core.**

### 23.5 Registration + deploy
The 5 registration points are §21.5; **`frontend/config/live-tool-slugs.ts` is the 410 trap** and the `TOOL_WRAPPER_VERSION` bump is the one non-idempotent step (guard it). Locales: **§A.13.48 three-agent native panel per locale**, rebuild never translate — expect every panel to rename the tool, and ⚠ **normalise the paid-plan name (`unlock`) against the shipped tools**, because the panels cannot see the product lexicon. Deploy: `git pull` → `cp "mini tools"/<key>.{js,html,json} /var/www/lcs-media/mini-tools/` ⚠ **including the `*.json`, and BEFORE `deploy.sh` runs the build** (§20.4) → `chown lcs-media:` → `deploy.sh` → `live-verify-`.

### 23.6 Standing traps, each bought by a real defect
⭐⭐ **THE ENGLISH SOURCE IS THE ONE NOBODY REVIEWS, AND EVERY LOCALE INHERITS IT.** All eleven #41 panels — asked only to write their own language — independently found the SAME six defects in my English: the title named **tape**, the one noun the tool forbids and a sibling owns, while all fifteen other strings said *strand*; nothing "unrolls" (a cord round an edge SLIDES OFF); "the bench" was a fourth named part in a three-part tool, and in sv/da/no it is a sibling tool's NAME; the screen-reader label omitted two of the five things on the bench while the next string told that same user to drag one of them; "how many of the shape's own WIDTHS" (a shape has one width); and "print FOR paper" is not English. **Give every panel the English as SOURCE, not as target, and ask it to audit — they are the only review that source ever gets.** · ⭐⭐ **A PANEL CAN FIND A BUG IN THE MODEL.** #41's French panel read the code and reported that at 0<t<1 with no flag the hint says "drag the flag" while `setFlag` refuses and no handle is drawn; I reproduced it before fixing. **Reproduce, then fix — never take it on trust, and never dismiss it because it came from a language panel.** · ⭐ **CHECK THE DEFINITE FORM IN EVERY SCANDINAVIAN LOCALE.** Swedish `bana` (track) has the definite `banan` — spelt identically to *banana*. "Skriv ut banan" would have shipped **"print the banana"** to seven-year-olds. The panel caught it; no English-side check could have. · ⭐ **CHOOSE EACH LOCALE'S NOUNS AGAINST THE SHIPPED LEXICON OF THE OTHER TOOLS, NOT AGAINST ENGLISH** — that is what makes them unrelated words instead of one word respelled eleven times, and it is routinely the *obvious* word that is already taken (#41 lost `Bahn`, `piste`, `bande`, `baan`, `räls`, `remsa`, `snor`, `spor`, `bane`, `nauha`, `naru`). Two #41 vetoes were mathematical: es `cuerda` is the term for a **chord of a circle** (the tool is full of circles), and fr `s'allonger` means *gets longer*, the exact misconception the tool exists to kill. · ⚠ **A DRAG-ONLY HANDLE IS DEAD TO A KEYBOARD, TO ASSISTIVE TECH, AND TO THE LIVENESS GATE** — a synthetic `.click()` never fires `pointerdown`. #41's flag scored DEAD on all nine paths in all three entitlement states until it also acted on click and on Enter/Space. · ⚠ **A "LAW" THE DESIGN ASSUMES MUST BE MEASURED BEFORE IT IS GATED, AND A THRESHOLD YOU INVENTED IS NOT A MEASUREMENT.** #41's non-vacuity check asserted an invented "20× margin" and failed a CORRECT tool at 6×; the real requirement — *a dropped segment lands outside the ±10% band* — is stronger, checkable, and passes. Fix WHAT is measured, never the number.
⭐⭐ **A GATE CAN BE VACUOUS ON A SELECTOR THE TOOL NEVER EMITS.** #40's production gate keyed on `.unh-tape[data-t="a"]`; the tool emits `.unh-tape-a`. Every tape assertion compared two EMPTY NodeLists, so it would have passed on a tool with **no tapes at all** — and it reported green on the parts that mattered least. **Assert non-vacuity FIRST** (the collection is non-empty, the count is in range), then assert anything about its contents. A `querySelectorAll` comparison is not evidence until you have shown it selected something. · ⭐ **SCOPE EVERY CONTENT BAN TO YOUR OWN PROSE — `document.body.textContent` INCLUDES NEXT'S RSC FLIGHT-DATA**, which serialises every sibling tool on the page. #40's no-named-unit ban read the *ruler's* correct slug `las-linjalen-mat-i-centimeter` and condemned 10 of 11 locales; the ruler legitimately owns centimetres because it owns 2.MD.A.1. This is the ban-too-wide trap in a new dress: **the fence working is not the fence failing.** · ⭐ **A POISON SET IS ONLY AS GOOD AS ITS EXAMPLES, AND A MIS-SORTED ONE TEACHES YOU TO LOOSEN A CORRECT REGEX.** #40 filed Swedish *"Tum blir det inte tal om här"* under MUST_PASS — but `tum` **is** the Swedish inch, so it belonged in MUST_FIRE. The same run showed the ban too NARROW in the other direction (`\btomme\b` missed the Norwegian plural *tommer*). Sort each example by what it MEANS, not by whether it currently passes. · ⭐ **WRITE ASSERTIONS FROM THE ARTEFACT, NOT FROM THE PLAN'S NARRATIVE.** #40's gate demanded both tapes open on the same unit because the *howToUse* copy said "start with both the same" — the tool deliberately opens 160/100 so one object shows two numbers in the first frame. The narrative described a BUTTON, not the opening state. · ⚠ **A REGISTER SCRIPT'S LOG MUST REPORT THE VALUE ACTUALLY WRITTEN** — #40's said `category "number"` while writing `'measurement'`, which is precisely the log that lets the #38 wrong-category defect survive an eyeball review. · ⚠ **`\s` inside a template literal degrades to a bare `s`** (`` `'${K}'\s*:` `` → `'key's*:`); it matched only by luck. Build regex sources from plain strings. · ⭐ **VERIFY THE MEASUREMENT BEFORE REPORTING THE DEFECT.** #40's hub check produced two false alarms in a row — a wrong section label (`"Measurement & Data"` vs the real `"Measurement, Shape & Data"`) and an anchor scraped out of the RSC payload instead of the rendered card. Both looked like shipped defects and neither was.
⭐⭐ **A BAN CAN BE TOO WIDE, AND ONLY THE OTHER-DIRECTION POISON TEST FINDS IT.** Draw Bag's likelihood ban covered the SUBJECT word as well as the verdict, so the German panel's own name `Der Zufallsbeutel` — from *Zufallsexperiment*, the real Grundschule register — would have failed the build; the same ban rejected KMK/BNCC/Lgr22 **official strand names** in landing prose. This is the recorded `par`-rejects-correct-French defect in a new dress, and a fence that rejects correct native prose teaches a panel to reword AROUND it instead of reporting it. **Every ban is now poison-tested in BOTH directions** — a must-fire string AND a must-pass string — and prose-level exemptions are an explicit, auditable list, never a loosened regex. · ⭐ **A gate that HANGS is a gate that SURVIVED**: an unbounded `while (canDraw)` loop met a mutation that stopped the record filling, and the harness scored it TIMED OUT, which it counts as survived — no loop in a gate may depend on the tool terminating. · ⭐ **The English source is a locale too**: seven native panels independently found that *"keep every piece you draw"* reads as sampling WITHOUT replacement — a mathematics inversion sitting in the EN string the other ten were built from. **Panels audit your source, not just their own copy.** · ⭐ **A wildcard in a coverage check manufactures coverage** (numeric placeholders matched with `.{1,60}?` matched any prose, so a string dead in all 11 locales reported as rendered) · ⭐ **a control must do WHAT ITS LABEL SAYS** — the generic liveness gate structurally cannot know this · ⭐ **containment is measured against the CARD**, not the inner box (`overflow-x` absorbs the evidence) · ⭐ **sweep every configuration, not just the default** · **A15 no dead strings** (scan for *referenced outside the strings declaration*; a key can be reached via a ternary or a lookup map) · **an offline fallback must degrade to the FREE TIER, not to nothing** · locale mutations must **self-anchor on the live file** · the mutation harness must **carry every data file the gate reads** into its tmp dir · **assert the MEDIAN, not the mean** · **`vh` is forbidden inside a manipulative** · **never an inline `background` shorthand** · **poison-test every narrowed regex**.

⭐ **Bought on #39 (The Lids), and each changed the shipped DESIGN, not the gate:**
- ⭐⭐ **MEASURE A "LAW" BEFORE YOU GATE IT.** Two invariants written into the design were refuted by the gate's own data: *"adding a lid shrinks the others"* (50 shrank, 77 held, **35 GREW**) and *"the largest lid never grows"* (28/81). Only two responses are allowed — **fix the design so the law becomes true, or delete the law.** Never soften a percentage until it passes; a threshold tuned to its own data has stopped meaning anything.
- ⭐ **HONEST IS NOT THE SAME AS LEGIBLE.** A lid drawn big enough to REACH every counter it claimed was truthful and unreadable — three circles a third of a table wide, overlapping into one mass. The fix changed what the apparatus DOES (seat the counters under the lid, in a packing it is sized to hold), not what it draws. **When a faithful rendering is unreadable, the model is wrong, not the renderer.**
- ⭐ **A HARNESS'S SILENT NO-OP HOLLOWS OUT THE NEXT ASSERTION.** A click helper that returned false quietly hit a legitimately-disabled control, and the very next check — *"the toggle is not swapped"* — passed because nothing had been toggled. **Every scripted interaction must fail loudly when it does not happen.**
- **A greedy `[\w.]*` backtracks across a dot** (`self.lower(` captured as `r`), so a swapped-toggle check condemned a CORRECT tool. Use `(?:\w+\.)*`, and poison-test on a synthetic correct pair AND a synthetic broken one.
- **A gate that measures nothing reports "measured nothing", not FAIL.** The canvas-floor poison ran on a table whose counters were all hidden, so it could never fire. **Poison every floor in a state where the thing it measures exists.**
- **A fixed-px element inside a percentage-positioned layout collapses at narrow widths** — 34px counters spaced as a % of the table overlapped into a blob at 320px. Size both in the same unit so non-overlap is STRUCTURAL.
- **Check the PATH, not just the logic:** the registration's thumbnail check pointed one directory too high and could never confirm. **A check that cannot pass is as useless as one that cannot fail.**
- **A model is only TOTAL if it is total:** `st || newState()` catches `null` and `0` and hands `[]` straight through to `.length`.
...
| **"continue"** / "continue building the second batch of the Swedish activities" (in plan mode) | **§20.9** — the **SWEDISH** activity fan-out (NOT pt-BR, which is complete) |
...
## 25. nt20-C — the LAST batch of 20 printable worksheet types × (1 base + 5 variations) × 11 locales = 1,320 (DESIGNED 2026-09-13/14; BUILT + LIVE 2026-09-20 — 1,295 decks + 1,295 landings, hub gate hard)

**SoT = `docs/worksheet-gen/b3-designs/README.md`** (the 20 types + ids, the per-type design files `<ID>-<key>.md`, the build recipe cloned from nt20-B/-VAR, the open items, the cross-type rulings) + memory `project_nt20c_design.md` (resume state). **Trigger: "build the last batch of 20 worksheet types and their expansion"** → read the README first, then the 20 design files, then run the recipe; never re-research selection or demand (four expert panels + 11 autocomplete harvests did it: `_PANEL-FINDINGS.md`), never re-design (each file is the contract; native panels refine per locale under `_STUDIO-BRIEF.md`). **Design phase COMPLETE 2026-09-14: all 20 final design files exist** (each with `_work/<ID>-{pedagogy,design,critic}.md`); the README carries the hub-gate expectation matrix (design total ≈1,247 of 1,320 — every gap is a recorded per-locale refusal) and open items 1-12 the build must clear first (the `÷` glyph in the shared array/number-line factories, `animal-facts.json`; item 1 "Free printable" is RULED — see below). **BUILD COMPLETE 2026-09-20:** Phases 0–8 shipped (memory `project_nt20c_worksheet_types.md` is the record; the 09-14 session was cut by the weekly limit mid-Phase-6 and resumed 09-20). 1,295 decks published + repointed, 1,295 landings live, `verify-hub-type-rows.js` now HARD in `deploy.sh`; the taxonomy carries 115 `axes.exercise-type` keys (29 apps + 86 printable families). ⭐ The per-locale landing panels, reading every render, surfaced ~40 shipped-string / bank / generator defects (title says circle while the page ticks; instructions naming boxes that do not exist; a truth face printing one fact twice through two frames; a cloze face repeating an answer; a title naming a five-verb pool the page draws two of) — every one fixed at the source and regenerated; the recipe's lesson: **a landing panel that must describe the RENDER is the cheapest review the sheets ever get — brief it as an audit, not as copywriting.** Open follow-ups are listed in the README status line.

⭐⭐ **Why past batches vanished from the `/worksheets` sidebar (measured 2026-09-13):** the rail is CORPUS-derived and grouped by taxonomy SUBJECT via `apps.<key>.default_subject` — a family key registered in `axes['exercise-type']` but WITHOUT `apps.<key>` is rendered NOWHERE in the rail while `?type=<key>` still works when typed; labels use `name.<locale>` with no en fallback; rows = the landing corpus only (`applyLandingFilters`, exact `coordinate.type`); the old hub strip is DELETED (`56f5a388`); the landing file cache refreshes only on deploy. Every new family therefore needs `apps.<key>` + slug/name ×11 + one landing per face per locale + commit + deploy, and the build session adds `scripts/verify-hub-type-rows.js` (expected rows per key × locale from the design files' refusal matrix; poison-tested; wired into `deploy.sh`).

Standing rules from the design phase: ONE fan mechanism for non-theme axes (`unitAxis`); themeless types draw from a cross-theme picture index; boundary-printing syllable faces use only TeX-agreed entries (EN approved boundaries are often rule-only); the grapheme `chunks` layer exists only in de/nl/sv/no; refusals lower the hub-gate expectation per locale explicitly, fillers never pad a page; **the "Free printable" meta lead is RULED (operator 2026-09-14): correct in SEO METADATA only (title / meta description / OG / JSON-LD — the free tier grants 3 PDF downloads a month) and forbidden in anything the user can SEE (landing h1 / eyebrow / body, the printed sheet, deck.html h1 / instruction, hub cards) — the build session lints visible fields for a free-predication per locale, never the bare word.**

---

## 26. nt10-D — the (then) last batch of 10 NEW printable worksheet types × (1 base + 5 variations) × 11 locales (DESIGNED 2026-09-21; **BUILT + LIVE 2026-09-23 — 652 decks + 652 landings, every deck repointed, hub gate HARD**)

**SoT = `docs/worksheet-gen/b4-designs/README.md`** (files, the 10 types + ids, the 12-step build recipe re-pointed to b4 names, open items, cross-type rulings, the hub-gate expectation matrix) + **`_PANEL-FINDINGS.md`** (the LOCK + per-locale native heads/tiers/harvest counts + per-type pedagogy/faces/traps + the honest click model ≈ 970-2,050/day at maturity) + the per-type design files `<ID>-<key>.md` (7 sections: identity · base page · faces 2-6 · native rebuild ×11 · data + gates · SEO · hub contract) + memory `project_nt10d_design.md` (the lock, every ruling the build obeys, the facts measured, the backlog). **Build trigger: "build the last batch of 10 worksheet types and their expansion"** → read the README first, then `_PANEL-FINDINGS.md`, then the 10 design files, then run the recipe; never re-research selection or demand (four expert panels + a seeded 11-locale autocomplete harvest `scripts/seo-research/harvest-candidates.js` + a Nordic re-probe + a water-cycle probe did it), never re-design (each file is the contract; native panels refine per `_STUDIO-BRIEF.md`).

**The lock (2026-09-21):** tangram K-353 · human-body K-354 · five-senses K-355 · weather K-356 · recycling K-357 · cloze G1-350 · odd-and-even G1-351 · pronouns G1-352 · question-words G1-353 · rounding G2-346 (faces from K-358+ / G1-354+ / G2-347+ / G3-385+). `dominoes` dropped (content-swap faces, 0 Nordic demand); `cube-nets` backlog (above band in 6 of 11). Rulings that bind the build: compound heads ×11 for human-body and weather (both bare heads are THEME slugs); printable-only decks ship NO answer key so no meta promises one; the 4 science types need an 11-locale science strand literal added additively to `strand-names.ts` (the OA/NBT rows also misrender for odd-and-even/rounding → per-locale literals from each final's table B); `lib/b4-common.js` (b3's `bank()` reads `data/b3/` only); NEW primitives `tangram.js` / `body-figure.js` / `water-cycle.js` / `bin.js` each with a render-measuring verify, plus additive `bands`/`numerals:false` on `thermometer.js`; `≈`/`→` are OUTSIDE every font unicode-range (SVG glyph, codepoints banned as text); the b3 science factories overflow the fi 677 stack / the 5-bin pill width → b4 components, never edit the factories; `hub-expectations.json` is exported by `tools/export-hub-expectations.js --batch=b4` and `verify-hub-type-rows.js` must be wired to read it (b3-only today) with `--warn-missing-keys` until the batch lands; refusals on record: recycling en F4, cloze da F3, pronouns es + fi F3; the taxonomy goes 115 → 125 keys at the build.

**BUILT + LIVE 2026-09-23.** **652 decks** published and audited N/N clean in all eleven locales (en 59 · de 60 · es 59 · pt 60 · fr 60 · it 60 · nl 59 · sv 59 · da 58 · no 60 · fi 58 — every gap a recorded per-locale refusal: recycling F4 in en/nl/sv/da/fi, cloze F3 in da, pronouns F3 in es + fi) · **652 landings** at `/<loc>/worksheets/<slug>`, each written from the render by a native panel, all 11 locales passing every locked similarity threshold with 0 duplicate titles or metas · **652 decks repointed** to their landings (missing=0 in all eleven) · IndexNow 652/652 HTTP 200 · taxonomy **125 keys** · `verify-hub-type-rows.js --expect=…b4-designs/hub-expectations.json` **HARD in `deploy.sh`** (the `--warn-missing-keys` escape dropped), 143 cells / 839 rows PASS. Hub render + order verifiers PASS ×11; `/<loc>/worksheets?type=<key>` returns the 6 faces live in de, fi and pt.

**The four traps this build paid for, each worth more than the fix:**
1. ⭐⭐ **A b4 string lives in TWO places and the family gate reads the one the page does not print.** `data/b4/locales/<fam>.<loc>.json → strings.<mode>` is what `verify-b4-<fam>.js` asserts on; `i18n/strings.<loc>.json → <ID>` is what the generator PRINTS. `apply-b4-locale.js` writes both, so a later hand-edit to a bank goes green and reaches no deck. Five corrected strings did exactly that. Gate: `tools/check-b4-string-parity.js` (every bank string must appear verbatim in the built i18n; refuses to report success on fewer than 10 banks) — 0 across **1,186 strings in 100 banks**.
2. ⭐⭐ **An answer-position tell must be measured in BOTH directions and on the SHIPPED instance.** G1-369's parity leak was not closed by the odd row width, it was INVERTED (83.3% opposite); the range now straddles the one-row/two-row boundary at a measured **50.0%**. G2-348 shipped `oddIdx [3,2,1,0]` — a staircase — **in all eleven locales at once**, because the instance seed carries no locale; a sweep of random seeds read 0.0% and looked like a refutation. Its sibling G1-370 is a recorded 100% exception with every legal alternative measured and refused.
3. ⭐ **`git status` is part of a deploy.** The Nordic half of the batch — 20 taxonomy slugs, 20 names, 40 topicMeta lines — sat modified and uncommitted while I read the local file and called it done. The hub gate failed the deploy with 43 findings, which is exactly what it exists for.
4. ⚠ **A stale server taxonomy silently produces ENGLISH slugs.** sv/da/no/fi published before that commit landed, so publish-cli fell through to `slug.en` and 235 decks shipped under English slugs (repoint then reported missing=52-54). Unpublish-and-republish does NOT recover it — the surviving `@@unique([language,titleHash])` on the archived rows blocks re-INSERT with `TITLE_NON_UNIQUE` — so the 235 archived rows had to be deleted under a strict predicate before the republish. **Verify the server's taxonomy carries the locale before publishing that locale.**

Resume pointer + per-family commits + panel findings: memory `project_nt10d_worksheet_types.md`; plan `resilient-spinning-goblet.md`.

The design-session "continue = resume the design" rule is RETIRED (design complete 2026-09-21): §20.9's "continue" mapping (the Swedish fan-out) is back in force. Two distinct triggers remain: "build the next tool" → §23; "build the last batch of 10 worksheet types and their expansion" → this section.

---

## 27. nt10-E — 10 MORE printable worksheet types × (1 base + 5 variations) × 11 locales (operator /goal 2026-09-23; **BUILT + LIVE 2026-09-23 — 624 decks + 624 landings, every deck repointed, IndexNow 624/624, hub gate HARD**)

§26 called nt10-D "the last batch"; the operator's /goal of 2026-09-23 overrode it. **SoT = `docs/worksheet-gen/b5-designs/README.md`** (the 10 types + ids, the recipe, the shipped hub matrix) + `_PANEL-FINDINGS.md` (the lock + click model) + the 10 finals `<ID>-<key>.md` + memory `project_nt10e_worksheet_types.md`. Tools are the b4 spine cloned to b5 (`lib/b5-common.js`, `templates/components-b5/`, `tools/*b5*`, `gen-b5-landings.js`, `b5-publish-locale.sh`).

**The ten:** 2d-shapes K-368 · road-safety K-369 · family K-370 · plants G1-376 · animal-life-cycles G1-377 · earth-and-space G1-378 · maps G1-379 · digraphs G1-380 · synonyms G2-358 · word-parts G2-359 (faces K-371–378 / G1-381–397 / G2-360–376 / G3-392–399). Shipped **624 of 660**: en 60 · de 60 · nl 59 · pt 59 · fr 59 · fi 59 · sv 54 · da 54 · no 54 · es 53 · it 53 — every gap a recorded refusal (digraphs whole family es/it/sv/da/no; digraphs F4 pt + nl; word-parts F1 it, F3 fi, F4 es/fr). Taxonomy **135** keys. es is **es-MX** (SEP/NEM, ALTO, catarina) — brief es panels as Mexico, never LOMLOE.

**How the quality was bought (the record for the next batch):** native content panel ×10 → apply → **landing panels briefed as an AUDIT of the render** (≈400 findings over three passes) → three generator/gate rounds + two native data rounds → re-render → landing touch-up. Every new rule is gated in every locale and poisoned both ways. The classes the landing panels found that no gate had: answer tells by POSITION or ROTATION (cut strips that are a rotation of the answer order; tags in a 3-2-1 staircase; STOP always left; signs printed in class pairs; box counts that give the group size — fixed by EQUAL group sizes); two right answers (a square offered "rectangle"; a sign sentence fitting two signs, now gated by a hand-read claim table `MEANING_READ`; synonym cards with two partners; a "said" frame taking two verbs); a fact naming its own column; pictures a child names differently (lamb/sheep, squirrel/fox, a lavender swatch as "Rechteck", a baby elephant for "big"); instructions naming apparatus that is not drawn (a stone, socks, "the plant" where one per row is wanted); a page-level French typography defect fixed at render in `page/shell.js` (fr only, text nodes only — strings keep plain spaces because they feed SEO titles).
⚠ A b5 string lives in the draft, the bank block AND `i18n/strings.<loc>.json` — apply locales from the draft ONE AT A TIME (shared files: taxonomy, strand-names, messages); `tools/check-b5-string-parity.js --all` must be 0 and `out/draft-drift.js` must show no drift before any fixer edits a draft.
⚠ A pooled gate sample that is too small invents a tell: the earth-and-space row-1 share read 63 % at 40 seeds and 54 % at 400 — raise the sample, never the threshold.
⚠ The seo-landing gate's "NO theme-noun in P1" lint is advisory for themeless families (nt10-D's landings carry it too); the binding checks are similarity + duplicate titles/metas (0 FAIL, 0 dups ×11).

**Live proof (2026-09-23):** `verify-hub-type-rows.js --expect=…b5-designs/hub-expectations.json` HARD in deploy.sh and PASS on the server (the 5 refused digraphs cells report "0 expected"); real-browser click on the sidebar entry lists exactly its worksheets (en 2D Shapes 6, fi Aurinkokunta 6 after opening its group), and a scripted pass over all 11 locales matched badge = rendered cards for every one of the 105 entries. Click model (panels' midpoints, 9-15-month maturity): ≈780-1,380/day, midpoint ≈1,080, near zero for the first 2-3 months.

---

## 28. nt5-F — 5 printable worksheet types × (1 base + 5 variations) × 11 locales (operator /goal 2026-09-23; **BUILT + LIVE 2026-09-25 — 318 decks + 318 landings, every deck repointed, IndexNow 318/318, hub gate HARD**)

Built in a cloud session, deployed from the PC. **SoT = `docs/worksheet-gen/b6-designs/README.md`** + `DEPLOY-HANDOVER.md` (the deploy recipe) + memory `project_nt5f_worksheet_types.md`. Tools are the b5 spine cloned to b6 (`tools/*b6*`, `b6-prepare-upload.js <loc>` on the PC → scp → `scripts/publish-cli/b6-publish-locale.sh <loc> [--confirm]` on Hetzner).

**The five:** story-sequencing K-379 · healthy-habits K-380 · habitats G1-398 · sink-or-float G1-399 · cursive-writing G2-377 (faces K-381–384 / G1-400–408 / G2-378–387 / G3-400–401). Shipped **318 of 330**: 30 in en/de/es/fr/it/pt/nl/da/no, 24 in sv + fi (cursive-writing refused whole-type: no joined school script, no Playwrite SE/FI). Taxonomy **140** keys. Cursive uses vendored Playwrite fonts in their own `cursive-fonts.css`; its gate (`qa/verify-b6-cursive-writing.js`) is judged on the PC only (Linux renders the fonts 1-2 px differently). de cursive ships VA + LA. Per-locale `banks.habitats.excludeAnimals` (da/de/es: moose).

**Also shipped:** G1-204 (science-sort, all 55 decks ×11) republished in place — the bank listed a plastic spoon as "sink"; now tools/bolt. 55 UPDATE, audit clean ×11, then the K-205 post-update injector list re-run (the update path writes a bare deck.html).

**Traps paid for on this deploy:**
1. ⚠ **Description band at publish.** en K-384's long title pushed every middle sentence past 170 chars, so the bare 117-char core shipped and publish-bulk refused it (`DESCRIPTION_LENGTH_TOO_SHORT`). No gate before the server dry-run checks the 120-170 band — check pooled ZIP descriptions on the PC before uploading.
2. ⚠ **A dangling legacy symlink fails a per-locale injector for the whole locale.** `decks/pt/chart-count → contagem-em-grafico-v1` (target gone since the June re-slug) made `inject-analytics-beacon.js --locale=pt` exit non-zero after applying all 30 new decks; the remaining pt steps were run by hand with `--slugs-file`.
3. The landings shipped with the first deploy before 10 locales' decks existed (about an hour of 404 download links); acceptable, but publish-then-deploy (b5 order) avoids it.

**Live proof (2026-09-25):** dry-run slugs = landing `canonicalDeckSlug` 318/318; audit clean ×11 except es 28/30 (2 × TITLE_LENGTH_TOO_LONG, see below); repoint missing=0 ×11; 318 landing URLs HTTP 200; IndexNow 318 → 200; `verify-hub-type-rows.js --expect=…b6-designs/hub-expectations.json` HARD in deploy.sh and PASS on the server; real-browser click on every sidebar entry in all 11 locales: badge 6 = 6 cards (cursive absent in sv/fi).

**Open:** es `secuencias-de-cuentos-g1402` + `-k381` titles longer than a SERP shows; the art/layout items in DEPLOY-HANDOVER §6; the dangling pt symlink; b5 + b6 decks carry no `lcs-meter-js` (dark paywall hook, publish scripts never ran `inject-meter.js`).
...
- **Host:** `65.108.5.250` (root). **SSH via KEY AUTH (rotation 2026-07-03):** `ssh -i %USERPROFILE%/.ssh/id_ed25519 root@65.108.5.250` (scp likewise). plink/pscp password auth retired; root password rotated + stored at `%USERPROFILE%\.lcs-secrets\hetzner-root-password.txt` (PC) / `/root/.root-password-rotated-2026-07-03` (server) — console fallback only, never in commands/docs. DB creds: `/opt/lessoncraftstudio/.deploy-env` (root:600, gitignored; `LCS_DB_PASSWORD`) sourced by deploy.sh/cron/scripts. Full runbook: `docs/ops/server-access.md`.
...
#### A.6.1 Payment-system protection (redeploy-safe) — payments LIVE 2026-07-12
Subscription payments went live 2026-07-12 (full record: [[project-ls-payments-live-2026-07-12]]). A redeploy CANNOT break payments, by construction + two guards:
- **Env persists:** all `LEMONSQUEEZY_*` vars live in source `frontend/.env.production`; `deploy.sh` copies it into every release (`cp .env.production "$RELEASE_DIR/.env.production"`) → each redeploy carries the webhook secret. **Zero-downtime:** a failed build aborts BEFORE the release-symlink flip, so the old working release keeps serving. Redeploys never touch the DB or run migrations.
- **Guard 1 — pre-build FAIL** (`deploy.sh`, after the hreflang check): aborts the deploy if `LEMONSQUEEZY_WEBHOOK_SECRET` / `API_KEY` / `STORE_ID` are missing/empty in source `.env.production` → a broken payment env never ships.
- **Guard 2 — post-flip canary** `scripts/payments/smoke-payment-webhook.js` (WARN + loud `!!! PAYMENT CANARY FAILED`): signs a synthetic `subscription_created` (LIVE product `1194166`) → asserts HTTP 200 + active `Subscription` + `subscriptionTier='full'`, asserts a bad signature → 401, then cleans up (leaves only a bare internal `deploy-smoke@lcs.internal` user). Proves signature-verify + product-allowlist + subscription-upsert + the tier-bridge still work on EVERY deploy.
- **Webhook secret** = `LEMONSQUEEZY_WEBHOOK_SECRET` in prod `.env.production` (must match the LS **live** webhook's signing secret). **The LIVE webhook must subscribe to the `subscription_*` events** (created/updated/cancelled/payment_success/payment_failed) — NOT just `order_*` (the handler log-and-ignores `order_*`). Live product ids: `1194166` (yearly) / `1194171` (monthly).
- **Payment-critical files (treat with care; a break = lost revenue):** `frontend/app/api/webhooks/lemonsqueezy/route.ts` (incl. `syncUserTierForSubscription` — bridges the LS subscription onto the legacy `user.subscriptionTier` that ~51 admin/UI read-sites use), `frontend/config/lemonsqueezy-product-config.ts`, `frontend/lib/entitlements.ts` (`isSubscriptionUsable`), `frontend/lib/subscription-helpers.ts` (`isLcsSubscriptionActive`).
...
#### A.7.2 Image-library WebP mirrors must be UNIFORM per directory (gate: `audit-theme-webp-coverage.js`)
The 33 apps request `/image-library-webp/themes-lossless/<dir>/<db-stem>@3x.webp` and **fall back silently to the raw `/images/<dir>/<file>` on 404**. Measured live: `animals` answers 200 for every variant; `flowers` answers 404 for every variant and uses raw throughout. **Both are correct — uniformity is the invariant, not coverage** (100 image dirs: 1 fully mirrored, 99 uniformly raw, 0 partial). The defect is a **half-mirrored** directory: a worksheet bakes its palette art at generate time and crops its reveal art separately, so one may read a variant while the other reads raw. In Grid Match that surfaces as **puzzle pieces changing picture when the child drops them into a cell**. Nothing errors and nothing logs.
**Two traps, each of which produced a false report before the real measurement:** (1) the raw tree writes `zoo_animals`, the mirror writes `zoo animals`, and Content-Manager uploads write a bare theme **CUID** (`/images/<themeId>/…` — the normal newer upload path, NOT corruption) — compare on a normalised key or 79 directories read as broken when none is; (2) the mirror's *filenames* are keyed by vocab noun for bulk-imported themes (`aster@3x.webp`) but by full DB stem for uploads — **the app settles which convention is live, not reasoning about it** (capture its actual requests). ⚠ **NEVER repair a missing mirror by symlinking to a similarly-named older theme** — names matched, artwork did not, and that *caused* the 2026-08-02 incident. Repair only with `generate-theme-webp-variants.js --dir=<dir> --apply` (raw source, `convert-png-to-webp.js` settings verbatim) then `--verify`, which decodes each variant beside its own source and fails unless they are the same picture. Wired into `deploy.sh`; poison-tested.
...
**tapTarget WARN is a real K-2 defect, not noise.** `audit-activity-mobile.js` emits a soft WARN when an interactive control is <36px (the K-2 minimum). 2026-06-18: the shared shell **keypad** (`.lcs-activity-keypad`, the `answerType:'number'` surface — ten-frame how-many/write-numeral + array build-array) shrank to 18px@iframe-216 (page-280) because the `@media(max-width:600px)` block kept a FIXED 6 columns. Fixed (authorized protected-core touch) by an added `@media(max-width:360px)` rule making the keypad `grid-template-columns: repeat(auto-fit, minmax(36px, 1fr))` — columns drop 6→5→4→3 as the iframe narrows so keys stay ≥36px, engaging only in the failure band (wider layouts unchanged). **Lesson: minmax's MAX, if a fixed length, drives auto-fit's column count — use `1fr` (not `var(--lcs-tap)`) so the 36px MIN packs columns densely.** **OPEN FOLLOW-UP (separate work unit, not yet done): audit ALL shell tap targets — chrome buttons, choice chips, Check/Next, drawer controls — for K-2 ≥36px down to 280px** (the keypad fix covered only the keypad). See [[shell-keypad-tap-target-fix]].
...
#### A.13.60 Exercise variety + shuffle (≥7 originals + post-pass order-only reshuffle) — STANDING default-on gate
**Every activity (all engines/standards/locales, permanently) needs ≥7 ORIGINAL distinct exercises + a post-pass ORDER-ONLY reshuffle so the child never gets the same sequence twice.** This is the **5th standing activity-build-workflow rule** (alongside deploy-once / local-test / native-per-locale / one-plan-stop) and a **default-on definition-of-done gate**: an activity is **OUT OF SPEC without it even if its commission never mentions variety** (waivable only by explicit operator instruction). **"DISTINCT" = DISTINCT TO THE CHILD** — genuinely different recognizable CONTENT/CONTEXT, NOT the same abstract thing reconfigured. ≥7 code-distinct configurations of one plain shape does NOT satisfy it (operator ruling 2026-06-17: the fraction partition activities' first "8/7 rounds" of plain teal rect/square/circle were rejected as ≈2-shapes-reshuffled). The fix + reference for genuine variety: partition a set of **genuinely DIFFERENT geometric shapes** — circle, square, rectangle, triangle, ellipse, diamond, hexagon, pentagon — each cut into exactly-equal parts (the variety is the shapes themselves; plain translated prompt "Cut the shape into N equal parts — halves/thirds/fourths", no per-locale authoring). (A themed-object dressing — pizza/cake/etc. over the same two base shapes — was tried and **rejected by the operator as still ≈2-shapes-reshuffled**; different SHAPES is the answer.) Each shape's partition is proven measured-equal by point-sampling (`scripts/verify-fractions-core.js`); add a shape only with an exact equal-area construction (a thirds example: only circle/triangle/hexagon/square/rect divide into exact thirds — don't force a shape into a cut it can't bisect exactly). **Shuffle = order-only** (re-orders the SAME validated items; never generates unvalidated content — correctness/geometry stays measured). **Canonical impl: the wrapper's `nextTask` hook, NOT `lcs-shell.js` (protected).** The shell's `tasks[]` path gives only a per-MOUNT shuffle that loops the same order each cycle; a `nextTask`-only tool owns ordering (`getTask`→`nextTask({index,completed})`; chrome/reload/resize gate on `tasks||nextTask`) → per-pass reshuffle with ZERO shell change. Pattern: drop `tasks`; hold `_pool` (≥7); `nextTask` builds a shuffled `order`, rebuilds on pool length/ref change, reshuffles (≠ previous) only on a FORWARD pass boundary `floor(index/N)>_curPass` (transition-driven → `getTask(0)` idempotent); no-op for N<2. The ≥7 floor is a DATA requirement (manifest pool — order-only can't synthesize a 7th). **Gate: `scripts/audit-activity-variety.js`** (static catalog pool/reshuffle scan + live puppeteer 2-pass probe asserting ≥7 distinct + pass-2≠pass-1 same-set). Reference impl `mini tools/fractions-activity.js` (`shuffledOrder`+`nextTask`), shipped on both fraction partition activities (8/7 pools) 2026-06-17. **Backlog: the other ~39 activities are still on the shell-loop (several <7) — convert each to the `nextTask` pattern + ≥7 when next touched** (`--strict` makes the backlog block). Full doctrine [[feedback-activity-variety-shuffle-rule]]; cross-ref §20.4, §21.4, §A.13.55.

#### A.13.61 Naming-standard label doctrine — formal curriculum term over the transparent form
For any activity whose standard is about **naming / classification** (a child labels or sorts by what a thing IS), the on-screen label uses the locale's **FORMAL curriculum term**, NOT a transparent/descriptive paraphrase — PROVIDED the native expert confirms the formal term is valid + natural for that grade in that locale. Auto-applies to every naming/classify activity; do not re-litigate per build. Origin: E3 #1 (2.G.A.1), where the es ensemble proposed transparent "N lados" labels and the operator ruled the formal polygon name ("Triángulos") because 2.G.A.1 is a NAMING standard. **Negation-label corollary (E3 #2, 1.G.A.1 defining-vs-non-defining):** a "Not a <shape>" box is authored as a FULL literal per-(target, locale) phrase by the native expert (de "Keine Dreiecke", es/pt/it plural "No son…/Não são…/Non sono…", fr "Pas des…", nl/Nordic "Geen…/Inga…/Ingen…", fi partitive-plural "Ei kolmioita") — NEVER a fill-in `Not a {shape}` token apposition (the §A.13.56 case-language trap, esp. Finnish). The POSITIVE box reuses the already-shipped formal polygon labels. Cross-ref §A.13.48/§A.13.54/§A.13.56, §20.10.

#### A.13.62 Desktop-first MEASURED visual-QA sweep (`visual-qa-activity.js`) — STANDING per-activity definition-of-done
**Operator-demanded 2026-06-23 ("create a workflow so visual discrepancies NEVER appear again").** Visual defects kept reaching the operator after "done" because verification happened at ONE viewport (phone 360) while the operator views DESKTOP (~768), and the old gates (`audit-activity-mobile.js`, per-activity `local-test`) only measured horizontal overflow at phone widths — never the three classes that actually shipped: **CUT-OFF at desktop, SPARSE (small content floating in big cards), TINY (content too small to count/read).** `scripts/visual-qa-activity.js --activity=<id>` is the standing gate: a generic puppeteer harness that serves `mini tools/` + `image-library-webp/` locally (no deploy), resolves the wrapper html + `LCS.mount` global by manifest id, FORCES every round via the `_order`/`LCS_reloadFirstTask` seam (§A.13.60 nextTask pattern), and at the viewport sweep **`320·360·412·768·1024·1366`** asserts per (viewport×round): **FITS** (bottom of the lowest CONTROL — answer cards + Check/Next — ≤ viewport, so trailing app padding does NOT mask as cut-off), **no overflow**, **NOT-TINY** (answer-card content bbox + standalone stage images ≥14px; decorative SVG — character eyes, check/badge icons, the small reference panel — are EXCLUDED), **NOT-SPARSE** (answer-card content ≥0.32 area / ≥0.45 width), **tap ≥44px**. Screenshots to `docs/audit-results/<tool>/qa/` at 360/768/1024 for the critic + operator. **DoD = harness GREEN → visual-critic agent on the FULL sweep (incl. desktop 768 AND 1024) → I personally Read the 768+1024+360 renders myself → git diff 0 protected-core/lcs-shell.** **NEVER move a threshold to pass** — fix the layout, or fix WHAT is measured if the gate catches decorative/chrome nodes (a correctness fix, never a threshold loosening). **The fix pattern that surfaced this (Mosaic Menders): content-sized answer cards that HUG their own content at a SHARED tile unit (`--mm-tu: clamp(18px,5vw,30px)`, card width `calc(var(--mm-c) * --mm-tu)`) in a `flex-wrap` centred row** → a short row of N on desktop (not sparse, fits), wrapping to bigger cards on a phone; size differences between cards are the HONEST decoy (correct ≠ biggest), not a salience leak. The harness PROVED itself on first run: caught real desktop/dense-round cut-off in BOTH already-"approved" activities (Echo Grove 4-group rounds 1119px; Mochi "8 donuts" past the phone fold) — both now surfaced as fix-backlog. Cross-ref §20.4, §21.4, §A.13.55 (mobile gate — complementary, narrow-width overflow/clip), §A.13.47, [[feedback-visual-qa-container-containment]].
...
publish-cli's ceilings under growth, in order: (1) time-death tolerance ~10K decks (≈59ms/deck → 10 min/batch); (2) within-batch slug-collision rate ~5-10K; (3) Sharp+chown overhead ~30-55K (CPU-bound); (4) stale-staging-dir lockout (any scale). Engineer in order: chunked batches > pre-collision-check > `fchown` > auto-cleanup. No memory/disk ceiling within 55K/250K. **[2026-09-11] staging auto-cleanup SHIPPED** (`6f7c9e04`: `publish-bulk` prunes its per-deck extraction dirs after a real publish, `--keep-staging` to retain; dry-run-only batches expire via §A.14.12) — it was "deferred until first lockout" and the deferral cost 13 GB.

#### A.14.2 Defer-trigger heuristic for performance commissions
Each commission has an explicit empirical trigger; **default-defer** rather than engineer-now (checkpoint/resume at 5K+/batch or first mid-batch death; collision pre-check at 5K+; `fchown` at 30K+; ~~stale-dir cleanup after first lockout~~ — shipped 2026-09-11, see §A.14.1/§A.14.12). ⚠ **Disk-growth items are NOT performance items and do not default-defer**: every file class a script creates needs its expiry written in the same commit (§A.14.12).
...
Backup gaps surface as **URGENT** (catastrophic FS loss is unrecoverable; gap is cheap to close). Verify backup OR file `[FIX][OPS]` for any asset-tree at `/var/www/lcs-media/<dir>/`. **Off-host backup SHIPPED 2026-09-11 as the PC pull** (§A.14.12 rule 5) — the "deferred trigger ~10 GB" below it was passed by 100 GB before it was acted on; a same-host tarball is never a backup of the disk it sits on.
...
#### A.14.12 Disk hygiene — every file class a script creates must have an expiry (2026-09-11 audit)
The 2026-09-11 audit found **309/436 GB used, ~145 GB of it four classes our own scripts create and never remove**: `deck.html.bak.*` retrofit safety copies (52 GB — the 8 `rewrite-deck-html-*`/`inject-deck-teaching-block`/`reconcile-deck-canonicals`/`strip-deck-hreflang` scripts each `copyFileSync` one per deck, never read back; `publish-wave` step 4 adds one per deck on EVERY wave), a second same-host deck tarball (67 GB — `backup-decks.sh` already prunes to 1 before tar, so "keep 2" protected nothing; now keep 1, `478d4ad2`), `.publish-cli-staging` extraction dirs (13 GB), and Next's runtime image cache left in retired releases (3 GB each). Plus `/tmp` (Ubuntu cleans it only at boot; no reboot since January) and a 60 MB/h pm2 error log from `t()` probes of unauthored `topicProse` keys (fixed with `t.has()`, `b279a58c`). **Standing rules:** (1) **`server-scripts/housekeeping.sh`** (cron Sun 04:30, `--dry-run` to preview) is THE place file classes expire — `.bak.*` >14 d, staging `batch-*` extraction dirs >7 d (reports kept), pm2 logs >14 d, `/root/*.log` >30 d; a new class = a new block there, in the commit that creates the class. (2) `deploy.sh` **aborts below 20 GB free** (`DEPLOY_MIN_FREE_GB`) and empties retired releases' `.next/cache/images`. (3) `/etc/tmpfiles.d/lcs-tmp-age.conf` expires `/tmp` after 14 d — stage wave ZIPs under `/var/www/lcs-media/_staging/<wave>/`, not `/tmp`. (4) **Never probe a message key with `t(key)`** — use `t.has(key)`; next-intl logs a stack per miss. (5) **There is NO same-host deck tarball any more** — the off-host backup is the PC pulling the tree over SSH every Sunday (`scripts/ops/pull-deck-backup.sh`, Task Scheduler "LCS deck backup pull", copies in `C:Userskgenlcs-backups`, keep 4; restore recipe in `docs/ops/server-access.md`). `backup-decks.sh` is retired. ⚠ The ZIPs on the PC are NOT a catalog backup (~27% coverage, all pre-retrofit) — only the pulled tree is. (6) ⭐⭐ **OPERATOR RULE (2026-09-11): the server hosts ONLY what it serves or builds; everything else — backups, archives, snapshots, one-off staging, superseded versions, records — lives on the PC** (`C:Userskgenlcs-backups` weekly pull; `C:Userskgenlcs-archive` cold archives via `scripts/ops/archive-from-server.sh <name> <path>… [--delete]`, which verifies the archive BEFORE deleting on the server). "The free space of the server is valuable and expensive unlike the local disk." The server checkout excludes `docs/audit-results` (git sparse-checkout); server DB-dump retention is 7 d (the PC keeps the history). Memory: `feedback_server_disk_is_expensive.md`. ⚠ Traps paid for while building the gates: a `releases/*/` glob includes the `current` SYMLINK — compare `readlink -f`, never the path (the dry-run caught the live release's cache in the rm list); a mock that back-dates a directory and THEN writes into it has reset its mtime (the poison failed against a correct script); server-side `curl` to `www` gets a Cloudflare 403 — origin checks use `--resolve www.lessoncraftstudio.com:443:127.0.0.1`. Record: memory `project_server_disk_cleanup_2026_09.md`.
