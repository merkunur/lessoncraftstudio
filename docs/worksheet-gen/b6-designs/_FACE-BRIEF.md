# nt5-F FACE BRIEF — the contract for building the FIVE variation faces of ONE family (Phase E)

You are building the five variation faces (§3 of `docs/worksheet-gen/b6-designs/<ID>-<key>.md`) of a family whose BASE spec is already committed and gated. The design file is the contract; the base spec, its component file, its EN bank, its gate and the build record `_work/<ID>-build.md` (read its "open items for the faces" — deferred face poisons, the seams the base left for you, refusals) are the substrate. `_BUILD-BRIEF.md` rules still apply (chrome budgets: body 722 (en/de 3-line chrome; measure your own family — pronouns measured 710) and 677 (the fi four-line title), the 36/44/56 element floors per band, no visible "free", every picture opened, `_buildWith` seam, stamps-only `verify()`).

## Ids are FIXED
`docs/worksheet-gen/b6-designs/_records/b6var-id-allocation.json` gives every (family, face) its id and band directory (`k`/`g1`/`g2`/`g3`) — some faces sit in ANOTHER band than the base (a K family's G1 face takes a G1 id and lives in `types/g1/`). Use exactly those ids; `gen-b6var-specs.js` refuses any other.
...
## Deliverable 1 — `scripts/worksheet-gen/tools/b6var-rows/<key>.js` (NEW; the only file the emitter reads)
```js
'use strict';
...
- `node tools/gate-variation-distinct.js --batch=b6 --diffs=2 --family=<key>` → must print "every variation differs …" (each face's resolved d2 config ≠ the base's d2 AND ≠ every sibling face).
...
## Deliverable 5 — the record: `docs/worksheet-gen/b6-designs/_work/<ID>-faces.md`
...
- The loader is `lib/b6-common.js bank(name, loc)` over `data/b6/<name>.js`; shared b3/b4 libs and other families' files are read-only.
- The release baseline was re-captured after all five b6 bases were committed (`out/b3-baseline.json`); `node tools/b3-baseline.js --check --quick` must print PASS (the base stays byte-identical). The 25 face ids are the accepted drift at the Phase E close — do not `--capture`.
