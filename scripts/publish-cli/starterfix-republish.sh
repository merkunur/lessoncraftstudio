#!/usr/bin/env bash
# starterfix-republish.sh <locale> [--confirm]
#
# 2026-09-21: in-place republish of the live decks the writing-frame STARTER fix
# (rulingBlock/factLane — K-335 G2-278 G2-299 G2-318 G2-339..342) and the G2-235
# ruler-registration fix changed (13 decks × 11 locales; ZIPs regenerated on the PC
# by scripts/worksheet-gen/tools/starterfix-regen.js, staged under
# /var/www/lcs-media/_staging/starterfix/<loc>/). Runs ON HETZNER. Cloned from
# divfix-republish.sh (the proven nt20-C C3 flow); only the batch name differs.
#
...
STAGE=/var/www/lcs-media/_staging/starterfix/$LOC
REC=/root/staging/starterfix; mkdir -p "$REC"
...
node scripts/publish-cli/index.js publish-bulk "$STAGE" --dry-run --batch-id "starterfix-$LOC-probe" > "$REC/$LOC-probe.log" 2>&1 || true
COLL=$ROOT/.publish-cli-staging/starterfix-$LOC-probe/_collisions.txt
...
if [ "$MODE" = "--confirm" ]; then
  node scripts/publish-cli/publish-wave.js "$STAGE" --locales="$LOC" --updates-manifest="$REC/$LOC-updates.json" --skip-preband --batch-id="starterfix-$LOC" --confirm 2>&1 | tee "$REC/$LOC-confirm.log" | grep -E "^\[|PUBLISHED|FAIL|ERROR|HALT|routed=|✓|✗|clean|audit" | tail -60
  echo "== $LOC routed:"; grep -c "routed=UPDATE" $ROOT/.publish-cli-staging/starterfix-$LOC/_results.txt || true
else
  node scripts/publish-cli/publish-wave.js "$STAGE" --locales="$LOC" --updates-manifest="$REC/$LOC-updates.json" --skip-preband --batch-id="starterfix-$LOC-dry" 2>&1 | tee "$REC/$LOC-dry.log" | grep -E "ok=|collisions=|errors=|halt|HALT|ERROR|DRY-RUN complete" | tail -12
fi
