/* =====================================================================
   audit-lids-locale-layout.js — TOOL #39, 11 locales x 6 widths
   ---------------------------------------------------------------------
   Run:  node scripts/audit-lids-locale-layout.js

   The sweep in local-test-lids.js measures ENGLISH at six widths. This
   measures the other ten, because German compounds and Finnish cases run
   long and a control that fits "Lift the lids" need not fit "Deckel
   ...
   lids are DOWN and the gate is showing, so this drives the tool into
  ...
  for (const loc of LOCALES) {
    for (const w of WIDTHS) {
      ...
      const m = await page.evaluate(() => {
        ...
        const ctrls = Array.from(document.querySelectorAll('.lid-chip,.lid-mark,.lid-lid')).filter(vis);
        const minCtrl = ctrls.length ? Math.min(...ctrls.map((e) => { const r = e.getBoundingClientRect(); return Math.min(r.width, r.height); })) : null;
        const texts = Array.from(document.querySelectorAll('.lid-hint,.lid-chip,.lid-mark,.lid-gate span,.lid-gate a')).filter((e) => vis(e) && (e.textContent || '').trim());
