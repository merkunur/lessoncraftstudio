...
**Known cosmetic residue (not auto-fixed):** 73 published decks (en 63, es 4, pt 3, de 2, fr 1) carry a `thanksgivinng` (double-n) typo in the slug + meta description, baked from an old theme-source typo (current derivation produces `thanksgiving` correctly). URLs resolve + are self-canonical; re-slugging indexed URLs is an outward-facing call surfaced to the operator rather than auto-executed. A fix would re-slug (301 old→new via the live old-slug redirect map) + republish-seo the description.
