...
(function (global) {
  ...
  function ottoSVG(mood) {
    ...
    return '<svg class="opb-owl-svg" viewBox="0 0 100 100" role="img" aria-label="' + (LANG === 'fr' ? 'Otto le hibou' : LANG === 'de' ? 'Otto, die Eule' : LANG === 'es' ? 'Otto, el búho' : LANG === 'pt' ? 'Otto, a coruja' : 'Otto the owl') + '">' +
  ...
  global.OttoPictureBookActivity = {
    ...
    strings: {
      title: { en: "Otto's Picture Book", de: 'Ottos Bilderbuch', fr: 'Le livre d’images d’Otto', es: 'El libro de imágenes de Otto', pt: 'O livro de desenhos do Otto' },
      prompt: { en: 'Which picture shows this part?', de: 'Welches Bild zeigt diesen Teil?', fr: 'Quelle image montre ce moment ?', es: '¿Qué imagen muestra este momento?', pt: 'Qual imagem mostra esta parte?' },
      ottoIntro: { en: 'I drew this story! Which picture shows the part I read?', de: 'Ich habe diese Geschichte gemalt! Welches Bild zeigt den Teil, den ich vorlese?', fr: 'J’ai dessiné cette histoire ! Quelle image montre le moment que je lis ?', es: '¡Yo dibujé este cuento! ¿Qué imagen muestra lo que leo?', pt: 'Eu desenhei esta história! Qual imagem mostra a parte que eu li?' },
      hearStory: { en: '📖 Hear the story', de: '📖 Geschichte anhören', fr: '📖 Écouter l’histoire', es: '📖 Escuchar el cuento', pt: '📖 Ouvir a história' },
      hearAgain: { en: '📖 Hear it again', de: '📖 Noch einmal anhören', fr: '📖 Réécouter', es: '📖 Escuchar otra vez', pt: '📖 Ouvir de novo' },
      thePart: { en: 'Otto reads:', de: 'Otto liest vor:', fr: 'Otto lit :', es: 'Otto lee:', pt: 'Otto lê:' },
      hintPick: { en: 'Tap the picture that shows this part!', de: 'Tippe auf das Bild, das diesen Teil zeigt!', fr: 'Tape l’image qui montre ce moment !', es: '¡Toca la imagen que muestra este momento!', pt: 'Toque na imagem que mostra esta parte!' },
      hintWrong: { en: 'That picture shows a different part — listen again.', de: 'Dieses Bild zeigt einen anderen Teil – hör noch einmal zu.', fr: 'Cette image montre un autre moment — réécoute.', es: 'Esa imagen muestra otro momento. Escucha otra vez.', pt: 'Essa imagem mostra outra parte — ouça de novo.' },
      ottoWin: { en: 'Yes! That picture shows it exactly!', de: 'Ja! Genau dieses Bild zeigt es!', fr: 'Oui ! Cette image le montre exactement !', es: '¡Sí! ¡Esa imagen lo muestra justo!', pt: 'Isso! Essa imagem mostra certinho!' }
    },
    defaults: {},
    ...
    render: function () {
      ...
      var sp = api.el('button', 'opb-spk'); sp.type = 'button'; sp.setAttribute('aria-label', LANG === 'fr' ? 'Lire la phrase' : LANG === 'de' ? 'Den Teil vorlesen' : LANG === 'es' ? 'Leer la oración' : LANG === 'pt' ? 'Ouvir a parte da história' : 'Read the part'); sp.textContent = '🔊';
      ...
      this.view.choices.forEach(function (ch, i) {
        ...
        card.setAttribute('aria-label', (LANG === 'fr' ? 'Image ' : LANG === 'de' ? 'Bild ' : LANG === 'es' ? 'Imagen ' : LANG === 'pt' ? 'Imagem ' : 'Picture ') + (i + 1));
