#!/usr/bin/env node
/* Wave 4 standalone coordinate enumeration (READ-ONLY): prepositions/multiplechoice + bingo/null × EN K.
 * Group by (type, mode, theme); canonical = earliest publishedAt; siblings = rest (bingo heavy collapse).
 * MUST run on Hetzner. Usage: node scripts/seo-landing/enum-wave4-literacyB.js
 */
'use strict';
...
(async function () {
  ...
  var rows = await c.deck.findMany({
    where: {
      ...
      OR: [
        { exerciseType: 'prepositions', exerciseMode: 'multiplechoice' },
        { exerciseType: 'bingo', exerciseMode: null }
      ]
    },
  ...
  rows.forEach(function (r) {
    ...
    var key = r.exerciseType + '::' + (r.exerciseMode || 'null') + '::' + theme;
  ...
  var coords = Object.keys(groups).sort().map(function (key) {
    ...
    var v = validateCoordinate ? validateCoordinate(type, modeForGate, theme, {}) : { valid: true, warn: null };
  ...
  console.log(JSON.stringify({
    ...
    sampleSlugs: valid.map(function (x) { return x.type + ' -> ' + x.canonical; }).filter(function (s, i, a) { return a.indexOf(s.split(' -> ')[0] + ' -> ' + s.split(' -> ')[1]) === i; }).slice(0, 6),
