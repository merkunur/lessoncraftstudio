...
/**
 * GET /api/admin/borders/update
 * Retrieves border themes with pagination support
 * Query params:
...
async function getHandler(request: NextRequest, userId: string) {
  try {
    ...
    try {
      ...
      // Otherwise, return theme list with image counts only
    ...
    return NextResponse.json(
      { error: 'Failed to read borders metadata', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
  }
}
...
/**
 * POST /api/admin/borders/update
 * Updates border metadata (translations, display names) in database
 */
async function postHandler(request: NextRequest, userId: string) {
  try {
    ...
    return NextResponse.json({
      success: true,
      message: 'Borders metadata updated successfully in database',
      themes: updatedThemes,
    });
  } catch (error) {
    console.error('Error updating borders metadata:', error);
    return NextResponse.json(
      { error: 'Failed to update borders metadata', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
  }
}
...
/**
 * DELETE /api/admin/borders/update
 * Deletes a border theme
 */
