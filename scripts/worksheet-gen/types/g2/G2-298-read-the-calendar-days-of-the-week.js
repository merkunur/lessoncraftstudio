/** G2-298 — Read the Calendar: Days of the Week. nt20-B-VAR variation of G2-277. */
'use strict';
const base = require('./G2-277-read-the-calendar.js');
// One object for all three levels: the waves ship d2 only, so a face must
// render identically whichever level is asked for. Spreading the base entry
// (not a JSON literal) carries function-valued params through intact.
const D = { ...base.difficulty[1], ...{"questions":["dayOfDate","countWeekday","firstDay","lastDay"],"stickers":2,"cellH":72} };
module.exports = {
  ...base,
  id: 'G2-298',
  slug: 'read-the-calendar-days-of-the-week',
  difficulty: { 1: D, 2: D, 3: D },
  i18n: { en: { title: "Read the Calendar: Days of the Week", instruction: "Every question is about the days. Read down the columns." } },
};
