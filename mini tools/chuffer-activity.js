...
(function (global) {
  ...
  global.ChufferActivity = {
    ...
    strings: {
      title: { en: "Chuffer's Switchyard", de: 'Chuffers Rangierbahnhof', fr: 'La gare de triage de Chuffer', es: 'El patio de Chuffer', pt: 'Pátio de Manobras do Chuffer', it: 'Lo scalo di Chuffer' },
      prompt: { en: 'Split the load and say each way!', de: 'Teile die Ladung auf und sag jeden Weg!', fr: 'Partage la cargaison et dis chaque façon !', es: '¡Reparte la carga y di todas las formas!', pt: 'Divida a carga e diga cada jeito!', it: 'Scomponi il carico e nomina ogni modo!' },
      ...
      covaryHint: { en: 'Move ONE crate — what is the new split?', de: 'Schiebe EINE Kiste – wie heißt die neue Zerlegung?', fr: 'Déplace UNE caisse — quelle est la nouvelle décomposition ?', es: 'Mueve UNA caja… ¿cuál es el nuevo reparto?', pt: 'Mova UMA caixa — qual é a nova divisão?', it: 'Sposta UNA cassa: qual è il nuovo modo?' },
      huntHint: { en: 'The route-book has a gap — find that way!', de: 'Im Fahrtenbuch fehlt ein Weg – finde ihn!', fr: 'Il manque une façon dans le carnet — trouve-la !', es: 'Falta una forma en la bitácora, ¡encuéntrala!', pt: 'Falta um jeito no caderno de rotas — ache esse!', it: 'Nel quaderno manca un modo: trovalo!' },
      maketenHint: { en: 'One car is sealed — find the partner to make 10!', de: 'Ein Wagen ist verschlossen – finde den Zehnerfreund!', fr: 'Un wagon est fermé — trouve le complément pour faire 10 !', es: 'Un vagón está cerrado: ¡encuentra la pareja del diez!', pt: 'Um vagão está lacrado — ache o amigo do dez!', it: 'Un vagone è chiuso: trova il suo amico del dieci!' },
      equationHint: { en: 'Build this exact load: {a} + {b}!', de: 'Baue genau diese Ladung: {a} + {b}!', fr: 'Construis exactement cette cargaison : {a} + {b} !', es: 'Arma justo esta carga: ¡{a} + {b}!', pt: 'Monte esta carga certinha: {a} + {b}!', it: 'Forma questo carico: {a} + {b}!' },
      mirrorHint: { en: 'You found {a} + {b} — what if we swap the cars?', de: 'Du hast {a} + {b} gefunden – was, wenn wir die Wagen tauschen?', fr: 'Tu as trouvé {a} + {b} — et si on échangeait les wagons ?', es: 'Encontraste {a} + {b}… ¿y si cambiamos los vagones?', pt: 'Você achou {a} + {b} — e se a gente trocar os vagões?', it: 'Hai fatto {a} + {b}: e se scambi i vagoni?' },
      judgeHint: { en: 'Chuffer says a route — is it NEW, or one we have?', de: 'Chuffer nennt einen Weg – ist er NEU oder haben wir ihn schon?', fr: 'Chuffer propose une façon — est-elle NOUVELLE, ou déjà trouvée ?', es: 'Chuffer dice una forma… ¿es NUEVA o ya la tenemos?', pt: 'O Chuffer diz uma rota — é NOVA, ou já temos?', it: 'Chuffer dice un modo: è NUOVO o già fatto?' },
      sayA: { en: '?', de: '?', fr: '?', es: '?', pt: '?', it: '?' },
      coupler: { en: 'Pull the coupler! 🔗', de: 'Zieh die Kupplung! 🔗', fr: 'Tire l’attelage ! 🔗', es: '¡Jala el enganche! 🔗', pt: 'Puxe o engate! 🔗', it: 'Tira il gancio! 🔗' },
      pickPrompt: { en: 'How many in this car?', de: 'Wie viele sind in diesem Wagen?', fr: 'Combien dans ce wagon ?', es: '¿Cuántas hay en este vagón?', pt: 'Quantas caixas neste vagão?', it: 'Quante casse in questo vagone?' },
      newRoute: { en: 'New route!', de: 'Neuer Weg!', fr: 'Nouvelle façon !', es: '¡Es NUEVA!', pt: 'Rota nova!', it: 'Modo nuovo!' },
      gotIt: { en: 'We have that!', de: 'Haben wir schon!', fr: 'On l’a déjà !', es: '¡Ya la tenemos!', pt: 'Essa a gente já tem!', it: 'Già fatto!' },
      swap: { en: 'Swap them!', de: 'Tausche sie!', fr: 'Échange-les !', es: '¡Cámbialos!', pt: 'Troque os vagões!', it: 'Scambiali!' },
      banked: { en: 'CLUNK! {a} and {b} — logged!', de: 'KLACK! {a} und {b} – eingetragen!', fr: 'CLAC ! {a} et {b} — enregistré !', es: '¡CLAC! {a} y {b}… ¡anotado!', pt: 'CLEC! {a} e {b} — anotado!', it: 'CLONK! {a} e {b}: segnato!' },
      mirror: { en: 'Same crates, just swapped — {a} and {b}, OR {b} and {a}!', de: 'Gleiche Kisten, nur getauscht – {a} und {b} ODER {b} und {a}!', fr: 'Les mêmes caisses, juste échangées — {a} et {b}, OU {b} et {a} !', es: 'Las mismas cajas, solo cambiadas: ¡{a} y {b} O {b} y {a}!', pt: 'As mesmas caixas, só trocadas — {a} e {b}, OU {b} e {a}!', it: 'Stesse casse scambiate: {a} e {b}, O {b} e {a}!' },
      dupe: { en: 'Chuffer already knows that route — find a NEW way!', de: 'Diesen Weg kennt Chuffer schon – finde einen NEUEN!', fr: 'Chuffer connaît déjà cette façon — trouve-en une NOUVELLE !', es: 'Chuffer ya conoce esa forma… ¡busca una NUEVA!', pt: 'O Chuffer já conhece essa rota — ache um jeito NOVO!', it: 'Chuffer conosce già questo modo: trovane uno NUOVO!' },
      mismatch: { en: "Let's count again — how many in each car?", de: 'Zählen wir noch mal – wie viele sind in jedem Wagen?', fr: 'Comptons encore — combien dans chaque wagon ?', es: 'Contemos otra vez… ¿cuántas hay en cada vagón?', pt: 'Vamos contar de novo — quantas caixas em cada vagão?', it: 'Contiamo di nuovo: quante casse per vagone?' },
      win: { en: 'Chuffer reached the station! 🚂🎉', de: 'Chuffer ist am Bahnhof angekommen! 🚂🎉', fr: 'Chuffer est arrivé à la gare ! 🚂🎉', es: '¡Chuffer llegó a la estación! 🚂🎉', pt: 'O Chuffer chegou na estação! 🚂🎉', it: 'Chuffer è arrivato alla stazione! 🚂🎉' },
      hintCheck: { en: 'Find every way to split the load.', de: 'Finde alle Wege, die Ladung zu zerlegen.', fr: 'Trouve toutes les façons de partager la cargaison.', es: 'Encuentra todas las formas de repartir la carga.', pt: 'Ache todos os jeitos de dividir a carga.', it: 'Trova ogni modo di scomporre il carico.' }
    },
    defaults: {},
    ...
    _car: function (which, count, sealed) {
      ...
      var cap = api.el('div', 'cf-carcap'); cap.textContent = (which === 'a' ? (LANG === 'es' ? 'Vagón azul' : LANG === 'de' ? 'Blauer Wagen' : (LANG === 'fr' ? 'Wagon bleu' : LANG === 'pt' ? 'Vagão azul' : LANG === 'it' ? 'Vagone azzurro' : 'Blue car')) : (LANG === 'es' ? 'Vagón rojo' : LANG === 'de' ? 'Roter Wagen' : (LANG === 'fr' ? 'Wagon rouge' : LANG === 'pt' ? 'Vagão vermelho' : LANG === 'it' ? 'Vagone rosso' : 'Red car'))) + (sealed ? ' 🔒' : ''); box.appendChild(cap);
    ...
    _couple: function () {
      ...
      if (res === 'recorded') {
        ...
        speak(numWord(this._whole()) + (LANG === 'es' ? ' es ' : LANG === 'de' ? ' ist ' : (LANG === 'fr' ? ' c’est ' : LANG === 'pt' ? ' é ' : LANG === 'it' ? ' è ' : ' is ')) + numWord(this.cstate.a) + (LANG === 'es' ? ' y ' : LANG === 'de' ? ' und ' : (LANG === 'fr' ? ' et ' : LANG === 'pt' ? ' e ' : LANG === 'it' ? ' e ' : ' and ')) + numWord(this.cstate.b));
    ...
    _renderJudge: function (root) {
      ...
      var main = api.el('div', 'cf-main');
      var yard = api.el('div', 'cf-yard');
      var claim = api.el('div', 'cf-claim'); claim.innerHTML = (LANG === 'es' ? 'Chuffer dice: <b>' : LANG === 'de' ? 'Chuffer sagt: <b>' : (LANG === 'fr' ? 'Chuffer dit : <b>' : LANG === 'pt' ? 'O Chuffer diz: <b>' : LANG === 'it' ? 'Chuffer dice: <b>' : 'Chuffer says: <b>')) + r.whole + ' = ' + prop.a + ' + ' + prop.b + '</b>'; yard.appendChild(claim);
    ...
    _judge: function (sayNew) {
      ...
      if (sayNew === isNew) {
        if (isNew) { s.manifest[key] = true; speak(numWord(r.whole) + (LANG === 'es' ? ' es ' : LANG === 'de' ? ' ist ' : (LANG === 'fr' ? ' c’est ' : LANG === 'pt' ? ' é ' : LANG === 'it' ? ' è ' : ' is ')) + numWord(prop.a) + (LANG === 'es' ? ' y ' : LANG === 'de' ? ' und ' : (LANG === 'fr' ? ' et ' : LANG === 'pt' ? ' e ' : LANG === 'it' ? ' e ' : ' and ')) + numWord(prop.b)); }
      ...
      } else { this.api.sound && this.api.sound(330); this.msg = (LANG === 'es') ? (isNew ? '¡Mira, todavía no está en la bitácora!' : 'Mira otra vez… esa ya la tenemos.') : (LANG === 'de') ? (isNew ? 'Schau – der steht noch nicht im Buch!' : 'Schau noch mal – den haben wir schon!') : (LANG === 'fr' ? (isNew ? 'Regarde — il n’est pas encore dans le carnet !' : 'Regarde encore — on l’a déjà, celui-là !') : (LANG === 'pt' ? (isNew ? 'Olha — ainda não está no caderno!' : 'Olha de novo — essa a gente já tem!') : LANG === 'it' ? (isNew ? 'Guarda: non è ancora sul quaderno!' : 'Guarda meglio: questo modo lo abbiamo già!') : (isNew ? "Look — it's not in the book yet!" : "Look again — we already have that one!"))); this.render(); }
    },

    _win: function () { this.solved = true; this.solvedCount = Math.min(this.solvedCount + 1, (this._pool && this._pool.length) || 9); this.api.sound && this.api.sound(940); this.render(); this.announce(this.api.t('win')); speak(LANG === 'es' ? 'Chuffer llegó a la estación' : LANG === 'de' ? 'Chuffer ist am Bahnhof angekommen' : (LANG === 'fr' ? 'Chuffer est arrivé à la gare' : LANG === 'pt' ? 'O Chuffer chegou na estação' : LANG === 'it' ? 'Chuffer è arrivato alla stazione' : 'Chuffer reached the station')); },

    _staircase: function () {
      var api = this.api, N = this._whole(), s = this.cstate, ways = Core.waysFor(N), book = api.el('div', 'cf-book');
      var title = api.el('div', 'cf-booktitle'); title.textContent = (LANG === 'es' ? 'La bitácora' : LANG === 'de' ? 'Fahrtenbuch' : (LANG === 'fr' ? 'Carnet de route' : LANG === 'pt' ? 'Caderno de rotas' : LANG === 'it' ? 'Quaderno di viaggio' : 'Route-book')); book.appendChild(title);
