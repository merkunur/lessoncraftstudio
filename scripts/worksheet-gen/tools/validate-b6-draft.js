#!/usr/bin/env node
/**
 * validate-b6-draft.js <locale> [path] [--no-probe] — the refuse-don't-guess gate for
 * a nt5-F native-panel draft (default path i18n/.draft-b6-<locale>.json).
 *
 * Draft shape (docs/worksheet-gen/b6-panel-brief.md):
 *   { locale,
 ...
 *              theme candidates of tools/gen-b6-waves.js) against the draft's blocks in a
 *              temp locales dir (B6_LOCALES_DIR) — a spec that refuses must be DECLARED in
 ...
 *
 * Poison-tested by tools/validate-b6-draft.test.js (an EN-derived draft PASSES; one
...
// bank module name per family key (nt5-F: none renamed; the map stays for parity with validate-b3-draft)
const BANK_OF = {}; // nt5-F: every bank is named after its family key (data/b6/<key>.js)
...
/** Writes the draft's blocks to a temp locales dir and runs tools/b6-probe-child.js there. */
function buildProbe(loc, draft) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'b6draft-'));
  for (const [b, block] of Object.entries(draft.banks || {})) fs.writeFileSync(path.join(dir, `${b}.${loc}.json`), JSON.stringify(block));
  const out = execFileSync(process.execPath, [path.join(__dirname, 'b6-probe-child.js'), loc], {
    env: { ...process.env, B6_LOCALES_DIR: dir, B6_DRAFT_SLUGS: JSON.stringify(Object.fromEntries(Object.entries(draft.families || {}).map(([k, f]) => [k, f && f.slug]))) }, encoding: 'utf8', maxBuffer: 64 * 1024 * 1024, cwd: WG,
  });
...
if (require.main === module) {
  ...
  const p = process.argv[3] && !process.argv[3].startsWith('--') ? process.argv[3] : path.join(WG, 'i18n', `.draft-b6-${loc}.json`);
  if (!loc) { console.error('usage: validate-b6-draft.js <locale> [path] [--no-probe]'); process.exit(2); }
