...
import '../homepage-v3/homepage-v3.css';
import './homepage-v4.css';

/* Homepage v4 PREVIEW layout — same pattern as ../homepage-v3/layout.tsx:
   robots noindex/nofollow (never indexed; promotion = import-swap in
   app/[locale]/page.tsx, this route then stays as the visual-diff safety
   net exactly like /preview/homepage-v3 does today).
...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: 'homepageV4.metadata' });
  return {
    title: t('title'),
...
export default function HomepageV4Layout({
  ...
  return (
    <>
      {/* Preview-route-scoped body override — identical to the v3 preview
          (deep teal #0E544A ground + chalk-on-teal endpaper pattern). Only
          renders while this layout is active. */}
      <style>{`
        body {
