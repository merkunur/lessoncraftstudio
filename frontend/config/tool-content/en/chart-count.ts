...
const content: ToolContent = {  seo: {
  ...
  tutorial: {
    ...
    steps: [
      ...
      },
      {
        ...
        description: 'In the Page Setup section, select your page size: Letter Portrait, Letter Landscape, A4 Portrait, A4 Landscape, Square (1200×1200), or enter a custom dimension. Choose a page background color — white is the default for clean worksheets. Select a decorative background theme and a border theme from the built-in library, each with an independent opacity slider (0–1, step 0.05). Background and border themes work independently, so you can pair a subtle watercolor background with a bold decorative border or any combination that fits your product style.',
      },
      {
        title: 'Choose Your Image Source',
        description: 'Open the Image Library panel and use the Worksheet Image Source dropdown to select a theme like Animals, Food, or Vehicles. The app randomly picks 6 images from that collection for automatic selection. Alternatively, switch to manual mode: browse the Image Library by theme or search by keyword, then click exactly 6 images. Selected images appear in a preview strip where you can click to remove and replace individual picks. With no theme selected, the app draws 6 random images from all available collections. Every mode guarantees exactly 6 image types per worksheet.',
      },
      {
      ...
      },
      {
        ...
        description: 'Use the Text Tools panel to add titles, labels, or instructions with seven font choices: Lexend Deca, Baloo 2, Nunito, Quicksand, Fredoka, Arial, and Verdana. Adjust text outline width from 0 to 10 with 0.5-step granularity for readable headings. Drag, resize, rotate, and reposition every element on the Fabric.js canvas. Use layer controls to manage stacking order, lock finished elements, and zoom from 25% to 300% for precision placement. Undo and redo up to 20 history states with Ctrl+Z and Ctrl+Y.',
      },
      {
  ...
  businessIdeas: [
    ...
    {
      title: 'Gumroad Complete Graphing Toolkit',
      description: 'Bundle picture graph worksheets across all 104 themes into a comprehensive graphing toolkit on Gumroad. Include 200+ worksheets spanning every major theme, each with its auto-generated answer key — 400+ total files. The toolkit format justifies premium pricing because buyers get a complete library rather than individual packs. Add new seasonal collections quarterly to drive repeat purchases. The four-file-per-session workflow (worksheet JPEG, worksheet PDF, answer key JPEG, answer key PDF) means building the entire toolkit is efficient and systematic.',
      platform: 'Gumroad',
    },
    {
  ...
  proTips: [
    ...
    },
    {
      ...
      description: 'The 4×5 scattered image grid randomly distributes 20 icons from 6 types with each type appearing 1–5 times. Every click of Generate produces a different counting challenge even with the same 6 images selected. Use this to rapidly create large worksheet bundles where no two pages are identical. Generate 15–20 worksheets per theme in minutes, each with its own unique distribution and matching answer key.',
    },
    {
      title: 'Use Both Background and Border Themes Together',
      description: 'Unlike apps with only border themes, the Chart Count Generator offers independent background and border themes with separate opacity controls. Set a subtle watercolor background at 15–25% opacity to add visual warmth without distracting from the image grid and bar chart. Layer a decorative border at 80–100% opacity for a polished frame. This dual-theme approach creates professional-looking worksheets that stand out in marketplace listings. Consistent background and border styling across a bundle creates a cohesive product look.',
    },
    {
  ...
  faq: [
    {
      ...
      answer: 'Yes. The tool offers a free trial with every feature unlocked — the dual-canvas system, scattered image grid generation, bar chart creation, auto-generated answer key with yellow-highlighted cells, all 104 themed image collections with 3,100+ illustrations, automatic and manual image selection, localized headers, background and border themes, name/date fields, grayscale export, and all download formats. No signup, no credit card required. Free trial downloads include a watermark. Purchase a commercial license to remove the watermark and unlock selling rights.',
    },
    {
    ...
    },
    {
      ...
      answer: 'The Chart Count Generator offers both background themes and border themes from the built-in library, each with an independent opacity slider (0–1, step 0.05). Set a subtle watercolor background at 20% opacity while keeping a bold decorative border at full strength, or any combination you prefer. Background themes fill the page area behind the image grid and chart, while border themes frame the outer edge. Together they add visual polish and increase perceived product quality without interfering with the worksheet content.',
    },
    {
  ...
  visuals: {
    ...
    videoTitle: 'How to Create Picture Graph Worksheets with Auto Answer Keys, Scattered Image Grids, and 104 Themed Collections — Step-by-Step Tutorial',
  },
};
