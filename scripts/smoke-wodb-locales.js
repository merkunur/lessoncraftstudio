#!/usr/bin/env node
/* All-11-locale mount smoke for wodb: the featured 2×2 grid renders,
   title/instruction localized, dock chips present, and zero non-404
   console errors. */
'use strict';
...
(async () => {
  ...
  for (const L of LOCALES) {
    errs.length = 0;
    await page.goto(`http://127.0.0.1:${PORT}/mini-tools/wodb.html?lang=${L}`, { waitUntil: 'networkidle0' });
    const got = await page.waitForSelector('.wdb-cell', { timeout: 8000 }).then(() => true).catch(() => false);
    const m = await page.evaluate(() => ({
      ...
      chips: document.querySelectorAll('.wdb-chip').length,
    }));
    const real = errs.filter((e) => !/404|Failed to load resource|net::ERR/i.test(e));
    const ok = got && m.cells === 4 && m.title && m.instr && m.name && m.chips >= 3 && real.length === 0;
    if (!ok) fails++;
    console.log(`${ok ? '✓' : '✗'} ${L}: "${m.title}" — featured="${m.name}" cells=${m.cells}${real.length ? ' ERRORS: ' + real.slice(0, 2).join(' | ') : ''}`);
  }
  await browser.close();
  server.close();
