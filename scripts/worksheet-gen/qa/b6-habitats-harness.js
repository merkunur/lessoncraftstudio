/**
 * b6-habitats-harness.js — the G1-398 `habitats` render harness shared by its primitive gates
 * (verify-habitat-tile, verify-animal-home, verify-b6-habitats). Cloned from
 * qa/b5-maps-harness.js (G1-379). Type-scoped; nothing shared is edited.
 *
 *   withBrowser(fn)                    one headless Chromium for a whole gate
 *   openDoc(page, name, bodyHtml, {filter, width})
 *                                      writes out/dev/G1-398-<name>.html with the shell's
 ...
 *                                      out/dev/G1-398-<name>-{colour,grey}.png
...
async function openDoc(page, name, bodyHtml, opts = {}) {
  ...
  const f = path.join(OUT, `G1-398-${name}.html`);
...
async function sheet(page, name, bodyHtml, opts = {}) {
  ...
  for (const [tag, filter] of [['colour', ''], ['grey', 'filter:grayscale(1);']]) {
    ...
    const p = path.join(OUT, `G1-398-${name}-${tag}.png`);
