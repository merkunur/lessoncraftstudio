...
(function (global) {
  ...
  function fawnSVG() {
    return '<svg class="fcg-fawn-svg" viewBox="0 0 100 100" role="img" aria-label="' + (LANG === 'fr' ? 'Fern le faon' : LANG === 'de' ? 'Fern, das Rehkitz' : LANG === 'es' ? 'Fern el cervatillo' : LANG === 'pt' ? 'Fern, o cervo' : LANG === 'it' ? 'Fern, il cerbiatto' : 'Fern the fawn') + '">' +
  ...
  global.FernClueGardenActivity = {
    ...
    strings: {
      title: { en: "Fern's Clue Garden", de: 'Ferns Rätselgarten', fr: 'Le jardin d’indices de Fern', es: 'El jardín de acertijos de Fern', pt: 'O jardim de pistas da Fern', it: 'Il giardino degli indizi di Fern' },
      prompt: { en: 'What does the word mean here?', de: 'Was bedeutet das Wort hier?', fr: 'Que veut dire le mot ici ?', es: '¿Qué significa la palabra aquí?', pt: 'O que a palavra quer dizer aqui?', it: 'Che cosa vuol dire la parola qui?' },
      fernIntro: { en: 'Read the whole sentence — the clues are around the word!', de: 'Hallo, ich bin Fern! Lies den ganzen Satz – er verrät dir wie ein kleines Rätsel, welche Bedeutung gemeint ist.', fr: 'Lis toute la phrase — les indices sont autour du mot !', es: '¡Hola, soy Fern! Lee toda la oración: es como un pequeño acertijo que te dice cuál es el significado correcto.', pt: 'Leia a frase toda — as pistas estão em volta da palavra!', it: 'Ciao, sono Fern! La frase intera è il tuo indizio.' },
      theAsk: { en: 'What does "{word}" mean in this sentence?', de: 'Was bedeutet „{word}“ in diesem Satz?', fr: 'Que veut dire « {word} » dans cette phrase ?', es: '¿Qué significa «{word}» en esta oración?', pt: 'O que “{word}” quer dizer nesta frase?', it: 'Che cosa vuol dire «{word}» in questa frase?' },
      ...
      win: { en: 'Yes! The sentence shows what it means. 🌿', de: 'Ja! Genau diese Bedeutung zeigt der Satz.', fr: 'Oui ! La phrase montre ce que le mot veut dire. 🌿', es: '¡Sí! Ese es el significado que muestra la oración.', pt: 'Isso! A frase mostra o sentido certo. 🌿', it: 'Sì! La frase ti mostra il significato giusto. 🌿' }
    },
    defaults: {},
    ...
    render: function () {
      ...
      var sp = api.el('button', 'fcg-spk'); sp.type = 'button'; sp.setAttribute('aria-label', (LANG === 'fr' ? 'écouter la phrase' : LANG === 'de' ? 'Satz anhören' : LANG === 'es' ? 'escuchar la oración' : LANG === 'pt' ? 'ouvir a frase' : LANG === 'it' ? 'ascolta la frase' : 'hear the sentence')); sp.textContent = '🔊';
