#!/usr/bin/env node
/* Generic NO coordinate enumeration (READ-ONLY) — the no fan (STEP 2), any non-picture-sort type.
 * Clone of enum-da-coords.js with language='no'. Dumps published no decks for an exercise-type,
 ...
 * (locale-validity exclusion; a US holiday Norwegian teachers don't search). NOT for picture-sort (its
 * "-vs-" grouping is a separate sub-slice — use enum-no-picturesort.js).
 *
 * Emits `level` per coordinate ONLY when --level=<key> is passed (readiness types: --level=1-trinn).
 * Numeric/banded types (addition/subtraction/code-addition/math-puzzle/math-worksheet) omit level here
 * and get a per-coordinate re-grade pass (by child-seen quantity: ≤10 concrete 1-trinn / ≤20 symbolic
 * 2-trinn / 21-100 3-trinn) before gen — DO NOT default their level.
 *
 * MUST run on Hetzner (DATABASE_URL -> local production Postgres).
 * Usage: node scripts/seo-landing/enum-no-coords.js --type=grid-match --level=1-trinn > scripts/seo-landing/no-grid-match-coordinates.json
 */
'use strict';
...
(async function () {
  ...
  var rows = await c.deck.findMany({
    where: { language: 'no', status: 'published', exerciseType: TYPE },
