...
(function (global) {
  ...
  function speak(text) {
    try { if (global.LCSAudio && global.LCSAudio.speak) { global.LCSAudio.speak({ type: 'word', text: text, lang: (LANG === 'es' ? 'es-MX' : LANG), rate: 0.95 }); return; }
      if (global.speechSynthesis && global.SpeechSynthesisUtterance) { var u = new global.SpeechSynthesisUtterance(text); u.rate = 0.95; u.lang = (LANG === 'fr' ? 'fr-FR' : LANG === 'de' ? 'de-DE' : LANG === 'es' ? 'es-MX' : 'en-US'); global.speechSynthesis.cancel(); global.speechSynthesis.speak(u); } } catch (e) {}
  }
  function shuffle(arr) { var a = arr.slice(), i, j, t; for (i = a.length - 1; i > 0; i--) { j = Math.floor(Math.random() * (i + 1)); t = a[i]; a[i] = a[j]; a[j] = t; } return a; }

  function armadilloSVG() {
    /* es: «armadillo» is the MX everyday word — NOT «tatú» (S. American) / «cusuco» (C. American). */
    return '<svg class="aff-arm-svg" viewBox="0 0 100 100" role="img" aria-label="' + (LANG === 'fr' ? 'Atlas le tatou' : LANG === 'de' ? 'Atlas, das Gürteltier' : LANG === 'es' ? 'Atlas, el armadillo' : 'Atlas the armadillo') + '">' +
  ...
  global.AtlasFactFilesActivity = {
    ...
    strings: {
      ...
      title: { en: "Atlas's Fact Files", de: 'Atlas\' Fakten-Kartei', fr: 'Les fiches d’Atlas', es: 'Las fichas de Atlas' },
      instruction: { en: 'Read the facts, then tap the answer to the question.', de: 'Lies den Sachtext und tippe dann auf die Antwort zur Frage.', fr: 'Lis le texte, puis tape la réponse à la question.', es: 'Lee el texto y toca la respuesta a la pregunta.' },
      prompt: { en: 'Read the facts, then tap the answer.', de: 'Lies den Sachtext und tippe auf die Antwort.', fr: 'Lis le texte, puis tape la réponse.', es: 'Lee el texto y toca la respuesta.' },
      atlasIntro: { en: 'Read my fact, then find the answer hiding inside it!', de: 'Hallo, ich bin Atlas! Lies meinen kleinen Sachtext ganz genau – die Antwort steckt mitten im Text.', fr: 'Lis bien mon petit texte : la réponse se cache dedans !', es: 'Soy Atlas. Lee mi texto: ahí está la respuesta.' },
      hintPick: { en: 'The answer is right there in the facts — read them again.', de: 'Die Antwort steht schon im Text. Lies sie dort noch einmal nach!', fr: 'La réponse est dans le texte — relis-le bien.', es: 'La respuesta está en el texto. Léelo otra vez.' },
      ...
      hintWrong: { en: 'Look back at the facts and find the part that answers it.', de: 'Schau noch einmal in den Text. Dort steht die richtige Antwort!', fr: 'Regarde encore dans le texte : la réponse s’y trouve.', es: 'Vuelve a leer el texto y busca la parte que lo dice.' },
      win: { en: 'Yes! You found the key detail. 📁', de: 'Super! Du hast die wichtige Information gefunden. 📁', fr: 'Bravo ! Tu as trouvé l’information. 📁', es: '¡Sí! Encontraste el dato clave. 📁' }
    },
    defaults: {},
    ...
    render: function () {
      ...
      var fileb = api.el('button', 'aff-fact'); fileb.type = 'button'; fileb.setAttribute('aria-label', (LANG === 'fr' ? 'écouter le texte' : LANG === 'de' ? 'Den Sachtext anhören' : LANG === 'es' ? 'Escuchar el texto' : 'hear the facts'));
