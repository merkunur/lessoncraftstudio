...
const TYPE_MAP = {
  ...
  'find-and-count':    { op: (c) => (c.mode === 'letter-spotting' ? 'Beginning Sounds' : 'Find and Count'), qual: (c) => (c.mode === 'letter-spotting' && c.letter ? `letter ${String(c.letter).toLowerCase()}` : '') },
...
const META_FLAVOR = {
  ...
  'find-and-count':    (c) => (c.mode === 'letter-spotting' ? `hunt for pictures that begin with the letter ${String(c.letter || '').toUpperCase()}`.trim() : 'find and count the objects'),
...
module.exports = { EN_JSON, GRADE_LABEL, META_CAP, META_FLAVOR, REPO_ROOT, TAXONOMY, TITLE_CAP, TYPE_MAP, buildMeta, buildTitle, cap, gradeLabel, loadTaxonomyThemes, themeDisplay };
