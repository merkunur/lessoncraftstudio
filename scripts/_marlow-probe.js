#!/usr/bin/env node
/* =====================================================================
   _marlow-probe.js — multi-state control-bottom probe for "The Harbor Post"
   (RI.2.6, clarity-first redesign of #72). Drives an inform + entertain +
   instruct round + a wrong-tap + resolved, and MEASURES the lowest control
   INCLUDING the shell `.lcs-activity-check` across 320·360·412·768·1024·1366,
   plus tap ≥44px on the bin chips, no overlap, no overflow.
   ===================================================================== */
'use strict';
...
(async () => {
  ...
  const solve = (ok) => page.evaluate((correct) => {
    const t = window.AuthorPurposeActivity, r = t._round, C = window.AuthorPurposeCore;
    const ans = C.oracle(r), purpose = correct ? ans : C.PURPOSES.find(x => x !== ans);
    const b = Array.from(document.querySelectorAll('.ap-cand')).find(x => x.getAttribute('data-purpose') === purpose); if (b) b.click();
  }, ok).then(() => sleep(50));

  const states = {
    'inform': async () => { await force('ap-i1'); },
    'entertain': async () => { await force('ap-e1'); },
    'instruct': async () => { await force('ap-s1'); },
    'wrong': async () => { await force('ap-i2'); await solve(false); },
    'resolved': async () => { await force('ap-e2'); await solve(true); }
  };

  async function controlBottom() {
    return await page.evaluate(() => {
      const sel = '.ap-root,.ap-note,.ap-hear,.ap-say,.ap-strip,.ap-cand,.ap-line-msg,.lcs-activity-check';
  ...
  async function checks() {
    return await page.evaluate(() => {
      const taps = Array.from(document.querySelectorAll('.ap-cand'));
  ...
  if (!(await page.evaluate(() => window.AuthorPurposeActivity._resolved))) fails.push('a correct tap did NOT resolve');
  await states.wrong();
  if (await page.evaluate(() => window.AuthorPurposeActivity._resolved)) fails.push('a wrong bin resolved — must not advance');

  const order = ['inform', 'entertain', 'instruct', 'wrong', 'resolved'];
  for (const [w, h] of VIEWPORTS) {
    ...
    for (const label of order) {
      ...
      if (tt.minTap < CTRL_MIN && tt.minTap !== 999 && !subFloor) fails.push(`${label} @${w}×${h}: a bin is ${tt.minTap}px (<${CTRL_MIN}px)`);
      if (tt.overlap > 0) fails.push(`${label} @${w}×${h}: ${tt.overlap} bin pair(s) overlap`);
  ...
  console.log('');
  if (fails.length) { console.error(`MARLOW-PROBE FAILED — ${fails.length}:`); fails.forEach(f => console.error('  • ' + f)); process.exit(1); }
  console.log(`MARLOW-PROBE PASSED — inform/entertain/instruct/wrong/resolved: a correct tap resolves + a wrong bin does not advance; lowest control (incl. shell Check) clears the fold by ≥${MARGIN}px across 320→1366; bins ≥${CTRL_MIN}px (off the 320 sub-floor), no overlap, no horizontal overflow.`);
  process.exit(0);
