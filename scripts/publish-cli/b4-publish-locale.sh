#!/usr/bin/env bash
# b4-publish-locale.sh <locale> [--confirm]
#
# nt10-D Phase 7 (clone of b3-publish-locale.sh, 2026-09-21): INSERT-publish of one locale's pooled b4 wave
# (the 10 base + up to 50 variation printable decks generated on the PC into
# scripts/worksheet-gen/out/upload/wave-b4-<loc>-all/ and scp'd to
# /var/www/lcs-media/_staging/b4-<loc>/). Runs ON HETZNER. The nt20-B recipe
# (memory project_nt20b_worksheet_types.md, README b4-designs step 9), scripted:
#
...
STAGE=/var/www/lcs-media/_staging/b4-$LOC
REC=/root/staging/b4; mkdir -p "$REC"
...
node scripts/publish-cli/index.js publish-bulk "$STAGE" --dry-run --batch-id "b4-$LOC-dry" > "$REC/$LOC-dry.log" 2>&1 || true
...
# 2. real publish
node scripts/publish-cli/index.js publish-bulk "$STAGE" --confirm --batch-id "b4-$LOC" > "$REC/$LOC-confirm.log" 2>&1 || { echo "publish-bulk --confirm FAILED — $REC/$LOC-confirm.log"; tail -30 "$REC/$LOC-confirm.log"; exit 2; }
