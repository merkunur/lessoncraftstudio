/* =====================================================================
   register-shape-stretcher.js — the seven registration points for #57.
   Run:  node scripts/register-shape-stretcher.js
   IDEMPOTENT: a second run must report every point as already done.

   Cloned from register-the-gap.js (#56), which is itself the #55 clone.
   ...
   ⭐ THREE CHECKS ADDED HERE, each non-vacuous by construction:
     · the `name` of the en record must equal the TOOL'S OWN
       strings.title.en, read out of `mini tools/shape-stretcher.js` —
...
const KEY = 'shape-stretcher';
const PREV = 'the-gap';
/* ⚠ 'geometry' is not a ToolCategory. A shape-and-space instrument is
   'measurement'; the union is parsed off source at point 6 and this
   value refused if it is not a member. */
const CATEGORY = 'measurement';
const VER_FROM = '7.91', VER_TO = '7.92';
...
/* ---- 0. PRE-FLIGHT, before a single byte is written --------------- */
{
  ...
  /* 0c. ⭐ slug collision over the WHOLE shipped set, with the
     non-vacuity control: if the set parses implausibly small the check
     is meaningless and must refuse rather than pass. */
  const shipped = new Set();
  const dirs = ['messages/tool-content', 'messages/maker-content'];
  for (const d of dirs) {
    ...
    for (const f of fs.readdirSync(abs).filter((x) => x.endsWith('.json'))) {
      ...
      for (const k of Object.keys(j)) {
        const e = j[k];
        if (e && typeof e === 'object' && typeof e.slug === 'string' && e.slug) shipped.add(e.slug);
      }
    }
  }
  if (shipped.size < 500) die(`shipped slug set parsed implausibly small (${shipped.size}) — refusing to certify no collision`);
  const clash = [];
  const mine = new Set();
  for (const loc of LOCALES) {
    const e = L[loc];
    ...
    /* ⚠ a slug already ours from an earlier run is not a collision — but
       the test is EQUALITY with what we previously wrote, not merely
       "something is there": a run that changed the slug would otherwise
       be waved through against a stale entry. */
    const ours = (JSON.parse(rd(`messages/tool-content/${loc}.json`))[KEY] || {}).slug;
    if (shipped.has(e.slug) && ours !== e.slug) clash.push(`${loc}:${e.slug}`);
    if (mine.has(e.slug)) clash.push(`${loc}:duplicate-within-this-tool:${e.slug}`);
    mine.add(e.slug);
  }
  if (clash.length) die('slug collision — ' + clash.join(' '));
  console.log(`  · pre-flight: 11 slugs url-safe and distinct against ${shipped.size} shipped slugs`);
