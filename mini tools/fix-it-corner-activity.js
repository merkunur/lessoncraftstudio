...
(function (global) {
  ...
  function fixitSVG() {
    return '<svg class="fic-mole-svg" viewBox="0 0 100 100" role="img" aria-label="' + (LANG === 'de' ? 'Fixit, der Reparatur-Maulwurf' : LANG === 'fr' ? 'Fixit, la taupe réparatrice' : LANG === 'es' ? 'Fixit, el topo reparador' : LANG === 'pt' ? 'Fixit, a toupeira reparadora' : LANG === 'it' ? 'Fixit, la talpa aggiustatutto' : 'Fixit the repair-mole') + '">' +
  ...
  global.FixItCornerActivity = {
    ...
    strings: {
      title: { en: "Fixit's Fix-It Corner", de: 'Fixits Reparatur-Ecke', fr: 'Le coin réparation de Fixit', es: 'El rincón de arreglos de Fixit', pt: 'O cantinho de conserto do Fixit', it: 'Il banco delle riparazioni di Fixit' },
      instruction: { en: 'Fixit can\'t read the tiny number — you\'re his sharp-eyed helper. Work out the part each gadget needs, pop it in, then tap Check.', de: 'Fixit kann die winzige Zahl nicht lesen – du bist sein scharfäugiger Helfer. Finde heraus, welchen Teil jedes Gerät braucht, setz ihn ein und tippe dann auf Prüfen.', fr: 'Fixit n’arrive pas à lire le tout petit nombre — c’est toi son aide aux yeux perçants. Trouve la pièce qu’il manque à chaque appareil, place-la, puis touche Vérifier.', es: 'Fixit no alcanza a ver el numerito — tú eres su ayudante de buen ojo. Descubre qué parte le falta a cada aparato, colócala y toca Comprobar.', pt: 'O Fixit não consegue enxergar o numerozinho — você é o ajudante de olho vivo dele. Descubra qual parte falta em cada aparelho, coloque no lugar e toque em Verificar.', it: 'Fixit non riesce a leggere il numero piccolino — tu sei il suo aiutante dagli occhi acuti. Scopri il pezzo che serve a ogni congegno, mettilo dentro e poi tocca Controlla.' },
      promptMake10: { en: 'Fill it up to exactly 10!', de: 'Füll es genau bis 10 auf!', fr: 'Remplis-le jusqu’à exactement 10 !', es: '¡Llénalo justo hasta 10!', pt: 'Complete até dar 10 certinho!', it: 'Riempilo fino a esattamente 10!' },
      promptMissingSum: { en: '{a} and {b} — how many in all?', de: '{a} und {b} – wie viele sind das zusammen?', fr: '{a} et {b} — combien ça fait en tout ?', es: '{a} y {b}, ¿cuántos son juntos?', pt: '{a} e {b}, quantos são no total?', it: '{a} e {b} — quanti in tutto?' },
      promptMissingAddend: { en: '{a} and how many more make {c}?', de: '{a} und wie viele mehr ergeben {c}?', fr: '{a} et combien de plus pour faire {c} ?', es: '{a} y ¿cuántos más hacen {c}?', pt: '{a} e mais quantos dão {c}?', it: '{a} e quanti ancora per fare {c}?' },
      promptMissingSubtrahend: { en: '{a} take away how many leaves {b}?', de: 'Von {a} wie viele weg, dann bleibt {b}?', fr: '{a}, on en enlève combien pour qu’il reste {b} ?', es: 'De {a}, ¿cuántos quitas para que queden {b}?', pt: 'De {a}, tirando quantos sobram {b}?', it: 'Parti da {a}: quanti ne togli per arrivare a {b}?' },
      promptMissingFirstAddend: { en: 'What plus {b} makes {c}?', de: 'Was plus {b} ergibt {c}?', fr: 'Combien plus {b} font {c} ?', es: '¿Qué número más {b} es {c}?', pt: 'Que número mais {b} dá {c}?', it: 'Quanto più {b} fa {c}?' },
      promptMissingMinuend: { en: 'Take away {b}, leaves {c} — start with what?', de: 'Nimm {b} weg, dann bleibt {c} – womit hast du angefangen?', fr: 'Enlève {b}, il reste {c} — on part de combien ?', es: 'Quitas {b}, quedan {c}: ¿con cuántos empezaste?', pt: 'Tirando {b}, sobram {c} — começou com quantos?', it: 'Togli {b}, rimane {c} — parti da quanto?' },
      promptBalance: { en: 'Make both sides match.', de: 'Mach beide Seiten gleich.', fr: 'Rends les deux côtés égaux.', es: 'Equilibra los dos lados.', pt: 'Deixe os dois lados iguais.', it: 'Fai combaciare i due lati.' },
      promptProduce: { en: 'Tap a number to hide — make the next puzzle!', de: 'Tippe eine Zahl an, um sie zu verstecken – mach das nächste Rätsel!', fr: 'Touche un nombre pour le cacher — fabrique la prochaine énigme !', es: 'Toca un número para esconderlo y arma el siguiente reto.', pt: 'Toque num número para esconder e monte o próximo desafio!', it: 'Tocca un numero da nascondere — crea il prossimo gioco!' },
      hop: { en: 'Hop', de: 'Hüpf', fr: 'Saute', es: 'Brinca', pt: 'Pula', it: 'Salta' },
      slide: { en: 'Slide together', de: 'Zusammenschieben', fr: 'Rassemble', es: 'Junta', pt: 'Junta', it: 'Uniscili' },
      whole: { en: 'the whole', de: 'das Ganze', fr: 'le tout', es: 'el todo', pt: 'o todo', it: 'il tutto' },
      winMake10: { en: 'Ten! It whirs to life — it can butter ten slices of toast at once!', de: 'Zehn! Es surrt los – es kann zehn Scheiben Toast auf einmal buttern!', fr: 'Dix ! L’appareil s’anime en vrombissant — il peut beurrer dix tartines à la fois !', es: '¡Diez! Arranca zumbando: ¡puede untar diez panes tostados a la vez!', pt: 'Dez! Ele ganha vida zumbindo: consegue passar manteiga em dez torradas de uma vez!', it: 'Dieci! Il congegno si accende ronzando — riesce a imburrare dieci fette di pane tutte insieme!' },
      winSum: { en: 'That\'s the whole! POP — it clicks awake!', de: 'Das ist das Ganze! KLACK – es springt an!', fr: 'C’est le tout ! CLAC — il se réveille !', es: '¡Ese es el todo! CLIC, ¡y arranca!', pt: 'Esse é o todo! PLOC — ele desperta com um clique!', it: 'Ecco il tutto! POP — si sveglia con uno scatto!' },
      winAddend: { en: '{x} more! It catches, clicks, and whirs to life!', de: '{x} mehr! Es greift, klickt und surrt los!', fr: '{x} de plus ! Il accroche, cliquette et s’anime !', es: '¡{x} más! ¡Agarra, hace clic y cobra vida zumbando!', pt: 'Mais {x}! Ele encaixa, dá um clique e ganha vida zumbindo!', it: '{x} ancora! Si aggancia, scatta e si accende ronzando!' },
      winSubtrahend: { en: '{x}! It catches, clicks, and whirs to life!', de: '{x}! Es greift, klickt und surrt los!', fr: '{x} ! Il accroche, cliquette et s’anime !', es: '¡{x}! ¡Agarra, hace clic y cobra vida zumbando!', pt: '{x}! Ele encaixa, dá um clique e ganha vida zumbindo!', it: '{x}! Si aggancia, scatta e si accende ronzando!' },
      ...
      winMinuend: { en: 'You started with {x}! Slid back together — it hums awake!', de: 'Du hast mit {x} angefangen! Wieder zusammengeschoben – es brummt los!', fr: 'Tu es parti de {x} ! Rassemblé — il se réveille en ronronnant !', es: '¡Empezaste con {x}! Se volvieron a juntar, ¡y arranca traqueteando!', pt: 'Você começou com {x}! Tudo se juntou de novo — e ele acorda ronronando!', it: 'Sei partito da {x}! Rimesso insieme — si sveglia ronzando!' },
      winBalance: { en: 'Level! Both sides match — it balances and dings!', de: 'Im Gleichgewicht! Beide Seiten sind gleich – es pendelt sich ein und klingelt!', fr: 'À l’équilibre ! Les deux côtés sont égaux — il se stabilise et sonne !', es: '¡En equilibrio! Los dos lados iguales: se acomoda y suena una campanita.', pt: 'Equilibrado! Os dois lados iguais — ele se ajeita e dá um din!', it: 'In pari! I due lati combaciano — si bilancia e fa din!' },
      winProduce: { en: 'A new puzzle! Fixit fixes it right up.', de: 'Ein neues Rätsel! Fixit repariert es im Nu.', fr: 'Une nouvelle énigme ! Fixit la répare en un clin d’œil.', es: '¡Un reto nuevo! Fixit lo arregla en un dos por tres.', pt: 'Um desafio novo! O Fixit conserta rapidinho.', it: 'Un nuovo gioco! Fixit lo aggiusta in un lampo.' },
      missOver: { en: 'Whoa — it over-revs! That\'s too many. {a} is a part of {c}, not all of it.', de: 'Hoppla – es dreht über! Das sind zu viele. {a} ist ein Teil von {c}, nicht das Ganze.', fr: 'Oh — il s’emballe ! C’est trop. {a}, c’est une partie de {c}, pas le tout.', es: '¡Uy, se pasa! Son demasiados. {a} es una parte de {c}, no el todo.', pt: 'Opa — ele acelera demais! São números demais. {a} é uma parte de {c}, não o todo.', it: 'Ehi — gira troppo forte! Sono troppi. {a} è una parte di {c}, non il tutto.' },
      missCopy: { en: 'Clunk — it stalls. That\'s the whole, not the missing part.', de: 'Klonk – es stockt. Das ist das Ganze, nicht der fehlende Teil.', fr: 'Clonk — il cale. C’est le tout, pas la partie qui manque.', es: 'Clonc, se atora. Ese es el todo, no la parte que falta.', pt: 'Cloc — ele trava. Esse é o todo, não a parte que falta.', it: 'Clanc — si blocca. Quello è il tutto, non il numero che manca.' },
      missOne: { en: 'Half a turn… so close! Count once more.', de: 'Eine halbe Umdrehung … fast! Zähl noch einmal.', fr: 'Un demi-tour… tout près ! Recompte une fois.', es: 'Media vuelta… ¡casi! Cuenta una vez más.', pt: 'Meia volta… quase! Conte mais uma vez.', it: 'Mezzo giro… ci sei quasi! Conta ancora una volta.' },
      missGeneric: { en: 'Not yet — let\'s check it together.', de: 'Noch nicht – lass es uns zusammen prüfen.', fr: 'Pas encore — vérifions ensemble.', es: 'Todavía no… vamos a revisarlo juntos.', pt: 'Ainda não… vamos conferir juntos.', it: 'Non ancora — controlliamo insieme.' },
      model: { en: '{a} plus {x} is {c}… or {c} take away {a} is {x}.', de: '{a} plus {x} ist {c} … oder {c} minus {a} ist {x}.', fr: '{a} plus {x} font {c}… ou {c} moins {a} font {x}.', es: '{a} más {x} es igual a {c}… o {c} menos {a} es igual a {x}.', pt: '{a} mais {x} é igual a {c}… ou {c} menos {a} é igual a {x}.', it: '{a} più {x} fa {c}… oppure {c} togli {a} fa {x}.' },
      escalate: { en: 'Fixit slides over the counting-rail — let\'s build it together!', de: 'Fixit schiebt die Zählschiene herüber – lass es uns zusammen bauen!', fr: 'Fixit fait glisser la règle à compter — construisons-le ensemble !', es: 'Fixit se desliza por el riel para contar: ¡vamos a armarlo juntos!', pt: 'O Fixit desliza até o trilho de contar — vamos montar juntos!', it: 'Fixit fa scivolare il binario dei numeri — costruiamolo insieme!' },
