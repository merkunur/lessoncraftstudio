#!/usr/bin/env node
/* Generic SV coordinate enumeration (READ-ONLY) — the sv fan (STEP 2), any non-picture-sort type.
 * Clone of enum-coords-de.js with language='sv'. Dumps published sv decks for an exercise-type,
 ...
 * (locale-validity exclusion, mirrors de). NOT for picture-sort (its "-vs-" grouping is a separate sub-slice).
 *
 * Emits `level` per coordinate ONLY when --level=<key> is passed (readiness types: --level=forskola).
 * Numeric/banded types (addition/subtraction/code-addition/math-puzzle) omit level here and get a
 * per-coordinate re-grade pass (by child-seen quantity) before gen — DO NOT default their level.
 *
 * MUST run on Hetzner (DATABASE_URL -> local production Postgres).
 * Usage: node scripts/seo-landing/enum-sv-coords.js --type=grid-match --level=forskola > scripts/seo-landing/sv-grid-match-coordinates.json
 */
'use strict';
...
(async function () {
  ...
  var rows = await c.deck.findMany({
    where: { language: 'sv', status: 'published', exerciseType: TYPE },
