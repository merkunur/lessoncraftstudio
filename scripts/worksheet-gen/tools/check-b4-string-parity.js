#!/usr/bin/env node
/**
 * check-b4-string-parity.js [--fix-list]
 *
 * WHY THIS EXISTS. A b4 family keeps its per-face title + instruction in TWO places:
 *   · `data/b4/locales/<family>.<loc>.json` -> `strings.<mode>.{title,instruction}`
 *     — what every `qa/verify-b4-<family>.js` reads and asserts on, and
 *   · `i18n/strings.<loc>.json` -> `<ID>.{title,instruction}`
 *     — what the generator actually PRINTS on the page and stamps into the manifest.
 *
 * `tools/apply-b4-locale.js` writes both from one draft, so they agree at apply time. A later
...
const BANK_DIR = path.join(ROOT, 'data/b4/locales');
