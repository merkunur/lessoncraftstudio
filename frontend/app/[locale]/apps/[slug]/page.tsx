...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  // SEO FIX: Removed ~5,900 lines of legacy hardcoded metadata
  // All product page SEO is now handled by the dynamic content registry above (getContentBySlug)
  // Legacy URLs without content files will use the fallback below with index: false
  // This eliminates duplicate canonical issues from hardcoded metadata blocks
  ...
  return {
    ...
    robots: {
      index: false,  // Don't index fallback URLs - only content registry pages should be indexed
      follow: true,
    },
    alternates: {
...
// SEO FIX: Removed page-level redirects - they are now handled at config level
// in next.config.js via redirects.js (generateProductPageRedirects)
// Page-level redirects create redirect chains that confuse Google and cause
