...
(function (global) {
  ...
  global.FoxForgeActivity = {
    ...
    strings: {
      title: { en: "Pip's Chocolate Forge", de: 'Pips Schokoladen-Werkstatt', fr: 'La fabrique de chocolat de Pip', es: 'El taller de Pip', pt: 'A Fábrica de Chocolate do Pip' },
      instruction: { en: '', de: '', fr: '', es: '', pt: '' },
      prompt: { en: 'Forge the order.', de: 'Gieße die Bestellung.', fr: 'Fabrique la commande.', es: '¡Vacía el pedido!', pt: 'Prepare o pedido.' },
      handOver: { en: 'Hand it over 🍫', de: 'Abgeben 🍫', fr: 'Livrer 🍫', es: 'Entregar 🍫', pt: 'Entregar 🍫' },
      tapToForge: { en: 'tap the bar to forge a piece', de: 'tippe auf die Tafel, um ein Stück zu gießen', fr: 'touche la barre pour fabriquer un morceau', es: 'Toca la barra para vaciar una pieza.', pt: 'toque na barra para moldar uma peça' },
      sayWelcome: { en: 'A new order! Pick the right piece by its SIZE.', de: 'Eine neue Bestellung! Wähle das richtige Stück nach seiner GRÖSSE.', fr: 'Une nouvelle commande ! Choisis le bon morceau d’après sa TAILLE.', es: '¡Un pedido nuevo! Elige la pieza correcta por su TAMAÑO.', pt: 'Um novo pedido! Escolha a peça certa pelo TAMANHO.' },
      sayWin: { en: 'Perfect order — thank you! 🍫', de: 'Perfekte Bestellung – danke! 🍫', fr: 'Commande parfaite — merci ! 🍫', es: '¡Pedido perfecto, gracias! 🍫', pt: 'Pedido perfeito — obrigado! 🍫' },
      sayPickMold: { en: 'Pick a piece-mold first.', de: 'Wähle zuerst eine Form.', fr: 'Choisis d’abord un moule.', es: 'Primero elige un molde.', pt: 'Primeiro escolha uma forma.' },
      sayUnequal: { en: "Those aren't equal — pick the fair mold!", de: 'Die sind nicht gleich groß – wähle die faire Form!', fr: 'Ces morceaux ne sont pas égaux — choisis le moule juste !', es: 'Esas no están iguales… ¡elige el molde parejo!', pt: 'Esses pedaços não são iguais — escolha a forma que faz partes iguais!' },
      sayShort: { en: 'Not enough yet — forge more pieces.', de: 'Noch nicht genug – gieße mehr Stücke.', fr: 'Pas encore assez — fabrique plus de morceaux.', es: 'Todavía faltan… vacía más piezas.', pt: 'Ainda falta — molde mais peças.' },
      sayOver: { en: "That's too much — tap a piece to take it back.", de: 'Das ist zu viel – tippe auf ein Stück, um es zurückzunehmen.', fr: 'C’est trop — touche un morceau pour le reprendre.', es: 'Es demasiado… toca una pieza para quitarla.', pt: 'Passou! Toque numa peça para tirar.' },
      sayWrongSize: { en: 'Hmm — those are the wrong size. Try another mold.', de: 'Hmm – das ist die falsche Größe. Probier eine andere Form.', fr: 'Hmm — ces morceaux ne sont pas de la bonne taille. Essaie un autre moule.', es: 'Mmm… ese tamaño no es. Prueba con otro molde.', pt: 'Hmm — o tamanho não é esse. Tente outra forma.' },
      sayAgain: { en: "Let's look again.", de: 'Schauen wir noch mal.', fr: 'Regardons encore une fois.', es: 'Vamos a mirar otra vez.', pt: 'Vamos tentar de novo!' },
      sayCross: { en: 'Yes — the same share, on a different shape!', de: 'Ja – der gleiche Anteil, nur in einer anderen Form!', fr: 'Oui — le même partage, sur une autre forme !', es: '¡Sí! La misma parte, ¡solo que con otra forma!', pt: 'Isso! A mesma parte, num formato diferente!' },
      crossYes: { en: 'Yes — same share! 🍕', de: 'Ja – gleicher Anteil! 🍕', fr: 'Oui — le même partage ! 🍕', es: '¡Sí, la misma parte! 🍕', pt: 'Isso — a mesma parte! 🍕' },
      hintCheck: { en: 'Pick the piece that fits the bar, forge it the right number of times, then hand it over.', de: 'Wähle das Stück, das zur Tafel passt, gieße es so oft wie nötig und gib sie dann ab.', fr: 'Choisis le morceau qui va avec la barre, fabrique-le le bon nombre de fois, puis livre la commande.', es: 'Elige la pieza que cabe en la barra, vacíala las veces que necesites y entrégala.', pt: 'Toque em Entregar quando o pedido estiver pronto.' }
    },
    defaults: {},
    ...
    _orderSvg: function (opts) {
      ...
      var sv = svg('svg', { viewBox: '0 0 ' + VW + ' ' + VH, class: 'ff-svg' + (twoBar ? ' ff-svg2' : ''), 'aria-label': (LANG === 'es' ? 'pedido de chocolate' : LANG === 'de' ? 'Schokoladen-Bestellung' : LANG === 'fr' ? 'commande de chocolat' : LANG === 'pt' ? 'barra de chocolate do pedido' : 'chocolate order bar') });
      ...
      for (var k = 0; k < n; k++) {
        ...
        if (opts.interactive) {
          g.setAttribute('role', 'button'); g.setAttribute('tabindex', '0'); g.setAttribute('aria-label', (LANG === 'es' ? 'toca para quitar esta pieza' : LANG === 'de' ? 'dieses Stück zurücknehmen' : LANG === 'fr' ? 'reprendre ce morceau' : LANG === 'pt' ? 'tirar esta peça' : 'take this piece back')); g.style.cursor = 'pointer';
    ...
    _moldButton: function (moldB, idx) {
      ...
      btn.setAttribute('aria-label', LANG === 'es' ? (unequal ? 'un molde disparejo' : 'un molde de piezas') : LANG === 'de' ? (unequal ? 'eine ungleiche Form' : 'eine Stück-Form') : LANG === 'fr' ? (unequal ? 'un moule inégal' : 'un moule à morceau') : LANG === 'pt' ? (unequal ? 'uma forma desigual' : 'uma forma de peça') : (unequal ? 'an uneven mold' : 'a piece mold'));
      var MW = 64, MH = 18;
    ...
    _renderBuild: function (root) {
      ...
      var wall = api.el('div', 'ff-moldwall'); wall.setAttribute('role', 'group'); wall.setAttribute('aria-label', (LANG === 'es' ? 'moldes de piezas: elige uno por su tamaño' : LANG === 'de' ? 'Stück-Formen – wähle eine nach ihrer Größe' : LANG === 'fr' ? 'moules à morceaux — choisis-en un d’après sa taille' : LANG === 'pt' ? 'formas de peça — escolha uma pelo tamanho' : 'piece molds — pick one by its size'));
    ...
    _renderCrossShape: function (root) {
      ...
      var sv = svg('svg', { viewBox: '0 0 ' + (W + 4) + ' ' + (BH + 4), class: 'ff-svg ff-svgbar', 'aria-label': (LANG === 'es' ? 'una barra con la mitad sombreada' : LANG === 'de' ? 'eine Tafel, bei der eine Hälfte gefärbt ist' : LANG === 'fr' ? 'une barre dont une moitié est colorée' : LANG === 'pt' ? 'uma barra com um meio pintado' : 'a bar with one half shaded') });
    ...
    _renderDone: function (root) {
      ...
      if (r && s && (s.isBuild || s.isNonscored)) {
        ...
        var sv = svg('svg', { viewBox: '0 0 ' + (W + 4) + ' ' + (BH + 4), class: 'ff-svg ff-donesvg', 'aria-label': (LANG === 'es' ? ('la barra de ' + fracWord(r.a, r.b) + ' terminada') : LANG === 'de' ? ('die fertige ' + fracWord(r.a, r.b) + '-Tafel') : LANG === 'fr' ? ('la barre ' + fracWord(r.a, r.b) + ' terminée') : LANG === 'pt' ? ('a barra de ' + fracWord(r.a, r.b) + ' pronta') : ('the finished ' + fracWord(r.a, r.b) + ' bar')) });
        sv.style.aspectRatio = ((W + 4) / (BH + 4)).toFixed(2);
        sv.appendChild(svg('rect', { x: 0, y: 2, width: W, height: BH, rx: 1.8, fill: '#fff', stroke: C.BAR, 'stroke-width': 0.9 }));
        var unit = W / r.b;
        for (var i = 0; i < r.a; i++) {
