#!/usr/bin/env node
/**
 * validate-b4-draft.js <locale> [path] [--no-probe] — the refuse-don't-guess gate for
 * a nt10-D native-panel draft (default path i18n/.draft-b4-<locale>.json).
 *
 ...
 *
 * Poison-tested by tools/validate-b4-draft.test.js (an EN-derived draft PASSES; one
...
/** Writes the draft's blocks to a temp locales dir and runs tools/b4-probe-child.js there. */
...
if (require.main === module) {
  ...
  const p = process.argv[3] && !process.argv[3].startsWith('--') ? process.argv[3] : path.join(WG, 'i18n', `.draft-b4-${loc}.json`);
  if (!loc) { console.error('usage: validate-b4-draft.js <locale> [path] [--no-probe]'); process.exit(2); }
