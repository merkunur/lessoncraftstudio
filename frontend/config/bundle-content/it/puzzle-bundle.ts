...
const content: BundleContent = {
  ...
  appsIncluded: [
    {
      ...
      description: 'Crea puzzle a griglia con pezzi mancanti dove un\'immagine tematica viene divisa in segmenti e alcuni pezzi vengono rimossi. gli utenti esaminano l\'immagine di riferimento completa e identificano quali pezzi appartengono agli spazi vuoti — un esercizio di ragionamento spaziale che sviluppa capacità di osservazione, memoria visiva e comprensione parte-tutto. Configura le dimensioni della griglia da griglie piccole per principianti a griglie più grandi per utenti avanzati, controllando quanti pezzi vengono rimossi e quanto diventa impegnativo il compito di identificazione. Il generatore gestisce automaticamente la suddivisione dell\'immagine, la rimozione dei pezzi e la creazione della chiave di risposta — tu scegli il tema e la difficoltà, e lo strumento produce una pagina puzzle pronta per la stampa con la soluzione corrispondente. I puzzle con pezzi mancanti sono una categoria di prodotti consolidata su Etsy e KDP perché attraggono una vasta fascia d\'età: i bambini della scuola dell\'infanzia si divertono con semplici griglie 2x2 mentre gli utenti più grandi affrontano configurazioni complesse che richiedono un\'attenta analisi spaziale. Ogni scheda è puramente visiva — nessun testo sulla pagina — rendendo ogni puzzle vendibile in tutto il mondo senza traduzione. Scegli tra oltre 100 set di immagini tematiche o carica le tue immagini per prodotti puzzle specifici per la tua nicchia.',
    },
    {
      title: 'Generatore di Schede Trova l\'Intruso',
      description: 'Genera schede di discriminazione visiva dove ogni riga mostra un gruppo di immagini simili con un elemento che differisce dagli altri. gli utenti identificano l\'elemento diverso in ogni riga — un esercizio che rafforza l\'attenzione ai dettagli, la scansione visiva e il pensiero categoriale. Il generatore supporta la selezione della modalità per ogni riga, permettendoti di mescolare diversi tipi di criteri per l\'elemento diverso all\'interno di una singola scheda: una riga potrebbe presentare una specie animale diversa, la successiva una variante di colore diversa e un\'altra un orientamento diverso. Questa varietà all\'interno di una singola pagina mantiene l\'attività coinvolgente e impedisce agli utenti di cadere in scorciatoie di abbinamento dei pattern. Configura il numero di righe per pagina e il numero di elementi per riga per scalare la difficoltà per diverse fasce d\'età. Ogni scheda genera automaticamente una chiave di risposta corrispondente, evidenziando l\'elemento intruso corretto in ogni riga. Trova l\'Intruso è uno dei formati di attività per la prima infanzia più riconoscibili — genitori e venditori lo cercano per nome, rendendolo una categoria di prodotti ad alto traffico. Ogni scheda è puramente visiva senza testo sulla pagina, quindi ogni puzzle funziona in qualsiasi lingua e qualsiasi mercato nel mondo.',
    },
    {
    ...
    },
    {
      ...
      description: 'Crea schede di labirinti e ricerca del percorso in tre modalità distinte. La modalità Percorso genera un tragitto visivo attraverso una griglia di immagini — gli utenti tracciano il percorso corretto seguendo una sequenza specifica di immagini, combinando scansione visiva con pensiero sequenziale. La modalità Labirinto negozico produce strutture di labirinto tradizionali con decorazioni di immagini tematiche all\'inizio e alla fine, offrendoti il formato labirinto familiare che genitori e venditori cercano con un appeal visivo aggiunto. La modalità Scegli il Percorso presenta un albero decisionale ramificato dove gli utenti selezionano il percorso corretto a ogni bivio basandosi su indizi visivi — un formato unico che combina la risoluzione di labirinti con il processo decisionale logico. La struttura a tre modalità ti permette di creare prodotti di labirinti diversificati da un singolo generatore: puzzle di percorso per utenti più piccoli, labirinti negozici per il pubblico più ampio e sfide a percorso ramificato per utenti pronti al problem solving a più passaggi. Configura la dimensione della griglia, la complessità del percorso e il numero di punti decisionali per controllare la difficoltà. Ogni modalità genera automaticamente una chiave di risposta che mostra il percorso corretto evidenziato sul labirinto. Tutte e tre le modalità producono schede puramente visive senza testo sulla pagina, quindi ogni labirinto funziona in qualsiasi lingua nel mondo. Le schede di labirinti sono una categoria bestseller perenne — non richiedono istruzioni oltre a "trova il percorso" e attraggono bambini, genitori ed imprenditori allo stesso modo.',
    },
  ],
  ...
  faq: [
    ...
    },
    {
      ...
      answer: 'Il Sudoku con Immagini usa una griglia 4x4 con immagini tematiche al posto della griglia standard 9x9 con numeri. gli utenti posizionano quattro immagini diverse in modo che ogni riga e ogni colonna contenga ogni immagine esattamente una volta. Il formato 4x4 è progettato specificamente per bambini piccoli che potrebbero non essere ancora a loro agio con i numeri — applicano le stesse competenze di deduzione logica usando immagini familiari come animali, cibo o veicoli. La griglia più piccola mantiene la sfida gestibile pur richiedendo un genuino ragionamento logico.',
    },
    {
      question: 'Quali sono le tre modalità di Percorso di Immagini?',
      answer: 'Percorso di Immagini offre tre modalità di labirinto distinte. La modalità Percorso crea un tragitto attraverso una griglia di immagini dove gli utenti tracciano la sequenza corretta. La modalità Labirinto negozico genera strutture di labirinto tradizionali con decorazioni di immagini tematiche all\'inizio e alla fine. La modalità Scegli il Percorso presenta alberi decisionali ramificati dove gli utenti selezionano il percorso corretto a ogni bivio. Tutte e tre le modalità producono chiavi di risposta che mostrano il percorso corretto evidenziato sul labirinto.',
    },
    {
      question: 'Tutti e quattro i generatori includono chiavi di risposta?',
      answer: 'Sì. Tutti e quattro i generatori producono chiavi di risposta automatiche con ogni scheda — questo è l\'unico pacchetto con copertura del 100% delle chiavi di risposta. Pezzi Mancanti mostra i posizionamenti corretti, Trova l\'Intruso evidenzia l\'elemento intruso in ogni riga, Sudoku mostra la griglia completata e Percorso di Immagini traccia il percorso corretto. Le chiavi di risposta sono un\'aspettativa fondamentale degli acquirenti su Etsy e Gumroad, e non dovrai mai risolvere i puzzle manualmente.',
    },
    {
      question: 'In quali formati posso scaricare?',
      answer: 'Ogni generatore produce file PDF pronti per la stampa e JPEG ad alta risoluzione. Scegli tra Letter Verticale, Letter Orizzontale, A4 Verticale, A4 Orizzontale, Quadrato o inserisci una dimensione personalizzata. Il PDF è ideale per pacchetti puzzle multi-pagina su Etsy e Gumroad. Il JPEG funziona bene per le pagine interne KDP e le anteprime sui social media. Tutti i file vengono scaricati a risoluzione di stampa — nessun ridimensionamento o post-elaborazione necessaria.',
    },
    {
  ...
  visuals: {
    ...
    sampleGallery: [
      { src: '/samples/italian/missing%20pieces/pezzi-mancanti-1.webp', alt: 'Puzzle pezzi mancanti con segmenti di griglia di immagini rimossi', caption: 'Puzzle Pezzi Mancanti' },
      { src: '/samples/italian/odd%20one%20out/trova-il-diverso-1-v10.webp', alt: 'Scheda trova l\'intruso con righe di immagini simili e una diversa', caption: 'Scheda Trova l\'Intruso' },
      ...
      { src: '/samples/italian/picture%20path/percorso-di-immagini-1.webp', alt: 'Labirinto percorso di immagini con tragitto tematico attraverso la griglia', caption: 'Labirinto Percorso di Immagini' },
    ],
