#!/usr/bin/env node
/* NO picture-sort "-vs-" PAIR enumeration (READ-ONLY). subjectTags[0]="X-vs-Y" combined key.
 * Group by pairKey; canonical = earliest publishedAt; siblings = rest. Parse left/right themes.
 * MUST run on Hetzner (DB). Usage: node scripts/seo-landing/enum-no-picturesort.js > scripts/seo-landing/no-picture-sort-coordinates.json
 */
'use strict';
...
(async function () {
  ...
  var rows = await c.deck.findMany({
    where: { language: 'no', status: 'published', exerciseType: 'picture-sort' },
  ...
  console.log(JSON.stringify({
    locale: 'no', exerciseType: 'picture-sort', rawRowCount: rows.length, pairCount: coords.length,
