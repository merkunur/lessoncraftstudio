...
- **PT ss/rr split** (commit `fed6838b`): `pt.js` — `ss`/`rr` are dígrafos que se separam (boundary BETWEEN the letters: `gi-ras-sol`, `pas-sa`, `os-so`), the one gate-viable fix of three native-reviewer disputes per §A.13.57 (TeX + vocab-phonics already agreed; count-preserving). Re-gated PT clean.
- **PT-hiatus + FI-seam settled counts** (commit `7393a989`): 8 settled `vocabulary-phonics.json` `syl` count corrections (PT tonic-i/u hiatus: `fri-o`/`ri-o`/`na-vi-o`/`pi-a`/`lu-a`/`me-lan-ci-a`/`fo-to-gra-fi-a`; FI compound-seam `lu-mi-uk-ko`) + the matching `pt.js` tonic-i/u-hiatus guard (`HAS_ACCENT` + `HIATUS_GUARD_SKIP`) and `fi.js` `COMPOUND_VOWEL_SEAMS` guard. These re-enabled the words the río/iã+o backlog had parked (see Backlog below).
- **policial.pt 3→4** (sv-carve-out commit `4d5ef88d`): `po-li-ci-al`; the shipped tonic-i/u guard already produced 4, only the vocab-phonics count was wrong → pt pool 890→891. (§10.3 single-word edit.)
...
**[STATUS 2026-06 — E14 fractions #2 fan-out CLOSED 11/11, closing commit `c2bb562c`.]** Both E14 recognition fraction activities are fully LIVE: #1 "Equal Halves and Fourths" (1.G.A.3) + #2 "Same Size, Different Shape" (2.G.A.3 non-congruence), EN + 10-locale fan-out, on the **E2 choice-board engine** (anchor-and-match + FRACTION_FIGURES; **0 protected-core lines git-definitive end-to-end**; `choice-board-activity.js?v=62`/`8.63`; full 11-locale + x-default hreflang on every #2 page, PT pt-BR on both head + body strip, no orphan). Per-locale construction validated by native ensembles (FI partitive · DE auch · FR aussi · ES también · IT anche+un'altra · PT Toque-em · NL V2/V-final+Controleer · SV BIFF+§20.10-no-recur · DA der+single-def · NO som+double-def) — doctrine §A.13.59; record [[project-e14-fractions-series]]. Also live this period: the **3,065-deck Danish (`da`) catalog wave** (`publish-wave.js`; [[project-danish-deck-wave]]). **NEXT: the worksheet-hub initiative** (operator-flagged plan-mode investigation) — do NOT auto-start; await the prompt. Standing backlog: E14 **active-partition #3** (tap-to-partition; needs `fractions-core.js`) + the deferred-by-decision residuals below. (Prior status lines are historical.)

**[STATUS 2026-06-03]** **E2 "Sort and Count" (K.MD.B.3) — EN build + 11-locale fan-out COMPLETE 11/11** (platform's 2nd Measurement & Data activity; classify-then-count over a mixed pile; EN `c6db46f5` → final NO `60927c7a`; wrapper `choice-board-activity.js?v=25`, `ACTIVITY_WRAPPER_VERSION='7.86'`). Fan-out used the **3-agent native ensemble per locale** (linguist + K-3 educator + B2C marketing), operator-mandated after a single-agent attempt was rejected — it caught a real **Finnish case-agreement bug** the single agent missed (the `{strand}` token can't inflect; see §A.13.56), plus an es register-bleed and fr/sv/da/no `labelObjects` + fr/es intro-length refinements. Fan-out-end cumulative sweep **clean**: the 3 cores + lcs-shell.* + Direction A CSS **byte-identical** across all arc commits (git-proven); hreflang chain = **12** (pt→pt-BR + x-default); all-locale mobile audit 14/14; sort-count passes the all-locale page audit in every locale. The 91 `noStrandLeak` page-audit fails are the documented pre-existing staleness on 13 OTHER activities × 7 case-heavy locales — NOT sort-count, NOT compare-length, NOT caused by the `EN_STRAND_NAMES += 'Measurement & Data'` fold. **Awaiting operator sign-off at 11/11.**

**(historical)** Before Sort and Count: **E2 "Comparing Length" (K.MD.A.2) 11/11** (EN `a5e37bdf` → NO `41232bdb`; v=13/7.73; §A.13.54). **E12 A3 "Expanded Form" (2.NBT.A.3) 11/11** (`97ddaad1`; `place-value-activity.js?v=36`, core `?v=25`) — E12 ships 3 activities × 11 = 33. **E4 match-pairs** K.OA.A.3 + 1.OA.D.7 + 2.NBT.A.3 all **11/11** (`b4ab207a`; `match-pairs-activity.js?v=37`). **Do NOT auto-resume the E8/E9 queue or any further E2/E12/E4 activity without an explicit prompt; all remain standing backlog.**

**[ARC CLOSED 2026-06-04] The variety + depth arc is complete — the residual is deferred-by-decision, not pending build.** E8 (es/it/pt/fr/fi) + E9 (de/nl/sv/da/no) syllable decks are LIVE at **28 words** (de + en at 30); the per-engine **depth pass** brought the activity pools to their ceilings — E1 ten-frame at range ceilings (11/21/11/21/9), E2's 11 activities at their principled caps (e.g. count-sides 4, describe-attribute 14 EN-only with heavy thin, the shape-art-bound ones), E4's 5 activities at ~18, E12's 3 at ~18, E7 CVC at 20 (EN-only, with documented short-i/o/u library gaps — short-i capped at pig+fig). The **Nordic school-convention arc** (§A.13.57: sv surgical carve-out, no strict-correct, da TeX-accepted) and **strand-name localization** (all 11 locales on national-curriculum domains) also shipped. Per-engine deck depths + per-locale curation: [[project-activities-live-inventory]]. Variety surfaces via the §20.2 per-session Fisher–Yates reshuffle.
...
**STATUS (2026-06-08):** 🏁 **THE EN ARC IS COMPLETE.** **LIVE + verified + CLOSED: pilot + Waves 1, 1b, 2, 3, 4, 5, 6, 7, 8 — 1812 EN landings.** All 8 EN waves live: two subjects (math + ELA), four grade bands (Preschool/K/Gr1/Gr2), all CCSS strands + the consolidated readiness class, three alignment shapes, a proven-durable live-query hub change, and a similarity discipline that held through every coupled slice / grade boundary / natural-floor / constraint-bound-floor case without a single gate failure surviving to ship. **Wave 3 (Math → Grade 1, first grade-boundary crossing)** = 142 Gr1 at `fe8c1b9d` (per-entry level wiring + ≤N quantity-ceiling instrument debuted; K-stays regression PASSED). **Wave 4 (Literacy × K, first cross-strand)** = 162 at `bfa35d7a` (word-guess 47 L.K.2.d + find-and-count 47 RF.K.3.a + prepositions 50 L.K.1.e + bingo 18 readiness; new "Language" CCSS-ELA strand wired). **Wave 5 (Logic × K) — the program's LARGEST wave (423)** = sudoku 100 + picture-sort 40 + odd-one-out 99 + find-objects 93 + matching 91, commits content `c8d4aa5f` + de-orphan `baa89b1f`→`04bfcbee`. W5 shipped 4 readiness classes (2 NEW labels) + matching standard (RF.K.3.a/RF.K.3.c) + **the program's FIRST live-query hub change (the de-orphan)** — all post-ship-verified (de-orphan dual-membership + anchor-precision 0-FP + type-hub-unregressed; K-stays PASSED). **Wave 6 (Spatial × K)** = 392 at `f669f448` (pattern-train 248 [5-mode, NEW "Patterning (readiness)" label] + pattern-worksheet 48 [conditional-SHIP per the ship-or-drop fork] + picture-trail 48 + shadow-match 48) — **all-readiness, 0 standard-bearers (no K.G); NO de-orphan**; the de-orphan was proved **durable + coexistent** (the first wave shipping after it — `/en/topic/animals` carries both W5 picture-sort `-vs-` AND W6 regular-tag landings). **Wave 7 (Readiness-class + Preschool)** = 285 at `16fe8a37` — the program's **FIRST below-K (Preschool) grade band**. Preschool (190): alphabet-train 37 + big-small findBig 51/orderAsc 50 + more-less 52; K (95): grid-match 47 + missing-pieces 48. All-readiness, 0 standard-bearers, **5 NEW readiness labels**; Preschool level-wiring (+1-line LEVELS entry, K-default-safe); **three-band regression PASS** (Preschool-renders-3-4 AND K-stays-5-6 AND Gr1-stays-6-7 — the first sub-K level live without shifting K/Gr1). Refined the natural-floor doctrine to **operation-vs-attribute** (grid-match/missing-pieces resisted 0.177 on shared operation; big-small collapsed 0.132 on shared attribute). **Wave 8 (Gr2 — the FIRST Gr2 grade band, the EN arc's LAST wave) = math-puzzle 33 + code-addition 50 + crossword 44 = 127 Gr2 at `e7cc0bd6`** (full-corpus re-grade yielded 33/50 — nine/three more than the breacher record; **2.NBT.B.5** [new NBT strand, reused from the 2.NBT activities] + **L.2.2.d** [Language/ELA, reused from W4]; **four-band regression PASS** Preschool-3-4/K-5-6/Gr1-6-7/Gr2-7-8 — the complete level axis; Gr1-vs-Gr2 same-type coexistence LEAK=0; constraint-bound null↔secret-word floor 0.230 accepted; repoint 212/1194/0). The Wave-5/6/7/8 doctrine dividends are folded into §22.1 below. **Total live EN: 1812.**
...
**Sequence (plan P3.b):** EN waves 1→8 — **1 LIVE** addition·image-image·K (43) → **1b LIVE** +image-number+mixed (143) → **2 LIVE** subtraction (cross-out+image-number) + chart-count·K (137) → **3 LIVE** Math re-leveled Gr1 (142) → **4 LIVE** Literacy·K (find-and-count + prepositions-mc + bingo + word-guess; 162) → **5 LIVE** Logic·K (sudoku + picture-sort + odd-one-out + find-objects + matching; 423 — the largest wave; **+ picture-sort de-orphan DONE**) → **6 LIVE** Spatial·K (pattern-train + pattern-worksheet + picture-trail + shadow-match; 392 — all-readiness; NEW "Patterning (readiness)" label; pattern-worksheet conditional-SHIP) → **7 LIVE** Readiness-class + Preschool (alphabet-train + big-small + grid-match + missing-pieces + more-less; 285 — FIRST sub-K band; 5 new readiness labels; Preschool level-wiring) → **8 LIVE** Gr2 (math-puzzle Gr2 + code-addition Gr2 + crossword; 127 — FIRST Gr2 band, the EN arc's LAST wave; 2.NBT.B.5 + L.2.2.d; four-band axis complete; `e7cc0bd6`). **🏁 EN waves 1-8 COMPLETE — 1812 landings.** Then the **locale fan-out** (next phase): full-market locales **de → es → nl → fr → it → pt → sv → da** (operator may reorder the middle eight on market priority) → **no, fi LAST** (sample-only; honesty-filter in as their decks publish) — each locale's **native ensemble** authors its own compat booleans + P1 skeletons + native keyword research + strand/label localization (first per-locale-ensemble engagement in the landing program). **The picture-sort de-orphan (the program's first live-query hub change) shipped in Wave 5** via `themeSubjectTagsWhere` `hasSome`-over-exact-`-vs-`-component-keys (§22.1), with its own verify + rollback.
...
**Key files:** route `frontend/app/[locale]/worksheets/[slug]/page.tsx` · lib `frontend/lib/seo/landing-content.ts` · content `frontend/content/seo-landing/en.json` · generators `scripts/seo-landing/{gen-wave1.js,gen-wave1b.js,gen-wave2-*.js,gen-wave3-*.js}` · gates `scripts/seo-landing/{gate.js,validity-gate.js}` + **quantity-ceiling `scripts/seo-landing/check-sum-ceiling.js`** (4-shape ≤N gate) · compat `frontend/content/seo-landing/theme-mechanic-compat.json` · coordinate enumerators `scripts/seo-landing/{enum-wave1b-coords.js,enum-wave2-*.js,enum-wave3-*.js}` + `*-coordinates.json` manifests · repoint `scripts/seo-landing/repoint-deck-canonical.js` (committed Wave 2 `ab11441e`; quote-terminated `__CANONICAL_URL__`→landing, `.precanonical-bak`) · hub verifier `scripts/seo-landing/verify-hub-autobind.js` (standing; `--type`, auto-bind + LEAK=0 + auto-bounding) · level map in the route's `LEVELS` const · ledger `docs/seo-landing/mechanic-ledger-mathK.md` · Gr2-candidate record (out-of-tree) `~/.claude/plans/wave3-mathpuzzle-gr2-breachers.md`. **0 protected-core lines** throughout (the 5 activity cores are never touched). Commits: pilot `32e9af1e`/`941c9501`/`8158fef9`; Part-2 `822dcc54`; Wave 1 `d2f12145`→`51ae864c`→`67a8ae18`; **Wave 1b `a681d48d`→`616a5a9a`** (cell-space floor + coprime); **Wave 2** (subtraction+chart-count K, repoint `ab11441e`); **Wave 3 `fe8c1b9d`** (142 Gr1 + per-entry level wiring + ≤N quantity-ceiling instrument; 71 Gr2 candidates out-of-tree); **Wave 4 `bfa35d7a`** (162 Literacy-K + "Language" CCSS-ELA strand); **Wave 5** content `c8d4aa5f` + de-orphan `baa89b1f`→`04bfcbee` (423 Logic-K + the first live-query hub change — `themeSubjectTagsWhere` in `frontend/lib/topic-decks.ts`, verifier `scripts/seo-landing/verify-deorphan.js`); **Wave 6 `f669f448`** (392 Spatial-K + NEW "Patterning (readiness)" label + conditional-ship pattern-worksheet; NO de-orphan — de-orphan proved durable+coexistent; repoint 541/4327, the heaviest; `gen-wave6-*` + `enum-wave6-*` + `wave6-*-coordinates.json`); **Wave 7 `16fe8a37`** (285 Preschool+K — FIRST sub-K grade band + Preschool LEVELS-add [`'preschool':{Preschool,3-4}`, K-default-safe]; 5 new readiness labels; operation-vs-attribute floor refinement; three-band regression PASS; repoint 737/5425, the heaviest in the program; `gen/enum-wave7-{alphabettrain,bigsmall,visualperception,moreless}` + `wave7-*-coordinates.json`); **🏁 Wave 8 `e7cc0bd6`** (127 Gr2 — math-puzzle 33 + code-addition 50 + crossword 44; the EN arc's LAST wave; Gr2 LEVELS-add `'grade-2':{Grade 2,7-8}` [four-band axis complete]; 2.NBT.B.5 [NBT strand reused] + L.2.2.d [Language/ELA reused]; full-corpus re-grade 33/50; null-mode-string→JS-null normalizer; `gen/enum-wave8-{mathpuzzle,codeaddition,crossword}` + `wave8-*-coordinates.json`; repoint 212/1194/0). **EN waves 1-8 CLOSED; next phase = locale fan-out.** **Landing-presence audit 2026-06-08** (all 7 operator-flagged types: chart-count/cryptogram/math-worksheet/picture-sort/treasure-hunt/word-scramble/wordsearch) = **CLEAN, no regression** — chart-count 43 + picture-sort 40 live + binding (LEAK=0 on both type hubs, 200 + self-canonical); the other 5 correctly landing-absent by ruling; the operator's observation reconciled to the deliberately-unlanded set (category (d) above), not a gap.
...
- **de state by step:** STEP 1 addition (117, 1.Klasse) · STEP 2 readiness (978 Vorschule across 13 types, incl. the picture-sort **de-orphan**) · STEP 3 numeric Klasse (code-addition 95 no-CCSS Rechenrätsel + crossword 45 L.2.2.d + subtraction 158 1.OA.D.8 + math-puzzle 121 **two-band**) · STEP 4 literacy (265: find-and-count/letter-spotting 33 + matching 44 RF.K.3.a + word-guess 94 L.K.2.d + prepositions **split** 94). N-band **vorschule 1057 / 1-klasse 548 / 2-klasse 174**. Commits per step in [[project-de-landing-fanout-investigation]].
