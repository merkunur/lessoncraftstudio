#!/usr/bin/env bash
# b3-publish-locale.sh <locale> [--confirm]
#
# nt20-C Phase 7 (2026-09-20): INSERT-publish of one locale's pooled b3 wave
# (the 20 base + up to 100 variation printable decks generated on the PC into
# scripts/worksheet-gen/out/upload/wave-b3-<loc>-all/ and scp'd to
# /var/www/lcs-media/_staging/b3-<loc>/). Runs ON HETZNER. The nt20-B recipe
# (memory project_nt20b_worksheet_types.md, README b3-designs step 9), scripted:
#
...
# repoint-deck-canonical.js --types=<20 keys> --locale=<loc>,
# refresh-deck-noindex-exempt.sh and indexnow-submit.js (see the plan).
set -euo pipefail
LOC="${1:?locale}"; MODE="${2:-}"
ROOT=/opt/lessoncraftstudio
STAGE=/var/www/lcs-media/_staging/b3-$LOC
REC=/root/staging/b3; mkdir -p "$REC"
...
node scripts/publish-cli/index.js publish-bulk "$STAGE" --dry-run --batch-id "b3-$LOC-dry" > "$REC/$LOC-dry.log" 2>&1 || true
...
# 2. real publish
node scripts/publish-cli/index.js publish-bulk "$STAGE" --confirm --batch-id "b3-$LOC" > "$REC/$LOC-confirm.log" 2>&1 || { echo "publish-bulk --confirm FAILED — $REC/$LOC-confirm.log"; tail -30 "$REC/$LOC-confirm.log"; exit 2; }
