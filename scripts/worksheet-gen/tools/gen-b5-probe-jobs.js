#!/usr/bin/env node
/**
 * gen-b5-probe-jobs.js — turn a wave plan into a render/batch.js job file, so a
 * whole wave can be probed through the REAL enumerator (theme pins, unit lists,
 * refusals) before cli.js generate makes ZIPs.
 *
 *   node tools/gen-b5-probe-jobs.js <waves/wave-b5-en.json> [--out=<jobs.json>] [--types=A,B]
 *
...
if (!waveFile) { console.error('usage: gen-b5-probe-jobs.js <wave.json> [--out=<jobs.json>] [--types=A,B]'); process.exit(2); }
...
const jobs = instances
  ...
  .map((i) => {
    ...
    if (theme === undefined) throw new Error('gen-b5-probe-jobs: instance for ' + type + ' carries neither cacheTheme nor theme: ' + JSON.stringify(i));
    ...
    if (themed && !theme) throw new Error('gen-b5-probe-jobs: themed spec ' + type + ' enumerated without a theme (' + i.deckId + ')');
...
console.log(`gen-b5-probe-jobs: ${jobs.length} jobs from ${plan.id || waveFile} → ${out}` + (skipped.length ? `; ${skipped.length} coordinate(s) REFUSED by the enumerator:` : ''));
