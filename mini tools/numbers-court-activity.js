...
(function (global) {
  ...
  function speak(text) {
    try {
      ...
      if (global.speechSynthesis && global.SpeechSynthesisUtterance) { var u = new global.SpeechSynthesisUtterance(text); u.rate = .95; u.lang = LANG === 'de' ? 'de-DE' : LANG === 'fr' ? 'fr-FR' : LANG === 'es' ? 'es-MX' : LANG === 'pt' ? 'pt-BR' : LANG === 'it' ? 'it-IT' : LANG === 'nl' ? 'nl-NL' : LANG === 'sv' ? 'sv-SE' : 'en-US'; global.speechSynthesis.cancel(); global.speechSynthesis.speak(u); }
    } catch (e) {}
  }
  ...
  global.NumbersCourtActivity = {
    ...
    strings: {
      /* nl — native ensemble (linguïst + pedagoog groep 4). RELATIONELE-GELIJKHEID LOCK: «=» = «aan
         beide kanten evenveel» / «in evenwicht» — NOOIT operationeel «wordt/maakt/is/geeft/het antwoord»
         in bevestigende copy (operationeel alleen binnen een ontkenning). Oordeel «Klopt!»/«Klopt niet!»
         (relationeel; NIET «Eerlijk» = moreel-eerlijk, drift). Kind = «rechter» (epiceen). Karakter =
         rechter Roos de schildpad (hernoemd van Tess — botst met #9 Ties). */
      title: { en: 'Numbers Court', de: 'Das Zahlengericht', fr: 'Le tribunal des nombres', es: 'El tribunal de los números', pt: 'O Tribunal dos Números', it: 'Il Tribunale dei Numeri', nl: 'De rechtbank van de getallen', sv: 'Sifferdomstolen' },
      prompt: { en: 'Is it fair, Judge?', de: 'Stimmt das?', fr: 'Est-ce vrai, Madame la juge ?', es: '¿Es justo, jueza?', pt: 'É justo, juíza?', it: 'È giusto, Giudice?', nl: 'Klopt het, rechter?', sv: 'Är det lika mycket på båda sidor?' },
      vTrue: { en: "It's fair!", de: 'Stimmt!', fr: 'Vrai', es: '¡Es justo!', pt: 'É justo!', it: 'È giusto!', nl: 'Klopt!', sv: 'Stämmer!' }, vFalse: { en: 'Catch it!', de: 'Stimmt nicht!', fr: 'Faux', es: '¡Atrápalo!', pt: 'Não é justo!', it: 'Non è giusto!', nl: 'Klopt niet!', sv: 'Stämmer inte!' },
      jFalse: { en: 'Tap the heavier side.', de: 'Tippe auf die schwerere Seite.', fr: 'Touche le plateau le plus lourd.', es: 'Toca el lado más pesado.', pt: 'Toque no lado mais pesado.', it: 'Tocca il piatto più pesante.', nl: 'Tik op de zwaardere kant.', sv: 'Tryck på den tyngre sidan.' },
      jTrue: { en: 'Tap each number to fill both pans.', de: 'Tippe jede Zahl an, um beide Waagschalen zu füllen.', fr: 'Touche chaque nombre pour remplir les deux plateaux.', es: 'Toca cada número para llenar los dos platillos.', pt: 'Toque em cada número para encher os dois pratos.', it: 'Tocca ogni numero per riempire i due piatti.', nl: 'Tik op elk getal om beide schalen te vullen.', sv: 'Tryck på varje tal och fyll båda vågskålarna.' },
      repair: { en: 'Make it fair — tap a tile for the slot.', de: 'Mach beide Seiten gleich viel — tippe ein Plättchen in die Lücke.', fr: 'Équilibre la balance — touche un jeton pour la case.', es: 'Deja los dos lados iguales: toca una ficha para el espacio.', pt: 'Deixe justo — toque numa ficha para o espaço.', it: 'Rendilo giusto: tocca una tessera per la casella vuota.', nl: 'Maak het in evenwicht — tik een kaartje in het lege vakje.', sv: 'Gör lika mycket på båda sidorna — tryck på ett sifferkort till rutan.' },
      sure: { en: 'Are you sure, Judge? Look again.', de: 'Bist du sicher? Schau noch mal.', fr: 'Hmm… regarde encore les deux côtés.', es: '¿Segura, jueza? Míralo otra vez.', pt: 'Tem certeza, juíza? Olhe de novo.', it: 'Ripensaci bene, Giudice. Guarda di nuovo.', nl: 'Weet je het zeker, rechter? Kijk nog eens goed.', sv: 'Hm… titta en gång till på båda sidorna, domare.' },
      reteach: { en: 'This much AND this much — are they the same amount?', de: 'So viel und so viel — ist das auf beiden Seiten gleich viel?', fr: 'Autant d’un côté, autant de l’autre — est-ce la même chose des deux côtés ?', es: 'Esto de un lado y esto del otro… ¿es la misma cantidad?', pt: 'Tanto assim e tanto assim — os dois lados têm a mesma quantidade?', it: 'Tanto da una parte e tanto dall\'altra: i due piatti hanno la stessa quantità?', nl: 'Zoveel én zoveel — is dat aan beide kanten evenveel?', sv: 'Räkna båda sidorna — är det lika mycket?' },
      winTrue: { en: 'Fair! Both sides are the same amount.', de: 'Stimmt! Beide Seiten sind gleich viel.', fr: 'Vrai ! Il y a la même chose des deux côtés.', es: '¡Justo! Los dos lados tienen la misma cantidad.', pt: 'Justo! Os dois lados têm a mesma quantidade.', it: 'Giusto! I due piatti hanno la stessa quantità.', nl: 'In evenwicht! Aan beide kanten staat evenveel.', sv: 'Stämmer! Det är lika mycket på båda sidorna.' },
      winFalse: { en: 'Caught it! The sides are not the same.', de: 'Erwischt! Die Seiten sind nicht gleich viel.', fr: 'Faux ! Les deux côtés ne sont pas pareils.', es: '¡Lo atrapaste! Los dos lados no son iguales.', pt: 'Pegou! Os dois lados não são iguais.', it: 'Preso! I due piatti non hanno la stessa quantità.', nl: 'Betrapt! De twee kanten zijn niet evenveel.', sv: 'Stämmer inte! Det är inte lika mycket på båda sidorna.' },
      winRepair: { en: 'Now it balances — fair, Judge!', de: 'Jetzt ist die Waage im Gleichgewicht — gleich viel!', fr: 'La balance s’équilibre — bien joué, Madame la juge !', es: '¡Ahora se equilibra… justo, jueza!', pt: 'Agora equilibra — justo, juíza!', it: 'Ora è in equilibrio: giusto, Giudice!', nl: 'Nu is de weegschaal in evenwicht, rechter!', sv: 'Nu väger vågen jämnt, domare — lika mycket på båda sidorna!' },
      regen: { en: 'Still not level — here is a fresh one, Judge.', de: 'Immer noch nicht im Gleichgewicht — hier ist eine neue Aufgabe.', fr: 'Toujours pas d’équilibre — voici une nouvelle affaire, Madame la juge.', es: 'Todavía no se equilibra… aquí tienes un caso nuevo, jueza.', pt: 'Ainda não equilibrou — aqui vai outro caso, juíza.', it: 'Ancora non è in equilibrio: ecco un nuovo caso, Giudice.', nl: 'Nog niet in evenwicht — hier is een nieuwe zaak, rechter.', sv: 'Vågen väger fortfarande inte jämnt — här kommer ett nytt fall, domare.' },
      hintCheck: { en: 'Give your verdict, then tap Check.', de: 'Fäll dein Urteil, dann tippe auf „Prüfen".', fr: 'Donne ton verdict, puis touche « Vérifier ».', es: 'Da tu veredicto y luego toca "Revisar".', pt: 'Dê o seu veredito e toque em "Verificar".', it: 'Dai il tuo verdetto, poi tocca "Verifica".', nl: 'Geef je oordeel en tik dan op Controleer.', sv: 'Tryck på Kontrollera när du är klar, domare.' },
      judgeName: { en: 'Judge Tess', de: 'Richterin Tess', fr: 'La juge Tess', es: 'La jueza Tess', pt: 'A juíza Tess', it: 'La giudice Tess', nl: 'rechter Roos', sv: 'Domare Tess' }
    },
    ...
    render: function () {
      ...
      /* decorative: the witness animal carries no information the child needs. Without this a
         screen reader announces the emoji's own name in ITS language ('rabbit face') in every
         locale — noise between the claim and the verdict buttons. */
      wit.setAttribute('aria-hidden', 'true');
    ...
    injectCSS: function () {
      ...
      var css = ''
        ...
        + '.nc-num-tap{background:#FFF6E6;border:2px solid rgba(20,107,94,.3);border-radius:10px;padding:1px 8px;cursor:pointer;min-height:44px;touch-action:manipulation;}'
        ...
        + '.nc-pan{position:absolute;top:-6px;width:clamp(60px,19vw,92px);min-height:44px;border:0;background:transparent;cursor:default;padding:0;}'
