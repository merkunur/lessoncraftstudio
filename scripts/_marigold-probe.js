#!/usr/bin/env node
/* =====================================================================
   _marigold-probe.js — multi-state control-bottom probe for "Marigold's
   Knowing Machine" (L.2.4.b/c affix, clarity-first). Drives apply / which
   ...
   plus tap ≥44px, no overlap, no horizontal overflow.
   ===================================================================== */
'use strict';
...
(async () => {
  ...
  const states = {
    'apply': async () => { await force('af-helpful'); },
    'which': async () => { await force('af-which-retie'); },
    'wrong': async () => { await force('af-unkind'); await solve(false); },
    'resolved': async () => { await force('af-colorful'); await solve(true); }
  };

  async function controlBottom() {
    return await page.evaluate(() => {
      const sel = '.af-root,.af-say,.af-board,.af-scene,.af-row,.af-cogrow,.af-cand,.af-line-msg,.lcs-activity-check';
  ...
  async function checks() {
    return await page.evaluate(() => {
      const taps = Array.from(document.querySelectorAll('.af-cand'));
  ...
  if (!(await page.evaluate(() => window.AffixActivity._resolved))) fails.push('a correct tap did NOT resolve');
  await states.wrong();
  if (await page.evaluate(() => window.AffixActivity._resolved)) fails.push('a wrong option resolved — must not advance');

  const order = ['apply', 'which', 'wrong', 'resolved'];
  ...
  console.log('');
  if (fails.length) { console.error(`MARIGOLD-PROBE FAILED — ${fails.length}:`); fails.forEach(f => console.error('  • ' + f)); process.exit(1); }
  console.log(`MARIGOLD-PROBE PASSED — apply/which/wrong/resolved: a correct tap resolves + a wrong one does not advance; lowest control (incl. shell Check) clears the fold by ≥${MARGIN}px across 320→1366; controls ≥${CTRL_MIN}px (off the 320 sub-floor), no overlap, no horizontal overflow.`);
  process.exit(0);
